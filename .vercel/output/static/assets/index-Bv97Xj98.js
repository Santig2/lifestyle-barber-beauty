function V0(n) {
  return n && n.__esModule && Object.prototype.hasOwnProperty.call(n, "default") ? n.default : n;
}
var ec = { exports: {} },
  wl = {};
var Zm;
function X0() {
  if (Zm) return wl;
  Zm = 1;
  var n = Symbol.for("react.transitional.element"),
    l = Symbol.for("react.fragment");
  function s(u, c, f) {
    var h = null;
    if ((f !== void 0 && (h = "" + f), c.key !== void 0 && (h = "" + c.key), "key" in c)) {
      f = {};
      for (var y in c) y !== "key" && (f[y] = c[y]);
    } else f = c;
    return ((c = f.ref), { $$typeof: n, type: u, key: h, ref: c !== void 0 ? c : null, props: f });
  }
  return ((wl.Fragment = l), (wl.jsx = s), (wl.jsxs = s), wl);
}
var Pm;
function K0() {
  return (Pm || ((Pm = 1), (ec.exports = X0())), ec.exports);
}
var P = K0(),
  nc = { exports: {} },
  ot = {};
var Fm;
function Z0() {
  if (Fm) return ot;
  Fm = 1;
  var n = Symbol.for("react.transitional.element"),
    l = Symbol.for("react.portal"),
    s = Symbol.for("react.fragment"),
    u = Symbol.for("react.strict_mode"),
    c = Symbol.for("react.profiler"),
    f = Symbol.for("react.consumer"),
    h = Symbol.for("react.context"),
    y = Symbol.for("react.forward_ref"),
    m = Symbol.for("react.suspense"),
    p = Symbol.for("react.memo"),
    S = Symbol.for("react.lazy"),
    g = Symbol.for("react.activity"),
    _ = Symbol.iterator;
  function E(C) {
    return C === null || typeof C != "object"
      ? null
      : ((C = (_ && C[_]) || C["@@iterator"]), typeof C == "function" ? C : null);
  }
  var x = {
      isMounted: function () {
        return !1;
      },
      enqueueForceUpdate: function () {},
      enqueueReplaceState: function () {},
      enqueueSetState: function () {},
    },
    z = Object.assign,
    R = {};
  function w(C, Q, k) {
    ((this.props = C), (this.context = Q), (this.refs = R), (this.updater = k || x));
  }
  ((w.prototype.isReactComponent = {}),
    (w.prototype.setState = function (C, Q) {
      if (typeof C != "object" && typeof C != "function" && C != null)
        throw Error(
          "takes an object of state variables to update or a function which returns an object of state variables.",
        );
      this.updater.enqueueSetState(this, C, Q, "setState");
    }),
    (w.prototype.forceUpdate = function (C) {
      this.updater.enqueueForceUpdate(this, C, "forceUpdate");
    }));
  function N() {}
  N.prototype = w.prototype;
  function X(C, Q, k) {
    ((this.props = C), (this.context = Q), (this.refs = R), (this.updater = k || x));
  }
  var Y = (X.prototype = new N());
  ((Y.constructor = X), z(Y, w.prototype), (Y.isPureReactComponent = !0));
  var K = Array.isArray;
  function J() {}
  var q = { H: null, A: null, T: null, S: null },
    Z = Object.prototype.hasOwnProperty;
  function I(C, Q, k) {
    var $ = k.ref;
    return { $$typeof: n, type: C, key: Q, ref: $ !== void 0 ? $ : null, props: k };
  }
  function ut(C, Q) {
    return I(C.type, Q, C.props);
  }
  function st(C) {
    return typeof C == "object" && C !== null && C.$$typeof === n;
  }
  function lt(C) {
    var Q = { "=": "=0", ":": "=2" };
    return (
      "$" +
      C.replace(/[=:]/g, function (k) {
        return Q[k];
      })
    );
  }
  var Wt = /\/+/g;
  function Nt(C, Q) {
    return typeof C == "object" && C !== null && C.key != null ? lt("" + C.key) : Q.toString(36);
  }
  function Gt(C) {
    switch (C.status) {
      case "fulfilled":
        return C.value;
      case "rejected":
        throw C.reason;
      default:
        switch (
          (typeof C.status == "string"
            ? C.then(J, J)
            : ((C.status = "pending"),
              C.then(
                function (Q) {
                  C.status === "pending" && ((C.status = "fulfilled"), (C.value = Q));
                },
                function (Q) {
                  C.status === "pending" && ((C.status = "rejected"), (C.reason = Q));
                },
              )),
          C.status)
        ) {
          case "fulfilled":
            return C.value;
          case "rejected":
            throw C.reason;
        }
    }
    throw C;
  }
  function j(C, Q, k, $, nt) {
    var dt = typeof C;
    (dt === "undefined" || dt === "boolean") && (C = null);
    var vt = !1;
    if (C === null) vt = !0;
    else
      switch (dt) {
        case "bigint":
        case "string":
        case "number":
          vt = !0;
          break;
        case "object":
          switch (C.$$typeof) {
            case n:
            case l:
              vt = !0;
              break;
            case S:
              return ((vt = C._init), j(vt(C._payload), Q, k, $, nt));
          }
      }
    if (vt)
      return (
        (nt = nt(C)),
        (vt = $ === "" ? "." + Nt(C, 0) : $),
        K(nt)
          ? ((k = ""),
            vt != null && (k = vt.replace(Wt, "$&/") + "/"),
            j(nt, Q, k, "", function (nn) {
              return nn;
            }))
          : nt != null &&
            (st(nt) &&
              (nt = ut(
                nt,
                k +
                  (nt.key == null || (C && C.key === nt.key)
                    ? ""
                    : ("" + nt.key).replace(Wt, "$&/") + "/") +
                  vt,
              )),
            Q.push(nt)),
        1
      );
    vt = 0;
    var Qt = $ === "" ? "." : $ + ":";
    if (K(C))
      for (var Ot = 0; Ot < C.length; Ot++)
        (($ = C[Ot]), (dt = Qt + Nt($, Ot)), (vt += j($, Q, k, dt, nt)));
    else if (((Ot = E(C)), typeof Ot == "function"))
      for (C = Ot.call(C), Ot = 0; !($ = C.next()).done; )
        (($ = $.value), (dt = Qt + Nt($, Ot++)), (vt += j($, Q, k, dt, nt)));
    else if (dt === "object") {
      if (typeof C.then == "function") return j(Gt(C), Q, k, $, nt);
      throw (
        (Q = String(C)),
        Error(
          "Objects are not valid as a React child (found: " +
            (Q === "[object Object]" ? "object with keys {" + Object.keys(C).join(", ") + "}" : Q) +
            "). If you meant to render a collection of children, use an array instead.",
        )
      );
    }
    return vt;
  }
  function F(C, Q, k) {
    if (C == null) return C;
    var $ = [],
      nt = 0;
    return (
      j(C, $, "", "", function (dt) {
        return Q.call(k, dt, nt++);
      }),
      $
    );
  }
  function it(C) {
    if (C._status === -1) {
      var Q = C._result;
      ((Q = Q()),
        Q.then(
          function (k) {
            (C._status === 0 || C._status === -1) && ((C._status = 1), (C._result = k));
          },
          function (k) {
            (C._status === 0 || C._status === -1) && ((C._status = 2), (C._result = k));
          },
        ),
        C._status === -1 && ((C._status = 0), (C._result = Q)));
    }
    if (C._status === 1) return C._result.default;
    throw C._result;
  }
  var Et =
      typeof reportError == "function"
        ? reportError
        : function (C) {
            if (typeof window == "object" && typeof window.ErrorEvent == "function") {
              var Q = new window.ErrorEvent("error", {
                bubbles: !0,
                cancelable: !0,
                message:
                  typeof C == "object" && C !== null && typeof C.message == "string"
                    ? String(C.message)
                    : String(C),
                error: C,
              });
              if (!window.dispatchEvent(Q)) return;
            } else if (typeof process == "object" && typeof process.emit == "function") {
              process.emit("uncaughtException", C);
              return;
            }
            console.error(C);
          },
    Rt = {
      map: F,
      forEach: function (C, Q, k) {
        F(
          C,
          function () {
            Q.apply(this, arguments);
          },
          k,
        );
      },
      count: function (C) {
        var Q = 0;
        return (
          F(C, function () {
            Q++;
          }),
          Q
        );
      },
      toArray: function (C) {
        return (
          F(C, function (Q) {
            return Q;
          }) || []
        );
      },
      only: function (C) {
        if (!st(C))
          throw Error("React.Children.only expected to receive a single React element child.");
        return C;
      },
    };
  return (
    (ot.Activity = g),
    (ot.Children = Rt),
    (ot.Component = w),
    (ot.Fragment = s),
    (ot.Profiler = c),
    (ot.PureComponent = X),
    (ot.StrictMode = u),
    (ot.Suspense = m),
    (ot.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = q),
    (ot.__COMPILER_RUNTIME = {
      __proto__: null,
      c: function (C) {
        return q.H.useMemoCache(C);
      },
    }),
    (ot.cache = function (C) {
      return function () {
        return C.apply(null, arguments);
      };
    }),
    (ot.cacheSignal = function () {
      return null;
    }),
    (ot.cloneElement = function (C, Q, k) {
      if (C == null) throw Error("The argument must be a React element, but you passed " + C + ".");
      var $ = z({}, C.props),
        nt = C.key;
      if (Q != null)
        for (dt in (Q.key !== void 0 && (nt = "" + Q.key), Q))
          !Z.call(Q, dt) ||
            dt === "key" ||
            dt === "__self" ||
            dt === "__source" ||
            (dt === "ref" && Q.ref === void 0) ||
            ($[dt] = Q[dt]);
      var dt = arguments.length - 2;
      if (dt === 1) $.children = k;
      else if (1 < dt) {
        for (var vt = Array(dt), Qt = 0; Qt < dt; Qt++) vt[Qt] = arguments[Qt + 2];
        $.children = vt;
      }
      return I(C.type, nt, $);
    }),
    (ot.createContext = function (C) {
      return (
        (C = {
          $$typeof: h,
          _currentValue: C,
          _currentValue2: C,
          _threadCount: 0,
          Provider: null,
          Consumer: null,
        }),
        (C.Provider = C),
        (C.Consumer = { $$typeof: f, _context: C }),
        C
      );
    }),
    (ot.createElement = function (C, Q, k) {
      var $,
        nt = {},
        dt = null;
      if (Q != null)
        for ($ in (Q.key !== void 0 && (dt = "" + Q.key), Q))
          Z.call(Q, $) && $ !== "key" && $ !== "__self" && $ !== "__source" && (nt[$] = Q[$]);
      var vt = arguments.length - 2;
      if (vt === 1) nt.children = k;
      else if (1 < vt) {
        for (var Qt = Array(vt), Ot = 0; Ot < vt; Ot++) Qt[Ot] = arguments[Ot + 2];
        nt.children = Qt;
      }
      if (C && C.defaultProps)
        for ($ in ((vt = C.defaultProps), vt)) nt[$] === void 0 && (nt[$] = vt[$]);
      return I(C, dt, nt);
    }),
    (ot.createRef = function () {
      return { current: null };
    }),
    (ot.forwardRef = function (C) {
      return { $$typeof: y, render: C };
    }),
    (ot.isValidElement = st),
    (ot.lazy = function (C) {
      return { $$typeof: S, _payload: { _status: -1, _result: C }, _init: it };
    }),
    (ot.memo = function (C, Q) {
      return { $$typeof: p, type: C, compare: Q === void 0 ? null : Q };
    }),
    (ot.startTransition = function (C) {
      var Q = q.T,
        k = {};
      q.T = k;
      try {
        var $ = C(),
          nt = q.S;
        (nt !== null && nt(k, $),
          typeof $ == "object" && $ !== null && typeof $.then == "function" && $.then(J, Et));
      } catch (dt) {
        Et(dt);
      } finally {
        (Q !== null && k.types !== null && (Q.types = k.types), (q.T = Q));
      }
    }),
    (ot.unstable_useCacheRefresh = function () {
      return q.H.useCacheRefresh();
    }),
    (ot.use = function (C) {
      return q.H.use(C);
    }),
    (ot.useActionState = function (C, Q, k) {
      return q.H.useActionState(C, Q, k);
    }),
    (ot.useCallback = function (C, Q) {
      return q.H.useCallback(C, Q);
    }),
    (ot.useContext = function (C) {
      return q.H.useContext(C);
    }),
    (ot.useDebugValue = function () {}),
    (ot.useDeferredValue = function (C, Q) {
      return q.H.useDeferredValue(C, Q);
    }),
    (ot.useEffect = function (C, Q) {
      return q.H.useEffect(C, Q);
    }),
    (ot.useEffectEvent = function (C) {
      return q.H.useEffectEvent(C);
    }),
    (ot.useId = function () {
      return q.H.useId();
    }),
    (ot.useImperativeHandle = function (C, Q, k) {
      return q.H.useImperativeHandle(C, Q, k);
    }),
    (ot.useInsertionEffect = function (C, Q) {
      return q.H.useInsertionEffect(C, Q);
    }),
    (ot.useLayoutEffect = function (C, Q) {
      return q.H.useLayoutEffect(C, Q);
    }),
    (ot.useMemo = function (C, Q) {
      return q.H.useMemo(C, Q);
    }),
    (ot.useOptimistic = function (C, Q) {
      return q.H.useOptimistic(C, Q);
    }),
    (ot.useReducer = function (C, Q, k) {
      return q.H.useReducer(C, Q, k);
    }),
    (ot.useRef = function (C) {
      return q.H.useRef(C);
    }),
    (ot.useState = function (C) {
      return q.H.useState(C);
    }),
    (ot.useSyncExternalStore = function (C, Q, k) {
      return q.H.useSyncExternalStore(C, Q, k);
    }),
    (ot.useTransition = function () {
      return q.H.useTransition();
    }),
    (ot.version = "19.2.6"),
    ot
  );
}
var Jm;
function Kl() {
  return (Jm || ((Jm = 1), (nc.exports = Z0())), nc.exports);
}
var tt = Kl();
const jl = V0(tt);
var ac = { exports: {} },
  Ol = {},
  ic = { exports: {} },
  lc = {};
var km;
function P0() {
  return (
    km ||
      ((km = 1),
      (function (n) {
        function l(j, F) {
          var it = j.length;
          j.push(F);
          t: for (; 0 < it; ) {
            var Et = (it - 1) >>> 1,
              Rt = j[Et];
            if (0 < c(Rt, F)) ((j[Et] = F), (j[it] = Rt), (it = Et));
            else break t;
          }
        }
        function s(j) {
          return j.length === 0 ? null : j[0];
        }
        function u(j) {
          if (j.length === 0) return null;
          var F = j[0],
            it = j.pop();
          if (it !== F) {
            j[0] = it;
            t: for (var Et = 0, Rt = j.length, C = Rt >>> 1; Et < C; ) {
              var Q = 2 * (Et + 1) - 1,
                k = j[Q],
                $ = Q + 1,
                nt = j[$];
              if (0 > c(k, it))
                $ < Rt && 0 > c(nt, k)
                  ? ((j[Et] = nt), (j[$] = it), (Et = $))
                  : ((j[Et] = k), (j[Q] = it), (Et = Q));
              else if ($ < Rt && 0 > c(nt, it)) ((j[Et] = nt), (j[$] = it), (Et = $));
              else break t;
            }
          }
          return F;
        }
        function c(j, F) {
          var it = j.sortIndex - F.sortIndex;
          return it !== 0 ? it : j.id - F.id;
        }
        if (
          ((n.unstable_now = void 0),
          typeof performance == "object" && typeof performance.now == "function")
        ) {
          var f = performance;
          n.unstable_now = function () {
            return f.now();
          };
        } else {
          var h = Date,
            y = h.now();
          n.unstable_now = function () {
            return h.now() - y;
          };
        }
        var m = [],
          p = [],
          S = 1,
          g = null,
          _ = 3,
          E = !1,
          x = !1,
          z = !1,
          R = !1,
          w = typeof setTimeout == "function" ? setTimeout : null,
          N = typeof clearTimeout == "function" ? clearTimeout : null,
          X = typeof setImmediate < "u" ? setImmediate : null;
        function Y(j) {
          for (var F = s(p); F !== null; ) {
            if (F.callback === null) u(p);
            else if (F.startTime <= j) (u(p), (F.sortIndex = F.expirationTime), l(m, F));
            else break;
            F = s(p);
          }
        }
        function K(j) {
          if (((z = !1), Y(j), !x))
            if (s(m) !== null) ((x = !0), J || ((J = !0), lt()));
            else {
              var F = s(p);
              F !== null && Gt(K, F.startTime - j);
            }
        }
        var J = !1,
          q = -1,
          Z = 5,
          I = -1;
        function ut() {
          return R ? !0 : !(n.unstable_now() - I < Z);
        }
        function st() {
          if (((R = !1), J)) {
            var j = n.unstable_now();
            I = j;
            var F = !0;
            try {
              t: {
                ((x = !1), z && ((z = !1), N(q), (q = -1)), (E = !0));
                var it = _;
                try {
                  e: {
                    for (Y(j), g = s(m); g !== null && !(g.expirationTime > j && ut()); ) {
                      var Et = g.callback;
                      if (typeof Et == "function") {
                        ((g.callback = null), (_ = g.priorityLevel));
                        var Rt = Et(g.expirationTime <= j);
                        if (((j = n.unstable_now()), typeof Rt == "function")) {
                          ((g.callback = Rt), Y(j), (F = !0));
                          break e;
                        }
                        (g === s(m) && u(m), Y(j));
                      } else u(m);
                      g = s(m);
                    }
                    if (g !== null) F = !0;
                    else {
                      var C = s(p);
                      (C !== null && Gt(K, C.startTime - j), (F = !1));
                    }
                  }
                  break t;
                } finally {
                  ((g = null), (_ = it), (E = !1));
                }
                F = void 0;
              }
            } finally {
              F ? lt() : (J = !1);
            }
          }
        }
        var lt;
        if (typeof X == "function")
          lt = function () {
            X(st);
          };
        else if (typeof MessageChannel < "u") {
          var Wt = new MessageChannel(),
            Nt = Wt.port2;
          ((Wt.port1.onmessage = st),
            (lt = function () {
              Nt.postMessage(null);
            }));
        } else
          lt = function () {
            w(st, 0);
          };
        function Gt(j, F) {
          q = w(function () {
            j(n.unstable_now());
          }, F);
        }
        ((n.unstable_IdlePriority = 5),
          (n.unstable_ImmediatePriority = 1),
          (n.unstable_LowPriority = 4),
          (n.unstable_NormalPriority = 3),
          (n.unstable_Profiling = null),
          (n.unstable_UserBlockingPriority = 2),
          (n.unstable_cancelCallback = function (j) {
            j.callback = null;
          }),
          (n.unstable_forceFrameRate = function (j) {
            0 > j || 125 < j
              ? console.error(
                  "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported",
                )
              : (Z = 0 < j ? Math.floor(1e3 / j) : 5);
          }),
          (n.unstable_getCurrentPriorityLevel = function () {
            return _;
          }),
          (n.unstable_next = function (j) {
            switch (_) {
              case 1:
              case 2:
              case 3:
                var F = 3;
                break;
              default:
                F = _;
            }
            var it = _;
            _ = F;
            try {
              return j();
            } finally {
              _ = it;
            }
          }),
          (n.unstable_requestPaint = function () {
            R = !0;
          }),
          (n.unstable_runWithPriority = function (j, F) {
            switch (j) {
              case 1:
              case 2:
              case 3:
              case 4:
              case 5:
                break;
              default:
                j = 3;
            }
            var it = _;
            _ = j;
            try {
              return F();
            } finally {
              _ = it;
            }
          }),
          (n.unstable_scheduleCallback = function (j, F, it) {
            var Et = n.unstable_now();
            switch (
              (typeof it == "object" && it !== null
                ? ((it = it.delay), (it = typeof it == "number" && 0 < it ? Et + it : Et))
                : (it = Et),
              j)
            ) {
              case 1:
                var Rt = -1;
                break;
              case 2:
                Rt = 250;
                break;
              case 5:
                Rt = 1073741823;
                break;
              case 4:
                Rt = 1e4;
                break;
              default:
                Rt = 5e3;
            }
            return (
              (Rt = it + Rt),
              (j = {
                id: S++,
                callback: F,
                priorityLevel: j,
                startTime: it,
                expirationTime: Rt,
                sortIndex: -1,
              }),
              it > Et
                ? ((j.sortIndex = it),
                  l(p, j),
                  s(m) === null && j === s(p) && (z ? (N(q), (q = -1)) : (z = !0), Gt(K, it - Et)))
                : ((j.sortIndex = Rt), l(m, j), x || E || ((x = !0), J || ((J = !0), lt()))),
              j
            );
          }),
          (n.unstable_shouldYield = ut),
          (n.unstable_wrapCallback = function (j) {
            var F = _;
            return function () {
              var it = _;
              _ = F;
              try {
                return j.apply(this, arguments);
              } finally {
                _ = it;
              }
            };
          }));
      })(lc)),
    lc
  );
}
var Im;
function F0() {
  return (Im || ((Im = 1), (ic.exports = P0())), ic.exports);
}
var sc = { exports: {} },
  fe = {};
var $m;
function J0() {
  if ($m) return fe;
  $m = 1;
  var n = Kl();
  function l(m) {
    var p = "https://react.dev/errors/" + m;
    if (1 < arguments.length) {
      p += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var S = 2; S < arguments.length; S++) p += "&args[]=" + encodeURIComponent(arguments[S]);
    }
    return (
      "Minified React error #" +
      m +
      "; visit " +
      p +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function s() {}
  var u = {
      d: {
        f: s,
        r: function () {
          throw Error(l(522));
        },
        D: s,
        C: s,
        L: s,
        m: s,
        X: s,
        S: s,
        M: s,
      },
      p: 0,
      findDOMNode: null,
    },
    c = Symbol.for("react.portal");
  function f(m, p, S) {
    var g = 3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
    return {
      $$typeof: c,
      key: g == null ? null : "" + g,
      children: m,
      containerInfo: p,
      implementation: S,
    };
  }
  var h = n.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  function y(m, p) {
    if (m === "font") return "";
    if (typeof p == "string") return p === "use-credentials" ? p : "";
  }
  return (
    (fe.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = u),
    (fe.createPortal = function (m, p) {
      var S = 2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
      if (!p || (p.nodeType !== 1 && p.nodeType !== 9 && p.nodeType !== 11)) throw Error(l(299));
      return f(m, p, null, S);
    }),
    (fe.flushSync = function (m) {
      var p = h.T,
        S = u.p;
      try {
        if (((h.T = null), (u.p = 2), m)) return m();
      } finally {
        ((h.T = p), (u.p = S), u.d.f());
      }
    }),
    (fe.preconnect = function (m, p) {
      typeof m == "string" &&
        (p
          ? ((p = p.crossOrigin),
            (p = typeof p == "string" ? (p === "use-credentials" ? p : "") : void 0))
          : (p = null),
        u.d.C(m, p));
    }),
    (fe.prefetchDNS = function (m) {
      typeof m == "string" && u.d.D(m);
    }),
    (fe.preinit = function (m, p) {
      if (typeof m == "string" && p && typeof p.as == "string") {
        var S = p.as,
          g = y(S, p.crossOrigin),
          _ = typeof p.integrity == "string" ? p.integrity : void 0,
          E = typeof p.fetchPriority == "string" ? p.fetchPriority : void 0;
        S === "style"
          ? u.d.S(m, typeof p.precedence == "string" ? p.precedence : void 0, {
              crossOrigin: g,
              integrity: _,
              fetchPriority: E,
            })
          : S === "script" &&
            u.d.X(m, {
              crossOrigin: g,
              integrity: _,
              fetchPriority: E,
              nonce: typeof p.nonce == "string" ? p.nonce : void 0,
            });
      }
    }),
    (fe.preinitModule = function (m, p) {
      if (typeof m == "string")
        if (typeof p == "object" && p !== null) {
          if (p.as == null || p.as === "script") {
            var S = y(p.as, p.crossOrigin);
            u.d.M(m, {
              crossOrigin: S,
              integrity: typeof p.integrity == "string" ? p.integrity : void 0,
              nonce: typeof p.nonce == "string" ? p.nonce : void 0,
            });
          }
        } else p == null && u.d.M(m);
    }),
    (fe.preload = function (m, p) {
      if (typeof m == "string" && typeof p == "object" && p !== null && typeof p.as == "string") {
        var S = p.as,
          g = y(S, p.crossOrigin);
        u.d.L(m, S, {
          crossOrigin: g,
          integrity: typeof p.integrity == "string" ? p.integrity : void 0,
          nonce: typeof p.nonce == "string" ? p.nonce : void 0,
          type: typeof p.type == "string" ? p.type : void 0,
          fetchPriority: typeof p.fetchPriority == "string" ? p.fetchPriority : void 0,
          referrerPolicy: typeof p.referrerPolicy == "string" ? p.referrerPolicy : void 0,
          imageSrcSet: typeof p.imageSrcSet == "string" ? p.imageSrcSet : void 0,
          imageSizes: typeof p.imageSizes == "string" ? p.imageSizes : void 0,
          media: typeof p.media == "string" ? p.media : void 0,
        });
      }
    }),
    (fe.preloadModule = function (m, p) {
      if (typeof m == "string")
        if (p) {
          var S = y(p.as, p.crossOrigin);
          u.d.m(m, {
            as: typeof p.as == "string" && p.as !== "script" ? p.as : void 0,
            crossOrigin: S,
            integrity: typeof p.integrity == "string" ? p.integrity : void 0,
          });
        } else u.d.m(m);
    }),
    (fe.requestFormReset = function (m) {
      u.d.r(m);
    }),
    (fe.unstable_batchedUpdates = function (m, p) {
      return m(p);
    }),
    (fe.useFormState = function (m, p, S) {
      return h.H.useFormState(m, p, S);
    }),
    (fe.useFormStatus = function () {
      return h.H.useHostTransitionStatus();
    }),
    (fe.version = "19.2.6"),
    fe
  );
}
var Wm;
function $y() {
  if (Wm) return sc.exports;
  Wm = 1;
  function n() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(n);
      } catch (l) {
        console.error(l);
      }
  }
  return (n(), (sc.exports = J0()), sc.exports);
}
var ty;
function k0() {
  if (ty) return Ol;
  ty = 1;
  var n = F0(),
    l = Kl(),
    s = $y();
  function u(t) {
    var e = "https://react.dev/errors/" + t;
    if (1 < arguments.length) {
      e += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var a = 2; a < arguments.length; a++) e += "&args[]=" + encodeURIComponent(arguments[a]);
    }
    return (
      "Minified React error #" +
      t +
      "; visit " +
      e +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function c(t) {
    return !(!t || (t.nodeType !== 1 && t.nodeType !== 9 && t.nodeType !== 11));
  }
  function f(t) {
    var e = t,
      a = t;
    if (t.alternate) for (; e.return; ) e = e.return;
    else {
      t = e;
      do ((e = t), (e.flags & 4098) !== 0 && (a = e.return), (t = e.return));
      while (t);
    }
    return e.tag === 3 ? a : null;
  }
  function h(t) {
    if (t.tag === 13) {
      var e = t.memoizedState;
      if ((e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)), e !== null))
        return e.dehydrated;
    }
    return null;
  }
  function y(t) {
    if (t.tag === 31) {
      var e = t.memoizedState;
      if ((e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)), e !== null))
        return e.dehydrated;
    }
    return null;
  }
  function m(t) {
    if (f(t) !== t) throw Error(u(188));
  }
  function p(t) {
    var e = t.alternate;
    if (!e) {
      if (((e = f(t)), e === null)) throw Error(u(188));
      return e !== t ? null : t;
    }
    for (var a = t, i = e; ; ) {
      var r = a.return;
      if (r === null) break;
      var o = r.alternate;
      if (o === null) {
        if (((i = r.return), i !== null)) {
          a = i;
          continue;
        }
        break;
      }
      if (r.child === o.child) {
        for (o = r.child; o; ) {
          if (o === a) return (m(r), t);
          if (o === i) return (m(r), e);
          o = o.sibling;
        }
        throw Error(u(188));
      }
      if (a.return !== i.return) ((a = r), (i = o));
      else {
        for (var d = !1, v = r.child; v; ) {
          if (v === a) {
            ((d = !0), (a = r), (i = o));
            break;
          }
          if (v === i) {
            ((d = !0), (i = r), (a = o));
            break;
          }
          v = v.sibling;
        }
        if (!d) {
          for (v = o.child; v; ) {
            if (v === a) {
              ((d = !0), (a = o), (i = r));
              break;
            }
            if (v === i) {
              ((d = !0), (i = o), (a = r));
              break;
            }
            v = v.sibling;
          }
          if (!d) throw Error(u(189));
        }
      }
      if (a.alternate !== i) throw Error(u(190));
    }
    if (a.tag !== 3) throw Error(u(188));
    return a.stateNode.current === a ? t : e;
  }
  function S(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t;
    for (t = t.child; t !== null; ) {
      if (((e = S(t)), e !== null)) return e;
      t = t.sibling;
    }
    return null;
  }
  var g = Object.assign,
    _ = Symbol.for("react.element"),
    E = Symbol.for("react.transitional.element"),
    x = Symbol.for("react.portal"),
    z = Symbol.for("react.fragment"),
    R = Symbol.for("react.strict_mode"),
    w = Symbol.for("react.profiler"),
    N = Symbol.for("react.consumer"),
    X = Symbol.for("react.context"),
    Y = Symbol.for("react.forward_ref"),
    K = Symbol.for("react.suspense"),
    J = Symbol.for("react.suspense_list"),
    q = Symbol.for("react.memo"),
    Z = Symbol.for("react.lazy"),
    I = Symbol.for("react.activity"),
    ut = Symbol.for("react.memo_cache_sentinel"),
    st = Symbol.iterator;
  function lt(t) {
    return t === null || typeof t != "object"
      ? null
      : ((t = (st && t[st]) || t["@@iterator"]), typeof t == "function" ? t : null);
  }
  var Wt = Symbol.for("react.client.reference");
  function Nt(t) {
    if (t == null) return null;
    if (typeof t == "function") return t.$$typeof === Wt ? null : t.displayName || t.name || null;
    if (typeof t == "string") return t;
    switch (t) {
      case z:
        return "Fragment";
      case w:
        return "Profiler";
      case R:
        return "StrictMode";
      case K:
        return "Suspense";
      case J:
        return "SuspenseList";
      case I:
        return "Activity";
    }
    if (typeof t == "object")
      switch (t.$$typeof) {
        case x:
          return "Portal";
        case X:
          return t.displayName || "Context";
        case N:
          return (t._context.displayName || "Context") + ".Consumer";
        case Y:
          var e = t.render;
          return (
            (t = t.displayName),
            t ||
              ((t = e.displayName || e.name || ""),
              (t = t !== "" ? "ForwardRef(" + t + ")" : "ForwardRef")),
            t
          );
        case q:
          return ((e = t.displayName || null), e !== null ? e : Nt(t.type) || "Memo");
        case Z:
          ((e = t._payload), (t = t._init));
          try {
            return Nt(t(e));
          } catch {}
      }
    return null;
  }
  var Gt = Array.isArray,
    j = l.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    F = s.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    it = { pending: !1, data: null, method: null, action: null },
    Et = [],
    Rt = -1;
  function C(t) {
    return { current: t };
  }
  function Q(t) {
    0 > Rt || ((t.current = Et[Rt]), (Et[Rt] = null), Rt--);
  }
  function k(t, e) {
    (Rt++, (Et[Rt] = t.current), (t.current = e));
  }
  var $ = C(null),
    nt = C(null),
    dt = C(null),
    vt = C(null);
  function Qt(t, e) {
    switch ((k(dt, e), k(nt, t), k($, null), e.nodeType)) {
      case 9:
      case 11:
        t = (t = e.documentElement) && (t = t.namespaceURI) ? ym(t) : 0;
        break;
      default:
        if (((t = e.tagName), (e = e.namespaceURI))) ((e = ym(e)), (t = pm(e, t)));
        else
          switch (t) {
            case "svg":
              t = 1;
              break;
            case "math":
              t = 2;
              break;
            default:
              t = 0;
          }
    }
    (Q($), k($, t));
  }
  function Ot() {
    (Q($), Q(nt), Q(dt));
  }
  function nn(t) {
    t.memoizedState !== null && k(vt, t);
    var e = $.current,
      a = pm(e, t.type);
    e !== a && (k(nt, t), k($, a));
  }
  function an(t) {
    (nt.current === t && (Q($), Q(nt)), vt.current === t && (Q(vt), (Al._currentValue = it)));
  }
  var On, Ui;
  function Fe(t) {
    if (On === void 0)
      try {
        throw Error();
      } catch (a) {
        var e = a.stack.trim().match(/\n( *(at )?)/);
        ((On = (e && e[1]) || ""),
          (Ui =
            -1 <
            a.stack.indexOf(`
    at`)
              ? " (<anonymous>)"
              : -1 < a.stack.indexOf("@")
                ? "@unknown:0:0"
                : ""));
      }
    return (
      `
` +
      On +
      t +
      Ui
    );
  }
  var Bi = !1;
  function qa(t, e) {
    if (!t || Bi) return "";
    Bi = !0;
    var a = Error.prepareStackTrace;
    Error.prepareStackTrace = void 0;
    try {
      var i = {
        DetermineComponentFrameRoot: function () {
          try {
            if (e) {
              var V = function () {
                throw Error();
              };
              if (
                (Object.defineProperty(V.prototype, "props", {
                  set: function () {
                    throw Error();
                  },
                }),
                typeof Reflect == "object" && Reflect.construct)
              ) {
                try {
                  Reflect.construct(V, []);
                } catch (B) {
                  var U = B;
                }
                Reflect.construct(t, [], V);
              } else {
                try {
                  V.call();
                } catch (B) {
                  U = B;
                }
                t.call(V.prototype);
              }
            } else {
              try {
                throw Error();
              } catch (B) {
                U = B;
              }
              (V = t()) && typeof V.catch == "function" && V.catch(function () {});
            }
          } catch (B) {
            if (B && U && typeof B.stack == "string") return [B.stack, U.stack];
          }
          return [null, null];
        },
      };
      i.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
      var r = Object.getOwnPropertyDescriptor(i.DetermineComponentFrameRoot, "name");
      r &&
        r.configurable &&
        Object.defineProperty(i.DetermineComponentFrameRoot, "name", {
          value: "DetermineComponentFrameRoot",
        });
      var o = i.DetermineComponentFrameRoot(),
        d = o[0],
        v = o[1];
      if (d && v) {
        var A = d.split(`
`),
          L = v.split(`
`);
        for (r = i = 0; i < A.length && !A[i].includes("DetermineComponentFrameRoot"); ) i++;
        for (; r < L.length && !L[r].includes("DetermineComponentFrameRoot"); ) r++;
        if (i === A.length || r === L.length)
          for (i = A.length - 1, r = L.length - 1; 1 <= i && 0 <= r && A[i] !== L[r]; ) r--;
        for (; 1 <= i && 0 <= r; i--, r--)
          if (A[i] !== L[r]) {
            if (i !== 1 || r !== 1)
              do
                if ((i--, r--, 0 > r || A[i] !== L[r])) {
                  var H =
                    `
` + A[i].replace(" at new ", " at ");
                  return (
                    t.displayName &&
                      H.includes("<anonymous>") &&
                      (H = H.replace("<anonymous>", t.displayName)),
                    H
                  );
                }
              while (1 <= i && 0 <= r);
            break;
          }
      }
    } finally {
      ((Bi = !1), (Error.prepareStackTrace = a));
    }
    return (a = t ? t.displayName || t.name : "") ? Fe(a) : "";
  }
  function Fl(t, e) {
    switch (t.tag) {
      case 26:
      case 27:
      case 5:
        return Fe(t.type);
      case 16:
        return Fe("Lazy");
      case 13:
        return t.child !== e && e !== null ? Fe("Suspense Fallback") : Fe("Suspense");
      case 19:
        return Fe("SuspenseList");
      case 0:
      case 15:
        return qa(t.type, !1);
      case 11:
        return qa(t.type.render, !1);
      case 1:
        return qa(t.type, !0);
      case 31:
        return Fe("Activity");
      default:
        return "";
    }
  }
  function ln(t) {
    try {
      var e = "",
        a = null;
      do ((e += Fl(t, a)), (a = t), (t = t.return));
      while (t);
      return e;
    } catch (i) {
      return (
        `
Error generating stack: ` +
        i.message +
        `
` +
        i.stack
      );
    }
  }
  var ca = Object.prototype.hasOwnProperty,
    Ve = n.unstable_scheduleCallback,
    Ni = n.unstable_cancelCallback,
    Jl = n.unstable_shouldYield,
    Gr = n.unstable_requestPaint,
    ce = n.unstable_now,
    Lt = n.unstable_getCurrentPriorityLevel,
    ae = n.unstable_ImmediatePriority,
    Je = n.unstable_UserBlockingPriority,
    Ya = n.unstable_NormalPriority,
    Rg = n.unstable_LowPriority,
    Fc = n.unstable_IdlePriority,
    Ag = n.log,
    xg = n.unstable_setDisableYieldValue,
    ji = null,
    Re = null;
  function zn(t) {
    if ((typeof Ag == "function" && xg(t), Re && typeof Re.setStrictMode == "function"))
      try {
        Re.setStrictMode(ji, t);
      } catch {}
  }
  var Ae = Math.clz32 ? Math.clz32 : wg,
    Mg = Math.log,
    Cg = Math.LN2;
  function wg(t) {
    return ((t >>>= 0), t === 0 ? 32 : (31 - ((Mg(t) / Cg) | 0)) | 0);
  }
  var kl = 256,
    Il = 262144,
    $l = 4194304;
  function fa(t) {
    var e = t & 42;
    if (e !== 0) return e;
    switch (t & -t) {
      case 1:
        return 1;
      case 2:
        return 2;
      case 4:
        return 4;
      case 8:
        return 8;
      case 16:
        return 16;
      case 32:
        return 32;
      case 64:
        return 64;
      case 128:
        return 128;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
        return t & 261888;
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return t & 3932160;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return t & 62914560;
      case 67108864:
        return 67108864;
      case 134217728:
        return 134217728;
      case 268435456:
        return 268435456;
      case 536870912:
        return 536870912;
      case 1073741824:
        return 0;
      default:
        return t;
    }
  }
  function Wl(t, e, a) {
    var i = t.pendingLanes;
    if (i === 0) return 0;
    var r = 0,
      o = t.suspendedLanes,
      d = t.pingedLanes;
    t = t.warmLanes;
    var v = i & 134217727;
    return (
      v !== 0
        ? ((i = v & ~o),
          i !== 0
            ? (r = fa(i))
            : ((d &= v), d !== 0 ? (r = fa(d)) : a || ((a = v & ~t), a !== 0 && (r = fa(a)))))
        : ((v = i & ~o),
          v !== 0
            ? (r = fa(v))
            : d !== 0
              ? (r = fa(d))
              : a || ((a = i & ~t), a !== 0 && (r = fa(a)))),
      r === 0
        ? 0
        : e !== 0 &&
            e !== r &&
            (e & o) === 0 &&
            ((o = r & -r), (a = e & -e), o >= a || (o === 32 && (a & 4194048) !== 0))
          ? e
          : r
    );
  }
  function Hi(t, e) {
    return (t.pendingLanes & ~(t.suspendedLanes & ~t.pingedLanes) & e) === 0;
  }
  function Og(t, e) {
    switch (t) {
      case 1:
      case 2:
      case 4:
      case 8:
      case 64:
        return e + 250;
      case 16:
      case 32:
      case 128:
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return e + 5e3;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return -1;
      case 67108864:
      case 134217728:
      case 268435456:
      case 536870912:
      case 1073741824:
        return -1;
      default:
        return -1;
    }
  }
  function Jc() {
    var t = $l;
    return (($l <<= 1), ($l & 62914560) === 0 && ($l = 4194304), t);
  }
  function Qr(t) {
    for (var e = [], a = 0; 31 > a; a++) e.push(t);
    return e;
  }
  function qi(t, e) {
    ((t.pendingLanes |= e),
      e !== 268435456 && ((t.suspendedLanes = 0), (t.pingedLanes = 0), (t.warmLanes = 0)));
  }
  function zg(t, e, a, i, r, o) {
    var d = t.pendingLanes;
    ((t.pendingLanes = a),
      (t.suspendedLanes = 0),
      (t.pingedLanes = 0),
      (t.warmLanes = 0),
      (t.expiredLanes &= a),
      (t.entangledLanes &= a),
      (t.errorRecoveryDisabledLanes &= a),
      (t.shellSuspendCounter = 0));
    var v = t.entanglements,
      A = t.expirationTimes,
      L = t.hiddenUpdates;
    for (a = d & ~a; 0 < a; ) {
      var H = 31 - Ae(a),
        V = 1 << H;
      ((v[H] = 0), (A[H] = -1));
      var U = L[H];
      if (U !== null)
        for (L[H] = null, H = 0; H < U.length; H++) {
          var B = U[H];
          B !== null && (B.lane &= -536870913);
        }
      a &= ~V;
    }
    (i !== 0 && kc(t, i, 0),
      o !== 0 && r === 0 && t.tag !== 0 && (t.suspendedLanes |= o & ~(d & ~e)));
  }
  function kc(t, e, a) {
    ((t.pendingLanes |= e), (t.suspendedLanes &= ~e));
    var i = 31 - Ae(e);
    ((t.entangledLanes |= e),
      (t.entanglements[i] = t.entanglements[i] | 1073741824 | (a & 261930)));
  }
  function Ic(t, e) {
    var a = (t.entangledLanes |= e);
    for (t = t.entanglements; a; ) {
      var i = 31 - Ae(a),
        r = 1 << i;
      ((r & e) | (t[i] & e) && (t[i] |= e), (a &= ~r));
    }
  }
  function $c(t, e) {
    var a = e & -e;
    return ((a = (a & 42) !== 0 ? 1 : Vr(a)), (a & (t.suspendedLanes | e)) !== 0 ? 0 : a);
  }
  function Vr(t) {
    switch (t) {
      case 2:
        t = 1;
        break;
      case 8:
        t = 4;
        break;
      case 32:
        t = 16;
        break;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        t = 128;
        break;
      case 268435456:
        t = 134217728;
        break;
      default:
        t = 0;
    }
    return t;
  }
  function Xr(t) {
    return ((t &= -t), 2 < t ? (8 < t ? ((t & 134217727) !== 0 ? 32 : 268435456) : 8) : 2);
  }
  function Wc() {
    var t = F.p;
    return t !== 0 ? t : ((t = window.event), t === void 0 ? 32 : qm(t.type));
  }
  function tf(t, e) {
    var a = F.p;
    try {
      return ((F.p = t), e());
    } finally {
      F.p = a;
    }
  }
  var Dn = Math.random().toString(36).slice(2),
    ie = "__reactFiber$" + Dn,
    ye = "__reactProps$" + Dn,
    Ga = "__reactContainer$" + Dn,
    Kr = "__reactEvents$" + Dn,
    Dg = "__reactListeners$" + Dn,
    Lg = "__reactHandles$" + Dn,
    ef = "__reactResources$" + Dn,
    Yi = "__reactMarker$" + Dn;
  function Zr(t) {
    (delete t[ie], delete t[ye], delete t[Kr], delete t[Dg], delete t[Lg]);
  }
  function Qa(t) {
    var e = t[ie];
    if (e) return e;
    for (var a = t.parentNode; a; ) {
      if ((e = a[Ga] || a[ie])) {
        if (((a = e.alternate), e.child !== null || (a !== null && a.child !== null)))
          for (t = Tm(t); t !== null; ) {
            if ((a = t[ie])) return a;
            t = Tm(t);
          }
        return e;
      }
      ((t = a), (a = t.parentNode));
    }
    return null;
  }
  function Va(t) {
    if ((t = t[ie] || t[Ga])) {
      var e = t.tag;
      if (e === 5 || e === 6 || e === 13 || e === 31 || e === 26 || e === 27 || e === 3) return t;
    }
    return null;
  }
  function Gi(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t.stateNode;
    throw Error(u(33));
  }
  function Xa(t) {
    var e = t[ef];
    return (e || (e = t[ef] = { hoistableStyles: new Map(), hoistableScripts: new Map() }), e);
  }
  function te(t) {
    t[Yi] = !0;
  }
  var nf = new Set(),
    af = {};
  function da(t, e) {
    (Ka(t, e), Ka(t + "Capture", e));
  }
  function Ka(t, e) {
    for (af[t] = e, t = 0; t < e.length; t++) nf.add(e[t]);
  }
  var Ug = RegExp(
      "^[:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD][:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040]*$",
    ),
    lf = {},
    sf = {};
  function Bg(t) {
    return ca.call(sf, t)
      ? !0
      : ca.call(lf, t)
        ? !1
        : Ug.test(t)
          ? (sf[t] = !0)
          : ((lf[t] = !0), !1);
  }
  function ts(t, e, a) {
    if (Bg(e))
      if (a === null) t.removeAttribute(e);
      else {
        switch (typeof a) {
          case "undefined":
          case "function":
          case "symbol":
            t.removeAttribute(e);
            return;
          case "boolean":
            var i = e.toLowerCase().slice(0, 5);
            if (i !== "data-" && i !== "aria-") {
              t.removeAttribute(e);
              return;
            }
        }
        t.setAttribute(e, "" + a);
      }
  }
  function es(t, e, a) {
    if (a === null) t.removeAttribute(e);
    else {
      switch (typeof a) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(e);
          return;
      }
      t.setAttribute(e, "" + a);
    }
  }
  function sn(t, e, a, i) {
    if (i === null) t.removeAttribute(a);
    else {
      switch (typeof i) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(a);
          return;
      }
      t.setAttributeNS(e, a, "" + i);
    }
  }
  function Le(t) {
    switch (typeof t) {
      case "bigint":
      case "boolean":
      case "number":
      case "string":
      case "undefined":
        return t;
      case "object":
        return t;
      default:
        return "";
    }
  }
  function rf(t) {
    var e = t.type;
    return (t = t.nodeName) && t.toLowerCase() === "input" && (e === "checkbox" || e === "radio");
  }
  function Ng(t, e, a) {
    var i = Object.getOwnPropertyDescriptor(t.constructor.prototype, e);
    if (
      !t.hasOwnProperty(e) &&
      typeof i < "u" &&
      typeof i.get == "function" &&
      typeof i.set == "function"
    ) {
      var r = i.get,
        o = i.set;
      return (
        Object.defineProperty(t, e, {
          configurable: !0,
          get: function () {
            return r.call(this);
          },
          set: function (d) {
            ((a = "" + d), o.call(this, d));
          },
        }),
        Object.defineProperty(t, e, { enumerable: i.enumerable }),
        {
          getValue: function () {
            return a;
          },
          setValue: function (d) {
            a = "" + d;
          },
          stopTracking: function () {
            ((t._valueTracker = null), delete t[e]);
          },
        }
      );
    }
  }
  function Pr(t) {
    if (!t._valueTracker) {
      var e = rf(t) ? "checked" : "value";
      t._valueTracker = Ng(t, e, "" + t[e]);
    }
  }
  function uf(t) {
    if (!t) return !1;
    var e = t._valueTracker;
    if (!e) return !0;
    var a = e.getValue(),
      i = "";
    return (
      t && (i = rf(t) ? (t.checked ? "true" : "false") : t.value),
      (t = i),
      t !== a ? (e.setValue(t), !0) : !1
    );
  }
  function ns(t) {
    if (((t = t || (typeof document < "u" ? document : void 0)), typeof t > "u")) return null;
    try {
      return t.activeElement || t.body;
    } catch {
      return t.body;
    }
  }
  var jg = /[\n"\\]/g;
  function Ue(t) {
    return t.replace(jg, function (e) {
      return "\\" + e.charCodeAt(0).toString(16) + " ";
    });
  }
  function Fr(t, e, a, i, r, o, d, v) {
    ((t.name = ""),
      d != null && typeof d != "function" && typeof d != "symbol" && typeof d != "boolean"
        ? (t.type = d)
        : t.removeAttribute("type"),
      e != null
        ? d === "number"
          ? ((e === 0 && t.value === "") || t.value != e) && (t.value = "" + Le(e))
          : t.value !== "" + Le(e) && (t.value = "" + Le(e))
        : (d !== "submit" && d !== "reset") || t.removeAttribute("value"),
      e != null
        ? Jr(t, d, Le(e))
        : a != null
          ? Jr(t, d, Le(a))
          : i != null && t.removeAttribute("value"),
      r == null && o != null && (t.defaultChecked = !!o),
      r != null && (t.checked = r && typeof r != "function" && typeof r != "symbol"),
      v != null && typeof v != "function" && typeof v != "symbol" && typeof v != "boolean"
        ? (t.name = "" + Le(v))
        : t.removeAttribute("name"));
  }
  function of(t, e, a, i, r, o, d, v) {
    if (
      (o != null &&
        typeof o != "function" &&
        typeof o != "symbol" &&
        typeof o != "boolean" &&
        (t.type = o),
      e != null || a != null)
    ) {
      if (!((o !== "submit" && o !== "reset") || e != null)) {
        Pr(t);
        return;
      }
      ((a = a != null ? "" + Le(a) : ""),
        (e = e != null ? "" + Le(e) : a),
        v || e === t.value || (t.value = e),
        (t.defaultValue = e));
    }
    ((i = i ?? r),
      (i = typeof i != "function" && typeof i != "symbol" && !!i),
      (t.checked = v ? t.checked : !!i),
      (t.defaultChecked = !!i),
      d != null &&
        typeof d != "function" &&
        typeof d != "symbol" &&
        typeof d != "boolean" &&
        (t.name = d),
      Pr(t));
  }
  function Jr(t, e, a) {
    (e === "number" && ns(t.ownerDocument) === t) ||
      t.defaultValue === "" + a ||
      (t.defaultValue = "" + a);
  }
  function Za(t, e, a, i) {
    if (((t = t.options), e)) {
      e = {};
      for (var r = 0; r < a.length; r++) e["$" + a[r]] = !0;
      for (a = 0; a < t.length; a++)
        ((r = e.hasOwnProperty("$" + t[a].value)),
          t[a].selected !== r && (t[a].selected = r),
          r && i && (t[a].defaultSelected = !0));
    } else {
      for (a = "" + Le(a), e = null, r = 0; r < t.length; r++) {
        if (t[r].value === a) {
          ((t[r].selected = !0), i && (t[r].defaultSelected = !0));
          return;
        }
        e !== null || t[r].disabled || (e = t[r]);
      }
      e !== null && (e.selected = !0);
    }
  }
  function cf(t, e, a) {
    if (e != null && ((e = "" + Le(e)), e !== t.value && (t.value = e), a == null)) {
      t.defaultValue !== e && (t.defaultValue = e);
      return;
    }
    t.defaultValue = a != null ? "" + Le(a) : "";
  }
  function ff(t, e, a, i) {
    if (e == null) {
      if (i != null) {
        if (a != null) throw Error(u(92));
        if (Gt(i)) {
          if (1 < i.length) throw Error(u(93));
          i = i[0];
        }
        a = i;
      }
      (a == null && (a = ""), (e = a));
    }
    ((a = Le(e)),
      (t.defaultValue = a),
      (i = t.textContent),
      i === a && i !== "" && i !== null && (t.value = i),
      Pr(t));
  }
  function Pa(t, e) {
    if (e) {
      var a = t.firstChild;
      if (a && a === t.lastChild && a.nodeType === 3) {
        a.nodeValue = e;
        return;
      }
    }
    t.textContent = e;
  }
  var Hg = new Set(
    "animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
      " ",
    ),
  );
  function df(t, e, a) {
    var i = e.indexOf("--") === 0;
    a == null || typeof a == "boolean" || a === ""
      ? i
        ? t.setProperty(e, "")
        : e === "float"
          ? (t.cssFloat = "")
          : (t[e] = "")
      : i
        ? t.setProperty(e, a)
        : typeof a != "number" || a === 0 || Hg.has(e)
          ? e === "float"
            ? (t.cssFloat = a)
            : (t[e] = ("" + a).trim())
          : (t[e] = a + "px");
  }
  function hf(t, e, a) {
    if (e != null && typeof e != "object") throw Error(u(62));
    if (((t = t.style), a != null)) {
      for (var i in a)
        !a.hasOwnProperty(i) ||
          (e != null && e.hasOwnProperty(i)) ||
          (i.indexOf("--") === 0
            ? t.setProperty(i, "")
            : i === "float"
              ? (t.cssFloat = "")
              : (t[i] = ""));
      for (var r in e) ((i = e[r]), e.hasOwnProperty(r) && a[r] !== i && df(t, r, i));
    } else for (var o in e) e.hasOwnProperty(o) && df(t, o, e[o]);
  }
  function kr(t) {
    if (t.indexOf("-") === -1) return !1;
    switch (t) {
      case "annotation-xml":
      case "color-profile":
      case "font-face":
      case "font-face-src":
      case "font-face-uri":
      case "font-face-format":
      case "font-face-name":
      case "missing-glyph":
        return !1;
      default:
        return !0;
    }
  }
  var qg = new Map([
      ["acceptCharset", "accept-charset"],
      ["htmlFor", "for"],
      ["httpEquiv", "http-equiv"],
      ["crossOrigin", "crossorigin"],
      ["accentHeight", "accent-height"],
      ["alignmentBaseline", "alignment-baseline"],
      ["arabicForm", "arabic-form"],
      ["baselineShift", "baseline-shift"],
      ["capHeight", "cap-height"],
      ["clipPath", "clip-path"],
      ["clipRule", "clip-rule"],
      ["colorInterpolation", "color-interpolation"],
      ["colorInterpolationFilters", "color-interpolation-filters"],
      ["colorProfile", "color-profile"],
      ["colorRendering", "color-rendering"],
      ["dominantBaseline", "dominant-baseline"],
      ["enableBackground", "enable-background"],
      ["fillOpacity", "fill-opacity"],
      ["fillRule", "fill-rule"],
      ["floodColor", "flood-color"],
      ["floodOpacity", "flood-opacity"],
      ["fontFamily", "font-family"],
      ["fontSize", "font-size"],
      ["fontSizeAdjust", "font-size-adjust"],
      ["fontStretch", "font-stretch"],
      ["fontStyle", "font-style"],
      ["fontVariant", "font-variant"],
      ["fontWeight", "font-weight"],
      ["glyphName", "glyph-name"],
      ["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
      ["glyphOrientationVertical", "glyph-orientation-vertical"],
      ["horizAdvX", "horiz-adv-x"],
      ["horizOriginX", "horiz-origin-x"],
      ["imageRendering", "image-rendering"],
      ["letterSpacing", "letter-spacing"],
      ["lightingColor", "lighting-color"],
      ["markerEnd", "marker-end"],
      ["markerMid", "marker-mid"],
      ["markerStart", "marker-start"],
      ["overlinePosition", "overline-position"],
      ["overlineThickness", "overline-thickness"],
      ["paintOrder", "paint-order"],
      ["panose-1", "panose-1"],
      ["pointerEvents", "pointer-events"],
      ["renderingIntent", "rendering-intent"],
      ["shapeRendering", "shape-rendering"],
      ["stopColor", "stop-color"],
      ["stopOpacity", "stop-opacity"],
      ["strikethroughPosition", "strikethrough-position"],
      ["strikethroughThickness", "strikethrough-thickness"],
      ["strokeDasharray", "stroke-dasharray"],
      ["strokeDashoffset", "stroke-dashoffset"],
      ["strokeLinecap", "stroke-linecap"],
      ["strokeLinejoin", "stroke-linejoin"],
      ["strokeMiterlimit", "stroke-miterlimit"],
      ["strokeOpacity", "stroke-opacity"],
      ["strokeWidth", "stroke-width"],
      ["textAnchor", "text-anchor"],
      ["textDecoration", "text-decoration"],
      ["textRendering", "text-rendering"],
      ["transformOrigin", "transform-origin"],
      ["underlinePosition", "underline-position"],
      ["underlineThickness", "underline-thickness"],
      ["unicodeBidi", "unicode-bidi"],
      ["unicodeRange", "unicode-range"],
      ["unitsPerEm", "units-per-em"],
      ["vAlphabetic", "v-alphabetic"],
      ["vHanging", "v-hanging"],
      ["vIdeographic", "v-ideographic"],
      ["vMathematical", "v-mathematical"],
      ["vectorEffect", "vector-effect"],
      ["vertAdvY", "vert-adv-y"],
      ["vertOriginX", "vert-origin-x"],
      ["vertOriginY", "vert-origin-y"],
      ["wordSpacing", "word-spacing"],
      ["writingMode", "writing-mode"],
      ["xmlnsXlink", "xmlns:xlink"],
      ["xHeight", "x-height"],
    ]),
    Yg =
      /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
  function as(t) {
    return Yg.test("" + t)
      ? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')"
      : t;
  }
  function rn() {}
  var Ir = null;
  function $r(t) {
    return (
      (t = t.target || t.srcElement || window),
      t.correspondingUseElement && (t = t.correspondingUseElement),
      t.nodeType === 3 ? t.parentNode : t
    );
  }
  var Fa = null,
    Ja = null;
  function mf(t) {
    var e = Va(t);
    if (e && (t = e.stateNode)) {
      var a = t[ye] || null;
      t: switch (((t = e.stateNode), e.type)) {
        case "input":
          if (
            (Fr(
              t,
              a.value,
              a.defaultValue,
              a.defaultValue,
              a.checked,
              a.defaultChecked,
              a.type,
              a.name,
            ),
            (e = a.name),
            a.type === "radio" && e != null)
          ) {
            for (a = t; a.parentNode; ) a = a.parentNode;
            for (
              a = a.querySelectorAll('input[name="' + Ue("" + e) + '"][type="radio"]'), e = 0;
              e < a.length;
              e++
            ) {
              var i = a[e];
              if (i !== t && i.form === t.form) {
                var r = i[ye] || null;
                if (!r) throw Error(u(90));
                Fr(
                  i,
                  r.value,
                  r.defaultValue,
                  r.defaultValue,
                  r.checked,
                  r.defaultChecked,
                  r.type,
                  r.name,
                );
              }
            }
            for (e = 0; e < a.length; e++) ((i = a[e]), i.form === t.form && uf(i));
          }
          break t;
        case "textarea":
          cf(t, a.value, a.defaultValue);
          break t;
        case "select":
          ((e = a.value), e != null && Za(t, !!a.multiple, e, !1));
      }
    }
  }
  var Wr = !1;
  function yf(t, e, a) {
    if (Wr) return t(e, a);
    Wr = !0;
    try {
      var i = t(e);
      return i;
    } finally {
      if (
        ((Wr = !1),
        (Fa !== null || Ja !== null) &&
          (Xs(), Fa && ((e = Fa), (t = Ja), (Ja = Fa = null), mf(e), t)))
      )
        for (e = 0; e < t.length; e++) mf(t[e]);
    }
  }
  function Qi(t, e) {
    var a = t.stateNode;
    if (a === null) return null;
    var i = a[ye] || null;
    if (i === null) return null;
    a = i[e];
    t: switch (e) {
      case "onClick":
      case "onClickCapture":
      case "onDoubleClick":
      case "onDoubleClickCapture":
      case "onMouseDown":
      case "onMouseDownCapture":
      case "onMouseMove":
      case "onMouseMoveCapture":
      case "onMouseUp":
      case "onMouseUpCapture":
      case "onMouseEnter":
        ((i = !i.disabled) ||
          ((t = t.type),
          (i = !(t === "button" || t === "input" || t === "select" || t === "textarea"))),
          (t = !i));
        break t;
      default:
        t = !1;
    }
    if (t) return null;
    if (a && typeof a != "function") throw Error(u(231, e, typeof a));
    return a;
  }
  var un = !(
      typeof window > "u" ||
      typeof window.document > "u" ||
      typeof window.document.createElement > "u"
    ),
    tu = !1;
  if (un)
    try {
      var Vi = {};
      (Object.defineProperty(Vi, "passive", {
        get: function () {
          tu = !0;
        },
      }),
        window.addEventListener("test", Vi, Vi),
        window.removeEventListener("test", Vi, Vi));
    } catch {
      tu = !1;
    }
  var Ln = null,
    eu = null,
    is = null;
  function pf() {
    if (is) return is;
    var t,
      e = eu,
      a = e.length,
      i,
      r = "value" in Ln ? Ln.value : Ln.textContent,
      o = r.length;
    for (t = 0; t < a && e[t] === r[t]; t++);
    var d = a - t;
    for (i = 1; i <= d && e[a - i] === r[o - i]; i++);
    return (is = r.slice(t, 1 < i ? 1 - i : void 0));
  }
  function ls(t) {
    var e = t.keyCode;
    return (
      "charCode" in t ? ((t = t.charCode), t === 0 && e === 13 && (t = 13)) : (t = e),
      t === 10 && (t = 13),
      32 <= t || t === 13 ? t : 0
    );
  }
  function ss() {
    return !0;
  }
  function gf() {
    return !1;
  }
  function pe(t) {
    function e(a, i, r, o, d) {
      ((this._reactName = a),
        (this._targetInst = r),
        (this.type = i),
        (this.nativeEvent = o),
        (this.target = d),
        (this.currentTarget = null));
      for (var v in t) t.hasOwnProperty(v) && ((a = t[v]), (this[v] = a ? a(o) : o[v]));
      return (
        (this.isDefaultPrevented = (
          o.defaultPrevented != null ? o.defaultPrevented : o.returnValue === !1
        )
          ? ss
          : gf),
        (this.isPropagationStopped = gf),
        this
      );
    }
    return (
      g(e.prototype, {
        preventDefault: function () {
          this.defaultPrevented = !0;
          var a = this.nativeEvent;
          a &&
            (a.preventDefault
              ? a.preventDefault()
              : typeof a.returnValue != "unknown" && (a.returnValue = !1),
            (this.isDefaultPrevented = ss));
        },
        stopPropagation: function () {
          var a = this.nativeEvent;
          a &&
            (a.stopPropagation
              ? a.stopPropagation()
              : typeof a.cancelBubble != "unknown" && (a.cancelBubble = !0),
            (this.isPropagationStopped = ss));
        },
        persist: function () {},
        isPersistent: ss,
      }),
      e
    );
  }
  var ha = {
      eventPhase: 0,
      bubbles: 0,
      cancelable: 0,
      timeStamp: function (t) {
        return t.timeStamp || Date.now();
      },
      defaultPrevented: 0,
      isTrusted: 0,
    },
    rs = pe(ha),
    Xi = g({}, ha, { view: 0, detail: 0 }),
    Gg = pe(Xi),
    nu,
    au,
    Ki,
    us = g({}, Xi, {
      screenX: 0,
      screenY: 0,
      clientX: 0,
      clientY: 0,
      pageX: 0,
      pageY: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      getModifierState: lu,
      button: 0,
      buttons: 0,
      relatedTarget: function (t) {
        return t.relatedTarget === void 0
          ? t.fromElement === t.srcElement
            ? t.toElement
            : t.fromElement
          : t.relatedTarget;
      },
      movementX: function (t) {
        return "movementX" in t
          ? t.movementX
          : (t !== Ki &&
              (Ki && t.type === "mousemove"
                ? ((nu = t.screenX - Ki.screenX), (au = t.screenY - Ki.screenY))
                : (au = nu = 0),
              (Ki = t)),
            nu);
      },
      movementY: function (t) {
        return "movementY" in t ? t.movementY : au;
      },
    }),
    vf = pe(us),
    Qg = g({}, us, { dataTransfer: 0 }),
    Vg = pe(Qg),
    Xg = g({}, Xi, { relatedTarget: 0 }),
    iu = pe(Xg),
    Kg = g({}, ha, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
    Zg = pe(Kg),
    Pg = g({}, ha, {
      clipboardData: function (t) {
        return "clipboardData" in t ? t.clipboardData : window.clipboardData;
      },
    }),
    Fg = pe(Pg),
    Jg = g({}, ha, { data: 0 }),
    Sf = pe(Jg),
    kg = {
      Esc: "Escape",
      Spacebar: " ",
      Left: "ArrowLeft",
      Up: "ArrowUp",
      Right: "ArrowRight",
      Down: "ArrowDown",
      Del: "Delete",
      Win: "OS",
      Menu: "ContextMenu",
      Apps: "ContextMenu",
      Scroll: "ScrollLock",
      MozPrintableKey: "Unidentified",
    },
    Ig = {
      8: "Backspace",
      9: "Tab",
      12: "Clear",
      13: "Enter",
      16: "Shift",
      17: "Control",
      18: "Alt",
      19: "Pause",
      20: "CapsLock",
      27: "Escape",
      32: " ",
      33: "PageUp",
      34: "PageDown",
      35: "End",
      36: "Home",
      37: "ArrowLeft",
      38: "ArrowUp",
      39: "ArrowRight",
      40: "ArrowDown",
      45: "Insert",
      46: "Delete",
      112: "F1",
      113: "F2",
      114: "F3",
      115: "F4",
      116: "F5",
      117: "F6",
      118: "F7",
      119: "F8",
      120: "F9",
      121: "F10",
      122: "F11",
      123: "F12",
      144: "NumLock",
      145: "ScrollLock",
      224: "Meta",
    },
    $g = { Alt: "altKey", Control: "ctrlKey", Meta: "metaKey", Shift: "shiftKey" };
  function Wg(t) {
    var e = this.nativeEvent;
    return e.getModifierState ? e.getModifierState(t) : (t = $g[t]) ? !!e[t] : !1;
  }
  function lu() {
    return Wg;
  }
  var tv = g({}, Xi, {
      key: function (t) {
        if (t.key) {
          var e = kg[t.key] || t.key;
          if (e !== "Unidentified") return e;
        }
        return t.type === "keypress"
          ? ((t = ls(t)), t === 13 ? "Enter" : String.fromCharCode(t))
          : t.type === "keydown" || t.type === "keyup"
            ? Ig[t.keyCode] || "Unidentified"
            : "";
      },
      code: 0,
      location: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      repeat: 0,
      locale: 0,
      getModifierState: lu,
      charCode: function (t) {
        return t.type === "keypress" ? ls(t) : 0;
      },
      keyCode: function (t) {
        return t.type === "keydown" || t.type === "keyup" ? t.keyCode : 0;
      },
      which: function (t) {
        return t.type === "keypress"
          ? ls(t)
          : t.type === "keydown" || t.type === "keyup"
            ? t.keyCode
            : 0;
      },
    }),
    ev = pe(tv),
    nv = g({}, us, {
      pointerId: 0,
      width: 0,
      height: 0,
      pressure: 0,
      tangentialPressure: 0,
      tiltX: 0,
      tiltY: 0,
      twist: 0,
      pointerType: 0,
      isPrimary: 0,
    }),
    bf = pe(nv),
    av = g({}, Xi, {
      touches: 0,
      targetTouches: 0,
      changedTouches: 0,
      altKey: 0,
      metaKey: 0,
      ctrlKey: 0,
      shiftKey: 0,
      getModifierState: lu,
    }),
    iv = pe(av),
    lv = g({}, ha, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
    sv = pe(lv),
    rv = g({}, us, {
      deltaX: function (t) {
        return "deltaX" in t ? t.deltaX : "wheelDeltaX" in t ? -t.wheelDeltaX : 0;
      },
      deltaY: function (t) {
        return "deltaY" in t
          ? t.deltaY
          : "wheelDeltaY" in t
            ? -t.wheelDeltaY
            : "wheelDelta" in t
              ? -t.wheelDelta
              : 0;
      },
      deltaZ: 0,
      deltaMode: 0,
    }),
    uv = pe(rv),
    ov = g({}, ha, { newState: 0, oldState: 0 }),
    cv = pe(ov),
    fv = [9, 13, 27, 32],
    su = un && "CompositionEvent" in window,
    Zi = null;
  un && "documentMode" in document && (Zi = document.documentMode);
  var dv = un && "TextEvent" in window && !Zi,
    _f = un && (!su || (Zi && 8 < Zi && 11 >= Zi)),
    Ef = " ",
    Tf = !1;
  function Rf(t, e) {
    switch (t) {
      case "keyup":
        return fv.indexOf(e.keyCode) !== -1;
      case "keydown":
        return e.keyCode !== 229;
      case "keypress":
      case "mousedown":
      case "focusout":
        return !0;
      default:
        return !1;
    }
  }
  function Af(t) {
    return ((t = t.detail), typeof t == "object" && "data" in t ? t.data : null);
  }
  var ka = !1;
  function hv(t, e) {
    switch (t) {
      case "compositionend":
        return Af(e);
      case "keypress":
        return e.which !== 32 ? null : ((Tf = !0), Ef);
      case "textInput":
        return ((t = e.data), t === Ef && Tf ? null : t);
      default:
        return null;
    }
  }
  function mv(t, e) {
    if (ka)
      return t === "compositionend" || (!su && Rf(t, e))
        ? ((t = pf()), (is = eu = Ln = null), (ka = !1), t)
        : null;
    switch (t) {
      case "paste":
        return null;
      case "keypress":
        if (!(e.ctrlKey || e.altKey || e.metaKey) || (e.ctrlKey && e.altKey)) {
          if (e.char && 1 < e.char.length) return e.char;
          if (e.which) return String.fromCharCode(e.which);
        }
        return null;
      case "compositionend":
        return _f && e.locale !== "ko" ? null : e.data;
      default:
        return null;
    }
  }
  var yv = {
    color: !0,
    date: !0,
    datetime: !0,
    "datetime-local": !0,
    email: !0,
    month: !0,
    number: !0,
    password: !0,
    range: !0,
    search: !0,
    tel: !0,
    text: !0,
    time: !0,
    url: !0,
    week: !0,
  };
  function xf(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return e === "input" ? !!yv[t.type] : e === "textarea";
  }
  function Mf(t, e, a, i) {
    (Fa ? (Ja ? Ja.push(i) : (Ja = [i])) : (Fa = i),
      (e = Is(e, "onChange")),
      0 < e.length &&
        ((a = new rs("onChange", "change", null, a, i)), t.push({ event: a, listeners: e })));
  }
  var Pi = null,
    Fi = null;
  function pv(t) {
    om(t, 0);
  }
  function os(t) {
    var e = Gi(t);
    if (uf(e)) return t;
  }
  function Cf(t, e) {
    if (t === "change") return e;
  }
  var wf = !1;
  if (un) {
    var ru;
    if (un) {
      var uu = "oninput" in document;
      if (!uu) {
        var Of = document.createElement("div");
        (Of.setAttribute("oninput", "return;"), (uu = typeof Of.oninput == "function"));
      }
      ru = uu;
    } else ru = !1;
    wf = ru && (!document.documentMode || 9 < document.documentMode);
  }
  function zf() {
    Pi && (Pi.detachEvent("onpropertychange", Df), (Fi = Pi = null));
  }
  function Df(t) {
    if (t.propertyName === "value" && os(Fi)) {
      var e = [];
      (Mf(e, Fi, t, $r(t)), yf(pv, e));
    }
  }
  function gv(t, e, a) {
    t === "focusin"
      ? (zf(), (Pi = e), (Fi = a), Pi.attachEvent("onpropertychange", Df))
      : t === "focusout" && zf();
  }
  function vv(t) {
    if (t === "selectionchange" || t === "keyup" || t === "keydown") return os(Fi);
  }
  function Sv(t, e) {
    if (t === "click") return os(e);
  }
  function bv(t, e) {
    if (t === "input" || t === "change") return os(e);
  }
  function _v(t, e) {
    return (t === e && (t !== 0 || 1 / t === 1 / e)) || (t !== t && e !== e);
  }
  var xe = typeof Object.is == "function" ? Object.is : _v;
  function Ji(t, e) {
    if (xe(t, e)) return !0;
    if (typeof t != "object" || t === null || typeof e != "object" || e === null) return !1;
    var a = Object.keys(t),
      i = Object.keys(e);
    if (a.length !== i.length) return !1;
    for (i = 0; i < a.length; i++) {
      var r = a[i];
      if (!ca.call(e, r) || !xe(t[r], e[r])) return !1;
    }
    return !0;
  }
  function Lf(t) {
    for (; t && t.firstChild; ) t = t.firstChild;
    return t;
  }
  function Uf(t, e) {
    var a = Lf(t);
    t = 0;
    for (var i; a; ) {
      if (a.nodeType === 3) {
        if (((i = t + a.textContent.length), t <= e && i >= e)) return { node: a, offset: e - t };
        t = i;
      }
      t: {
        for (; a; ) {
          if (a.nextSibling) {
            a = a.nextSibling;
            break t;
          }
          a = a.parentNode;
        }
        a = void 0;
      }
      a = Lf(a);
    }
  }
  function Bf(t, e) {
    return t && e
      ? t === e
        ? !0
        : t && t.nodeType === 3
          ? !1
          : e && e.nodeType === 3
            ? Bf(t, e.parentNode)
            : "contains" in t
              ? t.contains(e)
              : t.compareDocumentPosition
                ? !!(t.compareDocumentPosition(e) & 16)
                : !1
      : !1;
  }
  function Nf(t) {
    t =
      t != null && t.ownerDocument != null && t.ownerDocument.defaultView != null
        ? t.ownerDocument.defaultView
        : window;
    for (var e = ns(t.document); e instanceof t.HTMLIFrameElement; ) {
      try {
        var a = typeof e.contentWindow.location.href == "string";
      } catch {
        a = !1;
      }
      if (a) t = e.contentWindow;
      else break;
      e = ns(t.document);
    }
    return e;
  }
  function ou(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return (
      e &&
      ((e === "input" &&
        (t.type === "text" ||
          t.type === "search" ||
          t.type === "tel" ||
          t.type === "url" ||
          t.type === "password")) ||
        e === "textarea" ||
        t.contentEditable === "true")
    );
  }
  var Ev = un && "documentMode" in document && 11 >= document.documentMode,
    Ia = null,
    cu = null,
    ki = null,
    fu = !1;
  function jf(t, e, a) {
    var i = a.window === a ? a.document : a.nodeType === 9 ? a : a.ownerDocument;
    fu ||
      Ia == null ||
      Ia !== ns(i) ||
      ((i = Ia),
      "selectionStart" in i && ou(i)
        ? (i = { start: i.selectionStart, end: i.selectionEnd })
        : ((i = ((i.ownerDocument && i.ownerDocument.defaultView) || window).getSelection()),
          (i = {
            anchorNode: i.anchorNode,
            anchorOffset: i.anchorOffset,
            focusNode: i.focusNode,
            focusOffset: i.focusOffset,
          })),
      (ki && Ji(ki, i)) ||
        ((ki = i),
        (i = Is(cu, "onSelect")),
        0 < i.length &&
          ((e = new rs("onSelect", "select", null, e, a)),
          t.push({ event: e, listeners: i }),
          (e.target = Ia))));
  }
  function ma(t, e) {
    var a = {};
    return (
      (a[t.toLowerCase()] = e.toLowerCase()),
      (a["Webkit" + t] = "webkit" + e),
      (a["Moz" + t] = "moz" + e),
      a
    );
  }
  var $a = {
      animationend: ma("Animation", "AnimationEnd"),
      animationiteration: ma("Animation", "AnimationIteration"),
      animationstart: ma("Animation", "AnimationStart"),
      transitionrun: ma("Transition", "TransitionRun"),
      transitionstart: ma("Transition", "TransitionStart"),
      transitioncancel: ma("Transition", "TransitionCancel"),
      transitionend: ma("Transition", "TransitionEnd"),
    },
    du = {},
    Hf = {};
  un &&
    ((Hf = document.createElement("div").style),
    "AnimationEvent" in window ||
      (delete $a.animationend.animation,
      delete $a.animationiteration.animation,
      delete $a.animationstart.animation),
    "TransitionEvent" in window || delete $a.transitionend.transition);
  function ya(t) {
    if (du[t]) return du[t];
    if (!$a[t]) return t;
    var e = $a[t],
      a;
    for (a in e) if (e.hasOwnProperty(a) && a in Hf) return (du[t] = e[a]);
    return t;
  }
  var qf = ya("animationend"),
    Yf = ya("animationiteration"),
    Gf = ya("animationstart"),
    Tv = ya("transitionrun"),
    Rv = ya("transitionstart"),
    Av = ya("transitioncancel"),
    Qf = ya("transitionend"),
    Vf = new Map(),
    hu =
      "abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
        " ",
      );
  hu.push("scrollEnd");
  function Xe(t, e) {
    (Vf.set(t, e), da(e, [t]));
  }
  var cs =
      typeof reportError == "function"
        ? reportError
        : function (t) {
            if (typeof window == "object" && typeof window.ErrorEvent == "function") {
              var e = new window.ErrorEvent("error", {
                bubbles: !0,
                cancelable: !0,
                message:
                  typeof t == "object" && t !== null && typeof t.message == "string"
                    ? String(t.message)
                    : String(t),
                error: t,
              });
              if (!window.dispatchEvent(e)) return;
            } else if (typeof process == "object" && typeof process.emit == "function") {
              process.emit("uncaughtException", t);
              return;
            }
            console.error(t);
          },
    Be = [],
    Wa = 0,
    mu = 0;
  function fs() {
    for (var t = Wa, e = (mu = Wa = 0); e < t; ) {
      var a = Be[e];
      Be[e++] = null;
      var i = Be[e];
      Be[e++] = null;
      var r = Be[e];
      Be[e++] = null;
      var o = Be[e];
      if (((Be[e++] = null), i !== null && r !== null)) {
        var d = i.pending;
        (d === null ? (r.next = r) : ((r.next = d.next), (d.next = r)), (i.pending = r));
      }
      o !== 0 && Xf(a, r, o);
    }
  }
  function ds(t, e, a, i) {
    ((Be[Wa++] = t),
      (Be[Wa++] = e),
      (Be[Wa++] = a),
      (Be[Wa++] = i),
      (mu |= i),
      (t.lanes |= i),
      (t = t.alternate),
      t !== null && (t.lanes |= i));
  }
  function yu(t, e, a, i) {
    return (ds(t, e, a, i), hs(t));
  }
  function pa(t, e) {
    return (ds(t, null, null, e), hs(t));
  }
  function Xf(t, e, a) {
    t.lanes |= a;
    var i = t.alternate;
    i !== null && (i.lanes |= a);
    for (var r = !1, o = t.return; o !== null; )
      ((o.childLanes |= a),
        (i = o.alternate),
        i !== null && (i.childLanes |= a),
        o.tag === 22 && ((t = o.stateNode), t === null || t._visibility & 1 || (r = !0)),
        (t = o),
        (o = o.return));
    return t.tag === 3
      ? ((o = t.stateNode),
        r &&
          e !== null &&
          ((r = 31 - Ae(a)),
          (t = o.hiddenUpdates),
          (i = t[r]),
          i === null ? (t[r] = [e]) : i.push(e),
          (e.lane = a | 536870912)),
        o)
      : null;
  }
  function hs(t) {
    if (50 < vl) throw ((vl = 0), (Ao = null), Error(u(185)));
    for (var e = t.return; e !== null; ) ((t = e), (e = t.return));
    return t.tag === 3 ? t.stateNode : null;
  }
  var ti = {};
  function xv(t, e, a, i) {
    ((this.tag = t),
      (this.key = a),
      (this.sibling =
        this.child =
        this.return =
        this.stateNode =
        this.type =
        this.elementType =
          null),
      (this.index = 0),
      (this.refCleanup = this.ref = null),
      (this.pendingProps = e),
      (this.dependencies = this.memoizedState = this.updateQueue = this.memoizedProps = null),
      (this.mode = i),
      (this.subtreeFlags = this.flags = 0),
      (this.deletions = null),
      (this.childLanes = this.lanes = 0),
      (this.alternate = null));
  }
  function Me(t, e, a, i) {
    return new xv(t, e, a, i);
  }
  function pu(t) {
    return ((t = t.prototype), !(!t || !t.isReactComponent));
  }
  function on(t, e) {
    var a = t.alternate;
    return (
      a === null
        ? ((a = Me(t.tag, e, t.key, t.mode)),
          (a.elementType = t.elementType),
          (a.type = t.type),
          (a.stateNode = t.stateNode),
          (a.alternate = t),
          (t.alternate = a))
        : ((a.pendingProps = e),
          (a.type = t.type),
          (a.flags = 0),
          (a.subtreeFlags = 0),
          (a.deletions = null)),
      (a.flags = t.flags & 65011712),
      (a.childLanes = t.childLanes),
      (a.lanes = t.lanes),
      (a.child = t.child),
      (a.memoizedProps = t.memoizedProps),
      (a.memoizedState = t.memoizedState),
      (a.updateQueue = t.updateQueue),
      (e = t.dependencies),
      (a.dependencies = e === null ? null : { lanes: e.lanes, firstContext: e.firstContext }),
      (a.sibling = t.sibling),
      (a.index = t.index),
      (a.ref = t.ref),
      (a.refCleanup = t.refCleanup),
      a
    );
  }
  function Kf(t, e) {
    t.flags &= 65011714;
    var a = t.alternate;
    return (
      a === null
        ? ((t.childLanes = 0),
          (t.lanes = e),
          (t.child = null),
          (t.subtreeFlags = 0),
          (t.memoizedProps = null),
          (t.memoizedState = null),
          (t.updateQueue = null),
          (t.dependencies = null),
          (t.stateNode = null))
        : ((t.childLanes = a.childLanes),
          (t.lanes = a.lanes),
          (t.child = a.child),
          (t.subtreeFlags = 0),
          (t.deletions = null),
          (t.memoizedProps = a.memoizedProps),
          (t.memoizedState = a.memoizedState),
          (t.updateQueue = a.updateQueue),
          (t.type = a.type),
          (e = a.dependencies),
          (t.dependencies = e === null ? null : { lanes: e.lanes, firstContext: e.firstContext })),
      t
    );
  }
  function ms(t, e, a, i, r, o) {
    var d = 0;
    if (((i = t), typeof t == "function")) pu(t) && (d = 1);
    else if (typeof t == "string")
      d = z0(t, a, $.current) ? 26 : t === "html" || t === "head" || t === "body" ? 27 : 5;
    else
      t: switch (t) {
        case I:
          return ((t = Me(31, a, e, r)), (t.elementType = I), (t.lanes = o), t);
        case z:
          return ga(a.children, r, o, e);
        case R:
          ((d = 8), (r |= 24));
          break;
        case w:
          return ((t = Me(12, a, e, r | 2)), (t.elementType = w), (t.lanes = o), t);
        case K:
          return ((t = Me(13, a, e, r)), (t.elementType = K), (t.lanes = o), t);
        case J:
          return ((t = Me(19, a, e, r)), (t.elementType = J), (t.lanes = o), t);
        default:
          if (typeof t == "object" && t !== null)
            switch (t.$$typeof) {
              case X:
                d = 10;
                break t;
              case N:
                d = 9;
                break t;
              case Y:
                d = 11;
                break t;
              case q:
                d = 14;
                break t;
              case Z:
                ((d = 16), (i = null));
                break t;
            }
          ((d = 29), (a = Error(u(130, t === null ? "null" : typeof t, ""))), (i = null));
      }
    return ((e = Me(d, a, e, r)), (e.elementType = t), (e.type = i), (e.lanes = o), e);
  }
  function ga(t, e, a, i) {
    return ((t = Me(7, t, i, e)), (t.lanes = a), t);
  }
  function gu(t, e, a) {
    return ((t = Me(6, t, null, e)), (t.lanes = a), t);
  }
  function Zf(t) {
    var e = Me(18, null, null, 0);
    return ((e.stateNode = t), e);
  }
  function vu(t, e, a) {
    return (
      (e = Me(4, t.children !== null ? t.children : [], t.key, e)),
      (e.lanes = a),
      (e.stateNode = {
        containerInfo: t.containerInfo,
        pendingChildren: null,
        implementation: t.implementation,
      }),
      e
    );
  }
  var Pf = new WeakMap();
  function Ne(t, e) {
    if (typeof t == "object" && t !== null) {
      var a = Pf.get(t);
      return a !== void 0 ? a : ((e = { value: t, source: e, stack: ln(e) }), Pf.set(t, e), e);
    }
    return { value: t, source: e, stack: ln(e) };
  }
  var ei = [],
    ni = 0,
    ys = null,
    Ii = 0,
    je = [],
    He = 0,
    Un = null,
    ke = 1,
    Ie = "";
  function cn(t, e) {
    ((ei[ni++] = Ii), (ei[ni++] = ys), (ys = t), (Ii = e));
  }
  function Ff(t, e, a) {
    ((je[He++] = ke), (je[He++] = Ie), (je[He++] = Un), (Un = t));
    var i = ke;
    t = Ie;
    var r = 32 - Ae(i) - 1;
    ((i &= ~(1 << r)), (a += 1));
    var o = 32 - Ae(e) + r;
    if (30 < o) {
      var d = r - (r % 5);
      ((o = (i & ((1 << d) - 1)).toString(32)),
        (i >>= d),
        (r -= d),
        (ke = (1 << (32 - Ae(e) + r)) | (a << r) | i),
        (Ie = o + t));
    } else ((ke = (1 << o) | (a << r) | i), (Ie = t));
  }
  function Su(t) {
    t.return !== null && (cn(t, 1), Ff(t, 1, 0));
  }
  function bu(t) {
    for (; t === ys; ) ((ys = ei[--ni]), (ei[ni] = null), (Ii = ei[--ni]), (ei[ni] = null));
    for (; t === Un; )
      ((Un = je[--He]),
        (je[He] = null),
        (Ie = je[--He]),
        (je[He] = null),
        (ke = je[--He]),
        (je[He] = null));
  }
  function Jf(t, e) {
    ((je[He++] = ke), (je[He++] = Ie), (je[He++] = Un), (ke = e.id), (Ie = e.overflow), (Un = t));
  }
  var le = null,
    Ut = null,
    gt = !1,
    Bn = null,
    qe = !1,
    _u = Error(u(519));
  function Nn(t) {
    var e = Error(
      u(418, 1 < arguments.length && arguments[1] !== void 0 && arguments[1] ? "text" : "HTML", ""),
    );
    throw ($i(Ne(e, t)), _u);
  }
  function kf(t) {
    var e = t.stateNode,
      a = t.type,
      i = t.memoizedProps;
    switch (((e[ie] = t), (e[ye] = i), a)) {
      case "dialog":
        (mt("cancel", e), mt("close", e));
        break;
      case "iframe":
      case "object":
      case "embed":
        mt("load", e);
        break;
      case "video":
      case "audio":
        for (a = 0; a < bl.length; a++) mt(bl[a], e);
        break;
      case "source":
        mt("error", e);
        break;
      case "img":
      case "image":
      case "link":
        (mt("error", e), mt("load", e));
        break;
      case "details":
        mt("toggle", e);
        break;
      case "input":
        (mt("invalid", e),
          of(e, i.value, i.defaultValue, i.checked, i.defaultChecked, i.type, i.name, !0));
        break;
      case "select":
        mt("invalid", e);
        break;
      case "textarea":
        (mt("invalid", e), ff(e, i.value, i.defaultValue, i.children));
    }
    ((a = i.children),
      (typeof a != "string" && typeof a != "number" && typeof a != "bigint") ||
      e.textContent === "" + a ||
      i.suppressHydrationWarning === !0 ||
      hm(e.textContent, a)
        ? (i.popover != null && (mt("beforetoggle", e), mt("toggle", e)),
          i.onScroll != null && mt("scroll", e),
          i.onScrollEnd != null && mt("scrollend", e),
          i.onClick != null && (e.onclick = rn),
          (e = !0))
        : (e = !1),
      e || Nn(t, !0));
  }
  function If(t) {
    for (le = t.return; le; )
      switch (le.tag) {
        case 5:
        case 31:
        case 13:
          qe = !1;
          return;
        case 27:
        case 3:
          qe = !0;
          return;
        default:
          le = le.return;
      }
  }
  function ai(t) {
    if (t !== le) return !1;
    if (!gt) return (If(t), (gt = !0), !1);
    var e = t.tag,
      a;
    if (
      ((a = e !== 3 && e !== 27) &&
        ((a = e === 5) &&
          ((a = t.type), (a = !(a !== "form" && a !== "button") || Yo(t.type, t.memoizedProps))),
        (a = !a)),
      a && Ut && Nn(t),
      If(t),
      e === 13)
    ) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t)) throw Error(u(317));
      Ut = Em(t);
    } else if (e === 31) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t)) throw Error(u(317));
      Ut = Em(t);
    } else
      e === 27
        ? ((e = Ut), kn(t.type) ? ((t = Ko), (Ko = null), (Ut = t)) : (Ut = e))
        : (Ut = le ? Ge(t.stateNode.nextSibling) : null);
    return !0;
  }
  function va() {
    ((Ut = le = null), (gt = !1));
  }
  function Eu() {
    var t = Bn;
    return (t !== null && (be === null ? (be = t) : be.push.apply(be, t), (Bn = null)), t);
  }
  function $i(t) {
    Bn === null ? (Bn = [t]) : Bn.push(t);
  }
  var Tu = C(null),
    Sa = null,
    fn = null;
  function jn(t, e, a) {
    (k(Tu, e._currentValue), (e._currentValue = a));
  }
  function dn(t) {
    ((t._currentValue = Tu.current), Q(Tu));
  }
  function Ru(t, e, a) {
    for (; t !== null; ) {
      var i = t.alternate;
      if (
        ((t.childLanes & e) !== e
          ? ((t.childLanes |= e), i !== null && (i.childLanes |= e))
          : i !== null && (i.childLanes & e) !== e && (i.childLanes |= e),
        t === a)
      )
        break;
      t = t.return;
    }
  }
  function Au(t, e, a, i) {
    var r = t.child;
    for (r !== null && (r.return = t); r !== null; ) {
      var o = r.dependencies;
      if (o !== null) {
        var d = r.child;
        o = o.firstContext;
        t: for (; o !== null; ) {
          var v = o;
          o = r;
          for (var A = 0; A < e.length; A++)
            if (v.context === e[A]) {
              ((o.lanes |= a),
                (v = o.alternate),
                v !== null && (v.lanes |= a),
                Ru(o.return, a, t),
                i || (d = null));
              break t;
            }
          o = v.next;
        }
      } else if (r.tag === 18) {
        if (((d = r.return), d === null)) throw Error(u(341));
        ((d.lanes |= a), (o = d.alternate), o !== null && (o.lanes |= a), Ru(d, a, t), (d = null));
      } else d = r.child;
      if (d !== null) d.return = r;
      else
        for (d = r; d !== null; ) {
          if (d === t) {
            d = null;
            break;
          }
          if (((r = d.sibling), r !== null)) {
            ((r.return = d.return), (d = r));
            break;
          }
          d = d.return;
        }
      r = d;
    }
  }
  function ii(t, e, a, i) {
    t = null;
    for (var r = e, o = !1; r !== null; ) {
      if (!o) {
        if ((r.flags & 524288) !== 0) o = !0;
        else if ((r.flags & 262144) !== 0) break;
      }
      if (r.tag === 10) {
        var d = r.alternate;
        if (d === null) throw Error(u(387));
        if (((d = d.memoizedProps), d !== null)) {
          var v = r.type;
          xe(r.pendingProps.value, d.value) || (t !== null ? t.push(v) : (t = [v]));
        }
      } else if (r === vt.current) {
        if (((d = r.alternate), d === null)) throw Error(u(387));
        d.memoizedState.memoizedState !== r.memoizedState.memoizedState &&
          (t !== null ? t.push(Al) : (t = [Al]));
      }
      r = r.return;
    }
    (t !== null && Au(e, t, a, i), (e.flags |= 262144));
  }
  function ps(t) {
    for (t = t.firstContext; t !== null; ) {
      if (!xe(t.context._currentValue, t.memoizedValue)) return !0;
      t = t.next;
    }
    return !1;
  }
  function ba(t) {
    ((Sa = t), (fn = null), (t = t.dependencies), t !== null && (t.firstContext = null));
  }
  function se(t) {
    return $f(Sa, t);
  }
  function gs(t, e) {
    return (Sa === null && ba(t), $f(t, e));
  }
  function $f(t, e) {
    var a = e._currentValue;
    if (((e = { context: e, memoizedValue: a, next: null }), fn === null)) {
      if (t === null) throw Error(u(308));
      ((fn = e), (t.dependencies = { lanes: 0, firstContext: e }), (t.flags |= 524288));
    } else fn = fn.next = e;
    return a;
  }
  var Mv =
      typeof AbortController < "u"
        ? AbortController
        : function () {
            var t = [],
              e = (this.signal = {
                aborted: !1,
                addEventListener: function (a, i) {
                  t.push(i);
                },
              });
            this.abort = function () {
              ((e.aborted = !0),
                t.forEach(function (a) {
                  return a();
                }));
            };
          },
    Cv = n.unstable_scheduleCallback,
    wv = n.unstable_NormalPriority,
    Zt = {
      $$typeof: X,
      Consumer: null,
      Provider: null,
      _currentValue: null,
      _currentValue2: null,
      _threadCount: 0,
    };
  function xu() {
    return { controller: new Mv(), data: new Map(), refCount: 0 };
  }
  function Wi(t) {
    (t.refCount--,
      t.refCount === 0 &&
        Cv(wv, function () {
          t.controller.abort();
        }));
  }
  var tl = null,
    Mu = 0,
    li = 0,
    si = null;
  function Ov(t, e) {
    if (tl === null) {
      var a = (tl = []);
      ((Mu = 0),
        (li = zo()),
        (si = {
          status: "pending",
          value: void 0,
          then: function (i) {
            a.push(i);
          },
        }));
    }
    return (Mu++, e.then(Wf, Wf), e);
  }
  function Wf() {
    if (--Mu === 0 && tl !== null) {
      si !== null && (si.status = "fulfilled");
      var t = tl;
      ((tl = null), (li = 0), (si = null));
      for (var e = 0; e < t.length; e++) (0, t[e])();
    }
  }
  function zv(t, e) {
    var a = [],
      i = {
        status: "pending",
        value: null,
        reason: null,
        then: function (r) {
          a.push(r);
        },
      };
    return (
      t.then(
        function () {
          ((i.status = "fulfilled"), (i.value = e));
          for (var r = 0; r < a.length; r++) (0, a[r])(e);
        },
        function (r) {
          for (i.status = "rejected", i.reason = r, r = 0; r < a.length; r++) (0, a[r])(void 0);
        },
      ),
      i
    );
  }
  var td = j.S;
  j.S = function (t, e) {
    ((jh = ce()),
      typeof e == "object" && e !== null && typeof e.then == "function" && Ov(t, e),
      td !== null && td(t, e));
  };
  var _a = C(null);
  function Cu() {
    var t = _a.current;
    return t !== null ? t : zt.pooledCache;
  }
  function vs(t, e) {
    e === null ? k(_a, _a.current) : k(_a, e.pool);
  }
  function ed() {
    var t = Cu();
    return t === null ? null : { parent: Zt._currentValue, pool: t };
  }
  var ri = Error(u(460)),
    wu = Error(u(474)),
    Ss = Error(u(542)),
    bs = { then: function () {} };
  function nd(t) {
    return ((t = t.status), t === "fulfilled" || t === "rejected");
  }
  function ad(t, e, a) {
    switch (
      ((a = t[a]), a === void 0 ? t.push(e) : a !== e && (e.then(rn, rn), (e = a)), e.status)
    ) {
      case "fulfilled":
        return e.value;
      case "rejected":
        throw ((t = e.reason), ld(t), t);
      default:
        if (typeof e.status == "string") e.then(rn, rn);
        else {
          if (((t = zt), t !== null && 100 < t.shellSuspendCounter)) throw Error(u(482));
          ((t = e),
            (t.status = "pending"),
            t.then(
              function (i) {
                if (e.status === "pending") {
                  var r = e;
                  ((r.status = "fulfilled"), (r.value = i));
                }
              },
              function (i) {
                if (e.status === "pending") {
                  var r = e;
                  ((r.status = "rejected"), (r.reason = i));
                }
              },
            ));
        }
        switch (e.status) {
          case "fulfilled":
            return e.value;
          case "rejected":
            throw ((t = e.reason), ld(t), t);
        }
        throw ((Ta = e), ri);
    }
  }
  function Ea(t) {
    try {
      var e = t._init;
      return e(t._payload);
    } catch (a) {
      throw a !== null && typeof a == "object" && typeof a.then == "function" ? ((Ta = a), ri) : a;
    }
  }
  var Ta = null;
  function id() {
    if (Ta === null) throw Error(u(459));
    var t = Ta;
    return ((Ta = null), t);
  }
  function ld(t) {
    if (t === ri || t === Ss) throw Error(u(483));
  }
  var ui = null,
    el = 0;
  function _s(t) {
    var e = el;
    return ((el += 1), ui === null && (ui = []), ad(ui, t, e));
  }
  function nl(t, e) {
    ((e = e.props.ref), (t.ref = e !== void 0 ? e : null));
  }
  function Es(t, e) {
    throw e.$$typeof === _
      ? Error(u(525))
      : ((t = Object.prototype.toString.call(e)),
        Error(
          u(
            31,
            t === "[object Object]" ? "object with keys {" + Object.keys(e).join(", ") + "}" : t,
          ),
        ));
  }
  function sd(t) {
    function e(O, M) {
      if (t) {
        var D = O.deletions;
        D === null ? ((O.deletions = [M]), (O.flags |= 16)) : D.push(M);
      }
    }
    function a(O, M) {
      if (!t) return null;
      for (; M !== null; ) (e(O, M), (M = M.sibling));
      return null;
    }
    function i(O) {
      for (var M = new Map(); O !== null; )
        (O.key !== null ? M.set(O.key, O) : M.set(O.index, O), (O = O.sibling));
      return M;
    }
    function r(O, M) {
      return ((O = on(O, M)), (O.index = 0), (O.sibling = null), O);
    }
    function o(O, M, D) {
      return (
        (O.index = D),
        t
          ? ((D = O.alternate),
            D !== null
              ? ((D = D.index), D < M ? ((O.flags |= 67108866), M) : D)
              : ((O.flags |= 67108866), M))
          : ((O.flags |= 1048576), M)
      );
    }
    function d(O) {
      return (t && O.alternate === null && (O.flags |= 67108866), O);
    }
    function v(O, M, D, G) {
      return M === null || M.tag !== 6
        ? ((M = gu(D, O.mode, G)), (M.return = O), M)
        : ((M = r(M, D)), (M.return = O), M);
    }
    function A(O, M, D, G) {
      var at = D.type;
      return at === z
        ? H(O, M, D.props.children, G, D.key)
        : M !== null &&
            (M.elementType === at ||
              (typeof at == "object" && at !== null && at.$$typeof === Z && Ea(at) === M.type))
          ? ((M = r(M, D.props)), nl(M, D), (M.return = O), M)
          : ((M = ms(D.type, D.key, D.props, null, O.mode, G)), nl(M, D), (M.return = O), M);
    }
    function L(O, M, D, G) {
      return M === null ||
        M.tag !== 4 ||
        M.stateNode.containerInfo !== D.containerInfo ||
        M.stateNode.implementation !== D.implementation
        ? ((M = vu(D, O.mode, G)), (M.return = O), M)
        : ((M = r(M, D.children || [])), (M.return = O), M);
    }
    function H(O, M, D, G, at) {
      return M === null || M.tag !== 7
        ? ((M = ga(D, O.mode, G, at)), (M.return = O), M)
        : ((M = r(M, D)), (M.return = O), M);
    }
    function V(O, M, D) {
      if ((typeof M == "string" && M !== "") || typeof M == "number" || typeof M == "bigint")
        return ((M = gu("" + M, O.mode, D)), (M.return = O), M);
      if (typeof M == "object" && M !== null) {
        switch (M.$$typeof) {
          case E:
            return ((D = ms(M.type, M.key, M.props, null, O.mode, D)), nl(D, M), (D.return = O), D);
          case x:
            return ((M = vu(M, O.mode, D)), (M.return = O), M);
          case Z:
            return ((M = Ea(M)), V(O, M, D));
        }
        if (Gt(M) || lt(M)) return ((M = ga(M, O.mode, D, null)), (M.return = O), M);
        if (typeof M.then == "function") return V(O, _s(M), D);
        if (M.$$typeof === X) return V(O, gs(O, M), D);
        Es(O, M);
      }
      return null;
    }
    function U(O, M, D, G) {
      var at = M !== null ? M.key : null;
      if ((typeof D == "string" && D !== "") || typeof D == "number" || typeof D == "bigint")
        return at !== null ? null : v(O, M, "" + D, G);
      if (typeof D == "object" && D !== null) {
        switch (D.$$typeof) {
          case E:
            return D.key === at ? A(O, M, D, G) : null;
          case x:
            return D.key === at ? L(O, M, D, G) : null;
          case Z:
            return ((D = Ea(D)), U(O, M, D, G));
        }
        if (Gt(D) || lt(D)) return at !== null ? null : H(O, M, D, G, null);
        if (typeof D.then == "function") return U(O, M, _s(D), G);
        if (D.$$typeof === X) return U(O, M, gs(O, D), G);
        Es(O, D);
      }
      return null;
    }
    function B(O, M, D, G, at) {
      if ((typeof G == "string" && G !== "") || typeof G == "number" || typeof G == "bigint")
        return ((O = O.get(D) || null), v(M, O, "" + G, at));
      if (typeof G == "object" && G !== null) {
        switch (G.$$typeof) {
          case E:
            return ((O = O.get(G.key === null ? D : G.key) || null), A(M, O, G, at));
          case x:
            return ((O = O.get(G.key === null ? D : G.key) || null), L(M, O, G, at));
          case Z:
            return ((G = Ea(G)), B(O, M, D, G, at));
        }
        if (Gt(G) || lt(G)) return ((O = O.get(D) || null), H(M, O, G, at, null));
        if (typeof G.then == "function") return B(O, M, D, _s(G), at);
        if (G.$$typeof === X) return B(O, M, D, gs(M, G), at);
        Es(M, G);
      }
      return null;
    }
    function W(O, M, D, G) {
      for (
        var at = null, St = null, et = M, ft = (M = 0), pt = null;
        et !== null && ft < D.length;
        ft++
      ) {
        et.index > ft ? ((pt = et), (et = null)) : (pt = et.sibling);
        var bt = U(O, et, D[ft], G);
        if (bt === null) {
          et === null && (et = pt);
          break;
        }
        (t && et && bt.alternate === null && e(O, et),
          (M = o(bt, M, ft)),
          St === null ? (at = bt) : (St.sibling = bt),
          (St = bt),
          (et = pt));
      }
      if (ft === D.length) return (a(O, et), gt && cn(O, ft), at);
      if (et === null) {
        for (; ft < D.length; ft++)
          ((et = V(O, D[ft], G)),
            et !== null &&
              ((M = o(et, M, ft)), St === null ? (at = et) : (St.sibling = et), (St = et)));
        return (gt && cn(O, ft), at);
      }
      for (et = i(et); ft < D.length; ft++)
        ((pt = B(et, O, ft, D[ft], G)),
          pt !== null &&
            (t && pt.alternate !== null && et.delete(pt.key === null ? ft : pt.key),
            (M = o(pt, M, ft)),
            St === null ? (at = pt) : (St.sibling = pt),
            (St = pt)));
      return (
        t &&
          et.forEach(function (ea) {
            return e(O, ea);
          }),
        gt && cn(O, ft),
        at
      );
    }
    function rt(O, M, D, G) {
      if (D == null) throw Error(u(151));
      for (
        var at = null, St = null, et = M, ft = (M = 0), pt = null, bt = D.next();
        et !== null && !bt.done;
        ft++, bt = D.next()
      ) {
        et.index > ft ? ((pt = et), (et = null)) : (pt = et.sibling);
        var ea = U(O, et, bt.value, G);
        if (ea === null) {
          et === null && (et = pt);
          break;
        }
        (t && et && ea.alternate === null && e(O, et),
          (M = o(ea, M, ft)),
          St === null ? (at = ea) : (St.sibling = ea),
          (St = ea),
          (et = pt));
      }
      if (bt.done) return (a(O, et), gt && cn(O, ft), at);
      if (et === null) {
        for (; !bt.done; ft++, bt = D.next())
          ((bt = V(O, bt.value, G)),
            bt !== null &&
              ((M = o(bt, M, ft)), St === null ? (at = bt) : (St.sibling = bt), (St = bt)));
        return (gt && cn(O, ft), at);
      }
      for (et = i(et); !bt.done; ft++, bt = D.next())
        ((bt = B(et, O, ft, bt.value, G)),
          bt !== null &&
            (t && bt.alternate !== null && et.delete(bt.key === null ? ft : bt.key),
            (M = o(bt, M, ft)),
            St === null ? (at = bt) : (St.sibling = bt),
            (St = bt)));
      return (
        t &&
          et.forEach(function (Q0) {
            return e(O, Q0);
          }),
        gt && cn(O, ft),
        at
      );
    }
    function wt(O, M, D, G) {
      if (
        (typeof D == "object" &&
          D !== null &&
          D.type === z &&
          D.key === null &&
          (D = D.props.children),
        typeof D == "object" && D !== null)
      ) {
        switch (D.$$typeof) {
          case E:
            t: {
              for (var at = D.key; M !== null; ) {
                if (M.key === at) {
                  if (((at = D.type), at === z)) {
                    if (M.tag === 7) {
                      (a(O, M.sibling), (G = r(M, D.props.children)), (G.return = O), (O = G));
                      break t;
                    }
                  } else if (
                    M.elementType === at ||
                    (typeof at == "object" && at !== null && at.$$typeof === Z && Ea(at) === M.type)
                  ) {
                    (a(O, M.sibling), (G = r(M, D.props)), nl(G, D), (G.return = O), (O = G));
                    break t;
                  }
                  a(O, M);
                  break;
                } else e(O, M);
                M = M.sibling;
              }
              D.type === z
                ? ((G = ga(D.props.children, O.mode, G, D.key)), (G.return = O), (O = G))
                : ((G = ms(D.type, D.key, D.props, null, O.mode, G)),
                  nl(G, D),
                  (G.return = O),
                  (O = G));
            }
            return d(O);
          case x:
            t: {
              for (at = D.key; M !== null; ) {
                if (M.key === at)
                  if (
                    M.tag === 4 &&
                    M.stateNode.containerInfo === D.containerInfo &&
                    M.stateNode.implementation === D.implementation
                  ) {
                    (a(O, M.sibling), (G = r(M, D.children || [])), (G.return = O), (O = G));
                    break t;
                  } else {
                    a(O, M);
                    break;
                  }
                else e(O, M);
                M = M.sibling;
              }
              ((G = vu(D, O.mode, G)), (G.return = O), (O = G));
            }
            return d(O);
          case Z:
            return ((D = Ea(D)), wt(O, M, D, G));
        }
        if (Gt(D)) return W(O, M, D, G);
        if (lt(D)) {
          if (((at = lt(D)), typeof at != "function")) throw Error(u(150));
          return ((D = at.call(D)), rt(O, M, D, G));
        }
        if (typeof D.then == "function") return wt(O, M, _s(D), G);
        if (D.$$typeof === X) return wt(O, M, gs(O, D), G);
        Es(O, D);
      }
      return (typeof D == "string" && D !== "") || typeof D == "number" || typeof D == "bigint"
        ? ((D = "" + D),
          M !== null && M.tag === 6
            ? (a(O, M.sibling), (G = r(M, D)), (G.return = O), (O = G))
            : (a(O, M), (G = gu(D, O.mode, G)), (G.return = O), (O = G)),
          d(O))
        : a(O, M);
    }
    return function (O, M, D, G) {
      try {
        el = 0;
        var at = wt(O, M, D, G);
        return ((ui = null), at);
      } catch (et) {
        if (et === ri || et === Ss) throw et;
        var St = Me(29, et, null, O.mode);
        return ((St.lanes = G), (St.return = O), St);
      }
    };
  }
  var Ra = sd(!0),
    rd = sd(!1),
    Hn = !1;
  function Ou(t) {
    t.updateQueue = {
      baseState: t.memoizedState,
      firstBaseUpdate: null,
      lastBaseUpdate: null,
      shared: { pending: null, lanes: 0, hiddenCallbacks: null },
      callbacks: null,
    };
  }
  function zu(t, e) {
    ((t = t.updateQueue),
      e.updateQueue === t &&
        (e.updateQueue = {
          baseState: t.baseState,
          firstBaseUpdate: t.firstBaseUpdate,
          lastBaseUpdate: t.lastBaseUpdate,
          shared: t.shared,
          callbacks: null,
        }));
  }
  function qn(t) {
    return { lane: t, tag: 0, payload: null, callback: null, next: null };
  }
  function Yn(t, e, a) {
    var i = t.updateQueue;
    if (i === null) return null;
    if (((i = i.shared), (Tt & 2) !== 0)) {
      var r = i.pending;
      return (
        r === null ? (e.next = e) : ((e.next = r.next), (r.next = e)),
        (i.pending = e),
        (e = hs(t)),
        Xf(t, null, a),
        e
      );
    }
    return (ds(t, i, e, a), hs(t));
  }
  function al(t, e, a) {
    if (((e = e.updateQueue), e !== null && ((e = e.shared), (a & 4194048) !== 0))) {
      var i = e.lanes;
      ((i &= t.pendingLanes), (a |= i), (e.lanes = a), Ic(t, a));
    }
  }
  function Du(t, e) {
    var a = t.updateQueue,
      i = t.alternate;
    if (i !== null && ((i = i.updateQueue), a === i)) {
      var r = null,
        o = null;
      if (((a = a.firstBaseUpdate), a !== null)) {
        do {
          var d = { lane: a.lane, tag: a.tag, payload: a.payload, callback: null, next: null };
          (o === null ? (r = o = d) : (o = o.next = d), (a = a.next));
        } while (a !== null);
        o === null ? (r = o = e) : (o = o.next = e);
      } else r = o = e;
      ((a = {
        baseState: i.baseState,
        firstBaseUpdate: r,
        lastBaseUpdate: o,
        shared: i.shared,
        callbacks: i.callbacks,
      }),
        (t.updateQueue = a));
      return;
    }
    ((t = a.lastBaseUpdate),
      t === null ? (a.firstBaseUpdate = e) : (t.next = e),
      (a.lastBaseUpdate = e));
  }
  var Lu = !1;
  function il() {
    if (Lu) {
      var t = si;
      if (t !== null) throw t;
    }
  }
  function ll(t, e, a, i) {
    Lu = !1;
    var r = t.updateQueue;
    Hn = !1;
    var o = r.firstBaseUpdate,
      d = r.lastBaseUpdate,
      v = r.shared.pending;
    if (v !== null) {
      r.shared.pending = null;
      var A = v,
        L = A.next;
      ((A.next = null), d === null ? (o = L) : (d.next = L), (d = A));
      var H = t.alternate;
      H !== null &&
        ((H = H.updateQueue),
        (v = H.lastBaseUpdate),
        v !== d && (v === null ? (H.firstBaseUpdate = L) : (v.next = L), (H.lastBaseUpdate = A)));
    }
    if (o !== null) {
      var V = r.baseState;
      ((d = 0), (H = L = A = null), (v = o));
      do {
        var U = v.lane & -536870913,
          B = U !== v.lane;
        if (B ? (yt & U) === U : (i & U) === U) {
          (U !== 0 && U === li && (Lu = !0),
            H !== null &&
              (H = H.next =
                { lane: 0, tag: v.tag, payload: v.payload, callback: null, next: null }));
          t: {
            var W = t,
              rt = v;
            U = e;
            var wt = a;
            switch (rt.tag) {
              case 1:
                if (((W = rt.payload), typeof W == "function")) {
                  V = W.call(wt, V, U);
                  break t;
                }
                V = W;
                break t;
              case 3:
                W.flags = (W.flags & -65537) | 128;
              case 0:
                if (
                  ((W = rt.payload), (U = typeof W == "function" ? W.call(wt, V, U) : W), U == null)
                )
                  break t;
                V = g({}, V, U);
                break t;
              case 2:
                Hn = !0;
            }
          }
          ((U = v.callback),
            U !== null &&
              ((t.flags |= 64),
              B && (t.flags |= 8192),
              (B = r.callbacks),
              B === null ? (r.callbacks = [U]) : B.push(U)));
        } else
          ((B = { lane: U, tag: v.tag, payload: v.payload, callback: v.callback, next: null }),
            H === null ? ((L = H = B), (A = V)) : (H = H.next = B),
            (d |= U));
        if (((v = v.next), v === null)) {
          if (((v = r.shared.pending), v === null)) break;
          ((B = v),
            (v = B.next),
            (B.next = null),
            (r.lastBaseUpdate = B),
            (r.shared.pending = null));
        }
      } while (!0);
      (H === null && (A = V),
        (r.baseState = A),
        (r.firstBaseUpdate = L),
        (r.lastBaseUpdate = H),
        o === null && (r.shared.lanes = 0),
        (Kn |= d),
        (t.lanes = d),
        (t.memoizedState = V));
    }
  }
  function ud(t, e) {
    if (typeof t != "function") throw Error(u(191, t));
    t.call(e);
  }
  function od(t, e) {
    var a = t.callbacks;
    if (a !== null) for (t.callbacks = null, t = 0; t < a.length; t++) ud(a[t], e);
  }
  var oi = C(null),
    Ts = C(0);
  function cd(t, e) {
    ((t = _n), k(Ts, t), k(oi, e), (_n = t | e.baseLanes));
  }
  function Uu() {
    (k(Ts, _n), k(oi, oi.current));
  }
  function Bu() {
    ((_n = Ts.current), Q(oi), Q(Ts));
  }
  var Ce = C(null),
    Ye = null;
  function Gn(t) {
    var e = t.alternate;
    (k(Vt, Vt.current & 1),
      k(Ce, t),
      Ye === null && (e === null || oi.current !== null || e.memoizedState !== null) && (Ye = t));
  }
  function Nu(t) {
    (k(Vt, Vt.current), k(Ce, t), Ye === null && (Ye = t));
  }
  function fd(t) {
    t.tag === 22 ? (k(Vt, Vt.current), k(Ce, t), Ye === null && (Ye = t)) : Qn();
  }
  function Qn() {
    (k(Vt, Vt.current), k(Ce, Ce.current));
  }
  function we(t) {
    (Q(Ce), Ye === t && (Ye = null), Q(Vt));
  }
  var Vt = C(0);
  function Rs(t) {
    for (var e = t; e !== null; ) {
      if (e.tag === 13) {
        var a = e.memoizedState;
        if (a !== null && ((a = a.dehydrated), a === null || Vo(a) || Xo(a))) return e;
      } else if (
        e.tag === 19 &&
        (e.memoizedProps.revealOrder === "forwards" ||
          e.memoizedProps.revealOrder === "backwards" ||
          e.memoizedProps.revealOrder === "unstable_legacy-backwards" ||
          e.memoizedProps.revealOrder === "together")
      ) {
        if ((e.flags & 128) !== 0) return e;
      } else if (e.child !== null) {
        ((e.child.return = e), (e = e.child));
        continue;
      }
      if (e === t) break;
      for (; e.sibling === null; ) {
        if (e.return === null || e.return === t) return null;
        e = e.return;
      }
      ((e.sibling.return = e.return), (e = e.sibling));
    }
    return null;
  }
  var hn = 0,
    ct = null,
    Mt = null,
    Pt = null,
    As = !1,
    ci = !1,
    Aa = !1,
    xs = 0,
    sl = 0,
    fi = null,
    Dv = 0;
  function qt() {
    throw Error(u(321));
  }
  function ju(t, e) {
    if (e === null) return !1;
    for (var a = 0; a < e.length && a < t.length; a++) if (!xe(t[a], e[a])) return !1;
    return !0;
  }
  function Hu(t, e, a, i, r, o) {
    return (
      (hn = o),
      (ct = e),
      (e.memoizedState = null),
      (e.updateQueue = null),
      (e.lanes = 0),
      (j.H = t === null || t.memoizedState === null ? Fd : Wu),
      (Aa = !1),
      (o = a(i, r)),
      (Aa = !1),
      ci && (o = hd(e, a, i, r)),
      dd(t),
      o
    );
  }
  function dd(t) {
    j.H = ol;
    var e = Mt !== null && Mt.next !== null;
    if (((hn = 0), (Pt = Mt = ct = null), (As = !1), (sl = 0), (fi = null), e)) throw Error(u(300));
    t === null || Ft || ((t = t.dependencies), t !== null && ps(t) && (Ft = !0));
  }
  function hd(t, e, a, i) {
    ct = t;
    var r = 0;
    do {
      if ((ci && (fi = null), (sl = 0), (ci = !1), 25 <= r)) throw Error(u(301));
      if (((r += 1), (Pt = Mt = null), t.updateQueue != null)) {
        var o = t.updateQueue;
        ((o.lastEffect = null),
          (o.events = null),
          (o.stores = null),
          o.memoCache != null && (o.memoCache.index = 0));
      }
      ((j.H = Jd), (o = e(a, i)));
    } while (ci);
    return o;
  }
  function Lv() {
    var t = j.H,
      e = t.useState()[0];
    return (
      (e = typeof e.then == "function" ? rl(e) : e),
      (t = t.useState()[0]),
      (Mt !== null ? Mt.memoizedState : null) !== t && (ct.flags |= 1024),
      e
    );
  }
  function qu() {
    var t = xs !== 0;
    return ((xs = 0), t);
  }
  function Yu(t, e, a) {
    ((e.updateQueue = t.updateQueue), (e.flags &= -2053), (t.lanes &= ~a));
  }
  function Gu(t) {
    if (As) {
      for (t = t.memoizedState; t !== null; ) {
        var e = t.queue;
        (e !== null && (e.pending = null), (t = t.next));
      }
      As = !1;
    }
    ((hn = 0), (Pt = Mt = ct = null), (ci = !1), (sl = xs = 0), (fi = null));
  }
  function de() {
    var t = { memoizedState: null, baseState: null, baseQueue: null, queue: null, next: null };
    return (Pt === null ? (ct.memoizedState = Pt = t) : (Pt = Pt.next = t), Pt);
  }
  function Xt() {
    if (Mt === null) {
      var t = ct.alternate;
      t = t !== null ? t.memoizedState : null;
    } else t = Mt.next;
    var e = Pt === null ? ct.memoizedState : Pt.next;
    if (e !== null) ((Pt = e), (Mt = t));
    else {
      if (t === null) throw ct.alternate === null ? Error(u(467)) : Error(u(310));
      ((Mt = t),
        (t = {
          memoizedState: Mt.memoizedState,
          baseState: Mt.baseState,
          baseQueue: Mt.baseQueue,
          queue: Mt.queue,
          next: null,
        }),
        Pt === null ? (ct.memoizedState = Pt = t) : (Pt = Pt.next = t));
    }
    return Pt;
  }
  function Ms() {
    return { lastEffect: null, events: null, stores: null, memoCache: null };
  }
  function rl(t) {
    var e = sl;
    return (
      (sl += 1),
      fi === null && (fi = []),
      (t = ad(fi, t, e)),
      (e = ct),
      (Pt === null ? e.memoizedState : Pt.next) === null &&
        ((e = e.alternate), (j.H = e === null || e.memoizedState === null ? Fd : Wu)),
      t
    );
  }
  function Cs(t) {
    if (t !== null && typeof t == "object") {
      if (typeof t.then == "function") return rl(t);
      if (t.$$typeof === X) return se(t);
    }
    throw Error(u(438, String(t)));
  }
  function Qu(t) {
    var e = null,
      a = ct.updateQueue;
    if ((a !== null && (e = a.memoCache), e == null)) {
      var i = ct.alternate;
      i !== null &&
        ((i = i.updateQueue),
        i !== null &&
          ((i = i.memoCache),
          i != null &&
            (e = {
              data: i.data.map(function (r) {
                return r.slice();
              }),
              index: 0,
            })));
    }
    if (
      (e == null && (e = { data: [], index: 0 }),
      a === null && ((a = Ms()), (ct.updateQueue = a)),
      (a.memoCache = e),
      (a = e.data[e.index]),
      a === void 0)
    )
      for (a = e.data[e.index] = Array(t), i = 0; i < t; i++) a[i] = ut;
    return (e.index++, a);
  }
  function mn(t, e) {
    return typeof e == "function" ? e(t) : e;
  }
  function ws(t) {
    var e = Xt();
    return Vu(e, Mt, t);
  }
  function Vu(t, e, a) {
    var i = t.queue;
    if (i === null) throw Error(u(311));
    i.lastRenderedReducer = a;
    var r = t.baseQueue,
      o = i.pending;
    if (o !== null) {
      if (r !== null) {
        var d = r.next;
        ((r.next = o.next), (o.next = d));
      }
      ((e.baseQueue = r = o), (i.pending = null));
    }
    if (((o = t.baseState), r === null)) t.memoizedState = o;
    else {
      e = r.next;
      var v = (d = null),
        A = null,
        L = e,
        H = !1;
      do {
        var V = L.lane & -536870913;
        if (V !== L.lane ? (yt & V) === V : (hn & V) === V) {
          var U = L.revertLane;
          if (U === 0)
            (A !== null &&
              (A = A.next =
                {
                  lane: 0,
                  revertLane: 0,
                  gesture: null,
                  action: L.action,
                  hasEagerState: L.hasEagerState,
                  eagerState: L.eagerState,
                  next: null,
                }),
              V === li && (H = !0));
          else if ((hn & U) === U) {
            ((L = L.next), U === li && (H = !0));
            continue;
          } else
            ((V = {
              lane: 0,
              revertLane: L.revertLane,
              gesture: null,
              action: L.action,
              hasEagerState: L.hasEagerState,
              eagerState: L.eagerState,
              next: null,
            }),
              A === null ? ((v = A = V), (d = o)) : (A = A.next = V),
              (ct.lanes |= U),
              (Kn |= U));
          ((V = L.action), Aa && a(o, V), (o = L.hasEagerState ? L.eagerState : a(o, V)));
        } else
          ((U = {
            lane: V,
            revertLane: L.revertLane,
            gesture: L.gesture,
            action: L.action,
            hasEagerState: L.hasEagerState,
            eagerState: L.eagerState,
            next: null,
          }),
            A === null ? ((v = A = U), (d = o)) : (A = A.next = U),
            (ct.lanes |= V),
            (Kn |= V));
        L = L.next;
      } while (L !== null && L !== e);
      if (
        (A === null ? (d = o) : (A.next = v),
        !xe(o, t.memoizedState) && ((Ft = !0), H && ((a = si), a !== null)))
      )
        throw a;
      ((t.memoizedState = o), (t.baseState = d), (t.baseQueue = A), (i.lastRenderedState = o));
    }
    return (r === null && (i.lanes = 0), [t.memoizedState, i.dispatch]);
  }
  function Xu(t) {
    var e = Xt(),
      a = e.queue;
    if (a === null) throw Error(u(311));
    a.lastRenderedReducer = t;
    var i = a.dispatch,
      r = a.pending,
      o = e.memoizedState;
    if (r !== null) {
      a.pending = null;
      var d = (r = r.next);
      do ((o = t(o, d.action)), (d = d.next));
      while (d !== r);
      (xe(o, e.memoizedState) || (Ft = !0),
        (e.memoizedState = o),
        e.baseQueue === null && (e.baseState = o),
        (a.lastRenderedState = o));
    }
    return [o, i];
  }
  function md(t, e, a) {
    var i = ct,
      r = Xt(),
      o = gt;
    if (o) {
      if (a === void 0) throw Error(u(407));
      a = a();
    } else a = e();
    var d = !xe((Mt || r).memoizedState, a);
    if (
      (d && ((r.memoizedState = a), (Ft = !0)),
      (r = r.queue),
      Pu(gd.bind(null, i, r, t), [t]),
      r.getSnapshot !== e || d || (Pt !== null && Pt.memoizedState.tag & 1))
    ) {
      if (
        ((i.flags |= 2048),
        di(9, { destroy: void 0 }, pd.bind(null, i, r, a, e), null),
        zt === null)
      )
        throw Error(u(349));
      o || (hn & 127) !== 0 || yd(i, e, a);
    }
    return a;
  }
  function yd(t, e, a) {
    ((t.flags |= 16384),
      (t = { getSnapshot: e, value: a }),
      (e = ct.updateQueue),
      e === null
        ? ((e = Ms()), (ct.updateQueue = e), (e.stores = [t]))
        : ((a = e.stores), a === null ? (e.stores = [t]) : a.push(t)));
  }
  function pd(t, e, a, i) {
    ((e.value = a), (e.getSnapshot = i), vd(e) && Sd(t));
  }
  function gd(t, e, a) {
    return a(function () {
      vd(e) && Sd(t);
    });
  }
  function vd(t) {
    var e = t.getSnapshot;
    t = t.value;
    try {
      var a = e();
      return !xe(t, a);
    } catch {
      return !0;
    }
  }
  function Sd(t) {
    var e = pa(t, 2);
    e !== null && _e(e, t, 2);
  }
  function Ku(t) {
    var e = de();
    if (typeof t == "function") {
      var a = t;
      if (((t = a()), Aa)) {
        zn(!0);
        try {
          a();
        } finally {
          zn(!1);
        }
      }
    }
    return (
      (e.memoizedState = e.baseState = t),
      (e.queue = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: mn,
        lastRenderedState: t,
      }),
      e
    );
  }
  function bd(t, e, a, i) {
    return ((t.baseState = a), Vu(t, Mt, typeof i == "function" ? i : mn));
  }
  function Uv(t, e, a, i, r) {
    if (Ds(t)) throw Error(u(485));
    if (((t = e.action), t !== null)) {
      var o = {
        payload: r,
        action: t,
        next: null,
        isTransition: !0,
        status: "pending",
        value: null,
        reason: null,
        listeners: [],
        then: function (d) {
          o.listeners.push(d);
        },
      };
      (j.T !== null ? a(!0) : (o.isTransition = !1),
        i(o),
        (a = e.pending),
        a === null
          ? ((o.next = e.pending = o), _d(e, o))
          : ((o.next = a.next), (e.pending = a.next = o)));
    }
  }
  function _d(t, e) {
    var a = e.action,
      i = e.payload,
      r = t.state;
    if (e.isTransition) {
      var o = j.T,
        d = {};
      j.T = d;
      try {
        var v = a(r, i),
          A = j.S;
        (A !== null && A(d, v), Ed(t, e, v));
      } catch (L) {
        Zu(t, e, L);
      } finally {
        (o !== null && d.types !== null && (o.types = d.types), (j.T = o));
      }
    } else
      try {
        ((o = a(r, i)), Ed(t, e, o));
      } catch (L) {
        Zu(t, e, L);
      }
  }
  function Ed(t, e, a) {
    a !== null && typeof a == "object" && typeof a.then == "function"
      ? a.then(
          function (i) {
            Td(t, e, i);
          },
          function (i) {
            return Zu(t, e, i);
          },
        )
      : Td(t, e, a);
  }
  function Td(t, e, a) {
    ((e.status = "fulfilled"),
      (e.value = a),
      Rd(e),
      (t.state = a),
      (e = t.pending),
      e !== null &&
        ((a = e.next), a === e ? (t.pending = null) : ((a = a.next), (e.next = a), _d(t, a))));
  }
  function Zu(t, e, a) {
    var i = t.pending;
    if (((t.pending = null), i !== null)) {
      i = i.next;
      do ((e.status = "rejected"), (e.reason = a), Rd(e), (e = e.next));
      while (e !== i);
    }
    t.action = null;
  }
  function Rd(t) {
    t = t.listeners;
    for (var e = 0; e < t.length; e++) (0, t[e])();
  }
  function Ad(t, e) {
    return e;
  }
  function xd(t, e) {
    if (gt) {
      var a = zt.formState;
      if (a !== null) {
        t: {
          var i = ct;
          if (gt) {
            if (Ut) {
              e: {
                for (var r = Ut, o = qe; r.nodeType !== 8; ) {
                  if (!o) {
                    r = null;
                    break e;
                  }
                  if (((r = Ge(r.nextSibling)), r === null)) {
                    r = null;
                    break e;
                  }
                }
                ((o = r.data), (r = o === "F!" || o === "F" ? r : null));
              }
              if (r) {
                ((Ut = Ge(r.nextSibling)), (i = r.data === "F!"));
                break t;
              }
            }
            Nn(i);
          }
          i = !1;
        }
        i && (e = a[0]);
      }
    }
    return (
      (a = de()),
      (a.memoizedState = a.baseState = e),
      (i = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Ad,
        lastRenderedState: e,
      }),
      (a.queue = i),
      (a = Kd.bind(null, ct, i)),
      (i.dispatch = a),
      (i = Ku(!1)),
      (o = $u.bind(null, ct, !1, i.queue)),
      (i = de()),
      (r = { state: e, dispatch: null, action: t, pending: null }),
      (i.queue = r),
      (a = Uv.bind(null, ct, r, o, a)),
      (r.dispatch = a),
      (i.memoizedState = t),
      [e, a, !1]
    );
  }
  function Md(t) {
    var e = Xt();
    return Cd(e, Mt, t);
  }
  function Cd(t, e, a) {
    if (
      ((e = Vu(t, e, Ad)[0]),
      (t = ws(mn)[0]),
      typeof e == "object" && e !== null && typeof e.then == "function")
    )
      try {
        var i = rl(e);
      } catch (d) {
        throw d === ri ? Ss : d;
      }
    else i = e;
    e = Xt();
    var r = e.queue,
      o = r.dispatch;
    return (
      a !== e.memoizedState &&
        ((ct.flags |= 2048), di(9, { destroy: void 0 }, Bv.bind(null, r, a), null)),
      [i, o, t]
    );
  }
  function Bv(t, e) {
    t.action = e;
  }
  function wd(t) {
    var e = Xt(),
      a = Mt;
    if (a !== null) return Cd(e, a, t);
    (Xt(), (e = e.memoizedState), (a = Xt()));
    var i = a.queue.dispatch;
    return ((a.memoizedState = t), [e, i, !1]);
  }
  function di(t, e, a, i) {
    return (
      (t = { tag: t, create: a, deps: i, inst: e, next: null }),
      (e = ct.updateQueue),
      e === null && ((e = Ms()), (ct.updateQueue = e)),
      (a = e.lastEffect),
      a === null
        ? (e.lastEffect = t.next = t)
        : ((i = a.next), (a.next = t), (t.next = i), (e.lastEffect = t)),
      t
    );
  }
  function Od() {
    return Xt().memoizedState;
  }
  function Os(t, e, a, i) {
    var r = de();
    ((ct.flags |= t),
      (r.memoizedState = di(1 | e, { destroy: void 0 }, a, i === void 0 ? null : i)));
  }
  function zs(t, e, a, i) {
    var r = Xt();
    i = i === void 0 ? null : i;
    var o = r.memoizedState.inst;
    Mt !== null && i !== null && ju(i, Mt.memoizedState.deps)
      ? (r.memoizedState = di(e, o, a, i))
      : ((ct.flags |= t), (r.memoizedState = di(1 | e, o, a, i)));
  }
  function zd(t, e) {
    Os(8390656, 8, t, e);
  }
  function Pu(t, e) {
    zs(2048, 8, t, e);
  }
  function Nv(t) {
    ct.flags |= 4;
    var e = ct.updateQueue;
    if (e === null) ((e = Ms()), (ct.updateQueue = e), (e.events = [t]));
    else {
      var a = e.events;
      a === null ? (e.events = [t]) : a.push(t);
    }
  }
  function Dd(t) {
    var e = Xt().memoizedState;
    return (
      Nv({ ref: e, nextImpl: t }),
      function () {
        if ((Tt & 2) !== 0) throw Error(u(440));
        return e.impl.apply(void 0, arguments);
      }
    );
  }
  function Ld(t, e) {
    return zs(4, 2, t, e);
  }
  function Ud(t, e) {
    return zs(4, 4, t, e);
  }
  function Bd(t, e) {
    if (typeof e == "function") {
      t = t();
      var a = e(t);
      return function () {
        typeof a == "function" ? a() : e(null);
      };
    }
    if (e != null)
      return (
        (t = t()),
        (e.current = t),
        function () {
          e.current = null;
        }
      );
  }
  function Nd(t, e, a) {
    ((a = a != null ? a.concat([t]) : null), zs(4, 4, Bd.bind(null, e, t), a));
  }
  function Fu() {}
  function jd(t, e) {
    var a = Xt();
    e = e === void 0 ? null : e;
    var i = a.memoizedState;
    return e !== null && ju(e, i[1]) ? i[0] : ((a.memoizedState = [t, e]), t);
  }
  function Hd(t, e) {
    var a = Xt();
    e = e === void 0 ? null : e;
    var i = a.memoizedState;
    if (e !== null && ju(e, i[1])) return i[0];
    if (((i = t()), Aa)) {
      zn(!0);
      try {
        t();
      } finally {
        zn(!1);
      }
    }
    return ((a.memoizedState = [i, e]), i);
  }
  function Ju(t, e, a) {
    return a === void 0 || ((hn & 1073741824) !== 0 && (yt & 261930) === 0)
      ? (t.memoizedState = e)
      : ((t.memoizedState = a), (t = qh()), (ct.lanes |= t), (Kn |= t), a);
  }
  function qd(t, e, a, i) {
    return xe(a, e)
      ? a
      : oi.current !== null
        ? ((t = Ju(t, a, i)), xe(t, e) || (Ft = !0), t)
        : (hn & 42) === 0 || ((hn & 1073741824) !== 0 && (yt & 261930) === 0)
          ? ((Ft = !0), (t.memoizedState = a))
          : ((t = qh()), (ct.lanes |= t), (Kn |= t), e);
  }
  function Yd(t, e, a, i, r) {
    var o = F.p;
    F.p = o !== 0 && 8 > o ? o : 8;
    var d = j.T,
      v = {};
    ((j.T = v), $u(t, !1, e, a));
    try {
      var A = r(),
        L = j.S;
      if (
        (L !== null && L(v, A), A !== null && typeof A == "object" && typeof A.then == "function")
      ) {
        var H = zv(A, i);
        ul(t, e, H, De(t));
      } else ul(t, e, i, De(t));
    } catch (V) {
      ul(t, e, { then: function () {}, status: "rejected", reason: V }, De());
    } finally {
      ((F.p = o), d !== null && v.types !== null && (d.types = v.types), (j.T = d));
    }
  }
  function jv() {}
  function ku(t, e, a, i) {
    if (t.tag !== 5) throw Error(u(476));
    var r = Gd(t).queue;
    Yd(
      t,
      r,
      e,
      it,
      a === null
        ? jv
        : function () {
            return (Qd(t), a(i));
          },
    );
  }
  function Gd(t) {
    var e = t.memoizedState;
    if (e !== null) return e;
    e = {
      memoizedState: it,
      baseState: it,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: mn,
        lastRenderedState: it,
      },
      next: null,
    };
    var a = {};
    return (
      (e.next = {
        memoizedState: a,
        baseState: a,
        baseQueue: null,
        queue: {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: mn,
          lastRenderedState: a,
        },
        next: null,
      }),
      (t.memoizedState = e),
      (t = t.alternate),
      t !== null && (t.memoizedState = e),
      e
    );
  }
  function Qd(t) {
    var e = Gd(t);
    (e.next === null && (e = t.alternate.memoizedState), ul(t, e.next.queue, {}, De()));
  }
  function Iu() {
    return se(Al);
  }
  function Vd() {
    return Xt().memoizedState;
  }
  function Xd() {
    return Xt().memoizedState;
  }
  function Hv(t) {
    for (var e = t.return; e !== null; ) {
      switch (e.tag) {
        case 24:
        case 3:
          var a = De();
          t = qn(a);
          var i = Yn(e, t, a);
          (i !== null && (_e(i, e, a), al(i, e, a)), (e = { cache: xu() }), (t.payload = e));
          return;
      }
      e = e.return;
    }
  }
  function qv(t, e, a) {
    var i = De();
    ((a = {
      lane: i,
      revertLane: 0,
      gesture: null,
      action: a,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    }),
      Ds(t) ? Zd(e, a) : ((a = yu(t, e, a, i)), a !== null && (_e(a, t, i), Pd(a, e, i))));
  }
  function Kd(t, e, a) {
    var i = De();
    ul(t, e, a, i);
  }
  function ul(t, e, a, i) {
    var r = {
      lane: i,
      revertLane: 0,
      gesture: null,
      action: a,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    };
    if (Ds(t)) Zd(e, r);
    else {
      var o = t.alternate;
      if (
        t.lanes === 0 &&
        (o === null || o.lanes === 0) &&
        ((o = e.lastRenderedReducer), o !== null)
      )
        try {
          var d = e.lastRenderedState,
            v = o(d, a);
          if (((r.hasEagerState = !0), (r.eagerState = v), xe(v, d)))
            return (ds(t, e, r, 0), zt === null && fs(), !1);
        } catch {}
      if (((a = yu(t, e, r, i)), a !== null)) return (_e(a, t, i), Pd(a, e, i), !0);
    }
    return !1;
  }
  function $u(t, e, a, i) {
    if (
      ((i = {
        lane: 2,
        revertLane: zo(),
        gesture: null,
        action: i,
        hasEagerState: !1,
        eagerState: null,
        next: null,
      }),
      Ds(t))
    ) {
      if (e) throw Error(u(479));
    } else ((e = yu(t, a, i, 2)), e !== null && _e(e, t, 2));
  }
  function Ds(t) {
    var e = t.alternate;
    return t === ct || (e !== null && e === ct);
  }
  function Zd(t, e) {
    ci = As = !0;
    var a = t.pending;
    (a === null ? (e.next = e) : ((e.next = a.next), (a.next = e)), (t.pending = e));
  }
  function Pd(t, e, a) {
    if ((a & 4194048) !== 0) {
      var i = e.lanes;
      ((i &= t.pendingLanes), (a |= i), (e.lanes = a), Ic(t, a));
    }
  }
  var ol = {
    readContext: se,
    use: Cs,
    useCallback: qt,
    useContext: qt,
    useEffect: qt,
    useImperativeHandle: qt,
    useLayoutEffect: qt,
    useInsertionEffect: qt,
    useMemo: qt,
    useReducer: qt,
    useRef: qt,
    useState: qt,
    useDebugValue: qt,
    useDeferredValue: qt,
    useTransition: qt,
    useSyncExternalStore: qt,
    useId: qt,
    useHostTransitionStatus: qt,
    useFormState: qt,
    useActionState: qt,
    useOptimistic: qt,
    useMemoCache: qt,
    useCacheRefresh: qt,
  };
  ol.useEffectEvent = qt;
  var Fd = {
      readContext: se,
      use: Cs,
      useCallback: function (t, e) {
        return ((de().memoizedState = [t, e === void 0 ? null : e]), t);
      },
      useContext: se,
      useEffect: zd,
      useImperativeHandle: function (t, e, a) {
        ((a = a != null ? a.concat([t]) : null), Os(4194308, 4, Bd.bind(null, e, t), a));
      },
      useLayoutEffect: function (t, e) {
        return Os(4194308, 4, t, e);
      },
      useInsertionEffect: function (t, e) {
        Os(4, 2, t, e);
      },
      useMemo: function (t, e) {
        var a = de();
        e = e === void 0 ? null : e;
        var i = t();
        if (Aa) {
          zn(!0);
          try {
            t();
          } finally {
            zn(!1);
          }
        }
        return ((a.memoizedState = [i, e]), i);
      },
      useReducer: function (t, e, a) {
        var i = de();
        if (a !== void 0) {
          var r = a(e);
          if (Aa) {
            zn(!0);
            try {
              a(e);
            } finally {
              zn(!1);
            }
          }
        } else r = e;
        return (
          (i.memoizedState = i.baseState = r),
          (t = {
            pending: null,
            lanes: 0,
            dispatch: null,
            lastRenderedReducer: t,
            lastRenderedState: r,
          }),
          (i.queue = t),
          (t = t.dispatch = qv.bind(null, ct, t)),
          [i.memoizedState, t]
        );
      },
      useRef: function (t) {
        var e = de();
        return ((t = { current: t }), (e.memoizedState = t));
      },
      useState: function (t) {
        t = Ku(t);
        var e = t.queue,
          a = Kd.bind(null, ct, e);
        return ((e.dispatch = a), [t.memoizedState, a]);
      },
      useDebugValue: Fu,
      useDeferredValue: function (t, e) {
        var a = de();
        return Ju(a, t, e);
      },
      useTransition: function () {
        var t = Ku(!1);
        return ((t = Yd.bind(null, ct, t.queue, !0, !1)), (de().memoizedState = t), [!1, t]);
      },
      useSyncExternalStore: function (t, e, a) {
        var i = ct,
          r = de();
        if (gt) {
          if (a === void 0) throw Error(u(407));
          a = a();
        } else {
          if (((a = e()), zt === null)) throw Error(u(349));
          (yt & 127) !== 0 || yd(i, e, a);
        }
        r.memoizedState = a;
        var o = { value: a, getSnapshot: e };
        return (
          (r.queue = o),
          zd(gd.bind(null, i, o, t), [t]),
          (i.flags |= 2048),
          di(9, { destroy: void 0 }, pd.bind(null, i, o, a, e), null),
          a
        );
      },
      useId: function () {
        var t = de(),
          e = zt.identifierPrefix;
        if (gt) {
          var a = Ie,
            i = ke;
          ((a = (i & ~(1 << (32 - Ae(i) - 1))).toString(32) + a),
            (e = "_" + e + "R_" + a),
            (a = xs++),
            0 < a && (e += "H" + a.toString(32)),
            (e += "_"));
        } else ((a = Dv++), (e = "_" + e + "r_" + a.toString(32) + "_"));
        return (t.memoizedState = e);
      },
      useHostTransitionStatus: Iu,
      useFormState: xd,
      useActionState: xd,
      useOptimistic: function (t) {
        var e = de();
        e.memoizedState = e.baseState = t;
        var a = {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: null,
          lastRenderedState: null,
        };
        return ((e.queue = a), (e = $u.bind(null, ct, !0, a)), (a.dispatch = e), [t, e]);
      },
      useMemoCache: Qu,
      useCacheRefresh: function () {
        return (de().memoizedState = Hv.bind(null, ct));
      },
      useEffectEvent: function (t) {
        var e = de(),
          a = { impl: t };
        return (
          (e.memoizedState = a),
          function () {
            if ((Tt & 2) !== 0) throw Error(u(440));
            return a.impl.apply(void 0, arguments);
          }
        );
      },
    },
    Wu = {
      readContext: se,
      use: Cs,
      useCallback: jd,
      useContext: se,
      useEffect: Pu,
      useImperativeHandle: Nd,
      useInsertionEffect: Ld,
      useLayoutEffect: Ud,
      useMemo: Hd,
      useReducer: ws,
      useRef: Od,
      useState: function () {
        return ws(mn);
      },
      useDebugValue: Fu,
      useDeferredValue: function (t, e) {
        var a = Xt();
        return qd(a, Mt.memoizedState, t, e);
      },
      useTransition: function () {
        var t = ws(mn)[0],
          e = Xt().memoizedState;
        return [typeof t == "boolean" ? t : rl(t), e];
      },
      useSyncExternalStore: md,
      useId: Vd,
      useHostTransitionStatus: Iu,
      useFormState: Md,
      useActionState: Md,
      useOptimistic: function (t, e) {
        var a = Xt();
        return bd(a, Mt, t, e);
      },
      useMemoCache: Qu,
      useCacheRefresh: Xd,
    };
  Wu.useEffectEvent = Dd;
  var Jd = {
    readContext: se,
    use: Cs,
    useCallback: jd,
    useContext: se,
    useEffect: Pu,
    useImperativeHandle: Nd,
    useInsertionEffect: Ld,
    useLayoutEffect: Ud,
    useMemo: Hd,
    useReducer: Xu,
    useRef: Od,
    useState: function () {
      return Xu(mn);
    },
    useDebugValue: Fu,
    useDeferredValue: function (t, e) {
      var a = Xt();
      return Mt === null ? Ju(a, t, e) : qd(a, Mt.memoizedState, t, e);
    },
    useTransition: function () {
      var t = Xu(mn)[0],
        e = Xt().memoizedState;
      return [typeof t == "boolean" ? t : rl(t), e];
    },
    useSyncExternalStore: md,
    useId: Vd,
    useHostTransitionStatus: Iu,
    useFormState: wd,
    useActionState: wd,
    useOptimistic: function (t, e) {
      var a = Xt();
      return Mt !== null ? bd(a, Mt, t, e) : ((a.baseState = t), [t, a.queue.dispatch]);
    },
    useMemoCache: Qu,
    useCacheRefresh: Xd,
  };
  Jd.useEffectEvent = Dd;
  function to(t, e, a, i) {
    ((e = t.memoizedState),
      (a = a(i, e)),
      (a = a == null ? e : g({}, e, a)),
      (t.memoizedState = a),
      t.lanes === 0 && (t.updateQueue.baseState = a));
  }
  var eo = {
    enqueueSetState: function (t, e, a) {
      t = t._reactInternals;
      var i = De(),
        r = qn(i);
      ((r.payload = e),
        a != null && (r.callback = a),
        (e = Yn(t, r, i)),
        e !== null && (_e(e, t, i), al(e, t, i)));
    },
    enqueueReplaceState: function (t, e, a) {
      t = t._reactInternals;
      var i = De(),
        r = qn(i);
      ((r.tag = 1),
        (r.payload = e),
        a != null && (r.callback = a),
        (e = Yn(t, r, i)),
        e !== null && (_e(e, t, i), al(e, t, i)));
    },
    enqueueForceUpdate: function (t, e) {
      t = t._reactInternals;
      var a = De(),
        i = qn(a);
      ((i.tag = 2),
        e != null && (i.callback = e),
        (e = Yn(t, i, a)),
        e !== null && (_e(e, t, a), al(e, t, a)));
    },
  };
  function kd(t, e, a, i, r, o, d) {
    return (
      (t = t.stateNode),
      typeof t.shouldComponentUpdate == "function"
        ? t.shouldComponentUpdate(i, o, d)
        : e.prototype && e.prototype.isPureReactComponent
          ? !Ji(a, i) || !Ji(r, o)
          : !0
    );
  }
  function Id(t, e, a, i) {
    ((t = e.state),
      typeof e.componentWillReceiveProps == "function" && e.componentWillReceiveProps(a, i),
      typeof e.UNSAFE_componentWillReceiveProps == "function" &&
        e.UNSAFE_componentWillReceiveProps(a, i),
      e.state !== t && eo.enqueueReplaceState(e, e.state, null));
  }
  function xa(t, e) {
    var a = e;
    if ("ref" in e) {
      a = {};
      for (var i in e) i !== "ref" && (a[i] = e[i]);
    }
    if ((t = t.defaultProps)) {
      a === e && (a = g({}, a));
      for (var r in t) a[r] === void 0 && (a[r] = t[r]);
    }
    return a;
  }
  function $d(t) {
    cs(t);
  }
  function Wd(t) {
    console.error(t);
  }
  function th(t) {
    cs(t);
  }
  function Ls(t, e) {
    try {
      var a = t.onUncaughtError;
      a(e.value, { componentStack: e.stack });
    } catch (i) {
      setTimeout(function () {
        throw i;
      });
    }
  }
  function eh(t, e, a) {
    try {
      var i = t.onCaughtError;
      i(a.value, { componentStack: a.stack, errorBoundary: e.tag === 1 ? e.stateNode : null });
    } catch (r) {
      setTimeout(function () {
        throw r;
      });
    }
  }
  function no(t, e, a) {
    return (
      (a = qn(a)),
      (a.tag = 3),
      (a.payload = { element: null }),
      (a.callback = function () {
        Ls(t, e);
      }),
      a
    );
  }
  function nh(t) {
    return ((t = qn(t)), (t.tag = 3), t);
  }
  function ah(t, e, a, i) {
    var r = a.type.getDerivedStateFromError;
    if (typeof r == "function") {
      var o = i.value;
      ((t.payload = function () {
        return r(o);
      }),
        (t.callback = function () {
          eh(e, a, i);
        }));
    }
    var d = a.stateNode;
    d !== null &&
      typeof d.componentDidCatch == "function" &&
      (t.callback = function () {
        (eh(e, a, i),
          typeof r != "function" && (Zn === null ? (Zn = new Set([this])) : Zn.add(this)));
        var v = i.stack;
        this.componentDidCatch(i.value, { componentStack: v !== null ? v : "" });
      });
  }
  function Yv(t, e, a, i, r) {
    if (((a.flags |= 32768), i !== null && typeof i == "object" && typeof i.then == "function")) {
      if (((e = a.alternate), e !== null && ii(e, a, r, !0), (a = Ce.current), a !== null)) {
        switch (a.tag) {
          case 31:
          case 13:
            return (
              Ye === null ? Ks() : a.alternate === null && Yt === 0 && (Yt = 3),
              (a.flags &= -257),
              (a.flags |= 65536),
              (a.lanes = r),
              i === bs
                ? (a.flags |= 16384)
                : ((e = a.updateQueue),
                  e === null ? (a.updateQueue = new Set([i])) : e.add(i),
                  Co(t, i, r)),
              !1
            );
          case 22:
            return (
              (a.flags |= 65536),
              i === bs
                ? (a.flags |= 16384)
                : ((e = a.updateQueue),
                  e === null
                    ? ((e = { transitions: null, markerInstances: null, retryQueue: new Set([i]) }),
                      (a.updateQueue = e))
                    : ((a = e.retryQueue), a === null ? (e.retryQueue = new Set([i])) : a.add(i)),
                  Co(t, i, r)),
              !1
            );
        }
        throw Error(u(435, a.tag));
      }
      return (Co(t, i, r), Ks(), !1);
    }
    if (gt)
      return (
        (e = Ce.current),
        e !== null
          ? ((e.flags & 65536) === 0 && (e.flags |= 256),
            (e.flags |= 65536),
            (e.lanes = r),
            i !== _u && ((t = Error(u(422), { cause: i })), $i(Ne(t, a))))
          : (i !== _u && ((e = Error(u(423), { cause: i })), $i(Ne(e, a))),
            (t = t.current.alternate),
            (t.flags |= 65536),
            (r &= -r),
            (t.lanes |= r),
            (i = Ne(i, a)),
            (r = no(t.stateNode, i, r)),
            Du(t, r),
            Yt !== 4 && (Yt = 2)),
        !1
      );
    var o = Error(u(520), { cause: i });
    if (((o = Ne(o, a)), gl === null ? (gl = [o]) : gl.push(o), Yt !== 4 && (Yt = 2), e === null))
      return !0;
    ((i = Ne(i, a)), (a = e));
    do {
      switch (a.tag) {
        case 3:
          return (
            (a.flags |= 65536),
            (t = r & -r),
            (a.lanes |= t),
            (t = no(a.stateNode, i, t)),
            Du(a, t),
            !1
          );
        case 1:
          if (
            ((e = a.type),
            (o = a.stateNode),
            (a.flags & 128) === 0 &&
              (typeof e.getDerivedStateFromError == "function" ||
                (o !== null &&
                  typeof o.componentDidCatch == "function" &&
                  (Zn === null || !Zn.has(o)))))
          )
            return (
              (a.flags |= 65536),
              (r &= -r),
              (a.lanes |= r),
              (r = nh(r)),
              ah(r, t, a, i),
              Du(a, r),
              !1
            );
      }
      a = a.return;
    } while (a !== null);
    return !1;
  }
  var ao = Error(u(461)),
    Ft = !1;
  function re(t, e, a, i) {
    e.child = t === null ? rd(e, null, a, i) : Ra(e, t.child, a, i);
  }
  function ih(t, e, a, i, r) {
    a = a.render;
    var o = e.ref;
    if ("ref" in i) {
      var d = {};
      for (var v in i) v !== "ref" && (d[v] = i[v]);
    } else d = i;
    return (
      ba(e),
      (i = Hu(t, e, a, d, o, r)),
      (v = qu()),
      t !== null && !Ft
        ? (Yu(t, e, r), yn(t, e, r))
        : (gt && v && Su(e), (e.flags |= 1), re(t, e, i, r), e.child)
    );
  }
  function lh(t, e, a, i, r) {
    if (t === null) {
      var o = a.type;
      return typeof o == "function" && !pu(o) && o.defaultProps === void 0 && a.compare === null
        ? ((e.tag = 15), (e.type = o), sh(t, e, o, i, r))
        : ((t = ms(a.type, null, i, e, e.mode, r)), (t.ref = e.ref), (t.return = e), (e.child = t));
    }
    if (((o = t.child), !fo(t, r))) {
      var d = o.memoizedProps;
      if (((a = a.compare), (a = a !== null ? a : Ji), a(d, i) && t.ref === e.ref))
        return yn(t, e, r);
    }
    return ((e.flags |= 1), (t = on(o, i)), (t.ref = e.ref), (t.return = e), (e.child = t));
  }
  function sh(t, e, a, i, r) {
    if (t !== null) {
      var o = t.memoizedProps;
      if (Ji(o, i) && t.ref === e.ref)
        if (((Ft = !1), (e.pendingProps = i = o), fo(t, r))) (t.flags & 131072) !== 0 && (Ft = !0);
        else return ((e.lanes = t.lanes), yn(t, e, r));
    }
    return io(t, e, a, i, r);
  }
  function rh(t, e, a, i) {
    var r = i.children,
      o = t !== null ? t.memoizedState : null;
    if (
      (t === null &&
        e.stateNode === null &&
        (e.stateNode = {
          _visibility: 1,
          _pendingMarkers: null,
          _retryCache: null,
          _transitions: null,
        }),
      i.mode === "hidden")
    ) {
      if ((e.flags & 128) !== 0) {
        if (((o = o !== null ? o.baseLanes | a : a), t !== null)) {
          for (i = e.child = t.child, r = 0; i !== null; )
            ((r = r | i.lanes | i.childLanes), (i = i.sibling));
          i = r & ~o;
        } else ((i = 0), (e.child = null));
        return uh(t, e, o, a, i);
      }
      if ((a & 536870912) !== 0)
        ((e.memoizedState = { baseLanes: 0, cachePool: null }),
          t !== null && vs(e, o !== null ? o.cachePool : null),
          o !== null ? cd(e, o) : Uu(),
          fd(e));
      else return ((i = e.lanes = 536870912), uh(t, e, o !== null ? o.baseLanes | a : a, a, i));
    } else
      o !== null
        ? (vs(e, o.cachePool), cd(e, o), Qn(), (e.memoizedState = null))
        : (t !== null && vs(e, null), Uu(), Qn());
    return (re(t, e, r, a), e.child);
  }
  function cl(t, e) {
    return (
      (t !== null && t.tag === 22) ||
        e.stateNode !== null ||
        (e.stateNode = {
          _visibility: 1,
          _pendingMarkers: null,
          _retryCache: null,
          _transitions: null,
        }),
      e.sibling
    );
  }
  function uh(t, e, a, i, r) {
    var o = Cu();
    return (
      (o = o === null ? null : { parent: Zt._currentValue, pool: o }),
      (e.memoizedState = { baseLanes: a, cachePool: o }),
      t !== null && vs(e, null),
      Uu(),
      fd(e),
      t !== null && ii(t, e, i, !0),
      (e.childLanes = r),
      null
    );
  }
  function Us(t, e) {
    return (
      (e = Ns({ mode: e.mode, children: e.children }, t.mode)),
      (e.ref = t.ref),
      (t.child = e),
      (e.return = t),
      e
    );
  }
  function oh(t, e, a) {
    return (
      Ra(e, t.child, null, a),
      (t = Us(e, e.pendingProps)),
      (t.flags |= 2),
      we(e),
      (e.memoizedState = null),
      t
    );
  }
  function Gv(t, e, a) {
    var i = e.pendingProps,
      r = (e.flags & 128) !== 0;
    if (((e.flags &= -129), t === null)) {
      if (gt) {
        if (i.mode === "hidden") return ((t = Us(e, i)), (e.lanes = 536870912), cl(null, t));
        if (
          (Nu(e),
          (t = Ut)
            ? ((t = _m(t, qe)),
              (t = t !== null && t.data === "&" ? t : null),
              t !== null &&
                ((e.memoizedState = {
                  dehydrated: t,
                  treeContext: Un !== null ? { id: ke, overflow: Ie } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (a = Zf(t)),
                (a.return = e),
                (e.child = a),
                (le = e),
                (Ut = null)))
            : (t = null),
          t === null)
        )
          throw Nn(e);
        return ((e.lanes = 536870912), null);
      }
      return Us(e, i);
    }
    var o = t.memoizedState;
    if (o !== null) {
      var d = o.dehydrated;
      if ((Nu(e), r))
        if (e.flags & 256) ((e.flags &= -257), (e = oh(t, e, a)));
        else if (e.memoizedState !== null) ((e.child = t.child), (e.flags |= 128), (e = null));
        else throw Error(u(558));
      else if ((Ft || ii(t, e, a, !1), (r = (a & t.childLanes) !== 0), Ft || r)) {
        if (((i = zt), i !== null && ((d = $c(i, a)), d !== 0 && d !== o.retryLane)))
          throw ((o.retryLane = d), pa(t, d), _e(i, t, d), ao);
        (Ks(), (e = oh(t, e, a)));
      } else
        ((t = o.treeContext),
          (Ut = Ge(d.nextSibling)),
          (le = e),
          (gt = !0),
          (Bn = null),
          (qe = !1),
          t !== null && Jf(e, t),
          (e = Us(e, i)),
          (e.flags |= 4096));
      return e;
    }
    return (
      (t = on(t.child, { mode: i.mode, children: i.children })),
      (t.ref = e.ref),
      (e.child = t),
      (t.return = e),
      t
    );
  }
  function Bs(t, e) {
    var a = e.ref;
    if (a === null) t !== null && t.ref !== null && (e.flags |= 4194816);
    else {
      if (typeof a != "function" && typeof a != "object") throw Error(u(284));
      (t === null || t.ref !== a) && (e.flags |= 4194816);
    }
  }
  function io(t, e, a, i, r) {
    return (
      ba(e),
      (a = Hu(t, e, a, i, void 0, r)),
      (i = qu()),
      t !== null && !Ft
        ? (Yu(t, e, r), yn(t, e, r))
        : (gt && i && Su(e), (e.flags |= 1), re(t, e, a, r), e.child)
    );
  }
  function ch(t, e, a, i, r, o) {
    return (
      ba(e),
      (e.updateQueue = null),
      (a = hd(e, i, a, r)),
      dd(t),
      (i = qu()),
      t !== null && !Ft
        ? (Yu(t, e, o), yn(t, e, o))
        : (gt && i && Su(e), (e.flags |= 1), re(t, e, a, o), e.child)
    );
  }
  function fh(t, e, a, i, r) {
    if ((ba(e), e.stateNode === null)) {
      var o = ti,
        d = a.contextType;
      (typeof d == "object" && d !== null && (o = se(d)),
        (o = new a(i, o)),
        (e.memoizedState = o.state !== null && o.state !== void 0 ? o.state : null),
        (o.updater = eo),
        (e.stateNode = o),
        (o._reactInternals = e),
        (o = e.stateNode),
        (o.props = i),
        (o.state = e.memoizedState),
        (o.refs = {}),
        Ou(e),
        (d = a.contextType),
        (o.context = typeof d == "object" && d !== null ? se(d) : ti),
        (o.state = e.memoizedState),
        (d = a.getDerivedStateFromProps),
        typeof d == "function" && (to(e, a, d, i), (o.state = e.memoizedState)),
        typeof a.getDerivedStateFromProps == "function" ||
          typeof o.getSnapshotBeforeUpdate == "function" ||
          (typeof o.UNSAFE_componentWillMount != "function" &&
            typeof o.componentWillMount != "function") ||
          ((d = o.state),
          typeof o.componentWillMount == "function" && o.componentWillMount(),
          typeof o.UNSAFE_componentWillMount == "function" && o.UNSAFE_componentWillMount(),
          d !== o.state && eo.enqueueReplaceState(o, o.state, null),
          ll(e, i, o, r),
          il(),
          (o.state = e.memoizedState)),
        typeof o.componentDidMount == "function" && (e.flags |= 4194308),
        (i = !0));
    } else if (t === null) {
      o = e.stateNode;
      var v = e.memoizedProps,
        A = xa(a, v);
      o.props = A;
      var L = o.context,
        H = a.contextType;
      ((d = ti), typeof H == "object" && H !== null && (d = se(H)));
      var V = a.getDerivedStateFromProps;
      ((H = typeof V == "function" || typeof o.getSnapshotBeforeUpdate == "function"),
        (v = e.pendingProps !== v),
        H ||
          (typeof o.UNSAFE_componentWillReceiveProps != "function" &&
            typeof o.componentWillReceiveProps != "function") ||
          ((v || L !== d) && Id(e, o, i, d)),
        (Hn = !1));
      var U = e.memoizedState;
      ((o.state = U),
        ll(e, i, o, r),
        il(),
        (L = e.memoizedState),
        v || U !== L || Hn
          ? (typeof V == "function" && (to(e, a, V, i), (L = e.memoizedState)),
            (A = Hn || kd(e, a, A, i, U, L, d))
              ? (H ||
                  (typeof o.UNSAFE_componentWillMount != "function" &&
                    typeof o.componentWillMount != "function") ||
                  (typeof o.componentWillMount == "function" && o.componentWillMount(),
                  typeof o.UNSAFE_componentWillMount == "function" &&
                    o.UNSAFE_componentWillMount()),
                typeof o.componentDidMount == "function" && (e.flags |= 4194308))
              : (typeof o.componentDidMount == "function" && (e.flags |= 4194308),
                (e.memoizedProps = i),
                (e.memoizedState = L)),
            (o.props = i),
            (o.state = L),
            (o.context = d),
            (i = A))
          : (typeof o.componentDidMount == "function" && (e.flags |= 4194308), (i = !1)));
    } else {
      ((o = e.stateNode),
        zu(t, e),
        (d = e.memoizedProps),
        (H = xa(a, d)),
        (o.props = H),
        (V = e.pendingProps),
        (U = o.context),
        (L = a.contextType),
        (A = ti),
        typeof L == "object" && L !== null && (A = se(L)),
        (v = a.getDerivedStateFromProps),
        (L = typeof v == "function" || typeof o.getSnapshotBeforeUpdate == "function") ||
          (typeof o.UNSAFE_componentWillReceiveProps != "function" &&
            typeof o.componentWillReceiveProps != "function") ||
          ((d !== V || U !== A) && Id(e, o, i, A)),
        (Hn = !1),
        (U = e.memoizedState),
        (o.state = U),
        ll(e, i, o, r),
        il());
      var B = e.memoizedState;
      d !== V || U !== B || Hn || (t !== null && t.dependencies !== null && ps(t.dependencies))
        ? (typeof v == "function" && (to(e, a, v, i), (B = e.memoizedState)),
          (H =
            Hn ||
            kd(e, a, H, i, U, B, A) ||
            (t !== null && t.dependencies !== null && ps(t.dependencies)))
            ? (L ||
                (typeof o.UNSAFE_componentWillUpdate != "function" &&
                  typeof o.componentWillUpdate != "function") ||
                (typeof o.componentWillUpdate == "function" && o.componentWillUpdate(i, B, A),
                typeof o.UNSAFE_componentWillUpdate == "function" &&
                  o.UNSAFE_componentWillUpdate(i, B, A)),
              typeof o.componentDidUpdate == "function" && (e.flags |= 4),
              typeof o.getSnapshotBeforeUpdate == "function" && (e.flags |= 1024))
            : (typeof o.componentDidUpdate != "function" ||
                (d === t.memoizedProps && U === t.memoizedState) ||
                (e.flags |= 4),
              typeof o.getSnapshotBeforeUpdate != "function" ||
                (d === t.memoizedProps && U === t.memoizedState) ||
                (e.flags |= 1024),
              (e.memoizedProps = i),
              (e.memoizedState = B)),
          (o.props = i),
          (o.state = B),
          (o.context = A),
          (i = H))
        : (typeof o.componentDidUpdate != "function" ||
            (d === t.memoizedProps && U === t.memoizedState) ||
            (e.flags |= 4),
          typeof o.getSnapshotBeforeUpdate != "function" ||
            (d === t.memoizedProps && U === t.memoizedState) ||
            (e.flags |= 1024),
          (i = !1));
    }
    return (
      (o = i),
      Bs(t, e),
      (i = (e.flags & 128) !== 0),
      o || i
        ? ((o = e.stateNode),
          (a = i && typeof a.getDerivedStateFromError != "function" ? null : o.render()),
          (e.flags |= 1),
          t !== null && i
            ? ((e.child = Ra(e, t.child, null, r)), (e.child = Ra(e, null, a, r)))
            : re(t, e, a, r),
          (e.memoizedState = o.state),
          (t = e.child))
        : (t = yn(t, e, r)),
      t
    );
  }
  function dh(t, e, a, i) {
    return (va(), (e.flags |= 256), re(t, e, a, i), e.child);
  }
  var lo = { dehydrated: null, treeContext: null, retryLane: 0, hydrationErrors: null };
  function so(t) {
    return { baseLanes: t, cachePool: ed() };
  }
  function ro(t, e, a) {
    return ((t = t !== null ? t.childLanes & ~a : 0), e && (t |= ze), t);
  }
  function hh(t, e, a) {
    var i = e.pendingProps,
      r = !1,
      o = (e.flags & 128) !== 0,
      d;
    if (
      ((d = o) || (d = t !== null && t.memoizedState === null ? !1 : (Vt.current & 2) !== 0),
      d && ((r = !0), (e.flags &= -129)),
      (d = (e.flags & 32) !== 0),
      (e.flags &= -33),
      t === null)
    ) {
      if (gt) {
        if (
          (r ? Gn(e) : Qn(),
          (t = Ut)
            ? ((t = _m(t, qe)),
              (t = t !== null && t.data !== "&" ? t : null),
              t !== null &&
                ((e.memoizedState = {
                  dehydrated: t,
                  treeContext: Un !== null ? { id: ke, overflow: Ie } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (a = Zf(t)),
                (a.return = e),
                (e.child = a),
                (le = e),
                (Ut = null)))
            : (t = null),
          t === null)
        )
          throw Nn(e);
        return (Xo(t) ? (e.lanes = 32) : (e.lanes = 536870912), null);
      }
      var v = i.children;
      return (
        (i = i.fallback),
        r
          ? (Qn(),
            (r = e.mode),
            (v = Ns({ mode: "hidden", children: v }, r)),
            (i = ga(i, r, a, null)),
            (v.return = e),
            (i.return = e),
            (v.sibling = i),
            (e.child = v),
            (i = e.child),
            (i.memoizedState = so(a)),
            (i.childLanes = ro(t, d, a)),
            (e.memoizedState = lo),
            cl(null, i))
          : (Gn(e), uo(e, v))
      );
    }
    var A = t.memoizedState;
    if (A !== null && ((v = A.dehydrated), v !== null)) {
      if (o)
        e.flags & 256
          ? (Gn(e), (e.flags &= -257), (e = oo(t, e, a)))
          : e.memoizedState !== null
            ? (Qn(), (e.child = t.child), (e.flags |= 128), (e = null))
            : (Qn(),
              (v = i.fallback),
              (r = e.mode),
              (i = Ns({ mode: "visible", children: i.children }, r)),
              (v = ga(v, r, a, null)),
              (v.flags |= 2),
              (i.return = e),
              (v.return = e),
              (i.sibling = v),
              (e.child = i),
              Ra(e, t.child, null, a),
              (i = e.child),
              (i.memoizedState = so(a)),
              (i.childLanes = ro(t, d, a)),
              (e.memoizedState = lo),
              (e = cl(null, i)));
      else if ((Gn(e), Xo(v))) {
        if (((d = v.nextSibling && v.nextSibling.dataset), d)) var L = d.dgst;
        ((d = L),
          (i = Error(u(419))),
          (i.stack = ""),
          (i.digest = d),
          $i({ value: i, source: null, stack: null }),
          (e = oo(t, e, a)));
      } else if ((Ft || ii(t, e, a, !1), (d = (a & t.childLanes) !== 0), Ft || d)) {
        if (((d = zt), d !== null && ((i = $c(d, a)), i !== 0 && i !== A.retryLane)))
          throw ((A.retryLane = i), pa(t, i), _e(d, t, i), ao);
        (Vo(v) || Ks(), (e = oo(t, e, a)));
      } else
        Vo(v)
          ? ((e.flags |= 192), (e.child = t.child), (e = null))
          : ((t = A.treeContext),
            (Ut = Ge(v.nextSibling)),
            (le = e),
            (gt = !0),
            (Bn = null),
            (qe = !1),
            t !== null && Jf(e, t),
            (e = uo(e, i.children)),
            (e.flags |= 4096));
      return e;
    }
    return r
      ? (Qn(),
        (v = i.fallback),
        (r = e.mode),
        (A = t.child),
        (L = A.sibling),
        (i = on(A, { mode: "hidden", children: i.children })),
        (i.subtreeFlags = A.subtreeFlags & 65011712),
        L !== null ? (v = on(L, v)) : ((v = ga(v, r, a, null)), (v.flags |= 2)),
        (v.return = e),
        (i.return = e),
        (i.sibling = v),
        (e.child = i),
        cl(null, i),
        (i = e.child),
        (v = t.child.memoizedState),
        v === null
          ? (v = so(a))
          : ((r = v.cachePool),
            r !== null
              ? ((A = Zt._currentValue), (r = r.parent !== A ? { parent: A, pool: A } : r))
              : (r = ed()),
            (v = { baseLanes: v.baseLanes | a, cachePool: r })),
        (i.memoizedState = v),
        (i.childLanes = ro(t, d, a)),
        (e.memoizedState = lo),
        cl(t.child, i))
      : (Gn(e),
        (a = t.child),
        (t = a.sibling),
        (a = on(a, { mode: "visible", children: i.children })),
        (a.return = e),
        (a.sibling = null),
        t !== null &&
          ((d = e.deletions), d === null ? ((e.deletions = [t]), (e.flags |= 16)) : d.push(t)),
        (e.child = a),
        (e.memoizedState = null),
        a);
  }
  function uo(t, e) {
    return ((e = Ns({ mode: "visible", children: e }, t.mode)), (e.return = t), (t.child = e));
  }
  function Ns(t, e) {
    return ((t = Me(22, t, null, e)), (t.lanes = 0), t);
  }
  function oo(t, e, a) {
    return (
      Ra(e, t.child, null, a),
      (t = uo(e, e.pendingProps.children)),
      (t.flags |= 2),
      (e.memoizedState = null),
      t
    );
  }
  function mh(t, e, a) {
    t.lanes |= e;
    var i = t.alternate;
    (i !== null && (i.lanes |= e), Ru(t.return, e, a));
  }
  function co(t, e, a, i, r, o) {
    var d = t.memoizedState;
    d === null
      ? (t.memoizedState = {
          isBackwards: e,
          rendering: null,
          renderingStartTime: 0,
          last: i,
          tail: a,
          tailMode: r,
          treeForkCount: o,
        })
      : ((d.isBackwards = e),
        (d.rendering = null),
        (d.renderingStartTime = 0),
        (d.last = i),
        (d.tail = a),
        (d.tailMode = r),
        (d.treeForkCount = o));
  }
  function yh(t, e, a) {
    var i = e.pendingProps,
      r = i.revealOrder,
      o = i.tail;
    i = i.children;
    var d = Vt.current,
      v = (d & 2) !== 0;
    if (
      (v ? ((d = (d & 1) | 2), (e.flags |= 128)) : (d &= 1),
      k(Vt, d),
      re(t, e, i, a),
      (i = gt ? Ii : 0),
      !v && t !== null && (t.flags & 128) !== 0)
    )
      t: for (t = e.child; t !== null; ) {
        if (t.tag === 13) t.memoizedState !== null && mh(t, a, e);
        else if (t.tag === 19) mh(t, a, e);
        else if (t.child !== null) {
          ((t.child.return = t), (t = t.child));
          continue;
        }
        if (t === e) break t;
        for (; t.sibling === null; ) {
          if (t.return === null || t.return === e) break t;
          t = t.return;
        }
        ((t.sibling.return = t.return), (t = t.sibling));
      }
    switch (r) {
      case "forwards":
        for (a = e.child, r = null; a !== null; )
          ((t = a.alternate), t !== null && Rs(t) === null && (r = a), (a = a.sibling));
        ((a = r),
          a === null ? ((r = e.child), (e.child = null)) : ((r = a.sibling), (a.sibling = null)),
          co(e, !1, r, a, o, i));
        break;
      case "backwards":
      case "unstable_legacy-backwards":
        for (a = null, r = e.child, e.child = null; r !== null; ) {
          if (((t = r.alternate), t !== null && Rs(t) === null)) {
            e.child = r;
            break;
          }
          ((t = r.sibling), (r.sibling = a), (a = r), (r = t));
        }
        co(e, !0, a, null, o, i);
        break;
      case "together":
        co(e, !1, null, null, void 0, i);
        break;
      default:
        e.memoizedState = null;
    }
    return e.child;
  }
  function yn(t, e, a) {
    if (
      (t !== null && (e.dependencies = t.dependencies), (Kn |= e.lanes), (a & e.childLanes) === 0)
    )
      if (t !== null) {
        if ((ii(t, e, a, !1), (a & e.childLanes) === 0)) return null;
      } else return null;
    if (t !== null && e.child !== t.child) throw Error(u(153));
    if (e.child !== null) {
      for (t = e.child, a = on(t, t.pendingProps), e.child = a, a.return = e; t.sibling !== null; )
        ((t = t.sibling), (a = a.sibling = on(t, t.pendingProps)), (a.return = e));
      a.sibling = null;
    }
    return e.child;
  }
  function fo(t, e) {
    return (t.lanes & e) !== 0 ? !0 : ((t = t.dependencies), !!(t !== null && ps(t)));
  }
  function Qv(t, e, a) {
    switch (e.tag) {
      case 3:
        (Qt(e, e.stateNode.containerInfo), jn(e, Zt, t.memoizedState.cache), va());
        break;
      case 27:
      case 5:
        nn(e);
        break;
      case 4:
        Qt(e, e.stateNode.containerInfo);
        break;
      case 10:
        jn(e, e.type, e.memoizedProps.value);
        break;
      case 31:
        if (e.memoizedState !== null) return ((e.flags |= 128), Nu(e), null);
        break;
      case 13:
        var i = e.memoizedState;
        if (i !== null)
          return i.dehydrated !== null
            ? (Gn(e), (e.flags |= 128), null)
            : (a & e.child.childLanes) !== 0
              ? hh(t, e, a)
              : (Gn(e), (t = yn(t, e, a)), t !== null ? t.sibling : null);
        Gn(e);
        break;
      case 19:
        var r = (t.flags & 128) !== 0;
        if (
          ((i = (a & e.childLanes) !== 0),
          i || (ii(t, e, a, !1), (i = (a & e.childLanes) !== 0)),
          r)
        ) {
          if (i) return yh(t, e, a);
          e.flags |= 128;
        }
        if (
          ((r = e.memoizedState),
          r !== null && ((r.rendering = null), (r.tail = null), (r.lastEffect = null)),
          k(Vt, Vt.current),
          i)
        )
          break;
        return null;
      case 22:
        return ((e.lanes = 0), rh(t, e, a, e.pendingProps));
      case 24:
        jn(e, Zt, t.memoizedState.cache);
    }
    return yn(t, e, a);
  }
  function ph(t, e, a) {
    if (t !== null)
      if (t.memoizedProps !== e.pendingProps) Ft = !0;
      else {
        if (!fo(t, a) && (e.flags & 128) === 0) return ((Ft = !1), Qv(t, e, a));
        Ft = (t.flags & 131072) !== 0;
      }
    else ((Ft = !1), gt && (e.flags & 1048576) !== 0 && Ff(e, Ii, e.index));
    switch (((e.lanes = 0), e.tag)) {
      case 16:
        t: {
          var i = e.pendingProps;
          if (((t = Ea(e.elementType)), (e.type = t), typeof t == "function"))
            pu(t)
              ? ((i = xa(t, i)), (e.tag = 1), (e = fh(null, e, t, i, a)))
              : ((e.tag = 0), (e = io(null, e, t, i, a)));
          else {
            if (t != null) {
              var r = t.$$typeof;
              if (r === Y) {
                ((e.tag = 11), (e = ih(null, e, t, i, a)));
                break t;
              } else if (r === q) {
                ((e.tag = 14), (e = lh(null, e, t, i, a)));
                break t;
              }
            }
            throw ((e = Nt(t) || t), Error(u(306, e, "")));
          }
        }
        return e;
      case 0:
        return io(t, e, e.type, e.pendingProps, a);
      case 1:
        return ((i = e.type), (r = xa(i, e.pendingProps)), fh(t, e, i, r, a));
      case 3:
        t: {
          if ((Qt(e, e.stateNode.containerInfo), t === null)) throw Error(u(387));
          i = e.pendingProps;
          var o = e.memoizedState;
          ((r = o.element), zu(t, e), ll(e, i, null, a));
          var d = e.memoizedState;
          if (
            ((i = d.cache),
            jn(e, Zt, i),
            i !== o.cache && Au(e, [Zt], a, !0),
            il(),
            (i = d.element),
            o.isDehydrated)
          )
            if (
              ((o = { element: i, isDehydrated: !1, cache: d.cache }),
              (e.updateQueue.baseState = o),
              (e.memoizedState = o),
              e.flags & 256)
            ) {
              e = dh(t, e, i, a);
              break t;
            } else if (i !== r) {
              ((r = Ne(Error(u(424)), e)), $i(r), (e = dh(t, e, i, a)));
              break t;
            } else
              for (
                t = e.stateNode.containerInfo,
                  t.nodeType === 9
                    ? (t = t.body)
                    : (t = t.nodeName === "HTML" ? t.ownerDocument.body : t),
                  Ut = Ge(t.firstChild),
                  le = e,
                  gt = !0,
                  Bn = null,
                  qe = !0,
                  a = rd(e, null, i, a),
                  e.child = a;
                a;
              )
                ((a.flags = (a.flags & -3) | 4096), (a = a.sibling));
          else {
            if ((va(), i === r)) {
              e = yn(t, e, a);
              break t;
            }
            re(t, e, i, a);
          }
          e = e.child;
        }
        return e;
      case 26:
        return (
          Bs(t, e),
          t === null
            ? (a = Mm(e.type, null, e.pendingProps, null))
              ? (e.memoizedState = a)
              : gt ||
                ((a = e.type),
                (t = e.pendingProps),
                (i = $s(dt.current).createElement(a)),
                (i[ie] = e),
                (i[ye] = t),
                ue(i, a, t),
                te(i),
                (e.stateNode = i))
            : (e.memoizedState = Mm(e.type, t.memoizedProps, e.pendingProps, t.memoizedState)),
          null
        );
      case 27:
        return (
          nn(e),
          t === null &&
            gt &&
            ((i = e.stateNode = Rm(e.type, e.pendingProps, dt.current)),
            (le = e),
            (qe = !0),
            (r = Ut),
            kn(e.type) ? ((Ko = r), (Ut = Ge(i.firstChild))) : (Ut = r)),
          re(t, e, e.pendingProps.children, a),
          Bs(t, e),
          t === null && (e.flags |= 4194304),
          e.child
        );
      case 5:
        return (
          t === null &&
            gt &&
            ((r = i = Ut) &&
              ((i = v0(i, e.type, e.pendingProps, qe)),
              i !== null
                ? ((e.stateNode = i), (le = e), (Ut = Ge(i.firstChild)), (qe = !1), (r = !0))
                : (r = !1)),
            r || Nn(e)),
          nn(e),
          (r = e.type),
          (o = e.pendingProps),
          (d = t !== null ? t.memoizedProps : null),
          (i = o.children),
          Yo(r, o) ? (i = null) : d !== null && Yo(r, d) && (e.flags |= 32),
          e.memoizedState !== null && ((r = Hu(t, e, Lv, null, null, a)), (Al._currentValue = r)),
          Bs(t, e),
          re(t, e, i, a),
          e.child
        );
      case 6:
        return (
          t === null &&
            gt &&
            ((t = a = Ut) &&
              ((a = S0(a, e.pendingProps, qe)),
              a !== null ? ((e.stateNode = a), (le = e), (Ut = null), (t = !0)) : (t = !1)),
            t || Nn(e)),
          null
        );
      case 13:
        return hh(t, e, a);
      case 4:
        return (
          Qt(e, e.stateNode.containerInfo),
          (i = e.pendingProps),
          t === null ? (e.child = Ra(e, null, i, a)) : re(t, e, i, a),
          e.child
        );
      case 11:
        return ih(t, e, e.type, e.pendingProps, a);
      case 7:
        return (re(t, e, e.pendingProps, a), e.child);
      case 8:
        return (re(t, e, e.pendingProps.children, a), e.child);
      case 12:
        return (re(t, e, e.pendingProps.children, a), e.child);
      case 10:
        return ((i = e.pendingProps), jn(e, e.type, i.value), re(t, e, i.children, a), e.child);
      case 9:
        return (
          (r = e.type._context),
          (i = e.pendingProps.children),
          ba(e),
          (r = se(r)),
          (i = i(r)),
          (e.flags |= 1),
          re(t, e, i, a),
          e.child
        );
      case 14:
        return lh(t, e, e.type, e.pendingProps, a);
      case 15:
        return sh(t, e, e.type, e.pendingProps, a);
      case 19:
        return yh(t, e, a);
      case 31:
        return Gv(t, e, a);
      case 22:
        return rh(t, e, a, e.pendingProps);
      case 24:
        return (
          ba(e),
          (i = se(Zt)),
          t === null
            ? ((r = Cu()),
              r === null &&
                ((r = zt),
                (o = xu()),
                (r.pooledCache = o),
                o.refCount++,
                o !== null && (r.pooledCacheLanes |= a),
                (r = o)),
              (e.memoizedState = { parent: i, cache: r }),
              Ou(e),
              jn(e, Zt, r))
            : ((t.lanes & a) !== 0 && (zu(t, e), ll(e, null, null, a), il()),
              (r = t.memoizedState),
              (o = e.memoizedState),
              r.parent !== i
                ? ((r = { parent: i, cache: i }),
                  (e.memoizedState = r),
                  e.lanes === 0 && (e.memoizedState = e.updateQueue.baseState = r),
                  jn(e, Zt, i))
                : ((i = o.cache), jn(e, Zt, i), i !== r.cache && Au(e, [Zt], a, !0))),
          re(t, e, e.pendingProps.children, a),
          e.child
        );
      case 29:
        throw e.pendingProps;
    }
    throw Error(u(156, e.tag));
  }
  function pn(t) {
    t.flags |= 4;
  }
  function ho(t, e, a, i, r) {
    if (((e = (t.mode & 32) !== 0) && (e = !1), e)) {
      if (((t.flags |= 16777216), (r & 335544128) === r))
        if (t.stateNode.complete) t.flags |= 8192;
        else if (Vh()) t.flags |= 8192;
        else throw ((Ta = bs), wu);
    } else t.flags &= -16777217;
  }
  function gh(t, e) {
    if (e.type !== "stylesheet" || (e.state.loading & 4) !== 0) t.flags &= -16777217;
    else if (((t.flags |= 16777216), !Dm(e)))
      if (Vh()) t.flags |= 8192;
      else throw ((Ta = bs), wu);
  }
  function js(t, e) {
    (e !== null && (t.flags |= 4),
      t.flags & 16384 && ((e = t.tag !== 22 ? Jc() : 536870912), (t.lanes |= e), (pi |= e)));
  }
  function fl(t, e) {
    if (!gt)
      switch (t.tailMode) {
        case "hidden":
          e = t.tail;
          for (var a = null; e !== null; ) (e.alternate !== null && (a = e), (e = e.sibling));
          a === null ? (t.tail = null) : (a.sibling = null);
          break;
        case "collapsed":
          a = t.tail;
          for (var i = null; a !== null; ) (a.alternate !== null && (i = a), (a = a.sibling));
          i === null
            ? e || t.tail === null
              ? (t.tail = null)
              : (t.tail.sibling = null)
            : (i.sibling = null);
      }
  }
  function Bt(t) {
    var e = t.alternate !== null && t.alternate.child === t.child,
      a = 0,
      i = 0;
    if (e)
      for (var r = t.child; r !== null; )
        ((a |= r.lanes | r.childLanes),
          (i |= r.subtreeFlags & 65011712),
          (i |= r.flags & 65011712),
          (r.return = t),
          (r = r.sibling));
    else
      for (r = t.child; r !== null; )
        ((a |= r.lanes | r.childLanes),
          (i |= r.subtreeFlags),
          (i |= r.flags),
          (r.return = t),
          (r = r.sibling));
    return ((t.subtreeFlags |= i), (t.childLanes = a), e);
  }
  function Vv(t, e, a) {
    var i = e.pendingProps;
    switch ((bu(e), e.tag)) {
      case 16:
      case 15:
      case 0:
      case 11:
      case 7:
      case 8:
      case 12:
      case 9:
      case 14:
        return (Bt(e), null);
      case 1:
        return (Bt(e), null);
      case 3:
        return (
          (a = e.stateNode),
          (i = null),
          t !== null && (i = t.memoizedState.cache),
          e.memoizedState.cache !== i && (e.flags |= 2048),
          dn(Zt),
          Ot(),
          a.pendingContext && ((a.context = a.pendingContext), (a.pendingContext = null)),
          (t === null || t.child === null) &&
            (ai(e)
              ? pn(e)
              : t === null ||
                (t.memoizedState.isDehydrated && (e.flags & 256) === 0) ||
                ((e.flags |= 1024), Eu())),
          Bt(e),
          null
        );
      case 26:
        var r = e.type,
          o = e.memoizedState;
        return (
          t === null
            ? (pn(e), o !== null ? (Bt(e), gh(e, o)) : (Bt(e), ho(e, r, null, i, a)))
            : o
              ? o !== t.memoizedState
                ? (pn(e), Bt(e), gh(e, o))
                : (Bt(e), (e.flags &= -16777217))
              : ((t = t.memoizedProps), t !== i && pn(e), Bt(e), ho(e, r, t, i, a)),
          null
        );
      case 27:
        if ((an(e), (a = dt.current), (r = e.type), t !== null && e.stateNode != null))
          t.memoizedProps !== i && pn(e);
        else {
          if (!i) {
            if (e.stateNode === null) throw Error(u(166));
            return (Bt(e), null);
          }
          ((t = $.current), ai(e) ? kf(e) : ((t = Rm(r, i, a)), (e.stateNode = t), pn(e)));
        }
        return (Bt(e), null);
      case 5:
        if ((an(e), (r = e.type), t !== null && e.stateNode != null))
          t.memoizedProps !== i && pn(e);
        else {
          if (!i) {
            if (e.stateNode === null) throw Error(u(166));
            return (Bt(e), null);
          }
          if (((o = $.current), ai(e))) kf(e);
          else {
            var d = $s(dt.current);
            switch (o) {
              case 1:
                o = d.createElementNS("http://www.w3.org/2000/svg", r);
                break;
              case 2:
                o = d.createElementNS("http://www.w3.org/1998/Math/MathML", r);
                break;
              default:
                switch (r) {
                  case "svg":
                    o = d.createElementNS("http://www.w3.org/2000/svg", r);
                    break;
                  case "math":
                    o = d.createElementNS("http://www.w3.org/1998/Math/MathML", r);
                    break;
                  case "script":
                    ((o = d.createElement("div")),
                      (o.innerHTML = "<script><\/script>"),
                      (o = o.removeChild(o.firstChild)));
                    break;
                  case "select":
                    ((o =
                      typeof i.is == "string"
                        ? d.createElement("select", { is: i.is })
                        : d.createElement("select")),
                      i.multiple ? (o.multiple = !0) : i.size && (o.size = i.size));
                    break;
                  default:
                    o =
                      typeof i.is == "string"
                        ? d.createElement(r, { is: i.is })
                        : d.createElement(r);
                }
            }
            ((o[ie] = e), (o[ye] = i));
            t: for (d = e.child; d !== null; ) {
              if (d.tag === 5 || d.tag === 6) o.appendChild(d.stateNode);
              else if (d.tag !== 4 && d.tag !== 27 && d.child !== null) {
                ((d.child.return = d), (d = d.child));
                continue;
              }
              if (d === e) break t;
              for (; d.sibling === null; ) {
                if (d.return === null || d.return === e) break t;
                d = d.return;
              }
              ((d.sibling.return = d.return), (d = d.sibling));
            }
            e.stateNode = o;
            t: switch ((ue(o, r, i), r)) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                i = !!i.autoFocus;
                break t;
              case "img":
                i = !0;
                break t;
              default:
                i = !1;
            }
            i && pn(e);
          }
        }
        return (Bt(e), ho(e, e.type, t === null ? null : t.memoizedProps, e.pendingProps, a), null);
      case 6:
        if (t && e.stateNode != null) t.memoizedProps !== i && pn(e);
        else {
          if (typeof i != "string" && e.stateNode === null) throw Error(u(166));
          if (((t = dt.current), ai(e))) {
            if (((t = e.stateNode), (a = e.memoizedProps), (i = null), (r = le), r !== null))
              switch (r.tag) {
                case 27:
                case 5:
                  i = r.memoizedProps;
              }
            ((t[ie] = e),
              (t = !!(
                t.nodeValue === a ||
                (i !== null && i.suppressHydrationWarning === !0) ||
                hm(t.nodeValue, a)
              )),
              t || Nn(e, !0));
          } else ((t = $s(t).createTextNode(i)), (t[ie] = e), (e.stateNode = t));
        }
        return (Bt(e), null);
      case 31:
        if (((a = e.memoizedState), t === null || t.memoizedState !== null)) {
          if (((i = ai(e)), a !== null)) {
            if (t === null) {
              if (!i) throw Error(u(318));
              if (((t = e.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
                throw Error(u(557));
              t[ie] = e;
            } else (va(), (e.flags & 128) === 0 && (e.memoizedState = null), (e.flags |= 4));
            (Bt(e), (t = !1));
          } else
            ((a = Eu()),
              t !== null && t.memoizedState !== null && (t.memoizedState.hydrationErrors = a),
              (t = !0));
          if (!t) return e.flags & 256 ? (we(e), e) : (we(e), null);
          if ((e.flags & 128) !== 0) throw Error(u(558));
        }
        return (Bt(e), null);
      case 13:
        if (
          ((i = e.memoizedState),
          t === null || (t.memoizedState !== null && t.memoizedState.dehydrated !== null))
        ) {
          if (((r = ai(e)), i !== null && i.dehydrated !== null)) {
            if (t === null) {
              if (!r) throw Error(u(318));
              if (((r = e.memoizedState), (r = r !== null ? r.dehydrated : null), !r))
                throw Error(u(317));
              r[ie] = e;
            } else (va(), (e.flags & 128) === 0 && (e.memoizedState = null), (e.flags |= 4));
            (Bt(e), (r = !1));
          } else
            ((r = Eu()),
              t !== null && t.memoizedState !== null && (t.memoizedState.hydrationErrors = r),
              (r = !0));
          if (!r) return e.flags & 256 ? (we(e), e) : (we(e), null);
        }
        return (
          we(e),
          (e.flags & 128) !== 0
            ? ((e.lanes = a), e)
            : ((a = i !== null),
              (t = t !== null && t.memoizedState !== null),
              a &&
                ((i = e.child),
                (r = null),
                i.alternate !== null &&
                  i.alternate.memoizedState !== null &&
                  i.alternate.memoizedState.cachePool !== null &&
                  (r = i.alternate.memoizedState.cachePool.pool),
                (o = null),
                i.memoizedState !== null &&
                  i.memoizedState.cachePool !== null &&
                  (o = i.memoizedState.cachePool.pool),
                o !== r && (i.flags |= 2048)),
              a !== t && a && (e.child.flags |= 8192),
              js(e, e.updateQueue),
              Bt(e),
              null)
        );
      case 4:
        return (Ot(), t === null && Bo(e.stateNode.containerInfo), Bt(e), null);
      case 10:
        return (dn(e.type), Bt(e), null);
      case 19:
        if ((Q(Vt), (i = e.memoizedState), i === null)) return (Bt(e), null);
        if (((r = (e.flags & 128) !== 0), (o = i.rendering), o === null))
          if (r) fl(i, !1);
          else {
            if (Yt !== 0 || (t !== null && (t.flags & 128) !== 0))
              for (t = e.child; t !== null; ) {
                if (((o = Rs(t)), o !== null)) {
                  for (
                    e.flags |= 128,
                      fl(i, !1),
                      t = o.updateQueue,
                      e.updateQueue = t,
                      js(e, t),
                      e.subtreeFlags = 0,
                      t = a,
                      a = e.child;
                    a !== null;
                  )
                    (Kf(a, t), (a = a.sibling));
                  return (k(Vt, (Vt.current & 1) | 2), gt && cn(e, i.treeForkCount), e.child);
                }
                t = t.sibling;
              }
            i.tail !== null &&
              ce() > Qs &&
              ((e.flags |= 128), (r = !0), fl(i, !1), (e.lanes = 4194304));
          }
        else {
          if (!r)
            if (((t = Rs(o)), t !== null)) {
              if (
                ((e.flags |= 128),
                (r = !0),
                (t = t.updateQueue),
                (e.updateQueue = t),
                js(e, t),
                fl(i, !0),
                i.tail === null && i.tailMode === "hidden" && !o.alternate && !gt)
              )
                return (Bt(e), null);
            } else
              2 * ce() - i.renderingStartTime > Qs &&
                a !== 536870912 &&
                ((e.flags |= 128), (r = !0), fl(i, !1), (e.lanes = 4194304));
          i.isBackwards
            ? ((o.sibling = e.child), (e.child = o))
            : ((t = i.last), t !== null ? (t.sibling = o) : (e.child = o), (i.last = o));
        }
        return i.tail !== null
          ? ((t = i.tail),
            (i.rendering = t),
            (i.tail = t.sibling),
            (i.renderingStartTime = ce()),
            (t.sibling = null),
            (a = Vt.current),
            k(Vt, r ? (a & 1) | 2 : a & 1),
            gt && cn(e, i.treeForkCount),
            t)
          : (Bt(e), null);
      case 22:
      case 23:
        return (
          we(e),
          Bu(),
          (i = e.memoizedState !== null),
          t !== null
            ? (t.memoizedState !== null) !== i && (e.flags |= 8192)
            : i && (e.flags |= 8192),
          i
            ? (a & 536870912) !== 0 &&
              (e.flags & 128) === 0 &&
              (Bt(e), e.subtreeFlags & 6 && (e.flags |= 8192))
            : Bt(e),
          (a = e.updateQueue),
          a !== null && js(e, a.retryQueue),
          (a = null),
          t !== null &&
            t.memoizedState !== null &&
            t.memoizedState.cachePool !== null &&
            (a = t.memoizedState.cachePool.pool),
          (i = null),
          e.memoizedState !== null &&
            e.memoizedState.cachePool !== null &&
            (i = e.memoizedState.cachePool.pool),
          i !== a && (e.flags |= 2048),
          t !== null && Q(_a),
          null
        );
      case 24:
        return (
          (a = null),
          t !== null && (a = t.memoizedState.cache),
          e.memoizedState.cache !== a && (e.flags |= 2048),
          dn(Zt),
          Bt(e),
          null
        );
      case 25:
        return null;
      case 30:
        return null;
    }
    throw Error(u(156, e.tag));
  }
  function Xv(t, e) {
    switch ((bu(e), e.tag)) {
      case 1:
        return ((t = e.flags), t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null);
      case 3:
        return (
          dn(Zt),
          Ot(),
          (t = e.flags),
          (t & 65536) !== 0 && (t & 128) === 0 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 26:
      case 27:
      case 5:
        return (an(e), null);
      case 31:
        if (e.memoizedState !== null) {
          if ((we(e), e.alternate === null)) throw Error(u(340));
          va();
        }
        return ((t = e.flags), t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null);
      case 13:
        if ((we(e), (t = e.memoizedState), t !== null && t.dehydrated !== null)) {
          if (e.alternate === null) throw Error(u(340));
          va();
        }
        return ((t = e.flags), t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null);
      case 19:
        return (Q(Vt), null);
      case 4:
        return (Ot(), null);
      case 10:
        return (dn(e.type), null);
      case 22:
      case 23:
        return (
          we(e),
          Bu(),
          t !== null && Q(_a),
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 24:
        return (dn(Zt), null);
      case 25:
        return null;
      default:
        return null;
    }
  }
  function vh(t, e) {
    switch ((bu(e), e.tag)) {
      case 3:
        (dn(Zt), Ot());
        break;
      case 26:
      case 27:
      case 5:
        an(e);
        break;
      case 4:
        Ot();
        break;
      case 31:
        e.memoizedState !== null && we(e);
        break;
      case 13:
        we(e);
        break;
      case 19:
        Q(Vt);
        break;
      case 10:
        dn(e.type);
        break;
      case 22:
      case 23:
        (we(e), Bu(), t !== null && Q(_a));
        break;
      case 24:
        dn(Zt);
    }
  }
  function dl(t, e) {
    try {
      var a = e.updateQueue,
        i = a !== null ? a.lastEffect : null;
      if (i !== null) {
        var r = i.next;
        a = r;
        do {
          if ((a.tag & t) === t) {
            i = void 0;
            var o = a.create,
              d = a.inst;
            ((i = o()), (d.destroy = i));
          }
          a = a.next;
        } while (a !== r);
      }
    } catch (v) {
      xt(e, e.return, v);
    }
  }
  function Vn(t, e, a) {
    try {
      var i = e.updateQueue,
        r = i !== null ? i.lastEffect : null;
      if (r !== null) {
        var o = r.next;
        i = o;
        do {
          if ((i.tag & t) === t) {
            var d = i.inst,
              v = d.destroy;
            if (v !== void 0) {
              ((d.destroy = void 0), (r = e));
              var A = a,
                L = v;
              try {
                L();
              } catch (H) {
                xt(r, A, H);
              }
            }
          }
          i = i.next;
        } while (i !== o);
      }
    } catch (H) {
      xt(e, e.return, H);
    }
  }
  function Sh(t) {
    var e = t.updateQueue;
    if (e !== null) {
      var a = t.stateNode;
      try {
        od(e, a);
      } catch (i) {
        xt(t, t.return, i);
      }
    }
  }
  function bh(t, e, a) {
    ((a.props = xa(t.type, t.memoizedProps)), (a.state = t.memoizedState));
    try {
      a.componentWillUnmount();
    } catch (i) {
      xt(t, e, i);
    }
  }
  function hl(t, e) {
    try {
      var a = t.ref;
      if (a !== null) {
        switch (t.tag) {
          case 26:
          case 27:
          case 5:
            var i = t.stateNode;
            break;
          case 30:
            i = t.stateNode;
            break;
          default:
            i = t.stateNode;
        }
        typeof a == "function" ? (t.refCleanup = a(i)) : (a.current = i);
      }
    } catch (r) {
      xt(t, e, r);
    }
  }
  function $e(t, e) {
    var a = t.ref,
      i = t.refCleanup;
    if (a !== null)
      if (typeof i == "function")
        try {
          i();
        } catch (r) {
          xt(t, e, r);
        } finally {
          ((t.refCleanup = null), (t = t.alternate), t != null && (t.refCleanup = null));
        }
      else if (typeof a == "function")
        try {
          a(null);
        } catch (r) {
          xt(t, e, r);
        }
      else a.current = null;
  }
  function _h(t) {
    var e = t.type,
      a = t.memoizedProps,
      i = t.stateNode;
    try {
      t: switch (e) {
        case "button":
        case "input":
        case "select":
        case "textarea":
          a.autoFocus && i.focus();
          break t;
        case "img":
          a.src ? (i.src = a.src) : a.srcSet && (i.srcset = a.srcSet);
      }
    } catch (r) {
      xt(t, t.return, r);
    }
  }
  function mo(t, e, a) {
    try {
      var i = t.stateNode;
      (d0(i, t.type, a, e), (i[ye] = e));
    } catch (r) {
      xt(t, t.return, r);
    }
  }
  function Eh(t) {
    return (
      t.tag === 5 || t.tag === 3 || t.tag === 26 || (t.tag === 27 && kn(t.type)) || t.tag === 4
    );
  }
  function yo(t) {
    t: for (;;) {
      for (; t.sibling === null; ) {
        if (t.return === null || Eh(t.return)) return null;
        t = t.return;
      }
      for (
        t.sibling.return = t.return, t = t.sibling;
        t.tag !== 5 && t.tag !== 6 && t.tag !== 18;
      ) {
        if ((t.tag === 27 && kn(t.type)) || t.flags & 2 || t.child === null || t.tag === 4)
          continue t;
        ((t.child.return = t), (t = t.child));
      }
      if (!(t.flags & 2)) return t.stateNode;
    }
  }
  function po(t, e, a) {
    var i = t.tag;
    if (i === 5 || i === 6)
      ((t = t.stateNode),
        e
          ? (a.nodeType === 9
              ? a.body
              : a.nodeName === "HTML"
                ? a.ownerDocument.body
                : a
            ).insertBefore(t, e)
          : ((e = a.nodeType === 9 ? a.body : a.nodeName === "HTML" ? a.ownerDocument.body : a),
            e.appendChild(t),
            (a = a._reactRootContainer),
            a != null || e.onclick !== null || (e.onclick = rn)));
    else if (
      i !== 4 &&
      (i === 27 && kn(t.type) && ((a = t.stateNode), (e = null)), (t = t.child), t !== null)
    )
      for (po(t, e, a), t = t.sibling; t !== null; ) (po(t, e, a), (t = t.sibling));
  }
  function Hs(t, e, a) {
    var i = t.tag;
    if (i === 5 || i === 6) ((t = t.stateNode), e ? a.insertBefore(t, e) : a.appendChild(t));
    else if (i !== 4 && (i === 27 && kn(t.type) && (a = t.stateNode), (t = t.child), t !== null))
      for (Hs(t, e, a), t = t.sibling; t !== null; ) (Hs(t, e, a), (t = t.sibling));
  }
  function Th(t) {
    var e = t.stateNode,
      a = t.memoizedProps;
    try {
      for (var i = t.type, r = e.attributes; r.length; ) e.removeAttributeNode(r[0]);
      (ue(e, i, a), (e[ie] = t), (e[ye] = a));
    } catch (o) {
      xt(t, t.return, o);
    }
  }
  var gn = !1,
    Jt = !1,
    go = !1,
    Rh = typeof WeakSet == "function" ? WeakSet : Set,
    ee = null;
  function Kv(t, e) {
    if (((t = t.containerInfo), (Ho = lr), (t = Nf(t)), ou(t))) {
      if ("selectionStart" in t) var a = { start: t.selectionStart, end: t.selectionEnd };
      else
        t: {
          a = ((a = t.ownerDocument) && a.defaultView) || window;
          var i = a.getSelection && a.getSelection();
          if (i && i.rangeCount !== 0) {
            a = i.anchorNode;
            var r = i.anchorOffset,
              o = i.focusNode;
            i = i.focusOffset;
            try {
              (a.nodeType, o.nodeType);
            } catch {
              a = null;
              break t;
            }
            var d = 0,
              v = -1,
              A = -1,
              L = 0,
              H = 0,
              V = t,
              U = null;
            e: for (;;) {
              for (
                var B;
                V !== a || (r !== 0 && V.nodeType !== 3) || (v = d + r),
                  V !== o || (i !== 0 && V.nodeType !== 3) || (A = d + i),
                  V.nodeType === 3 && (d += V.nodeValue.length),
                  (B = V.firstChild) !== null;
              )
                ((U = V), (V = B));
              for (;;) {
                if (V === t) break e;
                if (
                  (U === a && ++L === r && (v = d),
                  U === o && ++H === i && (A = d),
                  (B = V.nextSibling) !== null)
                )
                  break;
                ((V = U), (U = V.parentNode));
              }
              V = B;
            }
            a = v === -1 || A === -1 ? null : { start: v, end: A };
          } else a = null;
        }
      a = a || { start: 0, end: 0 };
    } else a = null;
    for (qo = { focusedElem: t, selectionRange: a }, lr = !1, ee = e; ee !== null; )
      if (((e = ee), (t = e.child), (e.subtreeFlags & 1028) !== 0 && t !== null))
        ((t.return = e), (ee = t));
      else
        for (; ee !== null; ) {
          switch (((e = ee), (o = e.alternate), (t = e.flags), e.tag)) {
            case 0:
              if (
                (t & 4) !== 0 &&
                ((t = e.updateQueue), (t = t !== null ? t.events : null), t !== null)
              )
                for (a = 0; a < t.length; a++) ((r = t[a]), (r.ref.impl = r.nextImpl));
              break;
            case 11:
            case 15:
              break;
            case 1:
              if ((t & 1024) !== 0 && o !== null) {
                ((t = void 0),
                  (a = e),
                  (r = o.memoizedProps),
                  (o = o.memoizedState),
                  (i = a.stateNode));
                try {
                  var W = xa(a.type, r);
                  ((t = i.getSnapshotBeforeUpdate(W, o)),
                    (i.__reactInternalSnapshotBeforeUpdate = t));
                } catch (rt) {
                  xt(a, a.return, rt);
                }
              }
              break;
            case 3:
              if ((t & 1024) !== 0) {
                if (((t = e.stateNode.containerInfo), (a = t.nodeType), a === 9)) Qo(t);
                else if (a === 1)
                  switch (t.nodeName) {
                    case "HEAD":
                    case "HTML":
                    case "BODY":
                      Qo(t);
                      break;
                    default:
                      t.textContent = "";
                  }
              }
              break;
            case 5:
            case 26:
            case 27:
            case 6:
            case 4:
            case 17:
              break;
            default:
              if ((t & 1024) !== 0) throw Error(u(163));
          }
          if (((t = e.sibling), t !== null)) {
            ((t.return = e.return), (ee = t));
            break;
          }
          ee = e.return;
        }
  }
  function Ah(t, e, a) {
    var i = a.flags;
    switch (a.tag) {
      case 0:
      case 11:
      case 15:
        (Sn(t, a), i & 4 && dl(5, a));
        break;
      case 1:
        if ((Sn(t, a), i & 4))
          if (((t = a.stateNode), e === null))
            try {
              t.componentDidMount();
            } catch (d) {
              xt(a, a.return, d);
            }
          else {
            var r = xa(a.type, e.memoizedProps);
            e = e.memoizedState;
            try {
              t.componentDidUpdate(r, e, t.__reactInternalSnapshotBeforeUpdate);
            } catch (d) {
              xt(a, a.return, d);
            }
          }
        (i & 64 && Sh(a), i & 512 && hl(a, a.return));
        break;
      case 3:
        if ((Sn(t, a), i & 64 && ((t = a.updateQueue), t !== null))) {
          if (((e = null), a.child !== null))
            switch (a.child.tag) {
              case 27:
              case 5:
                e = a.child.stateNode;
                break;
              case 1:
                e = a.child.stateNode;
            }
          try {
            od(t, e);
          } catch (d) {
            xt(a, a.return, d);
          }
        }
        break;
      case 27:
        e === null && i & 4 && Th(a);
      case 26:
      case 5:
        (Sn(t, a), e === null && i & 4 && _h(a), i & 512 && hl(a, a.return));
        break;
      case 12:
        Sn(t, a);
        break;
      case 31:
        (Sn(t, a), i & 4 && Ch(t, a));
        break;
      case 13:
        (Sn(t, a),
          i & 4 && wh(t, a),
          i & 64 &&
            ((t = a.memoizedState),
            t !== null && ((t = t.dehydrated), t !== null && ((a = t0.bind(null, a)), b0(t, a)))));
        break;
      case 22:
        if (((i = a.memoizedState !== null || gn), !i)) {
          ((e = (e !== null && e.memoizedState !== null) || Jt), (r = gn));
          var o = Jt;
          ((gn = i),
            (Jt = e) && !o ? bn(t, a, (a.subtreeFlags & 8772) !== 0) : Sn(t, a),
            (gn = r),
            (Jt = o));
        }
        break;
      case 30:
        break;
      default:
        Sn(t, a);
    }
  }
  function xh(t) {
    var e = t.alternate;
    (e !== null && ((t.alternate = null), xh(e)),
      (t.child = null),
      (t.deletions = null),
      (t.sibling = null),
      t.tag === 5 && ((e = t.stateNode), e !== null && Zr(e)),
      (t.stateNode = null),
      (t.return = null),
      (t.dependencies = null),
      (t.memoizedProps = null),
      (t.memoizedState = null),
      (t.pendingProps = null),
      (t.stateNode = null),
      (t.updateQueue = null));
  }
  var jt = null,
    ge = !1;
  function vn(t, e, a) {
    for (a = a.child; a !== null; ) (Mh(t, e, a), (a = a.sibling));
  }
  function Mh(t, e, a) {
    if (Re && typeof Re.onCommitFiberUnmount == "function")
      try {
        Re.onCommitFiberUnmount(ji, a);
      } catch {}
    switch (a.tag) {
      case 26:
        (Jt || $e(a, e),
          vn(t, e, a),
          a.memoizedState
            ? a.memoizedState.count--
            : a.stateNode && ((a = a.stateNode), a.parentNode.removeChild(a)));
        break;
      case 27:
        Jt || $e(a, e);
        var i = jt,
          r = ge;
        (kn(a.type) && ((jt = a.stateNode), (ge = !1)),
          vn(t, e, a),
          El(a.stateNode),
          (jt = i),
          (ge = r));
        break;
      case 5:
        Jt || $e(a, e);
      case 6:
        if (((i = jt), (r = ge), (jt = null), vn(t, e, a), (jt = i), (ge = r), jt !== null))
          if (ge)
            try {
              (jt.nodeType === 9
                ? jt.body
                : jt.nodeName === "HTML"
                  ? jt.ownerDocument.body
                  : jt
              ).removeChild(a.stateNode);
            } catch (o) {
              xt(a, e, o);
            }
          else
            try {
              jt.removeChild(a.stateNode);
            } catch (o) {
              xt(a, e, o);
            }
        break;
      case 18:
        jt !== null &&
          (ge
            ? ((t = jt),
              Sm(
                t.nodeType === 9 ? t.body : t.nodeName === "HTML" ? t.ownerDocument.body : t,
                a.stateNode,
              ),
              Ri(t))
            : Sm(jt, a.stateNode));
        break;
      case 4:
        ((i = jt),
          (r = ge),
          (jt = a.stateNode.containerInfo),
          (ge = !0),
          vn(t, e, a),
          (jt = i),
          (ge = r));
        break;
      case 0:
      case 11:
      case 14:
      case 15:
        (Vn(2, a, e), Jt || Vn(4, a, e), vn(t, e, a));
        break;
      case 1:
        (Jt ||
          ($e(a, e), (i = a.stateNode), typeof i.componentWillUnmount == "function" && bh(a, e, i)),
          vn(t, e, a));
        break;
      case 21:
        vn(t, e, a);
        break;
      case 22:
        ((Jt = (i = Jt) || a.memoizedState !== null), vn(t, e, a), (Jt = i));
        break;
      default:
        vn(t, e, a);
    }
  }
  function Ch(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate), t !== null && ((t = t.memoizedState), t !== null))
    ) {
      t = t.dehydrated;
      try {
        Ri(t);
      } catch (a) {
        xt(e, e.return, a);
      }
    }
  }
  function wh(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate),
      t !== null && ((t = t.memoizedState), t !== null && ((t = t.dehydrated), t !== null)))
    )
      try {
        Ri(t);
      } catch (a) {
        xt(e, e.return, a);
      }
  }
  function Zv(t) {
    switch (t.tag) {
      case 31:
      case 13:
      case 19:
        var e = t.stateNode;
        return (e === null && (e = t.stateNode = new Rh()), e);
      case 22:
        return (
          (t = t.stateNode),
          (e = t._retryCache),
          e === null && (e = t._retryCache = new Rh()),
          e
        );
      default:
        throw Error(u(435, t.tag));
    }
  }
  function qs(t, e) {
    var a = Zv(t);
    e.forEach(function (i) {
      if (!a.has(i)) {
        a.add(i);
        var r = e0.bind(null, t, i);
        i.then(r, r);
      }
    });
  }
  function ve(t, e) {
    var a = e.deletions;
    if (a !== null)
      for (var i = 0; i < a.length; i++) {
        var r = a[i],
          o = t,
          d = e,
          v = d;
        t: for (; v !== null; ) {
          switch (v.tag) {
            case 27:
              if (kn(v.type)) {
                ((jt = v.stateNode), (ge = !1));
                break t;
              }
              break;
            case 5:
              ((jt = v.stateNode), (ge = !1));
              break t;
            case 3:
            case 4:
              ((jt = v.stateNode.containerInfo), (ge = !0));
              break t;
          }
          v = v.return;
        }
        if (jt === null) throw Error(u(160));
        (Mh(o, d, r),
          (jt = null),
          (ge = !1),
          (o = r.alternate),
          o !== null && (o.return = null),
          (r.return = null));
      }
    if (e.subtreeFlags & 13886) for (e = e.child; e !== null; ) (Oh(e, t), (e = e.sibling));
  }
  var Ke = null;
  function Oh(t, e) {
    var a = t.alternate,
      i = t.flags;
    switch (t.tag) {
      case 0:
      case 11:
      case 14:
      case 15:
        (ve(e, t), Se(t), i & 4 && (Vn(3, t, t.return), dl(3, t), Vn(5, t, t.return)));
        break;
      case 1:
        (ve(e, t),
          Se(t),
          i & 512 && (Jt || a === null || $e(a, a.return)),
          i & 64 &&
            gn &&
            ((t = t.updateQueue),
            t !== null &&
              ((i = t.callbacks),
              i !== null &&
                ((a = t.shared.hiddenCallbacks),
                (t.shared.hiddenCallbacks = a === null ? i : a.concat(i))))));
        break;
      case 26:
        var r = Ke;
        if ((ve(e, t), Se(t), i & 512 && (Jt || a === null || $e(a, a.return)), i & 4)) {
          var o = a !== null ? a.memoizedState : null;
          if (((i = t.memoizedState), a === null))
            if (i === null)
              if (t.stateNode === null) {
                t: {
                  ((i = t.type), (a = t.memoizedProps), (r = r.ownerDocument || r));
                  e: switch (i) {
                    case "title":
                      ((o = r.getElementsByTagName("title")[0]),
                        (!o ||
                          o[Yi] ||
                          o[ie] ||
                          o.namespaceURI === "http://www.w3.org/2000/svg" ||
                          o.hasAttribute("itemprop")) &&
                          ((o = r.createElement(i)),
                          r.head.insertBefore(o, r.querySelector("head > title"))),
                        ue(o, i, a),
                        (o[ie] = t),
                        te(o),
                        (i = o));
                      break t;
                    case "link":
                      var d = Om("link", "href", r).get(i + (a.href || ""));
                      if (d) {
                        for (var v = 0; v < d.length; v++)
                          if (
                            ((o = d[v]),
                            o.getAttribute("href") ===
                              (a.href == null || a.href === "" ? null : a.href) &&
                              o.getAttribute("rel") === (a.rel == null ? null : a.rel) &&
                              o.getAttribute("title") === (a.title == null ? null : a.title) &&
                              o.getAttribute("crossorigin") ===
                                (a.crossOrigin == null ? null : a.crossOrigin))
                          ) {
                            d.splice(v, 1);
                            break e;
                          }
                      }
                      ((o = r.createElement(i)), ue(o, i, a), r.head.appendChild(o));
                      break;
                    case "meta":
                      if ((d = Om("meta", "content", r).get(i + (a.content || "")))) {
                        for (v = 0; v < d.length; v++)
                          if (
                            ((o = d[v]),
                            o.getAttribute("content") ===
                              (a.content == null ? null : "" + a.content) &&
                              o.getAttribute("name") === (a.name == null ? null : a.name) &&
                              o.getAttribute("property") ===
                                (a.property == null ? null : a.property) &&
                              o.getAttribute("http-equiv") ===
                                (a.httpEquiv == null ? null : a.httpEquiv) &&
                              o.getAttribute("charset") === (a.charSet == null ? null : a.charSet))
                          ) {
                            d.splice(v, 1);
                            break e;
                          }
                      }
                      ((o = r.createElement(i)), ue(o, i, a), r.head.appendChild(o));
                      break;
                    default:
                      throw Error(u(468, i));
                  }
                  ((o[ie] = t), te(o), (i = o));
                }
                t.stateNode = i;
              } else zm(r, t.type, t.stateNode);
            else t.stateNode = wm(r, i, t.memoizedProps);
          else
            o !== i
              ? (o === null
                  ? a.stateNode !== null && ((a = a.stateNode), a.parentNode.removeChild(a))
                  : o.count--,
                i === null ? zm(r, t.type, t.stateNode) : wm(r, i, t.memoizedProps))
              : i === null && t.stateNode !== null && mo(t, t.memoizedProps, a.memoizedProps);
        }
        break;
      case 27:
        (ve(e, t),
          Se(t),
          i & 512 && (Jt || a === null || $e(a, a.return)),
          a !== null && i & 4 && mo(t, t.memoizedProps, a.memoizedProps));
        break;
      case 5:
        if ((ve(e, t), Se(t), i & 512 && (Jt || a === null || $e(a, a.return)), t.flags & 32)) {
          r = t.stateNode;
          try {
            Pa(r, "");
          } catch (W) {
            xt(t, t.return, W);
          }
        }
        (i & 4 &&
          t.stateNode != null &&
          ((r = t.memoizedProps), mo(t, r, a !== null ? a.memoizedProps : r)),
          i & 1024 && (go = !0));
        break;
      case 6:
        if ((ve(e, t), Se(t), i & 4)) {
          if (t.stateNode === null) throw Error(u(162));
          ((i = t.memoizedProps), (a = t.stateNode));
          try {
            a.nodeValue = i;
          } catch (W) {
            xt(t, t.return, W);
          }
        }
        break;
      case 3:
        if (
          ((er = null),
          (r = Ke),
          (Ke = Ws(e.containerInfo)),
          ve(e, t),
          (Ke = r),
          Se(t),
          i & 4 && a !== null && a.memoizedState.isDehydrated)
        )
          try {
            Ri(e.containerInfo);
          } catch (W) {
            xt(t, t.return, W);
          }
        go && ((go = !1), zh(t));
        break;
      case 4:
        ((i = Ke), (Ke = Ws(t.stateNode.containerInfo)), ve(e, t), Se(t), (Ke = i));
        break;
      case 12:
        (ve(e, t), Se(t));
        break;
      case 31:
        (ve(e, t),
          Se(t),
          i & 4 && ((i = t.updateQueue), i !== null && ((t.updateQueue = null), qs(t, i))));
        break;
      case 13:
        (ve(e, t),
          Se(t),
          t.child.flags & 8192 &&
            (t.memoizedState !== null) != (a !== null && a.memoizedState !== null) &&
            (Gs = ce()),
          i & 4 && ((i = t.updateQueue), i !== null && ((t.updateQueue = null), qs(t, i))));
        break;
      case 22:
        r = t.memoizedState !== null;
        var A = a !== null && a.memoizedState !== null,
          L = gn,
          H = Jt;
        if (((gn = L || r), (Jt = H || A), ve(e, t), (Jt = H), (gn = L), Se(t), i & 8192))
          t: for (
            e = t.stateNode,
              e._visibility = r ? e._visibility & -2 : e._visibility | 1,
              r && (a === null || A || gn || Jt || Ma(t)),
              a = null,
              e = t;
            ;
          ) {
            if (e.tag === 5 || e.tag === 26) {
              if (a === null) {
                A = a = e;
                try {
                  if (((o = A.stateNode), r))
                    ((d = o.style),
                      typeof d.setProperty == "function"
                        ? d.setProperty("display", "none", "important")
                        : (d.display = "none"));
                  else {
                    v = A.stateNode;
                    var V = A.memoizedProps.style,
                      U = V != null && V.hasOwnProperty("display") ? V.display : null;
                    v.style.display = U == null || typeof U == "boolean" ? "" : ("" + U).trim();
                  }
                } catch (W) {
                  xt(A, A.return, W);
                }
              }
            } else if (e.tag === 6) {
              if (a === null) {
                A = e;
                try {
                  A.stateNode.nodeValue = r ? "" : A.memoizedProps;
                } catch (W) {
                  xt(A, A.return, W);
                }
              }
            } else if (e.tag === 18) {
              if (a === null) {
                A = e;
                try {
                  var B = A.stateNode;
                  r ? bm(B, !0) : bm(A.stateNode, !1);
                } catch (W) {
                  xt(A, A.return, W);
                }
              }
            } else if (
              ((e.tag !== 22 && e.tag !== 23) || e.memoizedState === null || e === t) &&
              e.child !== null
            ) {
              ((e.child.return = e), (e = e.child));
              continue;
            }
            if (e === t) break t;
            for (; e.sibling === null; ) {
              if (e.return === null || e.return === t) break t;
              (a === e && (a = null), (e = e.return));
            }
            (a === e && (a = null), (e.sibling.return = e.return), (e = e.sibling));
          }
        i & 4 &&
          ((i = t.updateQueue),
          i !== null && ((a = i.retryQueue), a !== null && ((i.retryQueue = null), qs(t, a))));
        break;
      case 19:
        (ve(e, t),
          Se(t),
          i & 4 && ((i = t.updateQueue), i !== null && ((t.updateQueue = null), qs(t, i))));
        break;
      case 30:
        break;
      case 21:
        break;
      default:
        (ve(e, t), Se(t));
    }
  }
  function Se(t) {
    var e = t.flags;
    if (e & 2) {
      try {
        for (var a, i = t.return; i !== null; ) {
          if (Eh(i)) {
            a = i;
            break;
          }
          i = i.return;
        }
        if (a == null) throw Error(u(160));
        switch (a.tag) {
          case 27:
            var r = a.stateNode,
              o = yo(t);
            Hs(t, o, r);
            break;
          case 5:
            var d = a.stateNode;
            a.flags & 32 && (Pa(d, ""), (a.flags &= -33));
            var v = yo(t);
            Hs(t, v, d);
            break;
          case 3:
          case 4:
            var A = a.stateNode.containerInfo,
              L = yo(t);
            po(t, L, A);
            break;
          default:
            throw Error(u(161));
        }
      } catch (H) {
        xt(t, t.return, H);
      }
      t.flags &= -3;
    }
    e & 4096 && (t.flags &= -4097);
  }
  function zh(t) {
    if (t.subtreeFlags & 1024)
      for (t = t.child; t !== null; ) {
        var e = t;
        (zh(e), e.tag === 5 && e.flags & 1024 && e.stateNode.reset(), (t = t.sibling));
      }
  }
  function Sn(t, e) {
    if (e.subtreeFlags & 8772)
      for (e = e.child; e !== null; ) (Ah(t, e.alternate, e), (e = e.sibling));
  }
  function Ma(t) {
    for (t = t.child; t !== null; ) {
      var e = t;
      switch (e.tag) {
        case 0:
        case 11:
        case 14:
        case 15:
          (Vn(4, e, e.return), Ma(e));
          break;
        case 1:
          $e(e, e.return);
          var a = e.stateNode;
          (typeof a.componentWillUnmount == "function" && bh(e, e.return, a), Ma(e));
          break;
        case 27:
          El(e.stateNode);
        case 26:
        case 5:
          ($e(e, e.return), Ma(e));
          break;
        case 22:
          e.memoizedState === null && Ma(e);
          break;
        case 30:
          Ma(e);
          break;
        default:
          Ma(e);
      }
      t = t.sibling;
    }
  }
  function bn(t, e, a) {
    for (a = a && (e.subtreeFlags & 8772) !== 0, e = e.child; e !== null; ) {
      var i = e.alternate,
        r = t,
        o = e,
        d = o.flags;
      switch (o.tag) {
        case 0:
        case 11:
        case 15:
          (bn(r, o, a), dl(4, o));
          break;
        case 1:
          if ((bn(r, o, a), (i = o), (r = i.stateNode), typeof r.componentDidMount == "function"))
            try {
              r.componentDidMount();
            } catch (L) {
              xt(i, i.return, L);
            }
          if (((i = o), (r = i.updateQueue), r !== null)) {
            var v = i.stateNode;
            try {
              var A = r.shared.hiddenCallbacks;
              if (A !== null)
                for (r.shared.hiddenCallbacks = null, r = 0; r < A.length; r++) ud(A[r], v);
            } catch (L) {
              xt(i, i.return, L);
            }
          }
          (a && d & 64 && Sh(o), hl(o, o.return));
          break;
        case 27:
          Th(o);
        case 26:
        case 5:
          (bn(r, o, a), a && i === null && d & 4 && _h(o), hl(o, o.return));
          break;
        case 12:
          bn(r, o, a);
          break;
        case 31:
          (bn(r, o, a), a && d & 4 && Ch(r, o));
          break;
        case 13:
          (bn(r, o, a), a && d & 4 && wh(r, o));
          break;
        case 22:
          (o.memoizedState === null && bn(r, o, a), hl(o, o.return));
          break;
        case 30:
          break;
        default:
          bn(r, o, a);
      }
      e = e.sibling;
    }
  }
  function vo(t, e) {
    var a = null;
    (t !== null &&
      t.memoizedState !== null &&
      t.memoizedState.cachePool !== null &&
      (a = t.memoizedState.cachePool.pool),
      (t = null),
      e.memoizedState !== null &&
        e.memoizedState.cachePool !== null &&
        (t = e.memoizedState.cachePool.pool),
      t !== a && (t != null && t.refCount++, a != null && Wi(a)));
  }
  function So(t, e) {
    ((t = null),
      e.alternate !== null && (t = e.alternate.memoizedState.cache),
      (e = e.memoizedState.cache),
      e !== t && (e.refCount++, t != null && Wi(t)));
  }
  function Ze(t, e, a, i) {
    if (e.subtreeFlags & 10256) for (e = e.child; e !== null; ) (Dh(t, e, a, i), (e = e.sibling));
  }
  function Dh(t, e, a, i) {
    var r = e.flags;
    switch (e.tag) {
      case 0:
      case 11:
      case 15:
        (Ze(t, e, a, i), r & 2048 && dl(9, e));
        break;
      case 1:
        Ze(t, e, a, i);
        break;
      case 3:
        (Ze(t, e, a, i),
          r & 2048 &&
            ((t = null),
            e.alternate !== null && (t = e.alternate.memoizedState.cache),
            (e = e.memoizedState.cache),
            e !== t && (e.refCount++, t != null && Wi(t))));
        break;
      case 12:
        if (r & 2048) {
          (Ze(t, e, a, i), (t = e.stateNode));
          try {
            var o = e.memoizedProps,
              d = o.id,
              v = o.onPostCommit;
            typeof v == "function" &&
              v(d, e.alternate === null ? "mount" : "update", t.passiveEffectDuration, -0);
          } catch (A) {
            xt(e, e.return, A);
          }
        } else Ze(t, e, a, i);
        break;
      case 31:
        Ze(t, e, a, i);
        break;
      case 13:
        Ze(t, e, a, i);
        break;
      case 23:
        break;
      case 22:
        ((o = e.stateNode),
          (d = e.alternate),
          e.memoizedState !== null
            ? o._visibility & 2
              ? Ze(t, e, a, i)
              : ml(t, e)
            : o._visibility & 2
              ? Ze(t, e, a, i)
              : ((o._visibility |= 2), hi(t, e, a, i, (e.subtreeFlags & 10256) !== 0 || !1)),
          r & 2048 && vo(d, e));
        break;
      case 24:
        (Ze(t, e, a, i), r & 2048 && So(e.alternate, e));
        break;
      default:
        Ze(t, e, a, i);
    }
  }
  function hi(t, e, a, i, r) {
    for (r = r && ((e.subtreeFlags & 10256) !== 0 || !1), e = e.child; e !== null; ) {
      var o = t,
        d = e,
        v = a,
        A = i,
        L = d.flags;
      switch (d.tag) {
        case 0:
        case 11:
        case 15:
          (hi(o, d, v, A, r), dl(8, d));
          break;
        case 23:
          break;
        case 22:
          var H = d.stateNode;
          (d.memoizedState !== null
            ? H._visibility & 2
              ? hi(o, d, v, A, r)
              : ml(o, d)
            : ((H._visibility |= 2), hi(o, d, v, A, r)),
            r && L & 2048 && vo(d.alternate, d));
          break;
        case 24:
          (hi(o, d, v, A, r), r && L & 2048 && So(d.alternate, d));
          break;
        default:
          hi(o, d, v, A, r);
      }
      e = e.sibling;
    }
  }
  function ml(t, e) {
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; ) {
        var a = t,
          i = e,
          r = i.flags;
        switch (i.tag) {
          case 22:
            (ml(a, i), r & 2048 && vo(i.alternate, i));
            break;
          case 24:
            (ml(a, i), r & 2048 && So(i.alternate, i));
            break;
          default:
            ml(a, i);
        }
        e = e.sibling;
      }
  }
  var yl = 8192;
  function mi(t, e, a) {
    if (t.subtreeFlags & yl) for (t = t.child; t !== null; ) (Lh(t, e, a), (t = t.sibling));
  }
  function Lh(t, e, a) {
    switch (t.tag) {
      case 26:
        (mi(t, e, a),
          t.flags & yl && t.memoizedState !== null && D0(a, Ke, t.memoizedState, t.memoizedProps));
        break;
      case 5:
        mi(t, e, a);
        break;
      case 3:
      case 4:
        var i = Ke;
        ((Ke = Ws(t.stateNode.containerInfo)), mi(t, e, a), (Ke = i));
        break;
      case 22:
        t.memoizedState === null &&
          ((i = t.alternate),
          i !== null && i.memoizedState !== null
            ? ((i = yl), (yl = 16777216), mi(t, e, a), (yl = i))
            : mi(t, e, a));
        break;
      default:
        mi(t, e, a);
    }
  }
  function Uh(t) {
    var e = t.alternate;
    if (e !== null && ((t = e.child), t !== null)) {
      e.child = null;
      do ((e = t.sibling), (t.sibling = null), (t = e));
      while (t !== null);
    }
  }
  function pl(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var a = 0; a < e.length; a++) {
          var i = e[a];
          ((ee = i), Nh(i, t));
        }
      Uh(t);
    }
    if (t.subtreeFlags & 10256) for (t = t.child; t !== null; ) (Bh(t), (t = t.sibling));
  }
  function Bh(t) {
    switch (t.tag) {
      case 0:
      case 11:
      case 15:
        (pl(t), t.flags & 2048 && Vn(9, t, t.return));
        break;
      case 3:
        pl(t);
        break;
      case 12:
        pl(t);
        break;
      case 22:
        var e = t.stateNode;
        t.memoizedState !== null && e._visibility & 2 && (t.return === null || t.return.tag !== 13)
          ? ((e._visibility &= -3), Ys(t))
          : pl(t);
        break;
      default:
        pl(t);
    }
  }
  function Ys(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var a = 0; a < e.length; a++) {
          var i = e[a];
          ((ee = i), Nh(i, t));
        }
      Uh(t);
    }
    for (t = t.child; t !== null; ) {
      switch (((e = t), e.tag)) {
        case 0:
        case 11:
        case 15:
          (Vn(8, e, e.return), Ys(e));
          break;
        case 22:
          ((a = e.stateNode), a._visibility & 2 && ((a._visibility &= -3), Ys(e)));
          break;
        default:
          Ys(e);
      }
      t = t.sibling;
    }
  }
  function Nh(t, e) {
    for (; ee !== null; ) {
      var a = ee;
      switch (a.tag) {
        case 0:
        case 11:
        case 15:
          Vn(8, a, e);
          break;
        case 23:
        case 22:
          if (a.memoizedState !== null && a.memoizedState.cachePool !== null) {
            var i = a.memoizedState.cachePool.pool;
            i != null && i.refCount++;
          }
          break;
        case 24:
          Wi(a.memoizedState.cache);
      }
      if (((i = a.child), i !== null)) ((i.return = a), (ee = i));
      else
        t: for (a = t; ee !== null; ) {
          i = ee;
          var r = i.sibling,
            o = i.return;
          if ((xh(i), i === a)) {
            ee = null;
            break t;
          }
          if (r !== null) {
            ((r.return = o), (ee = r));
            break t;
          }
          ee = o;
        }
    }
  }
  var Pv = {
      getCacheForType: function (t) {
        var e = se(Zt),
          a = e.data.get(t);
        return (a === void 0 && ((a = t()), e.data.set(t, a)), a);
      },
      cacheSignal: function () {
        return se(Zt).controller.signal;
      },
    },
    Fv = typeof WeakMap == "function" ? WeakMap : Map,
    Tt = 0,
    zt = null,
    ht = null,
    yt = 0,
    At = 0,
    Oe = null,
    Xn = !1,
    yi = !1,
    bo = !1,
    _n = 0,
    Yt = 0,
    Kn = 0,
    Ca = 0,
    _o = 0,
    ze = 0,
    pi = 0,
    gl = null,
    be = null,
    Eo = !1,
    Gs = 0,
    jh = 0,
    Qs = 1 / 0,
    Vs = null,
    Zn = null,
    kt = 0,
    Pn = null,
    gi = null,
    En = 0,
    To = 0,
    Ro = null,
    Hh = null,
    vl = 0,
    Ao = null;
  function De() {
    return (Tt & 2) !== 0 && yt !== 0 ? yt & -yt : j.T !== null ? zo() : Wc();
  }
  function qh() {
    if (ze === 0)
      if ((yt & 536870912) === 0 || gt) {
        var t = Il;
        ((Il <<= 1), (Il & 3932160) === 0 && (Il = 262144), (ze = t));
      } else ze = 536870912;
    return ((t = Ce.current), t !== null && (t.flags |= 32), ze);
  }
  function _e(t, e, a) {
    (((t === zt && (At === 2 || At === 9)) || t.cancelPendingCommit !== null) &&
      (vi(t, 0), Fn(t, yt, ze, !1)),
      qi(t, a),
      ((Tt & 2) === 0 || t !== zt) &&
        (t === zt && ((Tt & 2) === 0 && (Ca |= a), Yt === 4 && Fn(t, yt, ze, !1)), We(t)));
  }
  function Yh(t, e, a) {
    if ((Tt & 6) !== 0) throw Error(u(327));
    var i = (!a && (e & 127) === 0 && (e & t.expiredLanes) === 0) || Hi(t, e),
      r = i ? Iv(t, e) : Mo(t, e, !0),
      o = i;
    do {
      if (r === 0) {
        yi && !i && Fn(t, e, 0, !1);
        break;
      } else {
        if (((a = t.current.alternate), o && !Jv(a))) {
          ((r = Mo(t, e, !1)), (o = !1));
          continue;
        }
        if (r === 2) {
          if (((o = e), t.errorRecoveryDisabledLanes & o)) var d = 0;
          else
            ((d = t.pendingLanes & -536870913), (d = d !== 0 ? d : d & 536870912 ? 536870912 : 0));
          if (d !== 0) {
            e = d;
            t: {
              var v = t;
              r = gl;
              var A = v.current.memoizedState.isDehydrated;
              if ((A && (vi(v, d).flags |= 256), (d = Mo(v, d, !1)), d !== 2)) {
                if (bo && !A) {
                  ((v.errorRecoveryDisabledLanes |= o), (Ca |= o), (r = 4));
                  break t;
                }
                ((o = be), (be = r), o !== null && (be === null ? (be = o) : be.push.apply(be, o)));
              }
              r = d;
            }
            if (((o = !1), r !== 2)) continue;
          }
        }
        if (r === 1) {
          (vi(t, 0), Fn(t, e, 0, !0));
          break;
        }
        t: {
          switch (((i = t), (o = r), o)) {
            case 0:
            case 1:
              throw Error(u(345));
            case 4:
              if ((e & 4194048) !== e) break;
            case 6:
              Fn(i, e, ze, !Xn);
              break t;
            case 2:
              be = null;
              break;
            case 3:
            case 5:
              break;
            default:
              throw Error(u(329));
          }
          if ((e & 62914560) === e && ((r = Gs + 300 - ce()), 10 < r)) {
            if ((Fn(i, e, ze, !Xn), Wl(i, 0, !0) !== 0)) break t;
            ((En = e),
              (i.timeoutHandle = gm(
                Gh.bind(null, i, a, be, Vs, Eo, e, ze, Ca, pi, Xn, o, "Throttled", -0, 0),
                r,
              )));
            break t;
          }
          Gh(i, a, be, Vs, Eo, e, ze, Ca, pi, Xn, o, null, -0, 0);
        }
      }
      break;
    } while (!0);
    We(t);
  }
  function Gh(t, e, a, i, r, o, d, v, A, L, H, V, U, B) {
    if (((t.timeoutHandle = -1), (V = e.subtreeFlags), V & 8192 || (V & 16785408) === 16785408)) {
      ((V = {
        stylesheets: null,
        count: 0,
        imgCount: 0,
        imgBytes: 0,
        suspenseyImages: [],
        waitingForImages: !0,
        waitingForViewTransition: !1,
        unsuspend: rn,
      }),
        Lh(e, o, V));
      var W = (o & 62914560) === o ? Gs - ce() : (o & 4194048) === o ? jh - ce() : 0;
      if (((W = L0(V, W)), W !== null)) {
        ((En = o),
          (t.cancelPendingCommit = W(Jh.bind(null, t, e, o, a, i, r, d, v, A, H, V, null, U, B))),
          Fn(t, o, d, !L));
        return;
      }
    }
    Jh(t, e, o, a, i, r, d, v, A);
  }
  function Jv(t) {
    for (var e = t; ; ) {
      var a = e.tag;
      if (
        (a === 0 || a === 11 || a === 15) &&
        e.flags & 16384 &&
        ((a = e.updateQueue), a !== null && ((a = a.stores), a !== null))
      )
        for (var i = 0; i < a.length; i++) {
          var r = a[i],
            o = r.getSnapshot;
          r = r.value;
          try {
            if (!xe(o(), r)) return !1;
          } catch {
            return !1;
          }
        }
      if (((a = e.child), e.subtreeFlags & 16384 && a !== null)) ((a.return = e), (e = a));
      else {
        if (e === t) break;
        for (; e.sibling === null; ) {
          if (e.return === null || e.return === t) return !0;
          e = e.return;
        }
        ((e.sibling.return = e.return), (e = e.sibling));
      }
    }
    return !0;
  }
  function Fn(t, e, a, i) {
    ((e &= ~_o),
      (e &= ~Ca),
      (t.suspendedLanes |= e),
      (t.pingedLanes &= ~e),
      i && (t.warmLanes |= e),
      (i = t.expirationTimes));
    for (var r = e; 0 < r; ) {
      var o = 31 - Ae(r),
        d = 1 << o;
      ((i[o] = -1), (r &= ~d));
    }
    a !== 0 && kc(t, a, e);
  }
  function Xs() {
    return (Tt & 6) === 0 ? (Sl(0), !1) : !0;
  }
  function xo() {
    if (ht !== null) {
      if (At === 0) var t = ht.return;
      else ((t = ht), (fn = Sa = null), Gu(t), (ui = null), (el = 0), (t = ht));
      for (; t !== null; ) (vh(t.alternate, t), (t = t.return));
      ht = null;
    }
  }
  function vi(t, e) {
    var a = t.timeoutHandle;
    (a !== -1 && ((t.timeoutHandle = -1), y0(a)),
      (a = t.cancelPendingCommit),
      a !== null && ((t.cancelPendingCommit = null), a()),
      (En = 0),
      xo(),
      (zt = t),
      (ht = a = on(t.current, null)),
      (yt = e),
      (At = 0),
      (Oe = null),
      (Xn = !1),
      (yi = Hi(t, e)),
      (bo = !1),
      (pi = ze = _o = Ca = Kn = Yt = 0),
      (be = gl = null),
      (Eo = !1),
      (e & 8) !== 0 && (e |= e & 32));
    var i = t.entangledLanes;
    if (i !== 0)
      for (t = t.entanglements, i &= e; 0 < i; ) {
        var r = 31 - Ae(i),
          o = 1 << r;
        ((e |= t[r]), (i &= ~o));
      }
    return ((_n = e), fs(), a);
  }
  function Qh(t, e) {
    ((ct = null),
      (j.H = ol),
      e === ri || e === Ss
        ? ((e = id()), (At = 3))
        : e === wu
          ? ((e = id()), (At = 4))
          : (At =
              e === ao
                ? 8
                : e !== null && typeof e == "object" && typeof e.then == "function"
                  ? 6
                  : 1),
      (Oe = e),
      ht === null && ((Yt = 1), Ls(t, Ne(e, t.current))));
  }
  function Vh() {
    var t = Ce.current;
    return t === null
      ? !0
      : (yt & 4194048) === yt
        ? Ye === null
        : (yt & 62914560) === yt || (yt & 536870912) !== 0
          ? t === Ye
          : !1;
  }
  function Xh() {
    var t = j.H;
    return ((j.H = ol), t === null ? ol : t);
  }
  function Kh() {
    var t = j.A;
    return ((j.A = Pv), t);
  }
  function Ks() {
    ((Yt = 4),
      Xn || ((yt & 4194048) !== yt && Ce.current !== null) || (yi = !0),
      ((Kn & 134217727) === 0 && (Ca & 134217727) === 0) || zt === null || Fn(zt, yt, ze, !1));
  }
  function Mo(t, e, a) {
    var i = Tt;
    Tt |= 2;
    var r = Xh(),
      o = Kh();
    ((zt !== t || yt !== e) && ((Vs = null), vi(t, e)), (e = !1));
    var d = Yt;
    t: do
      try {
        if (At !== 0 && ht !== null) {
          var v = ht,
            A = Oe;
          switch (At) {
            case 8:
              (xo(), (d = 6));
              break t;
            case 3:
            case 2:
            case 9:
            case 6:
              Ce.current === null && (e = !0);
              var L = At;
              if (((At = 0), (Oe = null), Si(t, v, A, L), a && yi)) {
                d = 0;
                break t;
              }
              break;
            default:
              ((L = At), (At = 0), (Oe = null), Si(t, v, A, L));
          }
        }
        (kv(), (d = Yt));
        break;
      } catch (H) {
        Qh(t, H);
      }
    while (!0);
    return (
      e && t.shellSuspendCounter++,
      (fn = Sa = null),
      (Tt = i),
      (j.H = r),
      (j.A = o),
      ht === null && ((zt = null), (yt = 0), fs()),
      d
    );
  }
  function kv() {
    for (; ht !== null; ) Zh(ht);
  }
  function Iv(t, e) {
    var a = Tt;
    Tt |= 2;
    var i = Xh(),
      r = Kh();
    zt !== t || yt !== e ? ((Vs = null), (Qs = ce() + 500), vi(t, e)) : (yi = Hi(t, e));
    t: do
      try {
        if (At !== 0 && ht !== null) {
          e = ht;
          var o = Oe;
          e: switch (At) {
            case 1:
              ((At = 0), (Oe = null), Si(t, e, o, 1));
              break;
            case 2:
            case 9:
              if (nd(o)) {
                ((At = 0), (Oe = null), Ph(e));
                break;
              }
              ((e = function () {
                ((At !== 2 && At !== 9) || zt !== t || (At = 7), We(t));
              }),
                o.then(e, e));
              break t;
            case 3:
              At = 7;
              break t;
            case 4:
              At = 5;
              break t;
            case 7:
              nd(o) ? ((At = 0), (Oe = null), Ph(e)) : ((At = 0), (Oe = null), Si(t, e, o, 7));
              break;
            case 5:
              var d = null;
              switch (ht.tag) {
                case 26:
                  d = ht.memoizedState;
                case 5:
                case 27:
                  var v = ht;
                  if (d ? Dm(d) : v.stateNode.complete) {
                    ((At = 0), (Oe = null));
                    var A = v.sibling;
                    if (A !== null) ht = A;
                    else {
                      var L = v.return;
                      L !== null ? ((ht = L), Zs(L)) : (ht = null);
                    }
                    break e;
                  }
              }
              ((At = 0), (Oe = null), Si(t, e, o, 5));
              break;
            case 6:
              ((At = 0), (Oe = null), Si(t, e, o, 6));
              break;
            case 8:
              (xo(), (Yt = 6));
              break t;
            default:
              throw Error(u(462));
          }
        }
        $v();
        break;
      } catch (H) {
        Qh(t, H);
      }
    while (!0);
    return (
      (fn = Sa = null),
      (j.H = i),
      (j.A = r),
      (Tt = a),
      ht !== null ? 0 : ((zt = null), (yt = 0), fs(), Yt)
    );
  }
  function $v() {
    for (; ht !== null && !Jl(); ) Zh(ht);
  }
  function Zh(t) {
    var e = ph(t.alternate, t, _n);
    ((t.memoizedProps = t.pendingProps), e === null ? Zs(t) : (ht = e));
  }
  function Ph(t) {
    var e = t,
      a = e.alternate;
    switch (e.tag) {
      case 15:
      case 0:
        e = ch(a, e, e.pendingProps, e.type, void 0, yt);
        break;
      case 11:
        e = ch(a, e, e.pendingProps, e.type.render, e.ref, yt);
        break;
      case 5:
        Gu(e);
      default:
        (vh(a, e), (e = ht = Kf(e, _n)), (e = ph(a, e, _n)));
    }
    ((t.memoizedProps = t.pendingProps), e === null ? Zs(t) : (ht = e));
  }
  function Si(t, e, a, i) {
    ((fn = Sa = null), Gu(e), (ui = null), (el = 0));
    var r = e.return;
    try {
      if (Yv(t, r, e, a, yt)) {
        ((Yt = 1), Ls(t, Ne(a, t.current)), (ht = null));
        return;
      }
    } catch (o) {
      if (r !== null) throw ((ht = r), o);
      ((Yt = 1), Ls(t, Ne(a, t.current)), (ht = null));
      return;
    }
    e.flags & 32768
      ? (gt || i === 1
          ? (t = !0)
          : yi || (yt & 536870912) !== 0
            ? (t = !1)
            : ((Xn = t = !0),
              (i === 2 || i === 9 || i === 3 || i === 6) &&
                ((i = Ce.current), i !== null && i.tag === 13 && (i.flags |= 16384))),
        Fh(e, t))
      : Zs(e);
  }
  function Zs(t) {
    var e = t;
    do {
      if ((e.flags & 32768) !== 0) {
        Fh(e, Xn);
        return;
      }
      t = e.return;
      var a = Vv(e.alternate, e, _n);
      if (a !== null) {
        ht = a;
        return;
      }
      if (((e = e.sibling), e !== null)) {
        ht = e;
        return;
      }
      ht = e = t;
    } while (e !== null);
    Yt === 0 && (Yt = 5);
  }
  function Fh(t, e) {
    do {
      var a = Xv(t.alternate, t);
      if (a !== null) {
        ((a.flags &= 32767), (ht = a));
        return;
      }
      if (
        ((a = t.return),
        a !== null && ((a.flags |= 32768), (a.subtreeFlags = 0), (a.deletions = null)),
        !e && ((t = t.sibling), t !== null))
      ) {
        ht = t;
        return;
      }
      ht = t = a;
    } while (t !== null);
    ((Yt = 6), (ht = null));
  }
  function Jh(t, e, a, i, r, o, d, v, A) {
    t.cancelPendingCommit = null;
    do Ps();
    while (kt !== 0);
    if ((Tt & 6) !== 0) throw Error(u(327));
    if (e !== null) {
      if (e === t.current) throw Error(u(177));
      if (
        ((o = e.lanes | e.childLanes),
        (o |= mu),
        zg(t, a, o, d, v, A),
        t === zt && ((ht = zt = null), (yt = 0)),
        (gi = e),
        (Pn = t),
        (En = a),
        (To = o),
        (Ro = r),
        (Hh = i),
        (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
          ? ((t.callbackNode = null),
            (t.callbackPriority = 0),
            n0(Ya, function () {
              return (tm(), null);
            }))
          : ((t.callbackNode = null), (t.callbackPriority = 0)),
        (i = (e.flags & 13878) !== 0),
        (e.subtreeFlags & 13878) !== 0 || i)
      ) {
        ((i = j.T), (j.T = null), (r = F.p), (F.p = 2), (d = Tt), (Tt |= 4));
        try {
          Kv(t, e, a);
        } finally {
          ((Tt = d), (F.p = r), (j.T = i));
        }
      }
      ((kt = 1), kh(), Ih(), $h());
    }
  }
  function kh() {
    if (kt === 1) {
      kt = 0;
      var t = Pn,
        e = gi,
        a = (e.flags & 13878) !== 0;
      if ((e.subtreeFlags & 13878) !== 0 || a) {
        ((a = j.T), (j.T = null));
        var i = F.p;
        F.p = 2;
        var r = Tt;
        Tt |= 4;
        try {
          Oh(e, t);
          var o = qo,
            d = Nf(t.containerInfo),
            v = o.focusedElem,
            A = o.selectionRange;
          if (d !== v && v && v.ownerDocument && Bf(v.ownerDocument.documentElement, v)) {
            if (A !== null && ou(v)) {
              var L = A.start,
                H = A.end;
              if ((H === void 0 && (H = L), "selectionStart" in v))
                ((v.selectionStart = L), (v.selectionEnd = Math.min(H, v.value.length)));
              else {
                var V = v.ownerDocument || document,
                  U = (V && V.defaultView) || window;
                if (U.getSelection) {
                  var B = U.getSelection(),
                    W = v.textContent.length,
                    rt = Math.min(A.start, W),
                    wt = A.end === void 0 ? rt : Math.min(A.end, W);
                  !B.extend && rt > wt && ((d = wt), (wt = rt), (rt = d));
                  var O = Uf(v, rt),
                    M = Uf(v, wt);
                  if (
                    O &&
                    M &&
                    (B.rangeCount !== 1 ||
                      B.anchorNode !== O.node ||
                      B.anchorOffset !== O.offset ||
                      B.focusNode !== M.node ||
                      B.focusOffset !== M.offset)
                  ) {
                    var D = V.createRange();
                    (D.setStart(O.node, O.offset),
                      B.removeAllRanges(),
                      rt > wt
                        ? (B.addRange(D), B.extend(M.node, M.offset))
                        : (D.setEnd(M.node, M.offset), B.addRange(D)));
                  }
                }
              }
            }
            for (V = [], B = v; (B = B.parentNode); )
              B.nodeType === 1 && V.push({ element: B, left: B.scrollLeft, top: B.scrollTop });
            for (typeof v.focus == "function" && v.focus(), v = 0; v < V.length; v++) {
              var G = V[v];
              ((G.element.scrollLeft = G.left), (G.element.scrollTop = G.top));
            }
          }
          ((lr = !!Ho), (qo = Ho = null));
        } finally {
          ((Tt = r), (F.p = i), (j.T = a));
        }
      }
      ((t.current = e), (kt = 2));
    }
  }
  function Ih() {
    if (kt === 2) {
      kt = 0;
      var t = Pn,
        e = gi,
        a = (e.flags & 8772) !== 0;
      if ((e.subtreeFlags & 8772) !== 0 || a) {
        ((a = j.T), (j.T = null));
        var i = F.p;
        F.p = 2;
        var r = Tt;
        Tt |= 4;
        try {
          Ah(t, e.alternate, e);
        } finally {
          ((Tt = r), (F.p = i), (j.T = a));
        }
      }
      kt = 3;
    }
  }
  function $h() {
    if (kt === 4 || kt === 3) {
      ((kt = 0), Gr());
      var t = Pn,
        e = gi,
        a = En,
        i = Hh;
      (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
        ? (kt = 5)
        : ((kt = 0), (gi = Pn = null), Wh(t, t.pendingLanes));
      var r = t.pendingLanes;
      if (
        (r === 0 && (Zn = null),
        Xr(a),
        (e = e.stateNode),
        Re && typeof Re.onCommitFiberRoot == "function")
      )
        try {
          Re.onCommitFiberRoot(ji, e, void 0, (e.current.flags & 128) === 128);
        } catch {}
      if (i !== null) {
        ((e = j.T), (r = F.p), (F.p = 2), (j.T = null));
        try {
          for (var o = t.onRecoverableError, d = 0; d < i.length; d++) {
            var v = i[d];
            o(v.value, { componentStack: v.stack });
          }
        } finally {
          ((j.T = e), (F.p = r));
        }
      }
      ((En & 3) !== 0 && Ps(),
        We(t),
        (r = t.pendingLanes),
        (a & 261930) !== 0 && (r & 42) !== 0 ? (t === Ao ? vl++ : ((vl = 0), (Ao = t))) : (vl = 0),
        Sl(0));
    }
  }
  function Wh(t, e) {
    (t.pooledCacheLanes &= e) === 0 &&
      ((e = t.pooledCache), e != null && ((t.pooledCache = null), Wi(e)));
  }
  function Ps() {
    return (kh(), Ih(), $h(), tm());
  }
  function tm() {
    if (kt !== 5) return !1;
    var t = Pn,
      e = To;
    To = 0;
    var a = Xr(En),
      i = j.T,
      r = F.p;
    try {
      ((F.p = 32 > a ? 32 : a), (j.T = null), (a = Ro), (Ro = null));
      var o = Pn,
        d = En;
      if (((kt = 0), (gi = Pn = null), (En = 0), (Tt & 6) !== 0)) throw Error(u(331));
      var v = Tt;
      if (
        ((Tt |= 4),
        Bh(o.current),
        Dh(o, o.current, d, a),
        (Tt = v),
        Sl(0, !1),
        Re && typeof Re.onPostCommitFiberRoot == "function")
      )
        try {
          Re.onPostCommitFiberRoot(ji, o);
        } catch {}
      return !0;
    } finally {
      ((F.p = r), (j.T = i), Wh(t, e));
    }
  }
  function em(t, e, a) {
    ((e = Ne(a, e)),
      (e = no(t.stateNode, e, 2)),
      (t = Yn(t, e, 2)),
      t !== null && (qi(t, 2), We(t)));
  }
  function xt(t, e, a) {
    if (t.tag === 3) em(t, t, a);
    else
      for (; e !== null; ) {
        if (e.tag === 3) {
          em(e, t, a);
          break;
        } else if (e.tag === 1) {
          var i = e.stateNode;
          if (
            typeof e.type.getDerivedStateFromError == "function" ||
            (typeof i.componentDidCatch == "function" && (Zn === null || !Zn.has(i)))
          ) {
            ((t = Ne(a, t)),
              (a = nh(2)),
              (i = Yn(e, a, 2)),
              i !== null && (ah(a, i, e, t), qi(i, 2), We(i)));
            break;
          }
        }
        e = e.return;
      }
  }
  function Co(t, e, a) {
    var i = t.pingCache;
    if (i === null) {
      i = t.pingCache = new Fv();
      var r = new Set();
      i.set(e, r);
    } else ((r = i.get(e)), r === void 0 && ((r = new Set()), i.set(e, r)));
    r.has(a) || ((bo = !0), r.add(a), (t = Wv.bind(null, t, e, a)), e.then(t, t));
  }
  function Wv(t, e, a) {
    var i = t.pingCache;
    (i !== null && i.delete(e),
      (t.pingedLanes |= t.suspendedLanes & a),
      (t.warmLanes &= ~a),
      zt === t &&
        (yt & a) === a &&
        (Yt === 4 || (Yt === 3 && (yt & 62914560) === yt && 300 > ce() - Gs)
          ? (Tt & 2) === 0 && vi(t, 0)
          : (_o |= a),
        pi === yt && (pi = 0)),
      We(t));
  }
  function nm(t, e) {
    (e === 0 && (e = Jc()), (t = pa(t, e)), t !== null && (qi(t, e), We(t)));
  }
  function t0(t) {
    var e = t.memoizedState,
      a = 0;
    (e !== null && (a = e.retryLane), nm(t, a));
  }
  function e0(t, e) {
    var a = 0;
    switch (t.tag) {
      case 31:
      case 13:
        var i = t.stateNode,
          r = t.memoizedState;
        r !== null && (a = r.retryLane);
        break;
      case 19:
        i = t.stateNode;
        break;
      case 22:
        i = t.stateNode._retryCache;
        break;
      default:
        throw Error(u(314));
    }
    (i !== null && i.delete(e), nm(t, a));
  }
  function n0(t, e) {
    return Ve(t, e);
  }
  var Fs = null,
    bi = null,
    wo = !1,
    Js = !1,
    Oo = !1,
    Jn = 0;
  function We(t) {
    (t !== bi && t.next === null && (bi === null ? (Fs = bi = t) : (bi = bi.next = t)),
      (Js = !0),
      wo || ((wo = !0), i0()));
  }
  function Sl(t, e) {
    if (!Oo && Js) {
      Oo = !0;
      do
        for (var a = !1, i = Fs; i !== null; ) {
          if (t !== 0) {
            var r = i.pendingLanes;
            if (r === 0) var o = 0;
            else {
              var d = i.suspendedLanes,
                v = i.pingedLanes;
              ((o = (1 << (31 - Ae(42 | t) + 1)) - 1),
                (o &= r & ~(d & ~v)),
                (o = o & 201326741 ? (o & 201326741) | 1 : o ? o | 2 : 0));
            }
            o !== 0 && ((a = !0), sm(i, o));
          } else
            ((o = yt),
              (o = Wl(
                i,
                i === zt ? o : 0,
                i.cancelPendingCommit !== null || i.timeoutHandle !== -1,
              )),
              (o & 3) === 0 || Hi(i, o) || ((a = !0), sm(i, o)));
          i = i.next;
        }
      while (a);
      Oo = !1;
    }
  }
  function a0() {
    am();
  }
  function am() {
    Js = wo = !1;
    var t = 0;
    Jn !== 0 && m0() && (t = Jn);
    for (var e = ce(), a = null, i = Fs; i !== null; ) {
      var r = i.next,
        o = im(i, e);
      (o === 0
        ? ((i.next = null), a === null ? (Fs = r) : (a.next = r), r === null && (bi = a))
        : ((a = i), (t !== 0 || (o & 3) !== 0) && (Js = !0)),
        (i = r));
    }
    ((kt !== 0 && kt !== 5) || Sl(t), Jn !== 0 && (Jn = 0));
  }
  function im(t, e) {
    for (
      var a = t.suspendedLanes,
        i = t.pingedLanes,
        r = t.expirationTimes,
        o = t.pendingLanes & -62914561;
      0 < o;
    ) {
      var d = 31 - Ae(o),
        v = 1 << d,
        A = r[d];
      (A === -1
        ? ((v & a) === 0 || (v & i) !== 0) && (r[d] = Og(v, e))
        : A <= e && (t.expiredLanes |= v),
        (o &= ~v));
    }
    if (
      ((e = zt),
      (a = yt),
      (a = Wl(t, t === e ? a : 0, t.cancelPendingCommit !== null || t.timeoutHandle !== -1)),
      (i = t.callbackNode),
      a === 0 || (t === e && (At === 2 || At === 9)) || t.cancelPendingCommit !== null)
    )
      return (i !== null && i !== null && Ni(i), (t.callbackNode = null), (t.callbackPriority = 0));
    if ((a & 3) === 0 || Hi(t, a)) {
      if (((e = a & -a), e === t.callbackPriority)) return e;
      switch ((i !== null && Ni(i), Xr(a))) {
        case 2:
        case 8:
          a = Je;
          break;
        case 32:
          a = Ya;
          break;
        case 268435456:
          a = Fc;
          break;
        default:
          a = Ya;
      }
      return (
        (i = lm.bind(null, t)),
        (a = Ve(a, i)),
        (t.callbackPriority = e),
        (t.callbackNode = a),
        e
      );
    }
    return (
      i !== null && i !== null && Ni(i),
      (t.callbackPriority = 2),
      (t.callbackNode = null),
      2
    );
  }
  function lm(t, e) {
    if (kt !== 0 && kt !== 5) return ((t.callbackNode = null), (t.callbackPriority = 0), null);
    var a = t.callbackNode;
    if (Ps() && t.callbackNode !== a) return null;
    var i = yt;
    return (
      (i = Wl(t, t === zt ? i : 0, t.cancelPendingCommit !== null || t.timeoutHandle !== -1)),
      i === 0
        ? null
        : (Yh(t, i, e),
          im(t, ce()),
          t.callbackNode != null && t.callbackNode === a ? lm.bind(null, t) : null)
    );
  }
  function sm(t, e) {
    if (Ps()) return null;
    Yh(t, e, !0);
  }
  function i0() {
    p0(function () {
      (Tt & 6) !== 0 ? Ve(ae, a0) : am();
    });
  }
  function zo() {
    if (Jn === 0) {
      var t = li;
      (t === 0 && ((t = kl), (kl <<= 1), (kl & 261888) === 0 && (kl = 256)), (Jn = t));
    }
    return Jn;
  }
  function rm(t) {
    return t == null || typeof t == "symbol" || typeof t == "boolean"
      ? null
      : typeof t == "function"
        ? t
        : as("" + t);
  }
  function um(t, e) {
    var a = e.ownerDocument.createElement("input");
    return (
      (a.name = e.name),
      (a.value = e.value),
      t.id && a.setAttribute("form", t.id),
      e.parentNode.insertBefore(a, e),
      (t = new FormData(t)),
      a.parentNode.removeChild(a),
      t
    );
  }
  function l0(t, e, a, i, r) {
    if (e === "submit" && a && a.stateNode === r) {
      var o = rm((r[ye] || null).action),
        d = i.submitter;
      d &&
        ((e = (e = d[ye] || null) ? rm(e.formAction) : d.getAttribute("formAction")),
        e !== null && ((o = e), (d = null)));
      var v = new rs("action", "action", null, i, r);
      t.push({
        event: v,
        listeners: [
          {
            instance: null,
            listener: function () {
              if (i.defaultPrevented) {
                if (Jn !== 0) {
                  var A = d ? um(r, d) : new FormData(r);
                  ku(a, { pending: !0, data: A, method: r.method, action: o }, null, A);
                }
              } else
                typeof o == "function" &&
                  (v.preventDefault(),
                  (A = d ? um(r, d) : new FormData(r)),
                  ku(a, { pending: !0, data: A, method: r.method, action: o }, o, A));
            },
            currentTarget: r,
          },
        ],
      });
    }
  }
  for (var Do = 0; Do < hu.length; Do++) {
    var Lo = hu[Do],
      s0 = Lo.toLowerCase(),
      r0 = Lo[0].toUpperCase() + Lo.slice(1);
    Xe(s0, "on" + r0);
  }
  (Xe(qf, "onAnimationEnd"),
    Xe(Yf, "onAnimationIteration"),
    Xe(Gf, "onAnimationStart"),
    Xe("dblclick", "onDoubleClick"),
    Xe("focusin", "onFocus"),
    Xe("focusout", "onBlur"),
    Xe(Tv, "onTransitionRun"),
    Xe(Rv, "onTransitionStart"),
    Xe(Av, "onTransitionCancel"),
    Xe(Qf, "onTransitionEnd"),
    Ka("onMouseEnter", ["mouseout", "mouseover"]),
    Ka("onMouseLeave", ["mouseout", "mouseover"]),
    Ka("onPointerEnter", ["pointerout", "pointerover"]),
    Ka("onPointerLeave", ["pointerout", "pointerover"]),
    da("onChange", "change click focusin focusout input keydown keyup selectionchange".split(" ")),
    da(
      "onSelect",
      "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
        " ",
      ),
    ),
    da("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]),
    da("onCompositionEnd", "compositionend focusout keydown keypress keyup mousedown".split(" ")),
    da(
      "onCompositionStart",
      "compositionstart focusout keydown keypress keyup mousedown".split(" "),
    ),
    da(
      "onCompositionUpdate",
      "compositionupdate focusout keydown keypress keyup mousedown".split(" "),
    ));
  var bl =
      "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
        " ",
      ),
    u0 = new Set(
      "beforetoggle cancel close invalid load scroll scrollend toggle".split(" ").concat(bl),
    );
  function om(t, e) {
    e = (e & 4) !== 0;
    for (var a = 0; a < t.length; a++) {
      var i = t[a],
        r = i.event;
      i = i.listeners;
      t: {
        var o = void 0;
        if (e)
          for (var d = i.length - 1; 0 <= d; d--) {
            var v = i[d],
              A = v.instance,
              L = v.currentTarget;
            if (((v = v.listener), A !== o && r.isPropagationStopped())) break t;
            ((o = v), (r.currentTarget = L));
            try {
              o(r);
            } catch (H) {
              cs(H);
            }
            ((r.currentTarget = null), (o = A));
          }
        else
          for (d = 0; d < i.length; d++) {
            if (
              ((v = i[d]),
              (A = v.instance),
              (L = v.currentTarget),
              (v = v.listener),
              A !== o && r.isPropagationStopped())
            )
              break t;
            ((o = v), (r.currentTarget = L));
            try {
              o(r);
            } catch (H) {
              cs(H);
            }
            ((r.currentTarget = null), (o = A));
          }
      }
    }
  }
  function mt(t, e) {
    var a = e[Kr];
    a === void 0 && (a = e[Kr] = new Set());
    var i = t + "__bubble";
    a.has(i) || (cm(e, t, 2, !1), a.add(i));
  }
  function Uo(t, e, a) {
    var i = 0;
    (e && (i |= 4), cm(a, t, i, e));
  }
  var ks = "_reactListening" + Math.random().toString(36).slice(2);
  function Bo(t) {
    if (!t[ks]) {
      ((t[ks] = !0),
        nf.forEach(function (a) {
          a !== "selectionchange" && (u0.has(a) || Uo(a, !1, t), Uo(a, !0, t));
        }));
      var e = t.nodeType === 9 ? t : t.ownerDocument;
      e === null || e[ks] || ((e[ks] = !0), Uo("selectionchange", !1, e));
    }
  }
  function cm(t, e, a, i) {
    switch (qm(e)) {
      case 2:
        var r = N0;
        break;
      case 8:
        r = j0;
        break;
      default:
        r = ko;
    }
    ((a = r.bind(null, e, a, t)),
      (r = void 0),
      !tu || (e !== "touchstart" && e !== "touchmove" && e !== "wheel") || (r = !0),
      i
        ? r !== void 0
          ? t.addEventListener(e, a, { capture: !0, passive: r })
          : t.addEventListener(e, a, !0)
        : r !== void 0
          ? t.addEventListener(e, a, { passive: r })
          : t.addEventListener(e, a, !1));
  }
  function No(t, e, a, i, r) {
    var o = i;
    if ((e & 1) === 0 && (e & 2) === 0 && i !== null)
      t: for (;;) {
        if (i === null) return;
        var d = i.tag;
        if (d === 3 || d === 4) {
          var v = i.stateNode.containerInfo;
          if (v === r) break;
          if (d === 4)
            for (d = i.return; d !== null; ) {
              var A = d.tag;
              if ((A === 3 || A === 4) && d.stateNode.containerInfo === r) return;
              d = d.return;
            }
          for (; v !== null; ) {
            if (((d = Qa(v)), d === null)) return;
            if (((A = d.tag), A === 5 || A === 6 || A === 26 || A === 27)) {
              i = o = d;
              continue t;
            }
            v = v.parentNode;
          }
        }
        i = i.return;
      }
    yf(function () {
      var L = o,
        H = $r(a),
        V = [];
      t: {
        var U = Vf.get(t);
        if (U !== void 0) {
          var B = rs,
            W = t;
          switch (t) {
            case "keypress":
              if (ls(a) === 0) break t;
            case "keydown":
            case "keyup":
              B = ev;
              break;
            case "focusin":
              ((W = "focus"), (B = iu));
              break;
            case "focusout":
              ((W = "blur"), (B = iu));
              break;
            case "beforeblur":
            case "afterblur":
              B = iu;
              break;
            case "click":
              if (a.button === 2) break t;
            case "auxclick":
            case "dblclick":
            case "mousedown":
            case "mousemove":
            case "mouseup":
            case "mouseout":
            case "mouseover":
            case "contextmenu":
              B = vf;
              break;
            case "drag":
            case "dragend":
            case "dragenter":
            case "dragexit":
            case "dragleave":
            case "dragover":
            case "dragstart":
            case "drop":
              B = Vg;
              break;
            case "touchcancel":
            case "touchend":
            case "touchmove":
            case "touchstart":
              B = iv;
              break;
            case qf:
            case Yf:
            case Gf:
              B = Zg;
              break;
            case Qf:
              B = sv;
              break;
            case "scroll":
            case "scrollend":
              B = Gg;
              break;
            case "wheel":
              B = uv;
              break;
            case "copy":
            case "cut":
            case "paste":
              B = Fg;
              break;
            case "gotpointercapture":
            case "lostpointercapture":
            case "pointercancel":
            case "pointerdown":
            case "pointermove":
            case "pointerout":
            case "pointerover":
            case "pointerup":
              B = bf;
              break;
            case "toggle":
            case "beforetoggle":
              B = cv;
          }
          var rt = (e & 4) !== 0,
            wt = !rt && (t === "scroll" || t === "scrollend"),
            O = rt ? (U !== null ? U + "Capture" : null) : U;
          rt = [];
          for (var M = L, D; M !== null; ) {
            var G = M;
            if (
              ((D = G.stateNode),
              (G = G.tag),
              (G !== 5 && G !== 26 && G !== 27) ||
                D === null ||
                O === null ||
                ((G = Qi(M, O)), G != null && rt.push(_l(M, G, D))),
              wt)
            )
              break;
            M = M.return;
          }
          0 < rt.length && ((U = new B(U, W, null, a, H)), V.push({ event: U, listeners: rt }));
        }
      }
      if ((e & 7) === 0) {
        t: {
          if (
            ((U = t === "mouseover" || t === "pointerover"),
            (B = t === "mouseout" || t === "pointerout"),
            U && a !== Ir && (W = a.relatedTarget || a.fromElement) && (Qa(W) || W[Ga]))
          )
            break t;
          if (
            (B || U) &&
            ((U =
              H.window === H
                ? H
                : (U = H.ownerDocument)
                  ? U.defaultView || U.parentWindow
                  : window),
            B
              ? ((W = a.relatedTarget || a.toElement),
                (B = L),
                (W = W ? Qa(W) : null),
                W !== null &&
                  ((wt = f(W)), (rt = W.tag), W !== wt || (rt !== 5 && rt !== 27 && rt !== 6)) &&
                  (W = null))
              : ((B = null), (W = L)),
            B !== W)
          ) {
            if (
              ((rt = vf),
              (G = "onMouseLeave"),
              (O = "onMouseEnter"),
              (M = "mouse"),
              (t === "pointerout" || t === "pointerover") &&
                ((rt = bf), (G = "onPointerLeave"), (O = "onPointerEnter"), (M = "pointer")),
              (wt = B == null ? U : Gi(B)),
              (D = W == null ? U : Gi(W)),
              (U = new rt(G, M + "leave", B, a, H)),
              (U.target = wt),
              (U.relatedTarget = D),
              (G = null),
              Qa(H) === L &&
                ((rt = new rt(O, M + "enter", W, a, H)),
                (rt.target = D),
                (rt.relatedTarget = wt),
                (G = rt)),
              (wt = G),
              B && W)
            )
              e: {
                for (rt = o0, O = B, M = W, D = 0, G = O; G; G = rt(G)) D++;
                G = 0;
                for (var at = M; at; at = rt(at)) G++;
                for (; 0 < D - G; ) ((O = rt(O)), D--);
                for (; 0 < G - D; ) ((M = rt(M)), G--);
                for (; D--; ) {
                  if (O === M || (M !== null && O === M.alternate)) {
                    rt = O;
                    break e;
                  }
                  ((O = rt(O)), (M = rt(M)));
                }
                rt = null;
              }
            else rt = null;
            (B !== null && fm(V, U, B, rt, !1), W !== null && wt !== null && fm(V, wt, W, rt, !0));
          }
        }
        t: {
          if (
            ((U = L ? Gi(L) : window),
            (B = U.nodeName && U.nodeName.toLowerCase()),
            B === "select" || (B === "input" && U.type === "file"))
          )
            var St = Cf;
          else if (xf(U))
            if (wf) St = bv;
            else {
              St = vv;
              var et = gv;
            }
          else
            ((B = U.nodeName),
              !B || B.toLowerCase() !== "input" || (U.type !== "checkbox" && U.type !== "radio")
                ? L && kr(L.elementType) && (St = Cf)
                : (St = Sv));
          if (St && (St = St(t, L))) {
            Mf(V, St, a, H);
            break t;
          }
          (et && et(t, U, L),
            t === "focusout" &&
              L &&
              U.type === "number" &&
              L.memoizedProps.value != null &&
              Jr(U, "number", U.value));
        }
        switch (((et = L ? Gi(L) : window), t)) {
          case "focusin":
            (xf(et) || et.contentEditable === "true") && ((Ia = et), (cu = L), (ki = null));
            break;
          case "focusout":
            ki = cu = Ia = null;
            break;
          case "mousedown":
            fu = !0;
            break;
          case "contextmenu":
          case "mouseup":
          case "dragend":
            ((fu = !1), jf(V, a, H));
            break;
          case "selectionchange":
            if (Ev) break;
          case "keydown":
          case "keyup":
            jf(V, a, H);
        }
        var ft;
        if (su)
          t: {
            switch (t) {
              case "compositionstart":
                var pt = "onCompositionStart";
                break t;
              case "compositionend":
                pt = "onCompositionEnd";
                break t;
              case "compositionupdate":
                pt = "onCompositionUpdate";
                break t;
            }
            pt = void 0;
          }
        else
          ka
            ? Rf(t, a) && (pt = "onCompositionEnd")
            : t === "keydown" && a.keyCode === 229 && (pt = "onCompositionStart");
        (pt &&
          (_f &&
            a.locale !== "ko" &&
            (ka || pt !== "onCompositionStart"
              ? pt === "onCompositionEnd" && ka && (ft = pf())
              : ((Ln = H), (eu = "value" in Ln ? Ln.value : Ln.textContent), (ka = !0))),
          (et = Is(L, pt)),
          0 < et.length &&
            ((pt = new Sf(pt, t, null, a, H)),
            V.push({ event: pt, listeners: et }),
            ft ? (pt.data = ft) : ((ft = Af(a)), ft !== null && (pt.data = ft)))),
          (ft = dv ? hv(t, a) : mv(t, a)) &&
            ((pt = Is(L, "onBeforeInput")),
            0 < pt.length &&
              ((et = new Sf("onBeforeInput", "beforeinput", null, a, H)),
              V.push({ event: et, listeners: pt }),
              (et.data = ft))),
          l0(V, t, L, a, H));
      }
      om(V, e);
    });
  }
  function _l(t, e, a) {
    return { instance: t, listener: e, currentTarget: a };
  }
  function Is(t, e) {
    for (var a = e + "Capture", i = []; t !== null; ) {
      var r = t,
        o = r.stateNode;
      if (
        ((r = r.tag),
        (r !== 5 && r !== 26 && r !== 27) ||
          o === null ||
          ((r = Qi(t, a)),
          r != null && i.unshift(_l(t, r, o)),
          (r = Qi(t, e)),
          r != null && i.push(_l(t, r, o))),
        t.tag === 3)
      )
        return i;
      t = t.return;
    }
    return [];
  }
  function o0(t) {
    if (t === null) return null;
    do t = t.return;
    while (t && t.tag !== 5 && t.tag !== 27);
    return t || null;
  }
  function fm(t, e, a, i, r) {
    for (var o = e._reactName, d = []; a !== null && a !== i; ) {
      var v = a,
        A = v.alternate,
        L = v.stateNode;
      if (((v = v.tag), A !== null && A === i)) break;
      ((v !== 5 && v !== 26 && v !== 27) ||
        L === null ||
        ((A = L),
        r
          ? ((L = Qi(a, o)), L != null && d.unshift(_l(a, L, A)))
          : r || ((L = Qi(a, o)), L != null && d.push(_l(a, L, A)))),
        (a = a.return));
    }
    d.length !== 0 && t.push({ event: e, listeners: d });
  }
  var c0 = /\r\n?/g,
    f0 = /\u0000|\uFFFD/g;
  function dm(t) {
    return (typeof t == "string" ? t : "" + t)
      .replace(
        c0,
        `
`,
      )
      .replace(f0, "");
  }
  function hm(t, e) {
    return ((e = dm(e)), dm(t) === e);
  }
  function Ct(t, e, a, i, r, o) {
    switch (a) {
      case "children":
        typeof i == "string"
          ? e === "body" || (e === "textarea" && i === "") || Pa(t, i)
          : (typeof i == "number" || typeof i == "bigint") && e !== "body" && Pa(t, "" + i);
        break;
      case "className":
        es(t, "class", i);
        break;
      case "tabIndex":
        es(t, "tabindex", i);
        break;
      case "dir":
      case "role":
      case "viewBox":
      case "width":
      case "height":
        es(t, a, i);
        break;
      case "style":
        hf(t, i, o);
        break;
      case "data":
        if (e !== "object") {
          es(t, "data", i);
          break;
        }
      case "src":
      case "href":
        if (i === "" && (e !== "a" || a !== "href")) {
          t.removeAttribute(a);
          break;
        }
        if (i == null || typeof i == "function" || typeof i == "symbol" || typeof i == "boolean") {
          t.removeAttribute(a);
          break;
        }
        ((i = as("" + i)), t.setAttribute(a, i));
        break;
      case "action":
      case "formAction":
        if (typeof i == "function") {
          t.setAttribute(
            a,
            "javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')",
          );
          break;
        } else
          typeof o == "function" &&
            (a === "formAction"
              ? (e !== "input" && Ct(t, e, "name", r.name, r, null),
                Ct(t, e, "formEncType", r.formEncType, r, null),
                Ct(t, e, "formMethod", r.formMethod, r, null),
                Ct(t, e, "formTarget", r.formTarget, r, null))
              : (Ct(t, e, "encType", r.encType, r, null),
                Ct(t, e, "method", r.method, r, null),
                Ct(t, e, "target", r.target, r, null)));
        if (i == null || typeof i == "symbol" || typeof i == "boolean") {
          t.removeAttribute(a);
          break;
        }
        ((i = as("" + i)), t.setAttribute(a, i));
        break;
      case "onClick":
        i != null && (t.onclick = rn);
        break;
      case "onScroll":
        i != null && mt("scroll", t);
        break;
      case "onScrollEnd":
        i != null && mt("scrollend", t);
        break;
      case "dangerouslySetInnerHTML":
        if (i != null) {
          if (typeof i != "object" || !("__html" in i)) throw Error(u(61));
          if (((a = i.__html), a != null)) {
            if (r.children != null) throw Error(u(60));
            t.innerHTML = a;
          }
        }
        break;
      case "multiple":
        t.multiple = i && typeof i != "function" && typeof i != "symbol";
        break;
      case "muted":
        t.muted = i && typeof i != "function" && typeof i != "symbol";
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "defaultValue":
      case "defaultChecked":
      case "innerHTML":
      case "ref":
        break;
      case "autoFocus":
        break;
      case "xlinkHref":
        if (i == null || typeof i == "function" || typeof i == "boolean" || typeof i == "symbol") {
          t.removeAttribute("xlink:href");
          break;
        }
        ((a = as("" + i)), t.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", a));
        break;
      case "contentEditable":
      case "spellCheck":
      case "draggable":
      case "value":
      case "autoReverse":
      case "externalResourcesRequired":
      case "focusable":
      case "preserveAlpha":
        i != null && typeof i != "function" && typeof i != "symbol"
          ? t.setAttribute(a, "" + i)
          : t.removeAttribute(a);
        break;
      case "inert":
      case "allowFullScreen":
      case "async":
      case "autoPlay":
      case "controls":
      case "default":
      case "defer":
      case "disabled":
      case "disablePictureInPicture":
      case "disableRemotePlayback":
      case "formNoValidate":
      case "hidden":
      case "loop":
      case "noModule":
      case "noValidate":
      case "open":
      case "playsInline":
      case "readOnly":
      case "required":
      case "reversed":
      case "scoped":
      case "seamless":
      case "itemScope":
        i && typeof i != "function" && typeof i != "symbol"
          ? t.setAttribute(a, "")
          : t.removeAttribute(a);
        break;
      case "capture":
      case "download":
        i === !0
          ? t.setAttribute(a, "")
          : i !== !1 && i != null && typeof i != "function" && typeof i != "symbol"
            ? t.setAttribute(a, i)
            : t.removeAttribute(a);
        break;
      case "cols":
      case "rows":
      case "size":
      case "span":
        i != null && typeof i != "function" && typeof i != "symbol" && !isNaN(i) && 1 <= i
          ? t.setAttribute(a, i)
          : t.removeAttribute(a);
        break;
      case "rowSpan":
      case "start":
        i == null || typeof i == "function" || typeof i == "symbol" || isNaN(i)
          ? t.removeAttribute(a)
          : t.setAttribute(a, i);
        break;
      case "popover":
        (mt("beforetoggle", t), mt("toggle", t), ts(t, "popover", i));
        break;
      case "xlinkActuate":
        sn(t, "http://www.w3.org/1999/xlink", "xlink:actuate", i);
        break;
      case "xlinkArcrole":
        sn(t, "http://www.w3.org/1999/xlink", "xlink:arcrole", i);
        break;
      case "xlinkRole":
        sn(t, "http://www.w3.org/1999/xlink", "xlink:role", i);
        break;
      case "xlinkShow":
        sn(t, "http://www.w3.org/1999/xlink", "xlink:show", i);
        break;
      case "xlinkTitle":
        sn(t, "http://www.w3.org/1999/xlink", "xlink:title", i);
        break;
      case "xlinkType":
        sn(t, "http://www.w3.org/1999/xlink", "xlink:type", i);
        break;
      case "xmlBase":
        sn(t, "http://www.w3.org/XML/1998/namespace", "xml:base", i);
        break;
      case "xmlLang":
        sn(t, "http://www.w3.org/XML/1998/namespace", "xml:lang", i);
        break;
      case "xmlSpace":
        sn(t, "http://www.w3.org/XML/1998/namespace", "xml:space", i);
        break;
      case "is":
        ts(t, "is", i);
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        (!(2 < a.length) || (a[0] !== "o" && a[0] !== "O") || (a[1] !== "n" && a[1] !== "N")) &&
          ((a = qg.get(a) || a), ts(t, a, i));
    }
  }
  function jo(t, e, a, i, r, o) {
    switch (a) {
      case "style":
        hf(t, i, o);
        break;
      case "dangerouslySetInnerHTML":
        if (i != null) {
          if (typeof i != "object" || !("__html" in i)) throw Error(u(61));
          if (((a = i.__html), a != null)) {
            if (r.children != null) throw Error(u(60));
            t.innerHTML = a;
          }
        }
        break;
      case "children":
        typeof i == "string"
          ? Pa(t, i)
          : (typeof i == "number" || typeof i == "bigint") && Pa(t, "" + i);
        break;
      case "onScroll":
        i != null && mt("scroll", t);
        break;
      case "onScrollEnd":
        i != null && mt("scrollend", t);
        break;
      case "onClick":
        i != null && (t.onclick = rn);
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "innerHTML":
      case "ref":
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        if (!af.hasOwnProperty(a))
          t: {
            if (
              a[0] === "o" &&
              a[1] === "n" &&
              ((r = a.endsWith("Capture")),
              (e = a.slice(2, r ? a.length - 7 : void 0)),
              (o = t[ye] || null),
              (o = o != null ? o[a] : null),
              typeof o == "function" && t.removeEventListener(e, o, r),
              typeof i == "function")
            ) {
              (typeof o != "function" &&
                o !== null &&
                (a in t ? (t[a] = null) : t.hasAttribute(a) && t.removeAttribute(a)),
                t.addEventListener(e, i, r));
              break t;
            }
            a in t ? (t[a] = i) : i === !0 ? t.setAttribute(a, "") : ts(t, a, i);
          }
    }
  }
  function ue(t, e, a) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "img":
        (mt("error", t), mt("load", t));
        var i = !1,
          r = !1,
          o;
        for (o in a)
          if (a.hasOwnProperty(o)) {
            var d = a[o];
            if (d != null)
              switch (o) {
                case "src":
                  i = !0;
                  break;
                case "srcSet":
                  r = !0;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  throw Error(u(137, e));
                default:
                  Ct(t, e, o, d, a, null);
              }
          }
        (r && Ct(t, e, "srcSet", a.srcSet, a, null), i && Ct(t, e, "src", a.src, a, null));
        return;
      case "input":
        mt("invalid", t);
        var v = (o = d = r = null),
          A = null,
          L = null;
        for (i in a)
          if (a.hasOwnProperty(i)) {
            var H = a[i];
            if (H != null)
              switch (i) {
                case "name":
                  r = H;
                  break;
                case "type":
                  d = H;
                  break;
                case "checked":
                  A = H;
                  break;
                case "defaultChecked":
                  L = H;
                  break;
                case "value":
                  o = H;
                  break;
                case "defaultValue":
                  v = H;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  if (H != null) throw Error(u(137, e));
                  break;
                default:
                  Ct(t, e, i, H, a, null);
              }
          }
        of(t, o, v, A, L, d, r, !1);
        return;
      case "select":
        (mt("invalid", t), (i = d = o = null));
        for (r in a)
          if (a.hasOwnProperty(r) && ((v = a[r]), v != null))
            switch (r) {
              case "value":
                o = v;
                break;
              case "defaultValue":
                d = v;
                break;
              case "multiple":
                i = v;
              default:
                Ct(t, e, r, v, a, null);
            }
        ((e = o),
          (a = d),
          (t.multiple = !!i),
          e != null ? Za(t, !!i, e, !1) : a != null && Za(t, !!i, a, !0));
        return;
      case "textarea":
        (mt("invalid", t), (o = r = i = null));
        for (d in a)
          if (a.hasOwnProperty(d) && ((v = a[d]), v != null))
            switch (d) {
              case "value":
                i = v;
                break;
              case "defaultValue":
                r = v;
                break;
              case "children":
                o = v;
                break;
              case "dangerouslySetInnerHTML":
                if (v != null) throw Error(u(91));
                break;
              default:
                Ct(t, e, d, v, a, null);
            }
        ff(t, i, r, o);
        return;
      case "option":
        for (A in a)
          a.hasOwnProperty(A) &&
            ((i = a[A]), i != null) &&
            (A === "selected"
              ? (t.selected = i && typeof i != "function" && typeof i != "symbol")
              : Ct(t, e, A, i, a, null));
        return;
      case "dialog":
        (mt("beforetoggle", t), mt("toggle", t), mt("cancel", t), mt("close", t));
        break;
      case "iframe":
      case "object":
        mt("load", t);
        break;
      case "video":
      case "audio":
        for (i = 0; i < bl.length; i++) mt(bl[i], t);
        break;
      case "image":
        (mt("error", t), mt("load", t));
        break;
      case "details":
        mt("toggle", t);
        break;
      case "embed":
      case "source":
      case "link":
        (mt("error", t), mt("load", t));
      case "area":
      case "base":
      case "br":
      case "col":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "track":
      case "wbr":
      case "menuitem":
        for (L in a)
          if (a.hasOwnProperty(L) && ((i = a[L]), i != null))
            switch (L) {
              case "children":
              case "dangerouslySetInnerHTML":
                throw Error(u(137, e));
              default:
                Ct(t, e, L, i, a, null);
            }
        return;
      default:
        if (kr(e)) {
          for (H in a)
            a.hasOwnProperty(H) && ((i = a[H]), i !== void 0 && jo(t, e, H, i, a, void 0));
          return;
        }
    }
    for (v in a) a.hasOwnProperty(v) && ((i = a[v]), i != null && Ct(t, e, v, i, a, null));
  }
  function d0(t, e, a, i) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "input":
        var r = null,
          o = null,
          d = null,
          v = null,
          A = null,
          L = null,
          H = null;
        for (B in a) {
          var V = a[B];
          if (a.hasOwnProperty(B) && V != null)
            switch (B) {
              case "checked":
                break;
              case "value":
                break;
              case "defaultValue":
                A = V;
              default:
                i.hasOwnProperty(B) || Ct(t, e, B, null, i, V);
            }
        }
        for (var U in i) {
          var B = i[U];
          if (((V = a[U]), i.hasOwnProperty(U) && (B != null || V != null)))
            switch (U) {
              case "type":
                o = B;
                break;
              case "name":
                r = B;
                break;
              case "checked":
                L = B;
                break;
              case "defaultChecked":
                H = B;
                break;
              case "value":
                d = B;
                break;
              case "defaultValue":
                v = B;
                break;
              case "children":
              case "dangerouslySetInnerHTML":
                if (B != null) throw Error(u(137, e));
                break;
              default:
                B !== V && Ct(t, e, U, B, i, V);
            }
        }
        Fr(t, d, v, A, L, H, o, r);
        return;
      case "select":
        B = d = v = U = null;
        for (o in a)
          if (((A = a[o]), a.hasOwnProperty(o) && A != null))
            switch (o) {
              case "value":
                break;
              case "multiple":
                B = A;
              default:
                i.hasOwnProperty(o) || Ct(t, e, o, null, i, A);
            }
        for (r in i)
          if (((o = i[r]), (A = a[r]), i.hasOwnProperty(r) && (o != null || A != null)))
            switch (r) {
              case "value":
                U = o;
                break;
              case "defaultValue":
                v = o;
                break;
              case "multiple":
                d = o;
              default:
                o !== A && Ct(t, e, r, o, i, A);
            }
        ((e = v),
          (a = d),
          (i = B),
          U != null
            ? Za(t, !!a, U, !1)
            : !!i != !!a && (e != null ? Za(t, !!a, e, !0) : Za(t, !!a, a ? [] : "", !1)));
        return;
      case "textarea":
        B = U = null;
        for (v in a)
          if (((r = a[v]), a.hasOwnProperty(v) && r != null && !i.hasOwnProperty(v)))
            switch (v) {
              case "value":
                break;
              case "children":
                break;
              default:
                Ct(t, e, v, null, i, r);
            }
        for (d in i)
          if (((r = i[d]), (o = a[d]), i.hasOwnProperty(d) && (r != null || o != null)))
            switch (d) {
              case "value":
                U = r;
                break;
              case "defaultValue":
                B = r;
                break;
              case "children":
                break;
              case "dangerouslySetInnerHTML":
                if (r != null) throw Error(u(91));
                break;
              default:
                r !== o && Ct(t, e, d, r, i, o);
            }
        cf(t, U, B);
        return;
      case "option":
        for (var W in a)
          ((U = a[W]),
            a.hasOwnProperty(W) &&
              U != null &&
              !i.hasOwnProperty(W) &&
              (W === "selected" ? (t.selected = !1) : Ct(t, e, W, null, i, U)));
        for (A in i)
          ((U = i[A]),
            (B = a[A]),
            i.hasOwnProperty(A) &&
              U !== B &&
              (U != null || B != null) &&
              (A === "selected"
                ? (t.selected = U && typeof U != "function" && typeof U != "symbol")
                : Ct(t, e, A, U, i, B)));
        return;
      case "img":
      case "link":
      case "area":
      case "base":
      case "br":
      case "col":
      case "embed":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "source":
      case "track":
      case "wbr":
      case "menuitem":
        for (var rt in a)
          ((U = a[rt]),
            a.hasOwnProperty(rt) && U != null && !i.hasOwnProperty(rt) && Ct(t, e, rt, null, i, U));
        for (L in i)
          if (((U = i[L]), (B = a[L]), i.hasOwnProperty(L) && U !== B && (U != null || B != null)))
            switch (L) {
              case "children":
              case "dangerouslySetInnerHTML":
                if (U != null) throw Error(u(137, e));
                break;
              default:
                Ct(t, e, L, U, i, B);
            }
        return;
      default:
        if (kr(e)) {
          for (var wt in a)
            ((U = a[wt]),
              a.hasOwnProperty(wt) &&
                U !== void 0 &&
                !i.hasOwnProperty(wt) &&
                jo(t, e, wt, void 0, i, U));
          for (H in i)
            ((U = i[H]),
              (B = a[H]),
              !i.hasOwnProperty(H) ||
                U === B ||
                (U === void 0 && B === void 0) ||
                jo(t, e, H, U, i, B));
          return;
        }
    }
    for (var O in a)
      ((U = a[O]),
        a.hasOwnProperty(O) && U != null && !i.hasOwnProperty(O) && Ct(t, e, O, null, i, U));
    for (V in i)
      ((U = i[V]),
        (B = a[V]),
        !i.hasOwnProperty(V) || U === B || (U == null && B == null) || Ct(t, e, V, U, i, B));
  }
  function mm(t) {
    switch (t) {
      case "css":
      case "script":
      case "font":
      case "img":
      case "image":
      case "input":
      case "link":
        return !0;
      default:
        return !1;
    }
  }
  function h0() {
    if (typeof performance.getEntriesByType == "function") {
      for (
        var t = 0, e = 0, a = performance.getEntriesByType("resource"), i = 0;
        i < a.length;
        i++
      ) {
        var r = a[i],
          o = r.transferSize,
          d = r.initiatorType,
          v = r.duration;
        if (o && v && mm(d)) {
          for (d = 0, v = r.responseEnd, i += 1; i < a.length; i++) {
            var A = a[i],
              L = A.startTime;
            if (L > v) break;
            var H = A.transferSize,
              V = A.initiatorType;
            H && mm(V) && ((A = A.responseEnd), (d += H * (A < v ? 1 : (v - L) / (A - L))));
          }
          if ((--i, (e += (8 * (o + d)) / (r.duration / 1e3)), t++, 10 < t)) break;
        }
      }
      if (0 < t) return e / t / 1e6;
    }
    return navigator.connection && ((t = navigator.connection.downlink), typeof t == "number")
      ? t
      : 5;
  }
  var Ho = null,
    qo = null;
  function $s(t) {
    return t.nodeType === 9 ? t : t.ownerDocument;
  }
  function ym(t) {
    switch (t) {
      case "http://www.w3.org/2000/svg":
        return 1;
      case "http://www.w3.org/1998/Math/MathML":
        return 2;
      default:
        return 0;
    }
  }
  function pm(t, e) {
    if (t === 0)
      switch (e) {
        case "svg":
          return 1;
        case "math":
          return 2;
        default:
          return 0;
      }
    return t === 1 && e === "foreignObject" ? 0 : t;
  }
  function Yo(t, e) {
    return (
      t === "textarea" ||
      t === "noscript" ||
      typeof e.children == "string" ||
      typeof e.children == "number" ||
      typeof e.children == "bigint" ||
      (typeof e.dangerouslySetInnerHTML == "object" &&
        e.dangerouslySetInnerHTML !== null &&
        e.dangerouslySetInnerHTML.__html != null)
    );
  }
  var Go = null;
  function m0() {
    var t = window.event;
    return t && t.type === "popstate" ? (t === Go ? !1 : ((Go = t), !0)) : ((Go = null), !1);
  }
  var gm = typeof setTimeout == "function" ? setTimeout : void 0,
    y0 = typeof clearTimeout == "function" ? clearTimeout : void 0,
    vm = typeof Promise == "function" ? Promise : void 0,
    p0 =
      typeof queueMicrotask == "function"
        ? queueMicrotask
        : typeof vm < "u"
          ? function (t) {
              return vm.resolve(null).then(t).catch(g0);
            }
          : gm;
  function g0(t) {
    setTimeout(function () {
      throw t;
    });
  }
  function kn(t) {
    return t === "head";
  }
  function Sm(t, e) {
    var a = e,
      i = 0;
    do {
      var r = a.nextSibling;
      if ((t.removeChild(a), r && r.nodeType === 8))
        if (((a = r.data), a === "/$" || a === "/&")) {
          if (i === 0) {
            (t.removeChild(r), Ri(e));
            return;
          }
          i--;
        } else if (a === "$" || a === "$?" || a === "$~" || a === "$!" || a === "&") i++;
        else if (a === "html") El(t.ownerDocument.documentElement);
        else if (a === "head") {
          ((a = t.ownerDocument.head), El(a));
          for (var o = a.firstChild; o; ) {
            var d = o.nextSibling,
              v = o.nodeName;
            (o[Yi] ||
              v === "SCRIPT" ||
              v === "STYLE" ||
              (v === "LINK" && o.rel.toLowerCase() === "stylesheet") ||
              a.removeChild(o),
              (o = d));
          }
        } else a === "body" && El(t.ownerDocument.body);
      a = r;
    } while (a);
    Ri(e);
  }
  function bm(t, e) {
    var a = t;
    t = 0;
    do {
      var i = a.nextSibling;
      if (
        (a.nodeType === 1
          ? e
            ? ((a._stashedDisplay = a.style.display), (a.style.display = "none"))
            : ((a.style.display = a._stashedDisplay || ""),
              a.getAttribute("style") === "" && a.removeAttribute("style"))
          : a.nodeType === 3 &&
            (e
              ? ((a._stashedText = a.nodeValue), (a.nodeValue = ""))
              : (a.nodeValue = a._stashedText || "")),
        i && i.nodeType === 8)
      )
        if (((a = i.data), a === "/$")) {
          if (t === 0) break;
          t--;
        } else (a !== "$" && a !== "$?" && a !== "$~" && a !== "$!") || t++;
      a = i;
    } while (a);
  }
  function Qo(t) {
    var e = t.firstChild;
    for (e && e.nodeType === 10 && (e = e.nextSibling); e; ) {
      var a = e;
      switch (((e = e.nextSibling), a.nodeName)) {
        case "HTML":
        case "HEAD":
        case "BODY":
          (Qo(a), Zr(a));
          continue;
        case "SCRIPT":
        case "STYLE":
          continue;
        case "LINK":
          if (a.rel.toLowerCase() === "stylesheet") continue;
      }
      t.removeChild(a);
    }
  }
  function v0(t, e, a, i) {
    for (; t.nodeType === 1; ) {
      var r = a;
      if (t.nodeName.toLowerCase() !== e.toLowerCase()) {
        if (!i && (t.nodeName !== "INPUT" || t.type !== "hidden")) break;
      } else if (i) {
        if (!t[Yi])
          switch (e) {
            case "meta":
              if (!t.hasAttribute("itemprop")) break;
              return t;
            case "link":
              if (
                ((o = t.getAttribute("rel")),
                o === "stylesheet" && t.hasAttribute("data-precedence"))
              )
                break;
              if (
                o !== r.rel ||
                t.getAttribute("href") !== (r.href == null || r.href === "" ? null : r.href) ||
                t.getAttribute("crossorigin") !== (r.crossOrigin == null ? null : r.crossOrigin) ||
                t.getAttribute("title") !== (r.title == null ? null : r.title)
              )
                break;
              return t;
            case "style":
              if (t.hasAttribute("data-precedence")) break;
              return t;
            case "script":
              if (
                ((o = t.getAttribute("src")),
                (o !== (r.src == null ? null : r.src) ||
                  t.getAttribute("type") !== (r.type == null ? null : r.type) ||
                  t.getAttribute("crossorigin") !==
                    (r.crossOrigin == null ? null : r.crossOrigin)) &&
                  o &&
                  t.hasAttribute("async") &&
                  !t.hasAttribute("itemprop"))
              )
                break;
              return t;
            default:
              return t;
          }
      } else if (e === "input" && t.type === "hidden") {
        var o = r.name == null ? null : "" + r.name;
        if (r.type === "hidden" && t.getAttribute("name") === o) return t;
      } else return t;
      if (((t = Ge(t.nextSibling)), t === null)) break;
    }
    return null;
  }
  function S0(t, e, a) {
    if (e === "") return null;
    for (; t.nodeType !== 3; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") && !a) ||
        ((t = Ge(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function _m(t, e) {
    for (; t.nodeType !== 8; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") && !e) ||
        ((t = Ge(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function Vo(t) {
    return t.data === "$?" || t.data === "$~";
  }
  function Xo(t) {
    return t.data === "$!" || (t.data === "$?" && t.ownerDocument.readyState !== "loading");
  }
  function b0(t, e) {
    var a = t.ownerDocument;
    if (t.data === "$~") t._reactRetry = e;
    else if (t.data !== "$?" || a.readyState !== "loading") e();
    else {
      var i = function () {
        (e(), a.removeEventListener("DOMContentLoaded", i));
      };
      (a.addEventListener("DOMContentLoaded", i), (t._reactRetry = i));
    }
  }
  function Ge(t) {
    for (; t != null; t = t.nextSibling) {
      var e = t.nodeType;
      if (e === 1 || e === 3) break;
      if (e === 8) {
        if (
          ((e = t.data),
          e === "$" ||
            e === "$!" ||
            e === "$?" ||
            e === "$~" ||
            e === "&" ||
            e === "F!" ||
            e === "F")
        )
          break;
        if (e === "/$" || e === "/&") return null;
      }
    }
    return t;
  }
  var Ko = null;
  function Em(t) {
    t = t.nextSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var a = t.data;
        if (a === "/$" || a === "/&") {
          if (e === 0) return Ge(t.nextSibling);
          e--;
        } else (a !== "$" && a !== "$!" && a !== "$?" && a !== "$~" && a !== "&") || e++;
      }
      t = t.nextSibling;
    }
    return null;
  }
  function Tm(t) {
    t = t.previousSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var a = t.data;
        if (a === "$" || a === "$!" || a === "$?" || a === "$~" || a === "&") {
          if (e === 0) return t;
          e--;
        } else (a !== "/$" && a !== "/&") || e++;
      }
      t = t.previousSibling;
    }
    return null;
  }
  function Rm(t, e, a) {
    switch (((e = $s(a)), t)) {
      case "html":
        if (((t = e.documentElement), !t)) throw Error(u(452));
        return t;
      case "head":
        if (((t = e.head), !t)) throw Error(u(453));
        return t;
      case "body":
        if (((t = e.body), !t)) throw Error(u(454));
        return t;
      default:
        throw Error(u(451));
    }
  }
  function El(t) {
    for (var e = t.attributes; e.length; ) t.removeAttributeNode(e[0]);
    Zr(t);
  }
  var Qe = new Map(),
    Am = new Set();
  function Ws(t) {
    return typeof t.getRootNode == "function"
      ? t.getRootNode()
      : t.nodeType === 9
        ? t
        : t.ownerDocument;
  }
  var Tn = F.d;
  F.d = { f: _0, r: E0, D: T0, C: R0, L: A0, m: x0, X: C0, S: M0, M: w0 };
  function _0() {
    var t = Tn.f(),
      e = Xs();
    return t || e;
  }
  function E0(t) {
    var e = Va(t);
    e !== null && e.tag === 5 && e.type === "form" ? Qd(e) : Tn.r(t);
  }
  var _i = typeof document > "u" ? null : document;
  function xm(t, e, a) {
    var i = _i;
    if (i && typeof e == "string" && e) {
      var r = Ue(e);
      ((r = 'link[rel="' + t + '"][href="' + r + '"]'),
        typeof a == "string" && (r += '[crossorigin="' + a + '"]'),
        Am.has(r) ||
          (Am.add(r),
          (t = { rel: t, crossOrigin: a, href: e }),
          i.querySelector(r) === null &&
            ((e = i.createElement("link")), ue(e, "link", t), te(e), i.head.appendChild(e))));
    }
  }
  function T0(t) {
    (Tn.D(t), xm("dns-prefetch", t, null));
  }
  function R0(t, e) {
    (Tn.C(t, e), xm("preconnect", t, e));
  }
  function A0(t, e, a) {
    Tn.L(t, e, a);
    var i = _i;
    if (i && t && e) {
      var r = 'link[rel="preload"][as="' + Ue(e) + '"]';
      e === "image" && a && a.imageSrcSet
        ? ((r += '[imagesrcset="' + Ue(a.imageSrcSet) + '"]'),
          typeof a.imageSizes == "string" && (r += '[imagesizes="' + Ue(a.imageSizes) + '"]'))
        : (r += '[href="' + Ue(t) + '"]');
      var o = r;
      switch (e) {
        case "style":
          o = Ei(t);
          break;
        case "script":
          o = Ti(t);
      }
      Qe.has(o) ||
        ((t = g(
          { rel: "preload", href: e === "image" && a && a.imageSrcSet ? void 0 : t, as: e },
          a,
        )),
        Qe.set(o, t),
        i.querySelector(r) !== null ||
          (e === "style" && i.querySelector(Tl(o))) ||
          (e === "script" && i.querySelector(Rl(o))) ||
          ((e = i.createElement("link")), ue(e, "link", t), te(e), i.head.appendChild(e)));
    }
  }
  function x0(t, e) {
    Tn.m(t, e);
    var a = _i;
    if (a && t) {
      var i = e && typeof e.as == "string" ? e.as : "script",
        r = 'link[rel="modulepreload"][as="' + Ue(i) + '"][href="' + Ue(t) + '"]',
        o = r;
      switch (i) {
        case "audioworklet":
        case "paintworklet":
        case "serviceworker":
        case "sharedworker":
        case "worker":
        case "script":
          o = Ti(t);
      }
      if (
        !Qe.has(o) &&
        ((t = g({ rel: "modulepreload", href: t }, e)), Qe.set(o, t), a.querySelector(r) === null)
      ) {
        switch (i) {
          case "audioworklet":
          case "paintworklet":
          case "serviceworker":
          case "sharedworker":
          case "worker":
          case "script":
            if (a.querySelector(Rl(o))) return;
        }
        ((i = a.createElement("link")), ue(i, "link", t), te(i), a.head.appendChild(i));
      }
    }
  }
  function M0(t, e, a) {
    Tn.S(t, e, a);
    var i = _i;
    if (i && t) {
      var r = Xa(i).hoistableStyles,
        o = Ei(t);
      e = e || "default";
      var d = r.get(o);
      if (!d) {
        var v = { loading: 0, preload: null };
        if ((d = i.querySelector(Tl(o)))) v.loading = 5;
        else {
          ((t = g({ rel: "stylesheet", href: t, "data-precedence": e }, a)),
            (a = Qe.get(o)) && Zo(t, a));
          var A = (d = i.createElement("link"));
          (te(A),
            ue(A, "link", t),
            (A._p = new Promise(function (L, H) {
              ((A.onload = L), (A.onerror = H));
            })),
            A.addEventListener("load", function () {
              v.loading |= 1;
            }),
            A.addEventListener("error", function () {
              v.loading |= 2;
            }),
            (v.loading |= 4),
            tr(d, e, i));
        }
        ((d = { type: "stylesheet", instance: d, count: 1, state: v }), r.set(o, d));
      }
    }
  }
  function C0(t, e) {
    Tn.X(t, e);
    var a = _i;
    if (a && t) {
      var i = Xa(a).hoistableScripts,
        r = Ti(t),
        o = i.get(r);
      o ||
        ((o = a.querySelector(Rl(r))),
        o ||
          ((t = g({ src: t, async: !0 }, e)),
          (e = Qe.get(r)) && Po(t, e),
          (o = a.createElement("script")),
          te(o),
          ue(o, "link", t),
          a.head.appendChild(o)),
        (o = { type: "script", instance: o, count: 1, state: null }),
        i.set(r, o));
    }
  }
  function w0(t, e) {
    Tn.M(t, e);
    var a = _i;
    if (a && t) {
      var i = Xa(a).hoistableScripts,
        r = Ti(t),
        o = i.get(r);
      o ||
        ((o = a.querySelector(Rl(r))),
        o ||
          ((t = g({ src: t, async: !0, type: "module" }, e)),
          (e = Qe.get(r)) && Po(t, e),
          (o = a.createElement("script")),
          te(o),
          ue(o, "link", t),
          a.head.appendChild(o)),
        (o = { type: "script", instance: o, count: 1, state: null }),
        i.set(r, o));
    }
  }
  function Mm(t, e, a, i) {
    var r = (r = dt.current) ? Ws(r) : null;
    if (!r) throw Error(u(446));
    switch (t) {
      case "meta":
      case "title":
        return null;
      case "style":
        return typeof a.precedence == "string" && typeof a.href == "string"
          ? ((e = Ei(a.href)),
            (a = Xa(r).hoistableStyles),
            (i = a.get(e)),
            i || ((i = { type: "style", instance: null, count: 0, state: null }), a.set(e, i)),
            i)
          : { type: "void", instance: null, count: 0, state: null };
      case "link":
        if (
          a.rel === "stylesheet" &&
          typeof a.href == "string" &&
          typeof a.precedence == "string"
        ) {
          t = Ei(a.href);
          var o = Xa(r).hoistableStyles,
            d = o.get(t);
          if (
            (d ||
              ((r = r.ownerDocument || r),
              (d = {
                type: "stylesheet",
                instance: null,
                count: 0,
                state: { loading: 0, preload: null },
              }),
              o.set(t, d),
              (o = r.querySelector(Tl(t))) && !o._p && ((d.instance = o), (d.state.loading = 5)),
              Qe.has(t) ||
                ((a = {
                  rel: "preload",
                  as: "style",
                  href: a.href,
                  crossOrigin: a.crossOrigin,
                  integrity: a.integrity,
                  media: a.media,
                  hrefLang: a.hrefLang,
                  referrerPolicy: a.referrerPolicy,
                }),
                Qe.set(t, a),
                o || O0(r, t, a, d.state))),
            e && i === null)
          )
            throw Error(u(528, ""));
          return d;
        }
        if (e && i !== null) throw Error(u(529, ""));
        return null;
      case "script":
        return (
          (e = a.async),
          (a = a.src),
          typeof a == "string" && e && typeof e != "function" && typeof e != "symbol"
            ? ((e = Ti(a)),
              (a = Xa(r).hoistableScripts),
              (i = a.get(e)),
              i || ((i = { type: "script", instance: null, count: 0, state: null }), a.set(e, i)),
              i)
            : { type: "void", instance: null, count: 0, state: null }
        );
      default:
        throw Error(u(444, t));
    }
  }
  function Ei(t) {
    return 'href="' + Ue(t) + '"';
  }
  function Tl(t) {
    return 'link[rel="stylesheet"][' + t + "]";
  }
  function Cm(t) {
    return g({}, t, { "data-precedence": t.precedence, precedence: null });
  }
  function O0(t, e, a, i) {
    t.querySelector('link[rel="preload"][as="style"][' + e + "]")
      ? (i.loading = 1)
      : ((e = t.createElement("link")),
        (i.preload = e),
        e.addEventListener("load", function () {
          return (i.loading |= 1);
        }),
        e.addEventListener("error", function () {
          return (i.loading |= 2);
        }),
        ue(e, "link", a),
        te(e),
        t.head.appendChild(e));
  }
  function Ti(t) {
    return '[src="' + Ue(t) + '"]';
  }
  function Rl(t) {
    return "script[async]" + t;
  }
  function wm(t, e, a) {
    if ((e.count++, e.instance === null))
      switch (e.type) {
        case "style":
          var i = t.querySelector('style[data-href~="' + Ue(a.href) + '"]');
          if (i) return ((e.instance = i), te(i), i);
          var r = g({}, a, {
            "data-href": a.href,
            "data-precedence": a.precedence,
            href: null,
            precedence: null,
          });
          return (
            (i = (t.ownerDocument || t).createElement("style")),
            te(i),
            ue(i, "style", r),
            tr(i, a.precedence, t),
            (e.instance = i)
          );
        case "stylesheet":
          r = Ei(a.href);
          var o = t.querySelector(Tl(r));
          if (o) return ((e.state.loading |= 4), (e.instance = o), te(o), o);
          ((i = Cm(a)),
            (r = Qe.get(r)) && Zo(i, r),
            (o = (t.ownerDocument || t).createElement("link")),
            te(o));
          var d = o;
          return (
            (d._p = new Promise(function (v, A) {
              ((d.onload = v), (d.onerror = A));
            })),
            ue(o, "link", i),
            (e.state.loading |= 4),
            tr(o, a.precedence, t),
            (e.instance = o)
          );
        case "script":
          return (
            (o = Ti(a.src)),
            (r = t.querySelector(Rl(o)))
              ? ((e.instance = r), te(r), r)
              : ((i = a),
                (r = Qe.get(o)) && ((i = g({}, a)), Po(i, r)),
                (t = t.ownerDocument || t),
                (r = t.createElement("script")),
                te(r),
                ue(r, "link", i),
                t.head.appendChild(r),
                (e.instance = r))
          );
        case "void":
          return null;
        default:
          throw Error(u(443, e.type));
      }
    else
      e.type === "stylesheet" &&
        (e.state.loading & 4) === 0 &&
        ((i = e.instance), (e.state.loading |= 4), tr(i, a.precedence, t));
    return e.instance;
  }
  function tr(t, e, a) {
    for (
      var i = a.querySelectorAll('link[rel="stylesheet"][data-precedence],style[data-precedence]'),
        r = i.length ? i[i.length - 1] : null,
        o = r,
        d = 0;
      d < i.length;
      d++
    ) {
      var v = i[d];
      if (v.dataset.precedence === e) o = v;
      else if (o !== r) break;
    }
    o
      ? o.parentNode.insertBefore(t, o.nextSibling)
      : ((e = a.nodeType === 9 ? a.head : a), e.insertBefore(t, e.firstChild));
  }
  function Zo(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.title == null && (t.title = e.title));
  }
  function Po(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.integrity == null && (t.integrity = e.integrity));
  }
  var er = null;
  function Om(t, e, a) {
    if (er === null) {
      var i = new Map(),
        r = (er = new Map());
      r.set(a, i);
    } else ((r = er), (i = r.get(a)), i || ((i = new Map()), r.set(a, i)));
    if (i.has(t)) return i;
    for (i.set(t, null), a = a.getElementsByTagName(t), r = 0; r < a.length; r++) {
      var o = a[r];
      if (
        !(o[Yi] || o[ie] || (t === "link" && o.getAttribute("rel") === "stylesheet")) &&
        o.namespaceURI !== "http://www.w3.org/2000/svg"
      ) {
        var d = o.getAttribute(e) || "";
        d = t + d;
        var v = i.get(d);
        v ? v.push(o) : i.set(d, [o]);
      }
    }
    return i;
  }
  function zm(t, e, a) {
    ((t = t.ownerDocument || t),
      t.head.insertBefore(a, e === "title" ? t.querySelector("head > title") : null));
  }
  function z0(t, e, a) {
    if (a === 1 || e.itemProp != null) return !1;
    switch (t) {
      case "meta":
      case "title":
        return !0;
      case "style":
        if (typeof e.precedence != "string" || typeof e.href != "string" || e.href === "") break;
        return !0;
      case "link":
        if (
          typeof e.rel != "string" ||
          typeof e.href != "string" ||
          e.href === "" ||
          e.onLoad ||
          e.onError
        )
          break;
        return e.rel === "stylesheet"
          ? ((t = e.disabled), typeof e.precedence == "string" && t == null)
          : !0;
      case "script":
        if (
          e.async &&
          typeof e.async != "function" &&
          typeof e.async != "symbol" &&
          !e.onLoad &&
          !e.onError &&
          e.src &&
          typeof e.src == "string"
        )
          return !0;
    }
    return !1;
  }
  function Dm(t) {
    return !(t.type === "stylesheet" && (t.state.loading & 3) === 0);
  }
  function D0(t, e, a, i) {
    if (
      a.type === "stylesheet" &&
      (typeof i.media != "string" || matchMedia(i.media).matches !== !1) &&
      (a.state.loading & 4) === 0
    ) {
      if (a.instance === null) {
        var r = Ei(i.href),
          o = e.querySelector(Tl(r));
        if (o) {
          ((e = o._p),
            e !== null &&
              typeof e == "object" &&
              typeof e.then == "function" &&
              (t.count++, (t = nr.bind(t)), e.then(t, t)),
            (a.state.loading |= 4),
            (a.instance = o),
            te(o));
          return;
        }
        ((o = e.ownerDocument || e),
          (i = Cm(i)),
          (r = Qe.get(r)) && Zo(i, r),
          (o = o.createElement("link")),
          te(o));
        var d = o;
        ((d._p = new Promise(function (v, A) {
          ((d.onload = v), (d.onerror = A));
        })),
          ue(o, "link", i),
          (a.instance = o));
      }
      (t.stylesheets === null && (t.stylesheets = new Map()),
        t.stylesheets.set(a, e),
        (e = a.state.preload) &&
          (a.state.loading & 3) === 0 &&
          (t.count++,
          (a = nr.bind(t)),
          e.addEventListener("load", a),
          e.addEventListener("error", a)));
    }
  }
  var Fo = 0;
  function L0(t, e) {
    return (
      t.stylesheets && t.count === 0 && ir(t, t.stylesheets),
      0 < t.count || 0 < t.imgCount
        ? function (a) {
            var i = setTimeout(function () {
              if ((t.stylesheets && ir(t, t.stylesheets), t.unsuspend)) {
                var o = t.unsuspend;
                ((t.unsuspend = null), o());
              }
            }, 6e4 + e);
            0 < t.imgBytes && Fo === 0 && (Fo = 62500 * h0());
            var r = setTimeout(
              function () {
                if (
                  ((t.waitingForImages = !1),
                  t.count === 0 && (t.stylesheets && ir(t, t.stylesheets), t.unsuspend))
                ) {
                  var o = t.unsuspend;
                  ((t.unsuspend = null), o());
                }
              },
              (t.imgBytes > Fo ? 50 : 800) + e,
            );
            return (
              (t.unsuspend = a),
              function () {
                ((t.unsuspend = null), clearTimeout(i), clearTimeout(r));
              }
            );
          }
        : null
    );
  }
  function nr() {
    if ((this.count--, this.count === 0 && (this.imgCount === 0 || !this.waitingForImages))) {
      if (this.stylesheets) ir(this, this.stylesheets);
      else if (this.unsuspend) {
        var t = this.unsuspend;
        ((this.unsuspend = null), t());
      }
    }
  }
  var ar = null;
  function ir(t, e) {
    ((t.stylesheets = null),
      t.unsuspend !== null &&
        (t.count++, (ar = new Map()), e.forEach(U0, t), (ar = null), nr.call(t)));
  }
  function U0(t, e) {
    if (!(e.state.loading & 4)) {
      var a = ar.get(t);
      if (a) var i = a.get(null);
      else {
        ((a = new Map()), ar.set(t, a));
        for (
          var r = t.querySelectorAll("link[data-precedence],style[data-precedence]"), o = 0;
          o < r.length;
          o++
        ) {
          var d = r[o];
          (d.nodeName === "LINK" || d.getAttribute("media") !== "not all") &&
            (a.set(d.dataset.precedence, d), (i = d));
        }
        i && a.set(null, i);
      }
      ((r = e.instance),
        (d = r.getAttribute("data-precedence")),
        (o = a.get(d) || i),
        o === i && a.set(null, r),
        a.set(d, r),
        this.count++,
        (i = nr.bind(this)),
        r.addEventListener("load", i),
        r.addEventListener("error", i),
        o
          ? o.parentNode.insertBefore(r, o.nextSibling)
          : ((t = t.nodeType === 9 ? t.head : t), t.insertBefore(r, t.firstChild)),
        (e.state.loading |= 4));
    }
  }
  var Al = {
    $$typeof: X,
    Provider: null,
    Consumer: null,
    _currentValue: it,
    _currentValue2: it,
    _threadCount: 0,
  };
  function B0(t, e, a, i, r, o, d, v, A) {
    ((this.tag = 1),
      (this.containerInfo = t),
      (this.pingCache = this.current = this.pendingChildren = null),
      (this.timeoutHandle = -1),
      (this.callbackNode =
        this.next =
        this.pendingContext =
        this.context =
        this.cancelPendingCommit =
          null),
      (this.callbackPriority = 0),
      (this.expirationTimes = Qr(-1)),
      (this.entangledLanes =
        this.shellSuspendCounter =
        this.errorRecoveryDisabledLanes =
        this.expiredLanes =
        this.warmLanes =
        this.pingedLanes =
        this.suspendedLanes =
        this.pendingLanes =
          0),
      (this.entanglements = Qr(0)),
      (this.hiddenUpdates = Qr(null)),
      (this.identifierPrefix = i),
      (this.onUncaughtError = r),
      (this.onCaughtError = o),
      (this.onRecoverableError = d),
      (this.pooledCache = null),
      (this.pooledCacheLanes = 0),
      (this.formState = A),
      (this.incompleteTransitions = new Map()));
  }
  function Lm(t, e, a, i, r, o, d, v, A, L, H, V) {
    return (
      (t = new B0(t, e, a, d, A, L, H, V, v)),
      (e = 1),
      o === !0 && (e |= 24),
      (o = Me(3, null, null, e)),
      (t.current = o),
      (o.stateNode = t),
      (e = xu()),
      e.refCount++,
      (t.pooledCache = e),
      e.refCount++,
      (o.memoizedState = { element: i, isDehydrated: a, cache: e }),
      Ou(o),
      t
    );
  }
  function Um(t) {
    return t ? ((t = ti), t) : ti;
  }
  function Bm(t, e, a, i, r, o) {
    ((r = Um(r)),
      i.context === null ? (i.context = r) : (i.pendingContext = r),
      (i = qn(e)),
      (i.payload = { element: a }),
      (o = o === void 0 ? null : o),
      o !== null && (i.callback = o),
      (a = Yn(t, i, e)),
      a !== null && (_e(a, t, e), al(a, t, e)));
  }
  function Nm(t, e) {
    if (((t = t.memoizedState), t !== null && t.dehydrated !== null)) {
      var a = t.retryLane;
      t.retryLane = a !== 0 && a < e ? a : e;
    }
  }
  function Jo(t, e) {
    (Nm(t, e), (t = t.alternate) && Nm(t, e));
  }
  function jm(t) {
    if (t.tag === 13 || t.tag === 31) {
      var e = pa(t, 67108864);
      (e !== null && _e(e, t, 67108864), Jo(t, 67108864));
    }
  }
  function Hm(t) {
    if (t.tag === 13 || t.tag === 31) {
      var e = De();
      e = Vr(e);
      var a = pa(t, e);
      (a !== null && _e(a, t, e), Jo(t, e));
    }
  }
  var lr = !0;
  function N0(t, e, a, i) {
    var r = j.T;
    j.T = null;
    var o = F.p;
    try {
      ((F.p = 2), ko(t, e, a, i));
    } finally {
      ((F.p = o), (j.T = r));
    }
  }
  function j0(t, e, a, i) {
    var r = j.T;
    j.T = null;
    var o = F.p;
    try {
      ((F.p = 8), ko(t, e, a, i));
    } finally {
      ((F.p = o), (j.T = r));
    }
  }
  function ko(t, e, a, i) {
    if (lr) {
      var r = Io(i);
      if (r === null) (No(t, e, i, sr, a), Ym(t, i));
      else if (q0(r, t, e, a, i)) i.stopPropagation();
      else if ((Ym(t, i), e & 4 && -1 < H0.indexOf(t))) {
        for (; r !== null; ) {
          var o = Va(r);
          if (o !== null)
            switch (o.tag) {
              case 3:
                if (((o = o.stateNode), o.current.memoizedState.isDehydrated)) {
                  var d = fa(o.pendingLanes);
                  if (d !== 0) {
                    var v = o;
                    for (v.pendingLanes |= 2, v.entangledLanes |= 2; d; ) {
                      var A = 1 << (31 - Ae(d));
                      ((v.entanglements[1] |= A), (d &= ~A));
                    }
                    (We(o), (Tt & 6) === 0 && ((Qs = ce() + 500), Sl(0)));
                  }
                }
                break;
              case 31:
              case 13:
                ((v = pa(o, 2)), v !== null && _e(v, o, 2), Xs(), Jo(o, 2));
            }
          if (((o = Io(i)), o === null && No(t, e, i, sr, a), o === r)) break;
          r = o;
        }
        r !== null && i.stopPropagation();
      } else No(t, e, i, null, a);
    }
  }
  function Io(t) {
    return ((t = $r(t)), $o(t));
  }
  var sr = null;
  function $o(t) {
    if (((sr = null), (t = Qa(t)), t !== null)) {
      var e = f(t);
      if (e === null) t = null;
      else {
        var a = e.tag;
        if (a === 13) {
          if (((t = h(e)), t !== null)) return t;
          t = null;
        } else if (a === 31) {
          if (((t = y(e)), t !== null)) return t;
          t = null;
        } else if (a === 3) {
          if (e.stateNode.current.memoizedState.isDehydrated)
            return e.tag === 3 ? e.stateNode.containerInfo : null;
          t = null;
        } else e !== t && (t = null);
      }
    }
    return ((sr = t), null);
  }
  function qm(t) {
    switch (t) {
      case "beforetoggle":
      case "cancel":
      case "click":
      case "close":
      case "contextmenu":
      case "copy":
      case "cut":
      case "auxclick":
      case "dblclick":
      case "dragend":
      case "dragstart":
      case "drop":
      case "focusin":
      case "focusout":
      case "input":
      case "invalid":
      case "keydown":
      case "keypress":
      case "keyup":
      case "mousedown":
      case "mouseup":
      case "paste":
      case "pause":
      case "play":
      case "pointercancel":
      case "pointerdown":
      case "pointerup":
      case "ratechange":
      case "reset":
      case "resize":
      case "seeked":
      case "submit":
      case "toggle":
      case "touchcancel":
      case "touchend":
      case "touchstart":
      case "volumechange":
      case "change":
      case "selectionchange":
      case "textInput":
      case "compositionstart":
      case "compositionend":
      case "compositionupdate":
      case "beforeblur":
      case "afterblur":
      case "beforeinput":
      case "blur":
      case "fullscreenchange":
      case "focus":
      case "hashchange":
      case "popstate":
      case "select":
      case "selectstart":
        return 2;
      case "drag":
      case "dragenter":
      case "dragexit":
      case "dragleave":
      case "dragover":
      case "mousemove":
      case "mouseout":
      case "mouseover":
      case "pointermove":
      case "pointerout":
      case "pointerover":
      case "scroll":
      case "touchmove":
      case "wheel":
      case "mouseenter":
      case "mouseleave":
      case "pointerenter":
      case "pointerleave":
        return 8;
      case "message":
        switch (Lt()) {
          case ae:
            return 2;
          case Je:
            return 8;
          case Ya:
          case Rg:
            return 32;
          case Fc:
            return 268435456;
          default:
            return 32;
        }
      default:
        return 32;
    }
  }
  var Wo = !1,
    In = null,
    $n = null,
    Wn = null,
    xl = new Map(),
    Ml = new Map(),
    ta = [],
    H0 =
      "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
        " ",
      );
  function Ym(t, e) {
    switch (t) {
      case "focusin":
      case "focusout":
        In = null;
        break;
      case "dragenter":
      case "dragleave":
        $n = null;
        break;
      case "mouseover":
      case "mouseout":
        Wn = null;
        break;
      case "pointerover":
      case "pointerout":
        xl.delete(e.pointerId);
        break;
      case "gotpointercapture":
      case "lostpointercapture":
        Ml.delete(e.pointerId);
    }
  }
  function Cl(t, e, a, i, r, o) {
    return t === null || t.nativeEvent !== o
      ? ((t = {
          blockedOn: e,
          domEventName: a,
          eventSystemFlags: i,
          nativeEvent: o,
          targetContainers: [r],
        }),
        e !== null && ((e = Va(e)), e !== null && jm(e)),
        t)
      : ((t.eventSystemFlags |= i),
        (e = t.targetContainers),
        r !== null && e.indexOf(r) === -1 && e.push(r),
        t);
  }
  function q0(t, e, a, i, r) {
    switch (e) {
      case "focusin":
        return ((In = Cl(In, t, e, a, i, r)), !0);
      case "dragenter":
        return (($n = Cl($n, t, e, a, i, r)), !0);
      case "mouseover":
        return ((Wn = Cl(Wn, t, e, a, i, r)), !0);
      case "pointerover":
        var o = r.pointerId;
        return (xl.set(o, Cl(xl.get(o) || null, t, e, a, i, r)), !0);
      case "gotpointercapture":
        return ((o = r.pointerId), Ml.set(o, Cl(Ml.get(o) || null, t, e, a, i, r)), !0);
    }
    return !1;
  }
  function Gm(t) {
    var e = Qa(t.target);
    if (e !== null) {
      var a = f(e);
      if (a !== null) {
        if (((e = a.tag), e === 13)) {
          if (((e = h(a)), e !== null)) {
            ((t.blockedOn = e),
              tf(t.priority, function () {
                Hm(a);
              }));
            return;
          }
        } else if (e === 31) {
          if (((e = y(a)), e !== null)) {
            ((t.blockedOn = e),
              tf(t.priority, function () {
                Hm(a);
              }));
            return;
          }
        } else if (e === 3 && a.stateNode.current.memoizedState.isDehydrated) {
          t.blockedOn = a.tag === 3 ? a.stateNode.containerInfo : null;
          return;
        }
      }
    }
    t.blockedOn = null;
  }
  function rr(t) {
    if (t.blockedOn !== null) return !1;
    for (var e = t.targetContainers; 0 < e.length; ) {
      var a = Io(t.nativeEvent);
      if (a === null) {
        a = t.nativeEvent;
        var i = new a.constructor(a.type, a);
        ((Ir = i), a.target.dispatchEvent(i), (Ir = null));
      } else return ((e = Va(a)), e !== null && jm(e), (t.blockedOn = a), !1);
      e.shift();
    }
    return !0;
  }
  function Qm(t, e, a) {
    rr(t) && a.delete(e);
  }
  function Y0() {
    ((Wo = !1),
      In !== null && rr(In) && (In = null),
      $n !== null && rr($n) && ($n = null),
      Wn !== null && rr(Wn) && (Wn = null),
      xl.forEach(Qm),
      Ml.forEach(Qm));
  }
  function ur(t, e) {
    t.blockedOn === e &&
      ((t.blockedOn = null),
      Wo || ((Wo = !0), n.unstable_scheduleCallback(n.unstable_NormalPriority, Y0)));
  }
  var or = null;
  function Vm(t) {
    or !== t &&
      ((or = t),
      n.unstable_scheduleCallback(n.unstable_NormalPriority, function () {
        or === t && (or = null);
        for (var e = 0; e < t.length; e += 3) {
          var a = t[e],
            i = t[e + 1],
            r = t[e + 2];
          if (typeof i != "function") {
            if ($o(i || a) === null) continue;
            break;
          }
          var o = Va(a);
          o !== null &&
            (t.splice(e, 3),
            (e -= 3),
            ku(o, { pending: !0, data: r, method: a.method, action: i }, i, r));
        }
      }));
  }
  function Ri(t) {
    function e(A) {
      return ur(A, t);
    }
    (In !== null && ur(In, t),
      $n !== null && ur($n, t),
      Wn !== null && ur(Wn, t),
      xl.forEach(e),
      Ml.forEach(e));
    for (var a = 0; a < ta.length; a++) {
      var i = ta[a];
      i.blockedOn === t && (i.blockedOn = null);
    }
    for (; 0 < ta.length && ((a = ta[0]), a.blockedOn === null); )
      (Gm(a), a.blockedOn === null && ta.shift());
    if (((a = (t.ownerDocument || t).$$reactFormReplay), a != null))
      for (i = 0; i < a.length; i += 3) {
        var r = a[i],
          o = a[i + 1],
          d = r[ye] || null;
        if (typeof o == "function") d || Vm(a);
        else if (d) {
          var v = null;
          if (o && o.hasAttribute("formAction")) {
            if (((r = o), (d = o[ye] || null))) v = d.formAction;
            else if ($o(r) !== null) continue;
          } else v = d.action;
          (typeof v == "function" ? (a[i + 1] = v) : (a.splice(i, 3), (i -= 3)), Vm(a));
        }
      }
  }
  function Xm() {
    function t(o) {
      o.canIntercept &&
        o.info === "react-transition" &&
        o.intercept({
          handler: function () {
            return new Promise(function (d) {
              return (r = d);
            });
          },
          focusReset: "manual",
          scroll: "manual",
        });
    }
    function e() {
      (r !== null && (r(), (r = null)), i || setTimeout(a, 20));
    }
    function a() {
      if (!i && !navigation.transition) {
        var o = navigation.currentEntry;
        o &&
          o.url != null &&
          navigation.navigate(o.url, {
            state: o.getState(),
            info: "react-transition",
            history: "replace",
          });
      }
    }
    if (typeof navigation == "object") {
      var i = !1,
        r = null;
      return (
        navigation.addEventListener("navigate", t),
        navigation.addEventListener("navigatesuccess", e),
        navigation.addEventListener("navigateerror", e),
        setTimeout(a, 100),
        function () {
          ((i = !0),
            navigation.removeEventListener("navigate", t),
            navigation.removeEventListener("navigatesuccess", e),
            navigation.removeEventListener("navigateerror", e),
            r !== null && (r(), (r = null)));
        }
      );
    }
  }
  function tc(t) {
    this._internalRoot = t;
  }
  ((cr.prototype.render = tc.prototype.render =
    function (t) {
      var e = this._internalRoot;
      if (e === null) throw Error(u(409));
      var a = e.current,
        i = De();
      Bm(a, i, t, e, null, null);
    }),
    (cr.prototype.unmount = tc.prototype.unmount =
      function () {
        var t = this._internalRoot;
        if (t !== null) {
          this._internalRoot = null;
          var e = t.containerInfo;
          (Bm(t.current, 2, null, t, null, null), Xs(), (e[Ga] = null));
        }
      }));
  function cr(t) {
    this._internalRoot = t;
  }
  cr.prototype.unstable_scheduleHydration = function (t) {
    if (t) {
      var e = Wc();
      t = { blockedOn: null, target: t, priority: e };
      for (var a = 0; a < ta.length && e !== 0 && e < ta[a].priority; a++);
      (ta.splice(a, 0, t), a === 0 && Gm(t));
    }
  };
  var Km = l.version;
  if (Km !== "19.2.6") throw Error(u(527, Km, "19.2.6"));
  F.findDOMNode = function (t) {
    var e = t._reactInternals;
    if (e === void 0)
      throw typeof t.render == "function"
        ? Error(u(188))
        : ((t = Object.keys(t).join(",")), Error(u(268, t)));
    return ((t = p(e)), (t = t !== null ? S(t) : null), (t = t === null ? null : t.stateNode), t);
  };
  var G0 = {
    bundleType: 0,
    version: "19.2.6",
    rendererPackageName: "react-dom",
    currentDispatcherRef: j,
    reconcilerVersion: "19.2.6",
  };
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
    var fr = __REACT_DEVTOOLS_GLOBAL_HOOK__;
    if (!fr.isDisabled && fr.supportsFiber)
      try {
        ((ji = fr.inject(G0)), (Re = fr));
      } catch {}
  }
  return (
    (Ol.createRoot = function (t, e) {
      if (!c(t)) throw Error(u(299));
      var a = !1,
        i = "",
        r = $d,
        o = Wd,
        d = th;
      return (
        e != null &&
          (e.unstable_strictMode === !0 && (a = !0),
          e.identifierPrefix !== void 0 && (i = e.identifierPrefix),
          e.onUncaughtError !== void 0 && (r = e.onUncaughtError),
          e.onCaughtError !== void 0 && (o = e.onCaughtError),
          e.onRecoverableError !== void 0 && (d = e.onRecoverableError)),
        (e = Lm(t, 1, !1, null, null, a, i, null, r, o, d, Xm)),
        (t[Ga] = e.current),
        Bo(t),
        new tc(e)
      );
    }),
    (Ol.hydrateRoot = function (t, e, a) {
      if (!c(t)) throw Error(u(299));
      var i = !1,
        r = "",
        o = $d,
        d = Wd,
        v = th,
        A = null;
      return (
        a != null &&
          (a.unstable_strictMode === !0 && (i = !0),
          a.identifierPrefix !== void 0 && (r = a.identifierPrefix),
          a.onUncaughtError !== void 0 && (o = a.onUncaughtError),
          a.onCaughtError !== void 0 && (d = a.onCaughtError),
          a.onRecoverableError !== void 0 && (v = a.onRecoverableError),
          a.formState !== void 0 && (A = a.formState)),
        (e = Lm(t, 1, !0, e, a ?? null, i, r, A, o, d, v, Xm)),
        (e.context = Um(null)),
        (a = e.current),
        (i = De()),
        (i = Vr(i)),
        (r = qn(i)),
        (r.callback = null),
        Yn(a, r, i),
        (a = i),
        (e.current.lanes = a),
        qi(e, a),
        We(e),
        (t[Ga] = e.current),
        Bo(t),
        new cr(e)
      );
    }),
    (Ol.version = "19.2.6"),
    Ol
  );
}
var ey;
function I0() {
  if (ey) return ac.exports;
  ey = 1;
  function n() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(n);
      } catch (l) {
        console.error(l);
      }
  }
  return (n(), (ac.exports = k0()), ac.exports);
}
var $0 = I0(),
  W0 = "__TSS_CONTEXT",
  _c = Symbol.for("TSS_SERVER_FUNCTION"),
  tS = "application/x-tss-framed",
  Rn = { JSON: 0, CHUNK: 1, END: 2, ERROR: 3 },
  eS = /;\s*v=(\d+)/;
function nS(n) {
  const l = n.match(eS);
  return l ? parseInt(l[1], 10) : void 0;
}
function aS(n) {
  const l = nS(n);
  if (l !== void 0 && l !== 1)
    throw new Error(
      `Incompatible framed protocol version: server=${l}, client=1. Please ensure client and server are using compatible versions.`,
    );
}
var Wy = () => window.__TSS_START_OPTIONS__;
const tp = !1;
function Hl(n) {
  return n[n.length - 1];
}
function iS(n) {
  return typeof n == "function";
}
function Oa(n, l) {
  return iS(n) ? n(l) : n;
}
const ep = Object.prototype.hasOwnProperty,
  ny = Object.prototype.propertyIsEnumerable;
function np(n) {
  for (const l in n) if (ep.call(n, l)) return !0;
  return !1;
}
const lS = () => Object.create(null),
  wa = (n, l) => za(n, l, lS);
function za(n, l, s = () => ({}), u = 0) {
  if (n === l) return n;
  if (u > 500) return l;
  const c = l,
    f = ly(n) && ly(c);
  if (!f && !(Rr(n) && Rr(c))) return c;
  const h = f ? n : ay(n);
  if (!h) return c;
  const y = f ? c : ay(c);
  if (!y) return c;
  const m = h.length,
    p = y.length,
    S = f ? new Array(p) : s();
  let g = 0;
  for (let _ = 0; _ < p; _++) {
    const E = f ? _ : y[_],
      x = n[E],
      z = c[E];
    if (x === z) {
      ((S[E] = x), (f ? _ < m : ep.call(n, E)) && g++);
      continue;
    }
    if (x === null || z === null || typeof x != "object" || typeof z != "object") {
      S[E] = z;
      continue;
    }
    const R = za(x, z, s, u + 1);
    ((S[E] = R), R === x && g++);
  }
  return m === p && g === m ? n : S;
}
function ay(n) {
  const l = Object.getOwnPropertyNames(n);
  for (const c of l) if (!ny.call(n, c)) return !1;
  const s = Object.getOwnPropertySymbols(n);
  if (s.length === 0) return l;
  const u = l;
  for (const c of s) {
    if (!ny.call(n, c)) return !1;
    u.push(c);
  }
  return u;
}
function Rr(n) {
  if (!iy(n)) return !1;
  const l = n.constructor;
  if (typeof l > "u") return !0;
  const s = l.prototype;
  return !(!iy(s) || !s.hasOwnProperty("isPrototypeOf"));
}
function iy(n) {
  return Object.prototype.toString.call(n) === "[object Object]";
}
function ly(n) {
  return Array.isArray(n) && n.length === Object.keys(n).length;
}
function he(n, l, s) {
  if (n === l) return !0;
  if (typeof n != typeof l) return !1;
  if (Array.isArray(n) && Array.isArray(l)) {
    if (n.length !== l.length) return !1;
    for (let u = 0, c = n.length; u < c; u++) if (!he(n[u], l[u], s)) return !1;
    return !0;
  }
  if (Rr(n) && Rr(l)) {
    const u = s?.ignoreUndefined ?? !0;
    if (s?.partial) {
      for (const h in l) if ((!u || l[h] !== void 0) && !he(n[h], l[h], s)) return !1;
      return !0;
    }
    let c = 0;
    if (!u) c = Object.keys(n).length;
    else for (const h in n) n[h] !== void 0 && c++;
    let f = 0;
    for (const h in l) if ((!u || l[h] !== void 0) && (f++, f > c || !he(n[h], l[h], s))) return !1;
    return c === f;
  }
  return !1;
}
function Na(n) {
  let l, s;
  const u = new Promise((c, f) => {
    ((l = c), (s = f));
  });
  return (
    (u.status = "pending"),
    (u.resolve = (c) => {
      ((u.status = "resolved"), (u.value = c), l(c), n?.(c));
    }),
    (u.reject = (c) => {
      ((u.status = "rejected"), s(c));
    }),
    u
  );
}
function sS(n) {
  return typeof n?.message != "string"
    ? !1
    : n.message.startsWith("Failed to fetch dynamically imported module") ||
        n.message.startsWith("error loading dynamically imported module") ||
        n.message.startsWith("Importing a module script failed");
}
function ql(n) {
  return !!(n && typeof n == "object" && typeof n.then == "function");
}
function rS(n) {
  return n.replace(/[\x00-\x1f\x7f]/g, "");
}
function sy(n) {
  let l;
  try {
    l = decodeURI(n);
  } catch {
    l = n.replaceAll(/%[0-9A-F]{2}/gi, (s) => {
      try {
        return decodeURI(s);
      } catch {
        return s;
      }
    });
  }
  return rS(l);
}
const uS = ["http:", "https:", "mailto:", "tel:"];
function Ar(n, l) {
  if (!n) return !1;
  try {
    const s = new URL(n);
    return !l.has(s.protocol);
  } catch {
    return !1;
  }
}
const oS = {
    "&": "\\u0026",
    ">": "\\u003e",
    "<": "\\u003c",
    "\u2028": "\\u2028",
    "\u2029": "\\u2029",
  },
  cS = /[&><\u2028\u2029]/g;
function fS(n) {
  return n.replace(cS, (l) => oS[l]);
}
function zl(n) {
  if (!n) return { path: n, handledProtocolRelativeURL: !1 };
  if (!/[%\\\x00-\x1f\x7f]/.test(n) && !n.startsWith("//"))
    return { path: n, handledProtocolRelativeURL: !1 };
  const l = /%25|%5C/gi;
  let s = 0,
    u = "",
    c;
  for (; (c = l.exec(n)) !== null; ) ((u += sy(n.slice(s, c.index)) + c[0]), (s = l.lastIndex));
  u = u + sy(s ? n.slice(s) : n);
  let f = !1;
  return (
    u.startsWith("//") && ((f = !0), (u = "/" + u.replace(/^\/+/, ""))),
    { path: u, handledProtocolRelativeURL: f }
  );
}
function dS(n) {
  return /\s|[^\u0000-\u007F]/.test(n) ? n.replace(/\s|[^\u0000-\u007F]/gu, encodeURIComponent) : n;
}
function hS(n, l) {
  if (n === l) return !0;
  if (n.length !== l.length) return !1;
  for (let s = 0; s < n.length; s++) if (n[s] !== l[s]) return !1;
  return !0;
}
function Te() {
  throw new Error("Invariant failed");
}
function Yl(n) {
  const l = new Map();
  let s, u;
  const c = (f) => {
    f.next &&
      (f.prev
        ? ((f.prev.next = f.next),
          (f.next.prev = f.prev),
          (f.next = void 0),
          u && ((u.next = f), (f.prev = u)))
        : ((f.next.prev = void 0),
          (s = f.next),
          (f.next = void 0),
          u && ((f.prev = u), (u.next = f))),
      (u = f));
  };
  return {
    get(f) {
      const h = l.get(f);
      if (h) return (c(h), h.value);
    },
    set(f, h) {
      if (l.size >= n && s) {
        const m = s;
        (l.delete(m.key),
          m.next && ((s = m.next), (m.next.prev = void 0)),
          m === u && (u = void 0));
      }
      const y = l.get(f);
      if (y) ((y.value = h), c(y));
      else {
        const m = { key: f, value: h, prev: u };
        (u && (u.next = m), (u = m), s || (s = m), l.set(f, m));
      }
    },
    clear() {
      (l.clear(), (s = void 0), (u = void 0));
    },
  };
}
const ia = 4,
  ap = 5;
function mS(n) {
  const l = n.indexOf("{");
  if (l === -1) return null;
  const s = n.indexOf("}", l);
  return s === -1 || l + 1 >= n.length ? null : [l, s];
}
function ip(n, l, s = new Uint16Array(6)) {
  const u = n.indexOf("/", l),
    c = u === -1 ? n.length : u,
    f = n.substring(l, c);
  if (!f || !f.includes("$"))
    return ((s[0] = 0), (s[1] = l), (s[2] = l), (s[3] = c), (s[4] = c), (s[5] = c), s);
  if (f === "$") {
    const y = n.length;
    return ((s[0] = 2), (s[1] = l), (s[2] = l), (s[3] = y), (s[4] = y), (s[5] = y), s);
  }
  if (f.charCodeAt(0) === 36)
    return ((s[0] = 1), (s[1] = l), (s[2] = l + 1), (s[3] = c), (s[4] = c), (s[5] = c), s);
  const h = mS(f);
  if (h) {
    const [y, m] = h,
      p = f.charCodeAt(y + 1);
    if (p === 45) {
      if (y + 2 < f.length && f.charCodeAt(y + 2) === 36) {
        const S = y + 3,
          g = m;
        if (S < g)
          return (
            (s[0] = 3),
            (s[1] = l + y),
            (s[2] = l + S),
            (s[3] = l + g),
            (s[4] = l + m + 1),
            (s[5] = c),
            s
          );
      }
    } else if (p === 36) {
      const S = y + 1,
        g = y + 2;
      return g === m
        ? ((s[0] = 2),
          (s[1] = l + y),
          (s[2] = l + S),
          (s[3] = l + g),
          (s[4] = l + m + 1),
          (s[5] = n.length),
          s)
        : ((s[0] = 1),
          (s[1] = l + y),
          (s[2] = l + g),
          (s[3] = l + m),
          (s[4] = l + m + 1),
          (s[5] = c),
          s);
    }
  }
  return ((s[0] = 0), (s[1] = l), (s[2] = l), (s[3] = c), (s[4] = c), (s[5] = c), s);
}
function Lr(n, l, s, u, c, f, h) {
  h?.(s);
  let y = u;
  {
    const m = s.fullPath ?? s.from,
      p = m.length,
      S = s.options?.caseSensitive ?? n,
      g = s.options?.params?.parse ?? s.options?.parseParams;
    for (; y < p; ) {
      const E = ip(m, y, l);
      let x;
      const z = y,
        R = E[5];
      switch (((y = R + 1), f++, E[0])) {
        case 0: {
          const w = m.substring(E[2], E[3]);
          if (S) {
            const N = c.static?.get(w);
            if (N) x = N;
            else {
              c.static ??= new Map();
              const X = Da(s.fullPath ?? s.from);
              ((X.parent = c), (X.depth = f), (x = X), c.static.set(w, X));
            }
          } else {
            const N = w.toLowerCase(),
              X = c.staticInsensitive?.get(N);
            if (X) x = X;
            else {
              c.staticInsensitive ??= new Map();
              const Y = Da(s.fullPath ?? s.from);
              ((Y.parent = c), (Y.depth = f), (x = Y), c.staticInsensitive.set(N, Y));
            }
          }
          break;
        }
        case 1: {
          const w = m.substring(z, E[1]),
            N = m.substring(E[4], R),
            X = S && !!(w || N),
            Y = w ? (X ? w : w.toLowerCase()) : void 0,
            K = N ? (X ? N : N.toLowerCase()) : void 0,
            J =
              !g &&
              c.dynamic?.find(
                (q) => !q.parse && q.caseSensitive === X && q.prefix === Y && q.suffix === K,
              );
          if (J) x = J;
          else {
            const q = uc(1, s.fullPath ?? s.from, X, Y, K);
            ((x = q), (q.depth = f), (q.parent = c), (c.dynamic ??= []), c.dynamic.push(q));
          }
          break;
        }
        case 3: {
          const w = m.substring(z, E[1]),
            N = m.substring(E[4], R),
            X = S && !!(w || N),
            Y = w ? (X ? w : w.toLowerCase()) : void 0,
            K = N ? (X ? N : N.toLowerCase()) : void 0,
            J =
              !g &&
              c.optional?.find(
                (q) => !q.parse && q.caseSensitive === X && q.prefix === Y && q.suffix === K,
              );
          if (J) x = J;
          else {
            const q = uc(3, s.fullPath ?? s.from, X, Y, K);
            ((x = q), (q.parent = c), (q.depth = f), (c.optional ??= []), c.optional.push(q));
          }
          break;
        }
        case 2: {
          const w = m.substring(z, E[1]),
            N = m.substring(E[4], R),
            X = S && !!(w || N),
            Y = w ? (X ? w : w.toLowerCase()) : void 0,
            K = N ? (X ? N : N.toLowerCase()) : void 0,
            J = uc(2, s.fullPath ?? s.from, X, Y, K);
          ((x = J), (J.parent = c), (J.depth = f), (c.wildcard ??= []), c.wildcard.push(J));
        }
      }
      c = x;
    }
    if (g && s.children && !s.isRoot && s.id && s.id.charCodeAt(s.id.lastIndexOf("/") + 1) === 95) {
      const E = Da(s.fullPath ?? s.from);
      ((E.kind = ap),
        (E.parent = c),
        f++,
        (E.depth = f),
        (c.pathless ??= []),
        c.pathless.push(E),
        (c = E));
    }
    const _ = (s.path || !s.children) && !s.isRoot;
    if (_ && m.endsWith("/")) {
      const E = Da(s.fullPath ?? s.from);
      ((E.kind = ia), (E.parent = c), f++, (E.depth = f), (c.index = E), (c = E));
    }
    ((c.parse = g ?? null),
      (c.priority = s.options?.params?.priority ?? 0),
      _ && !c.route && ((c.route = s), (c.fullPath = s.fullPath ?? s.from)));
  }
  if (s.children) for (const m of s.children) Lr(n, l, m, y, c, f, h);
}
function rc(n, l) {
  if (n.parse && !l.parse) return -1;
  if (!n.parse && l.parse) return 1;
  if (n.parse && l.parse && (n.priority || l.priority)) return l.priority - n.priority;
  if (n.prefix && l.prefix && n.prefix !== l.prefix) {
    if (n.prefix.startsWith(l.prefix)) return -1;
    if (l.prefix.startsWith(n.prefix)) return 1;
  }
  if (n.suffix && l.suffix && n.suffix !== l.suffix) {
    if (n.suffix.endsWith(l.suffix)) return -1;
    if (l.suffix.endsWith(n.suffix)) return 1;
  }
  return n.prefix && !l.prefix
    ? -1
    : !n.prefix && l.prefix
      ? 1
      : n.suffix && !l.suffix
        ? -1
        : !n.suffix && l.suffix
          ? 1
          : n.caseSensitive && !l.caseSensitive
            ? -1
            : !n.caseSensitive && l.caseSensitive
              ? 1
              : 0;
}
function na(n) {
  if (n.pathless) for (const l of n.pathless) na(l);
  if (n.static) for (const l of n.static.values()) na(l);
  if (n.staticInsensitive) for (const l of n.staticInsensitive.values()) na(l);
  if (n.dynamic?.length) {
    n.dynamic.sort(rc);
    for (const l of n.dynamic) na(l);
  }
  if (n.optional?.length) {
    n.optional.sort(rc);
    for (const l of n.optional) na(l);
  }
  if (n.wildcard?.length) {
    n.wildcard.sort(rc);
    for (const l of n.wildcard) na(l);
  }
}
function Da(n) {
  return {
    kind: 0,
    depth: 0,
    pathless: null,
    index: null,
    static: null,
    staticInsensitive: null,
    dynamic: null,
    optional: null,
    wildcard: null,
    route: null,
    fullPath: n,
    parent: null,
    parse: null,
    priority: 0,
  };
}
function uc(n, l, s, u, c) {
  return {
    kind: n,
    depth: 0,
    pathless: null,
    index: null,
    static: null,
    staticInsensitive: null,
    dynamic: null,
    optional: null,
    wildcard: null,
    route: null,
    fullPath: l,
    parent: null,
    parse: null,
    priority: 0,
    caseSensitive: s,
    prefix: u,
    suffix: c,
  };
}
function yS(n, l) {
  const s = Da("/"),
    u = new Uint16Array(6);
  for (const c of n) Lr(!1, u, c, 1, s, 0);
  (na(s), (l.masksTree = s), (l.flatCache = Yl(1e3)));
}
function pS(n, l) {
  n ||= "/";
  const s = l.flatCache.get(n);
  if (s) return s;
  const u = Bc(n, l.masksTree);
  return (l.flatCache.set(n, u), u);
}
function gS(n, l, s, u, c) {
  ((n ||= "/"), (u ||= "/"));
  const f = l ? `case\0${n}` : n;
  let h = c.singleCache.get(f);
  return (
    h || ((h = Da("/")), Lr(l, new Uint16Array(6), { from: n }, 1, h, 0), c.singleCache.set(f, h)),
    Bc(u, h, s)
  );
}
function vS(n, l, s = !1) {
  const u = s ? n : `nofuzz\0${n}`,
    c = l.matchCache.get(u);
  if (c !== void 0) return c;
  n ||= "/";
  let f;
  try {
    f = Bc(n, l.segmentTree, s);
  } catch (h) {
    if (h instanceof URIError) f = null;
    else throw h;
  }
  return (f && (f.branch = sp(f.route)), l.matchCache.set(u, f), f);
}
function SS(n) {
  return n === "/" ? n : n.replace(/\/{1,}$/, "");
}
function bS(n, l = !1, s) {
  const u = Da(n.fullPath),
    c = new Uint16Array(6),
    f = {},
    h = {};
  let y = 0;
  return (
    Lr(l, c, n, 1, u, 0, (m) => {
      if ((s?.(m, y), m.id in f && Te(), (f[m.id] = m), y !== 0 && m.path)) {
        const p = SS(m.fullPath);
        (!h[p] || m.fullPath.endsWith("/")) && (h[p] = m);
      }
      y++;
    }),
    na(u),
    {
      processedTree: {
        segmentTree: u,
        singleCache: Yl(1e3),
        matchCache: Yl(1e3),
        flatCache: null,
        masksTree: null,
      },
      routesById: f,
      routesByPath: h,
    }
  );
}
function Bc(n, l, s = !1) {
  const u = n.split("/"),
    c = ES(n, u, l, s);
  if (!c) return null;
  const [f] = lp(n, u, c);
  return { route: c.node.route, rawParams: f };
}
function lp(n, l, s) {
  const u = _S(s.node);
  let c = null;
  const f = Object.create(null);
  let h = s.extract?.part ?? 0,
    y = s.extract?.node ?? 0,
    m = s.extract?.path ?? 0,
    p = s.extract?.segment ?? 0;
  for (; y < u.length; h++, y++, m++, p++) {
    const S = u[y];
    if (S.kind === ia) break;
    if (S.kind === ap) {
      (p--, h--, m--);
      continue;
    }
    const g = l[h],
      _ = m;
    if ((g && (m += g.length), S.kind === 1)) {
      c ??= s.node.fullPath.split("/");
      const E = c[p],
        x = S.prefix?.length ?? 0;
      if (E.charCodeAt(x) === 123) {
        const z = S.suffix?.length ?? 0,
          R = E.substring(x + 2, E.length - z - 1),
          w = g.substring(x, g.length - z);
        f[R] = decodeURIComponent(w);
      } else {
        const z = E.substring(1);
        f[z] = decodeURIComponent(g);
      }
    } else if (S.kind === 3) {
      if (s.skipped & (1 << y)) {
        (h--, (m = _ - 1));
        continue;
      }
      c ??= s.node.fullPath.split("/");
      const E = c[p],
        x = S.prefix?.length ?? 0,
        z = S.suffix?.length ?? 0,
        R = E.substring(x + 3, E.length - z - 1),
        w = S.suffix || S.prefix ? g.substring(x, g.length - z) : g;
      w && (f[R] = decodeURIComponent(w));
    } else if (S.kind === 2) {
      const E = S,
        x = n.substring(_ + (E.prefix?.length ?? 0), n.length - (E.suffix?.length ?? 0)),
        z = decodeURIComponent(x);
      ((f["*"] = z), (f._splat = z));
      break;
    }
  }
  return (
    s.rawParams && Object.assign(f, s.rawParams),
    [f, { part: h, node: y, path: m, segment: p }]
  );
}
function sp(n) {
  const l = [n];
  for (; n.parentRoute; ) ((n = n.parentRoute), l.push(n));
  return (l.reverse(), l);
}
function _S(n) {
  const l = Array(n.depth + 1);
  do ((l[n.depth] = n), (n = n.parent));
  while (n);
  return l;
}
function ES(n, l, s, u) {
  if (n === "/" && s.index) return { node: s.index, skipped: 0 };
  const c = !Hl(l),
    f = c && n !== "/",
    h = l.length - (c ? 1 : 0),
    y = [{ node: s, index: 1, skipped: 0, depth: 1, statics: 0, dynamics: 0, optionals: 0 }];
  let m = null,
    p = null;
  for (; y.length; ) {
    const S = y.pop(),
      { node: g, index: _, skipped: E, depth: x, statics: z, dynamics: R, optionals: w } = S;
    let { extract: N, rawParams: X } = S;
    if (g.kind === 2 && g.route && !hr(p, S)) continue;
    if (g.parse) {
      if (!ry(n, l, S)) continue;
      ((X = S.rawParams), (N = S.extract));
    }
    u && g.route && g.kind !== ia && hr(m, S) && (m = S);
    const Y = _ === h;
    if (
      Y &&
      (g.route && (!f || g.kind === ia || g.kind === 2) && hr(p, S) && (p = S),
      !g.optional && !g.wildcard && !g.index && !g.pathless)
    )
      continue;
    const K = Y ? void 0 : l[_];
    let J;
    if (Y && g.index) {
      const q = {
        node: g.index,
        index: _,
        skipped: E,
        depth: x + 1,
        statics: z,
        dynamics: R,
        optionals: w,
        extract: N,
        rawParams: X,
      };
      let Z = !0;
      if ((g.index.parse && (ry(n, l, q) || (Z = !1)), Z)) {
        if (!R && !w && !E && TS(z, h)) return q;
        hr(p, q) && (p = q);
      }
    }
    if (g.wildcard)
      for (let q = g.wildcard.length - 1; q >= 0; q--) {
        const Z = g.wildcard[q],
          { prefix: I, suffix: ut } = Z;
        if (!(I && (Y || !(Z.caseSensitive ? K : (J ??= K.toLowerCase())).startsWith(I)))) {
          if (ut) {
            if (Y) continue;
            const st = l.slice(_).join("/").slice(-ut.length);
            if ((Z.caseSensitive ? st : st.toLowerCase()) !== ut) continue;
          }
          y.push({
            node: Z,
            index: h,
            skipped: E,
            depth: x + 1,
            statics: z,
            dynamics: R,
            optionals: w,
            extract: N,
            rawParams: X,
          });
        }
      }
    if (g.optional) {
      const q = E | (1 << x),
        Z = x + 1;
      for (let I = g.optional.length - 1; I >= 0; I--) {
        const ut = g.optional[I];
        y.push({
          node: ut,
          index: _,
          skipped: q,
          depth: Z,
          statics: z,
          dynamics: R,
          optionals: w,
          extract: N,
          rawParams: X,
        });
      }
      if (!Y)
        for (let I = g.optional.length - 1; I >= 0; I--) {
          const ut = g.optional[I],
            { prefix: st, suffix: lt } = ut;
          if (st || lt) {
            const Wt = ut.caseSensitive ? K : (J ??= K.toLowerCase());
            if ((st && !Wt.startsWith(st)) || (lt && !Wt.endsWith(lt))) continue;
          }
          y.push({
            node: ut,
            index: _ + 1,
            skipped: E,
            depth: Z,
            statics: z,
            dynamics: R,
            optionals: w + dr(h, _),
            extract: N,
            rawParams: X,
          });
        }
    }
    if (!Y && g.dynamic && K)
      for (let q = g.dynamic.length - 1; q >= 0; q--) {
        const Z = g.dynamic[q],
          { prefix: I, suffix: ut } = Z;
        if (I || ut) {
          const st = Z.caseSensitive ? K : (J ??= K.toLowerCase());
          if ((I && !st.startsWith(I)) || (ut && !st.endsWith(ut))) continue;
        }
        y.push({
          node: Z,
          index: _ + 1,
          skipped: E,
          depth: x + 1,
          statics: z,
          dynamics: R + dr(h, _),
          optionals: w,
          extract: N,
          rawParams: X,
        });
      }
    if (!Y && g.staticInsensitive) {
      const q = g.staticInsensitive.get((J ??= K.toLowerCase()));
      q &&
        y.push({
          node: q,
          index: _ + 1,
          skipped: E,
          depth: x + 1,
          statics: z + dr(h, _),
          dynamics: R,
          optionals: w,
          extract: N,
          rawParams: X,
        });
    }
    if (!Y && g.static) {
      const q = g.static.get(K);
      q &&
        y.push({
          node: q,
          index: _ + 1,
          skipped: E,
          depth: x + 1,
          statics: z + dr(h, _),
          dynamics: R,
          optionals: w,
          extract: N,
          rawParams: X,
        });
    }
    if (g.pathless) {
      const q = x + 1;
      for (let Z = g.pathless.length - 1; Z >= 0; Z--) {
        const I = g.pathless[Z];
        y.push({
          node: I,
          index: _,
          skipped: E,
          depth: q,
          statics: z,
          dynamics: R,
          optionals: w,
          extract: N,
          rawParams: X,
        });
      }
    }
  }
  if (p) return p;
  if (u && m) {
    let S = m.index;
    for (let _ = 0; _ < m.index; _++) S += l[_].length;
    const g = S === n.length ? "/" : n.slice(S);
    return ((m.rawParams ??= Object.create(null)), (m.rawParams["**"] = decodeURIComponent(g)), m);
  }
  return null;
}
function dr(n, l) {
  return 2 ** (n - l - 1);
}
function TS(n, l) {
  return n === 2 ** (l - 1) - 1;
}
function ry(n, l, s) {
  let u, c;
  try {
    [u, c] = lp(n, l, s);
  } catch {
    return null;
  }
  if (((s.rawParams = u), (s.extract = c), !s.node.parse)) return !0;
  try {
    if (s.node.parse(u) === !1) return null;
  } catch {}
  return !0;
}
function hr(n, l) {
  return n
    ? l.statics > n.statics ||
        (l.statics === n.statics &&
          (l.dynamics > n.dynamics ||
            (l.dynamics === n.dynamics &&
              (l.optionals > n.optionals ||
                (l.optionals === n.optionals &&
                  ((l.node.kind === ia) > (n.node.kind === ia) ||
                    ((l.node.kind === ia) == (n.node.kind === ia) && l.depth > n.depth)))))))
    : !0;
}
function Sr(n) {
  return Nc(n.filter((l) => l !== void 0).join("/"));
}
function Nc(n) {
  return n.replace(/\/{2,}/g, "/");
}
function rp(n) {
  return n === "/" ? n : n.replace(/^\/{1,}/, "");
}
function An(n) {
  const l = n.length;
  return l > 1 && n[l - 1] === "/" ? n.replace(/\/{1,}$/, "") : n;
}
function up(n) {
  return An(rp(n));
}
function xr(n, l) {
  return n?.endsWith("/") && n !== "/" && n !== `${l}/` ? n.slice(0, -1) : n;
}
function RS(n, l, s) {
  return xr(n, s) === xr(l, s);
}
function AS({ base: n, to: l, trailingSlash: s = "never", cache: u }) {
  const c = l.startsWith("/"),
    f = !c && l === ".";
  let h;
  if (u) {
    h = c ? l : f ? n : n + "\0" + l;
    const p = u.get(h);
    if (p) return p;
  }
  let y;
  if (f) y = n.split("/");
  else if (c) y = l.split("/");
  else {
    for (y = n.split("/"); y.length > 1 && Hl(y) === ""; ) y.pop();
    const p = l.split("/");
    for (let S = 0, g = p.length; S < g; S++) {
      const _ = p[S];
      _ === ""
        ? S
          ? S === g - 1 && y.push(_)
          : (y = [_])
        : _ === ".."
          ? y.pop()
          : _ === "." || y.push(_);
    }
  }
  y.length > 1 && (Hl(y) === "" ? s === "never" && y.pop() : s === "always" && y.push(""));
  const m = Nc(y.join("/")) || "/";
  return (h && u && u.set(h, m), m);
}
function xS(n) {
  const l = new Map(n.map((c) => [encodeURIComponent(c), c])),
    s = Array.from(l.keys())
      .map((c) => c.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
      .join("|"),
    u = new RegExp(s, "g");
  return (c) => c.replace(u, (f) => l.get(f) ?? f);
}
function oc(n, l, s) {
  const u = l[n];
  return typeof u != "string"
    ? u
    : n === "_splat"
      ? /^[a-zA-Z0-9\-._~!/]*$/.test(u)
        ? u
        : u
            .split("/")
            .map((c) => oy(c, s))
            .join("/")
      : oy(u, s);
}
function uy({ path: n, params: l, decoder: s, ...u }) {
  let c = !1;
  const f = Object.create(null);
  if (!n || n === "/") return { interpolatedPath: "/", usedParams: f, isMissingParams: c };
  if (!n.includes("$")) return { interpolatedPath: n, usedParams: f, isMissingParams: c };
  const h = n.length;
  let y = 0,
    m,
    p = "";
  for (; y < h; ) {
    const S = y;
    m = ip(n, S, m);
    const g = m[5];
    if (((y = g + 1), S === g)) continue;
    const _ = m[0];
    if (_ === 0) {
      p += "/" + n.substring(S, g);
      continue;
    }
    if (_ === 2) {
      const E = l._splat;
      ((f._splat = E), (f["*"] = E));
      const x = n.substring(S, m[1]),
        z = n.substring(m[4], g);
      if (!E) {
        ((c = !0), (x || z) && (p += "/" + x + z));
        continue;
      }
      const R = oc("_splat", l, s);
      p += "/" + x + R + z;
      continue;
    }
    if (_ === 1) {
      const E = n.substring(m[2], m[3]);
      (!c && !(E in l) && (c = !0), (f[E] = l[E]));
      const x = n.substring(S, m[1]),
        z = n.substring(m[4], g),
        R = oc(E, l, s) ?? "undefined";
      p += "/" + x + R + z;
      continue;
    }
    if (_ === 3) {
      const E = n.substring(m[2], m[3]),
        x = l[E];
      if (x == null) continue;
      f[E] = x;
      const z = n.substring(S, m[1]),
        R = n.substring(m[4], g),
        w = oc(E, l, s) ?? "";
      p += "/" + z + w + R;
      continue;
    }
  }
  return (
    n.endsWith("/") && (p += "/"),
    { usedParams: f, interpolatedPath: p || "/", isMissingParams: c }
  );
}
function oy(n, l) {
  const s = encodeURIComponent(n);
  return l?.(s) ?? s;
}
function ne(n) {
  return n?.isNotFound === !0;
}
function op(n, l = String) {
  const s = new URLSearchParams();
  for (const u in n) {
    const c = n[u];
    c !== void 0 && s.set(u, l(c));
  }
  return s.toString();
}
function cc(n) {
  return n ? (n === "false" ? !1 : n === "true" ? !0 : +n * 0 === 0 && +n + "" === n ? +n : n) : "";
}
function MS(n) {
  const l = new URLSearchParams(n),
    s = Object.create(null);
  for (const [u, c] of l.entries()) {
    const f = s[u];
    f == null ? (s[u] = cc(c)) : Array.isArray(f) ? f.push(cc(c)) : (s[u] = [f, cc(c)]);
  }
  return s;
}
const CS = OS(JSON.parse),
  wS = zS(JSON.stringify, JSON.parse);
function OS(n) {
  return (l) => {
    l[0] === "?" && (l = l.substring(1));
    const s = MS(l);
    for (const u in s) {
      const c = s[u];
      if (typeof c == "string")
        try {
          s[u] = n(c);
        } catch {}
    }
    return s;
  };
}
function zS(n, l) {
  const s = typeof l == "function";
  function u(c) {
    if (typeof c == "object" && c !== null)
      try {
        return n(c);
      } catch {}
    else if (s && typeof c == "string")
      try {
        return (l(c), n(c));
      } catch {}
    return c;
  }
  return (c) => {
    const f = op(c, u);
    return f ? `?${f}` : "";
  };
}
const Ua = "__root__";
function cp(n) {
  if (
    ((n.statusCode = n.statusCode || n.code || 307),
    !n._builtLocation && !n.reloadDocument && typeof n.href == "string")
  )
    try {
      (new URL(n.href), (n.reloadDocument = !0));
    } catch {}
  const l = new Headers(n.headers);
  n.href && l.get("Location") === null && l.set("Location", n.href);
  const s = new Response(null, { status: n.statusCode, headers: l });
  if (((s.options = n), n.throw)) throw s;
  return s;
}
function Ee(n) {
  return n instanceof Response && !!n.options;
}
function DS(n) {
  if (n !== null && typeof n == "object" && n.isSerializedRedirect) return cp(n);
}
const Ec = (n) => {
    if (!n.rendered) return ((n.rendered = !0), n.onReady?.());
  },
  LS = (n) =>
    n.stores.matchesId.get().some((l) => n.stores.matchStores.get(l)?.get()._forcePending),
  Ur = (n, l) => !!(n.preload && !n.router.stores.matchStores.has(l)),
  Ba = (n, l, s = !0) => {
    const u = { ...(n.router.options.context ?? {}) },
      c = s ? l : l - 1;
    for (let f = 0; f <= c; f++) {
      const h = n.matches[f];
      if (!h) continue;
      const y = n.router.getMatch(h.id);
      y && Object.assign(u, y.__routeContext, y.__beforeLoadContext);
    }
    return u;
  },
  cy = (n, l) => {
    if (!n.matches.length) return;
    const s = l.routeId,
      u = n.matches.findIndex((h) => h.routeId === n.router.routeTree.id),
      c = u >= 0 ? u : 0;
    let f = s
      ? n.matches.findIndex((h) => h.routeId === s)
      : (n.firstBadMatchIndex ?? n.matches.length - 1);
    f < 0 && (f = c);
    for (let h = f; h >= 0; h--) {
      const y = n.matches[h];
      if (n.router.looseRoutesById[y.routeId].options.notFoundComponent) return h;
    }
    return s ? f : c;
  },
  la = (n, l, s) => {
    if (!(!Ee(s) && !ne(s)))
      throw (
        (Ee(s) && s.redirectHandled && !s.options.reloadDocument) ||
          (l &&
            (l._nonReactive.beforeLoadPromise?.resolve(),
            l._nonReactive.loaderPromise?.resolve(),
            (l._nonReactive.beforeLoadPromise = void 0),
            (l._nonReactive.loaderPromise = void 0),
            (l._nonReactive.error = s),
            n.updateMatch(l.id, (u) => ({
              ...u,
              status: Ee(s)
                ? "redirected"
                : ne(s)
                  ? "notFound"
                  : u.status === "pending"
                    ? "success"
                    : u.status,
              context: Ba(n, l.index),
              isFetching: !1,
              error: s,
            })),
            ne(s) && !s.routeId && (s.routeId = l.routeId),
            l._nonReactive.loadPromise?.resolve()),
          Ee(s) &&
            ((n.rendered = !0),
            (s.options._fromLocation = n.location),
            (s.redirectHandled = !0),
            (s = n.router.resolveRedirect(s)))),
        s
      );
  },
  fp = (n, l) => {
    const s = n.router.getMatch(l);
    return !!(!s || s._nonReactive.dehydrated);
  },
  fy = (n, l, s) => {
    const u = Ba(n, s);
    n.updateMatch(l, (c) => ({ ...c, context: u }));
  },
  Dl = (n, l, s, u) => {
    const { id: c, routeId: f } = n.matches[l],
      h = n.router.looseRoutesById[f];
    if (s instanceof Promise) throw s;
    ((s.routerCode = u), (n.firstBadMatchIndex ??= l), la(n, n.router.getMatch(c), s));
    try {
      h.options.onError?.(s);
    } catch (y) {
      ((s = y), la(n, n.router.getMatch(c), s));
    }
    (n.updateMatch(
      c,
      (y) => (
        y._nonReactive.beforeLoadPromise?.resolve(),
        (y._nonReactive.beforeLoadPromise = void 0),
        y._nonReactive.loadPromise?.resolve(),
        {
          ...y,
          error: s,
          status: "error",
          isFetching: !1,
          updatedAt: Date.now(),
          abortController: new AbortController(),
        }
      ),
    ),
      !n.preload && !Ee(s) && !ne(s) && (n.serialError ??= s));
  },
  dp = (n, l, s, u) => {
    if (u._nonReactive.pendingTimeout !== void 0) return;
    const c = s.options.pendingMs ?? n.router.options.defaultPendingMs;
    if (
      n.onReady &&
      !Ur(n, l) &&
      (s.options.loader || s.options.beforeLoad || mp(s)) &&
      typeof c == "number" &&
      c !== 1 / 0 &&
      (s.options.pendingComponent ?? n.router.options?.defaultPendingComponent)
    ) {
      const f = setTimeout(() => {
        Ec(n);
      }, c);
      u._nonReactive.pendingTimeout = f;
    }
  },
  US = (n, l, s) => {
    const u = n.router.getMatch(l);
    if (!u._nonReactive.beforeLoadPromise && !u._nonReactive.loaderPromise) return;
    dp(n, l, s, u);
    const c = () => {
      const f = n.router.getMatch(l);
      f.preload && (f.status === "redirected" || f.status === "notFound") && la(n, f, f.error);
    };
    return u._nonReactive.beforeLoadPromise ? u._nonReactive.beforeLoadPromise.then(c) : c();
  },
  BS = (n, l, s, u) => {
    const c = n.router.getMatch(l);
    let f = c._nonReactive.loadPromise;
    c._nonReactive.loadPromise = Na(() => {
      (f?.resolve(), (f = void 0));
    });
    const { paramsError: h, searchError: y } = c;
    (h && Dl(n, s, h, "PARSE_PARAMS"), y && Dl(n, s, y, "VALIDATE_SEARCH"), dp(n, l, u, c));
    const m = new AbortController();
    let p = !1;
    const S = () => {
        p ||
          ((p = !0),
          n.updateMatch(l, (Y) => ({
            ...Y,
            isFetching: "beforeLoad",
            fetchCount: Y.fetchCount + 1,
            abortController: m,
          })));
      },
      g = () => {
        (c._nonReactive.beforeLoadPromise?.resolve(),
          (c._nonReactive.beforeLoadPromise = void 0),
          n.updateMatch(l, (Y) => ({ ...Y, isFetching: !1 })));
      };
    if (!u.options.beforeLoad) {
      n.router.batch(() => {
        (S(), g());
      });
      return;
    }
    c._nonReactive.beforeLoadPromise = Na();
    const _ = { ...Ba(n, s, !1), ...c.__routeContext },
      { search: E, params: x, cause: z } = c,
      R = Ur(n, l),
      w = {
        search: E,
        abortController: m,
        params: x,
        preload: R,
        context: _,
        location: n.location,
        navigate: (Y) => n.router.navigate({ ...Y, _fromLocation: n.location }),
        buildLocation: n.router.buildLocation,
        cause: R ? "preload" : z,
        matches: n.matches,
        routeId: u.id,
        ...n.router.options.additionalContext,
      },
      N = (Y) => {
        if (Y === void 0) {
          n.router.batch(() => {
            (S(), g());
          });
          return;
        }
        ((Ee(Y) || ne(Y)) && (S(), Dl(n, s, Y, "BEFORE_LOAD")),
          n.router.batch(() => {
            (S(), n.updateMatch(l, (K) => ({ ...K, __beforeLoadContext: Y })), g());
          }));
      };
    let X;
    try {
      if (((X = u.options.beforeLoad(w)), ql(X)))
        return (
          S(),
          X.catch((Y) => {
            Dl(n, s, Y, "BEFORE_LOAD");
          }).then(N)
        );
    } catch (Y) {
      (S(), Dl(n, s, Y, "BEFORE_LOAD"));
    }
    N(X);
  },
  NS = (n, l) => {
    const { id: s, routeId: u } = n.matches[l],
      c = n.router.looseRoutesById[u],
      f = () => y(),
      h = () => BS(n, s, l, c),
      y = () => {
        if (fp(n, s)) return;
        const m = US(n, s, c);
        return ql(m) ? m.then(h) : h();
      };
    return f();
  },
  jS = (n, l, s) => {
    const u = n.router.getMatch(l);
    if (!u || (!s.options.head && !s.options.scripts && !s.options.headers)) return;
    const c = {
      ssr: n.router.options.ssr,
      matches: n.matches,
      match: u,
      params: u.params,
      loaderData: u.loaderData,
    };
    return Promise.all([s.options.head?.(c), s.options.scripts?.(c), s.options.headers?.(c)]).then(
      ([f, h, y]) => ({
        meta: f?.meta,
        links: f?.links,
        headScripts: f?.scripts,
        headers: y,
        scripts: h,
        styles: f?.styles,
      }),
    );
  },
  hp = (n, l, s, u, c) => {
    const f = l[u - 1],
      { params: h, loaderDeps: y, abortController: m, cause: p } = n.router.getMatch(s),
      S = Ba(n, u),
      g = Ur(n, s);
    return {
      params: h,
      deps: y,
      preload: !!g,
      parentMatchPromise: f,
      abortController: m,
      context: S,
      location: n.location,
      navigate: (_) => n.router.navigate({ ..._, _fromLocation: n.location }),
      cause: g ? "preload" : p,
      route: c,
      ...n.router.options.additionalContext,
    };
  },
  dy = async (n, l, s, u, c) => {
    try {
      const f = n.router.getMatch(s);
      try {
        (!(tp ?? n.router.isServer) || f.ssr === !0) && Gl(c);
        const h = c.options.loader,
          y = typeof h == "function" ? h : h?.handler,
          m = y?.(hp(n, l, s, u, c)),
          p = !!y && ql(m);
        if (
          ((p ||
            c._lazyPromise ||
            c._componentsPromise ||
            c.options.head ||
            c.options.scripts ||
            c.options.headers ||
            f._nonReactive.minPendingPromise) &&
            n.updateMatch(s, (g) => ({ ...g, isFetching: "loader" })),
          y)
        ) {
          const g = p ? await m : m;
          (la(n, n.router.getMatch(s), g),
            g !== void 0 && n.updateMatch(s, (_) => ({ ..._, loaderData: g })));
        }
        c._lazyPromise && (await c._lazyPromise);
        const S = f._nonReactive.minPendingPromise;
        (S && (await S),
          c._componentsPromise && (await c._componentsPromise),
          n.updateMatch(s, (g) => ({
            ...g,
            error: void 0,
            context: Ba(n, u),
            status: "success",
            isFetching: !1,
            updatedAt: Date.now(),
          })));
      } catch (h) {
        let y = h;
        if (y?.name === "AbortError") {
          if (f.abortController.signal.aborted) {
            (f._nonReactive.loaderPromise?.resolve(), (f._nonReactive.loaderPromise = void 0));
            return;
          }
          n.updateMatch(s, (p) => ({
            ...p,
            status: p.status === "pending" ? "success" : p.status,
            isFetching: !1,
            context: Ba(n, u),
          }));
          return;
        }
        const m = f._nonReactive.minPendingPromise;
        (m && (await m),
          ne(h) && (await c.options.notFoundComponent?.preload?.()),
          la(n, n.router.getMatch(s), h));
        try {
          c.options.onError?.(h);
        } catch (p) {
          ((y = p), la(n, n.router.getMatch(s), p));
        }
        (!Ee(y) && !ne(y) && (await Gl(c, ["errorComponent"])),
          n.updateMatch(s, (p) => ({
            ...p,
            error: y,
            context: Ba(n, u),
            status: "error",
            isFetching: !1,
          })));
      }
    } catch (f) {
      const h = n.router.getMatch(s);
      (h && (h._nonReactive.loaderPromise = void 0), la(n, h, f));
    }
  },
  HS = async (n, l, s) => {
    async function u(E, x, z, R, w) {
      const N = Date.now() - x.updatedAt,
        X = E
          ? (w.options.preloadStaleTime ?? n.router.options.defaultPreloadStaleTime ?? 3e4)
          : (w.options.staleTime ?? n.router.options.defaultStaleTime ?? 0),
        Y = w.options.shouldReload,
        K = typeof Y == "function" ? Y(hp(n, l, c, s, w)) : Y,
        { status: J, invalid: q } = R,
        Z = N >= X && (!!n.forceStaleReload || R.cause === "enter" || (z !== void 0 && z !== R.id));
      ((h = J === "success" && (q || (K ?? Z))),
        (E && w.options.preload === !1) ||
          (h && !n.sync && S
            ? ((y = !0),
              (async () => {
                try {
                  await dy(n, l, c, s, w);
                  const I = n.router.getMatch(c);
                  (I._nonReactive.loaderPromise?.resolve(),
                    I._nonReactive.loadPromise?.resolve(),
                    (I._nonReactive.loaderPromise = void 0),
                    (I._nonReactive.loadPromise = void 0));
                } catch (I) {
                  Ee(I) && (await n.router.navigate(I.options));
                }
              })())
            : J !== "success" || h
              ? await dy(n, l, c, s, w)
              : fy(n, c, s)));
    }
    const { id: c, routeId: f } = n.matches[s];
    let h = !1,
      y = !1;
    const m = n.router.looseRoutesById[f],
      p = m.options.loader,
      S =
        ((typeof p == "function" ? void 0 : p?.staleReloadMode) ??
          n.router.options.defaultStaleReloadMode) !== "blocking";
    if (fp(n, c)) {
      if (!n.router.getMatch(c)) return n.matches[s];
      fy(n, c, s);
    } else {
      const E = n.router.getMatch(c),
        x = n.router.stores.matchesId.get()[s],
        z =
          ((x && n.router.stores.matchStores.get(x)) || null)?.routeId === f
            ? x
            : n.router.stores.matches.get().find((w) => w.routeId === f)?.id,
        R = Ur(n, c);
      if (E._nonReactive.loaderPromise) {
        if (E.status === "success" && !n.sync && !E.preload && S) return E;
        await E._nonReactive.loaderPromise;
        const w = n.router.getMatch(c),
          N = w._nonReactive.error || w.error;
        (N && la(n, w, N), w.status === "pending" && (await u(R, E, z, w, m)));
      } else {
        const w = R && !n.router.stores.matchStores.has(c),
          N = n.router.getMatch(c);
        ((N._nonReactive.loaderPromise = Na()),
          w !== N.preload && n.updateMatch(c, (X) => ({ ...X, preload: w })),
          await u(R, E, z, N, m));
      }
    }
    const g = n.router.getMatch(c);
    (y ||
      (g._nonReactive.loaderPromise?.resolve(),
      g._nonReactive.loadPromise?.resolve(),
      (g._nonReactive.loadPromise = void 0)),
      clearTimeout(g._nonReactive.pendingTimeout),
      (g._nonReactive.pendingTimeout = void 0),
      y || (g._nonReactive.loaderPromise = void 0),
      (g._nonReactive.dehydrated = void 0));
    const _ = y ? g.isFetching : !1;
    return _ !== g.isFetching || g.invalid !== !1
      ? (n.updateMatch(c, (E) => ({ ...E, isFetching: _, invalid: !1 })), n.router.getMatch(c))
      : g;
  };
async function hy(n) {
  const l = n,
    s = [];
  LS(l.router) && Ec(l);
  let u;
  for (let _ = 0; _ < l.matches.length; _++) {
    try {
      const E = NS(l, _);
      ql(E) && (await E);
    } catch (E) {
      if (Ee(E)) throw E;
      if (ne(E)) u = E;
      else if (!l.preload) throw E;
      break;
    }
    if (l.serialError || l.firstBadMatchIndex != null) break;
  }
  const c = l.firstBadMatchIndex ?? l.matches.length,
    f = u && !l.preload ? cy(l, u) : void 0,
    h = u && l.preload ? 0 : f !== void 0 ? Math.min(f + 1, c) : c;
  let y, m;
  for (let _ = 0; _ < h; _++) s.push(HS(l, s, _));
  try {
    await Promise.all(s);
  } catch {
    const _ = await Promise.allSettled(s);
    for (const E of _) {
      if (E.status !== "rejected") continue;
      const x = E.reason;
      if (Ee(x)) throw x;
      ne(x) ? (y ??= x) : (m ??= x);
    }
    if (m !== void 0) throw m;
  }
  const p = y ?? (u && !l.preload ? u : void 0);
  let S = l.firstBadMatchIndex !== void 0 ? l.firstBadMatchIndex : l.matches.length - 1;
  if (!p && u && l.preload) return l.matches;
  if (p) {
    const _ = cy(l, p);
    _ === void 0 && Te();
    const E = l.matches[_],
      x = l.router.looseRoutesById[E.routeId],
      z = l.router.options?.defaultNotFoundComponent;
    (!x.options.notFoundComponent && z && (x.options.notFoundComponent = z),
      (p.routeId = E.routeId));
    const R = E.routeId === l.router.routeTree.id;
    (l.updateMatch(E.id, (w) => ({
      ...w,
      ...(R
        ? { status: "success", globalNotFound: !0, error: void 0 }
        : { status: "notFound", error: p }),
      isFetching: !1,
    })),
      (S = _),
      await Gl(x, ["notFoundComponent"]));
  } else if (!l.preload) {
    const _ = l.matches[0];
    _.globalNotFound ||
      (l.router.getMatch(_.id)?.globalNotFound &&
        l.updateMatch(_.id, (E) => ({ ...E, globalNotFound: !1, error: void 0 })));
  }
  if (l.serialError && l.firstBadMatchIndex !== void 0) {
    const _ = l.router.looseRoutesById[l.matches[l.firstBadMatchIndex].routeId];
    await Gl(_, ["errorComponent"]);
  }
  for (let _ = 0; _ <= S; _++) {
    const { id: E, routeId: x } = l.matches[_],
      z = l.router.looseRoutesById[x];
    try {
      const R = jS(l, E, z);
      if (R) {
        const w = await R;
        l.updateMatch(E, (N) => ({ ...N, ...w }));
      }
    } catch (R) {
      console.error(`Error executing head for route ${x}:`, R);
    }
  }
  const g = Ec(l);
  if ((ql(g) && (await g), p)) throw p;
  if (l.serialError && !l.preload && !l.onReady) throw l.serialError;
  return l.matches;
}
function my(n, l) {
  const s = l.map((u) => n.options[u]?.preload?.()).filter(Boolean);
  if (s.length !== 0) return Promise.all(s);
}
function Gl(n, l = br) {
  !n._lazyLoaded &&
    n._lazyPromise === void 0 &&
    (n.lazyFn
      ? (n._lazyPromise = n.lazyFn().then((u) => {
          const { id: c, ...f } = u.options;
          (Object.assign(n.options, f), (n._lazyLoaded = !0), (n._lazyPromise = void 0));
        }))
      : (n._lazyLoaded = !0));
  const s = () =>
    n._componentsLoaded
      ? void 0
      : l === br
        ? (() => {
            if (n._componentsPromise === void 0) {
              const u = my(n, br);
              u
                ? (n._componentsPromise = u.then(() => {
                    ((n._componentsLoaded = !0), (n._componentsPromise = void 0));
                  }))
                : (n._componentsLoaded = !0);
            }
            return n._componentsPromise;
          })()
        : my(n, l);
  return n._lazyPromise ? n._lazyPromise.then(s) : s();
}
function mp(n) {
  for (const l of br) if (n.options[l]?.preload) return !0;
  return !1;
}
const br = ["component", "errorComponent", "pendingComponent", "notFoundComponent"];
function qS(n) {
  return {
    input: ({ url: l }) => {
      for (const s of n) l = Tc(s, l);
      return l;
    },
    output: ({ url: l }) => {
      for (let s = n.length - 1; s >= 0; s--) l = yp(n[s], l);
      return l;
    },
  };
}
function YS(n) {
  const l = up(n.basepath),
    s = `/${l}`,
    u = n.caseSensitive ? s : s.toLowerCase(),
    c = `${u}/`;
  return {
    input: ({ url: f }) => {
      const h = n.caseSensitive ? f.pathname : f.pathname.toLowerCase();
      return (
        h === u ? (f.pathname = "/") : h.startsWith(c) && (f.pathname = f.pathname.slice(s.length)),
        f
      );
    },
    output: ({ url: f }) => ((f.pathname = Sr(["/", l, f.pathname])), f),
  };
}
function Tc(n, l) {
  const s = n?.input?.({ url: l });
  if (s) {
    if (typeof s == "string") return new URL(s);
    if (s instanceof URL) return s;
  }
  return l;
}
function yp(n, l) {
  const s = n?.output?.({ url: l });
  if (s) {
    if (typeof s == "string") return new URL(s);
    if (s instanceof URL) return s;
  }
  return l;
}
function GS(n, l) {
  const { createMutableStore: s, createReadonlyStore: u, batch: c, init: f } = l,
    h = new Map(),
    y = new Map(),
    m = new Map(),
    p = s(n.status),
    S = s(n.loadedAt),
    g = s(n.isLoading),
    _ = s(n.isTransitioning),
    E = s(n.location),
    x = s(n.resolvedLocation),
    z = s(n.statusCode),
    R = s(n.redirect),
    w = s([]),
    N = s([]),
    X = s([]),
    Y = u(() => fc(h, w.get())),
    K = u(() => fc(y, N.get())),
    J = u(() => fc(m, X.get())),
    q = u(() => w.get()[0]),
    Z = u(() => w.get().some((F) => h.get(F)?.get().status === "pending")),
    I = u(() => ({
      locationHref: E.get().href,
      resolvedLocationHref: x.get()?.href,
      status: p.get(),
    })),
    ut = u(() => ({
      status: p.get(),
      loadedAt: S.get(),
      isLoading: g.get(),
      isTransitioning: _.get(),
      matches: Y.get(),
      location: E.get(),
      resolvedLocation: x.get(),
      statusCode: z.get(),
      redirect: R.get(),
    })),
    st = Yl(64);
  function lt(F) {
    let it = st.get(F);
    return (
      it ||
        ((it = u(() => {
          const Et = w.get();
          for (const Rt of Et) {
            const C = h.get(Rt);
            if (C && C.routeId === F) return C.get();
          }
        })),
        st.set(F, it)),
      it
    );
  }
  const Wt = {
    status: p,
    loadedAt: S,
    isLoading: g,
    isTransitioning: _,
    location: E,
    resolvedLocation: x,
    statusCode: z,
    redirect: R,
    matchesId: w,
    pendingIds: N,
    cachedIds: X,
    matches: Y,
    pendingMatches: K,
    cachedMatches: J,
    firstId: q,
    hasPending: Z,
    matchRouteDeps: I,
    matchStores: h,
    pendingMatchStores: y,
    cachedMatchStores: m,
    __store: ut,
    getRouteMatchStore: lt,
    setMatches: Nt,
    setPending: Gt,
    setCached: j,
  };
  (Nt(n.matches), f?.(Wt));
  function Nt(F) {
    dc(F, h, w, s, c);
  }
  function Gt(F) {
    dc(F, y, N, s, c);
  }
  function j(F) {
    dc(F, m, X, s, c);
  }
  return Wt;
}
function fc(n, l) {
  const s = [];
  for (const u of l) {
    const c = n.get(u);
    c && s.push(c.get());
  }
  return s;
}
function dc(n, l, s, u, c) {
  const f = n.map((y) => y.id),
    h = new Set(f);
  c(() => {
    for (const y of l.keys()) h.has(y) || l.delete(y);
    for (const y of n) {
      const m = l.get(y.id);
      if (!m) {
        const p = u(y);
        ((p.routeId = y.routeId), l.set(y.id, p));
        continue;
      }
      ((m.routeId = y.routeId), m.get() !== y && m.set(y));
    }
    hS(s.get(), f) || s.set(f);
  });
}
var sa = "__TSR_index",
  yy = "popstate",
  py = "beforeunload";
function QS(n) {
  let l = n.getLocation();
  const s = new Set(),
    u = (h) => {
      ((l = n.getLocation()), s.forEach((y) => y({ location: l, action: h })));
    },
    c = (h) => {
      (n.notifyOnIndexChange ?? !0) ? u(h) : (l = n.getLocation());
    },
    f = async ({ task: h, navigateOpts: y, ...m }) => {
      if (y?.ignoreBlocker ?? !1) {
        h();
        return;
      }
      const p = n.getBlockers?.() ?? [],
        S = m.type === "PUSH" || m.type === "REPLACE";
      if (typeof document < "u" && p.length && S)
        for (const g of p) {
          const _ = Mr(m.path, m.state);
          if (await g.blockerFn({ currentLocation: l, nextLocation: _, action: m.type })) {
            n.onBlocked?.();
            return;
          }
        }
      h();
    };
  return {
    get location() {
      return l;
    },
    get length() {
      return n.getLength();
    },
    subscribers: s,
    subscribe: (h) => (
      s.add(h),
      () => {
        s.delete(h);
      }
    ),
    push: (h, y, m) => {
      const p = l.state[sa];
      ((y = gy(p + 1, y)),
        f({
          task: () => {
            (n.pushState(h, y), u({ type: "PUSH" }));
          },
          navigateOpts: m,
          type: "PUSH",
          path: h,
          state: y,
        }));
    },
    replace: (h, y, m) => {
      const p = l.state[sa];
      ((y = gy(p, y)),
        f({
          task: () => {
            (n.replaceState(h, y), u({ type: "REPLACE" }));
          },
          navigateOpts: m,
          type: "REPLACE",
          path: h,
          state: y,
        }));
    },
    go: (h, y) => {
      f({
        task: () => {
          (n.go(h), c({ type: "GO", index: h }));
        },
        navigateOpts: y,
        type: "GO",
      });
    },
    back: (h) => {
      f({
        task: () => {
          (n.back(h?.ignoreBlocker ?? !1), c({ type: "BACK" }));
        },
        navigateOpts: h,
        type: "BACK",
      });
    },
    forward: (h) => {
      f({
        task: () => {
          (n.forward(h?.ignoreBlocker ?? !1), c({ type: "FORWARD" }));
        },
        navigateOpts: h,
        type: "FORWARD",
      });
    },
    canGoBack: () => l.state[sa] !== 0,
    createHref: (h) => n.createHref(h),
    block: (h) => {
      if (!n.setBlockers) return () => {};
      const y = n.getBlockers?.() ?? [];
      return (
        n.setBlockers([...y, h]),
        () => {
          const m = n.getBlockers?.() ?? [];
          n.setBlockers?.(m.filter((p) => p !== h));
        }
      );
    },
    flush: () => n.flush?.(),
    destroy: () => n.destroy?.(),
    notify: u,
  };
}
function gy(n, l) {
  l || (l = {});
  const s = jc();
  return { ...l, key: s, __TSR_key: s, [sa]: n };
}
function VS(n) {
  const l = typeof document < "u" ? window : void 0,
    s = l.history.pushState,
    u = l.history.replaceState;
  let c = [];
  const f = () => c,
    h = (Z) => (c = Z),
    y = (Z) => Z,
    m = () => Mr(`${l.location.pathname}${l.location.search}${l.location.hash}`, l.history.state);
  if (!l.history.state?.__TSR_key && !l.history.state?.key) {
    const Z = jc();
    l.history.replaceState({ [sa]: 0, key: Z, __TSR_key: Z }, "");
  }
  let p = m(),
    S,
    g = !1,
    _ = !1,
    E = !1,
    x = !1;
  const z = () => p;
  let R, w;
  const N = () => {
      R &&
        ((q._ignoreSubscribers = !0),
        (R.isPush ? l.history.pushState : l.history.replaceState)(R.state, "", R.href),
        (q._ignoreSubscribers = !1),
        (R = void 0),
        (w = void 0),
        (S = void 0));
    },
    X = (Z, I, ut) => {
      const st = y(I);
      (w || (S = p),
        (p = Mr(I, ut)),
        (R = { href: st, state: ut, isPush: R?.isPush || Z === "push" }),
        w || (w = Promise.resolve().then(() => N())));
    },
    Y = (Z) => {
      ((p = m()), q.notify({ type: Z }));
    },
    K = async () => {
      if (_) {
        _ = !1;
        return;
      }
      const Z = m(),
        I = Z.state[sa] - p.state[sa],
        ut = I === 1,
        st = I === -1,
        lt = (!ut && !st) || g;
      g = !1;
      const Wt = lt ? "GO" : st ? "BACK" : "FORWARD",
        Nt = lt ? { type: "GO", index: I } : { type: st ? "BACK" : "FORWARD" };
      if (E) E = !1;
      else {
        const Gt = f();
        if (typeof document < "u" && Gt.length) {
          for (const j of Gt)
            if (await j.blockerFn({ currentLocation: p, nextLocation: Z, action: Wt })) {
              ((_ = !0), l.history.go(1), q.notify(Nt));
              return;
            }
        }
      }
      ((p = m()), q.notify(Nt));
    },
    J = (Z) => {
      if (x) {
        x = !1;
        return;
      }
      let I = !1;
      const ut = f();
      if (typeof document < "u" && ut.length)
        for (const st of ut) {
          const lt = st.enableBeforeUnload ?? !0;
          if (lt === !0) {
            I = !0;
            break;
          }
          if (typeof lt == "function" && lt() === !0) {
            I = !0;
            break;
          }
        }
      if (I) return (Z.preventDefault(), (Z.returnValue = ""));
    },
    q = QS({
      getLocation: z,
      getLength: () => l.history.length,
      pushState: (Z, I) => X("push", Z, I),
      replaceState: (Z, I) => X("replace", Z, I),
      back: (Z) => (Z && (E = !0), (x = !0), l.history.back()),
      forward: (Z) => {
        (Z && (E = !0), (x = !0), l.history.forward());
      },
      go: (Z) => {
        ((g = !0), l.history.go(Z));
      },
      createHref: (Z) => y(Z),
      flush: N,
      destroy: () => {
        ((l.history.pushState = s),
          (l.history.replaceState = u),
          l.removeEventListener(py, J, { capture: !0 }),
          l.removeEventListener(yy, K));
      },
      onBlocked: () => {
        S && p !== S && (p = S);
      },
      getBlockers: f,
      setBlockers: h,
      notifyOnIndexChange: !1,
    });
  return (
    l.addEventListener(py, J, { capture: !0 }),
    l.addEventListener(yy, K),
    (l.history.pushState = function (...Z) {
      const I = s.apply(l.history, Z);
      return (q._ignoreSubscribers || Y("PUSH"), I);
    }),
    (l.history.replaceState = function (...Z) {
      const I = u.apply(l.history, Z);
      return (q._ignoreSubscribers || Y("REPLACE"), I);
    }),
    q
  );
}
function XS(n) {
  let l = n.replace(/[\x00-\x1f\x7f]/g, "");
  return (l.startsWith("//") && (l = "/" + l.replace(/^\/+/, "")), l);
}
function Mr(n, l) {
  const s = XS(n),
    u = s.indexOf("#"),
    c = s.indexOf("?"),
    f = jc();
  return {
    href: s,
    pathname: s.substring(0, u > 0 ? (c > 0 ? Math.min(u, c) : u) : c > 0 ? c : s.length),
    hash: u > -1 ? s.substring(u) : "",
    search: c > -1 ? s.slice(c, u === -1 ? void 0 : u) : "",
    state: l || { [sa]: 0, key: f, __TSR_key: f },
  };
}
function jc() {
  return (Math.random() + 1).toString(36).substring(7);
}
function KS(n) {
  return n instanceof Error ? { name: n.name, message: n.message } : { data: n };
}
function zi(n, l) {
  const s = l,
    u = n;
  return {
    fromLocation: s,
    toLocation: u,
    pathChanged: s?.pathname !== u.pathname,
    hrefChanged: s?.href !== u.href,
    hashChanged: s?.hash !== u.hash,
  };
}
const Rc = new WeakMap();
var ZS = class {
    constructor(n, l) {
      ((this.tempLocationKey = `${Math.round(Math.random() * 1e7)}`),
        (this.resetNextScroll = !0),
        (this.shouldViewTransition = void 0),
        (this.isViewTransitionTypesSupported = void 0),
        (this.subscribers = new Set()),
        (this.isScrollRestoring = !1),
        (this.isScrollRestorationSetup = !1),
        (this.routeBranchCache = new WeakMap()),
        (this.startTransition = (s) => s()),
        (this.update = (s) => {
          const u = this.options,
            c = this.basepath ?? u?.basepath ?? "/",
            f = this.basepath === void 0,
            h = u?.rewrite;
          if (
            ((this.options = { ...u, ...s }),
            (this.isServer = this.options.isServer ?? typeof document > "u"),
            (this.protocolAllowlist = new Set(this.options.protocolAllowlist)),
            this.options.pathParamsAllowedCharacters &&
              (this.pathParamsDecoder = xS(this.options.pathParamsAllowedCharacters)),
            (!this.history || (this.options.history && this.options.history !== this.history)) &&
              (this.options.history
                ? (this.history = this.options.history)
                : (this.history = VS())),
            (this.origin = this.options.origin),
            this.origin ||
              (window?.origin && window.origin !== "null"
                ? (this.origin = window.origin)
                : (this.origin = "http://localhost")),
            this.history && this.updateLatestLocation(),
            this.options.routeTree !== this.routeTree)
          ) {
            this.routeTree = this.options.routeTree;
            let S;
            ((this.resolvePathCache = Yl(1e3)), (S = this.buildRouteTree()), this.setRoutes(S));
          }
          if (!this.stores && this.latestLocation) {
            const S = this.getStoreConfig(this);
            ((this.batch = S.batch), (this.stores = GS(FS(this.latestLocation), S)), lb(this));
          }
          let y = !1;
          const m = this.options.basepath ?? "/",
            p = this.options.rewrite;
          if (f || c !== m || h !== p) {
            this.basepath = m;
            const S = [],
              g = up(m);
            (g && g !== "/" && S.push(YS({ basepath: m })),
              p && S.push(p),
              (this.rewrite = S.length === 0 ? void 0 : S.length === 1 ? S[0] : qS(S)),
              this.history && this.updateLatestLocation(),
              (y = !0));
          }
          (y && this.stores && this.stores.location.set(this.latestLocation),
            typeof window < "u" &&
              "CSS" in window &&
              typeof window.CSS?.supports == "function" &&
              (this.isViewTransitionTypesSupported = window.CSS.supports(
                "selector(:active-view-transition-type(a))",
              )));
        }),
        (this.updateLatestLocation = () => {
          this.latestLocation = this.parseLocation(this.history.location, this.latestLocation);
        }),
        (this.buildRouteTree = () => {
          const s = bS(this.routeTree, this.options.caseSensitive, (u, c) => {
            u.init({ originalIndex: c });
          });
          return (this.options.routeMasks && yS(this.options.routeMasks, s.processedTree), s);
        }),
        (this.subscribe = (s, u) => {
          const c = { eventType: s, fn: u };
          return (
            this.subscribers.add(c),
            () => {
              this.subscribers.delete(c);
            }
          );
        }),
        (this.emit = (s) => {
          this.subscribers.forEach((u) => {
            u.eventType === s.type && u.fn(s);
          });
        }),
        (this.parseLocation = (s, u) => {
          const c = ({ pathname: m, search: p, hash: S, href: g, state: _ }) => {
              if (!this.rewrite && !/[ \x00-\x1f\x7f\u0080-\uffff]/.test(m)) {
                const w = this.options.parseSearch(p),
                  N = this.options.stringifySearch(w);
                return {
                  href: m + N + S,
                  publicHref: m + N + S,
                  pathname: zl(m).path,
                  external: !1,
                  searchStr: N,
                  search: wa(u?.search, w),
                  hash: zl(S.slice(1)).path,
                  state: za(u?.state, _),
                };
              }
              const E = new URL(g, this.origin),
                x = Tc(this.rewrite, E),
                z = this.options.parseSearch(x.search),
                R = this.options.stringifySearch(z);
              return (
                (x.search = R),
                {
                  href: x.href.replace(x.origin, ""),
                  publicHref: g,
                  pathname: zl(x.pathname).path,
                  external: !!this.rewrite && x.origin !== this.origin,
                  searchStr: R,
                  search: wa(u?.search, z),
                  hash: zl(x.hash.slice(1)).path,
                  state: za(u?.state, _),
                }
              );
            },
            f = c(s),
            { __tempLocation: h, __tempKey: y } = f.state;
          if (h && (!y || y === this.tempLocationKey)) {
            const m = c(h);
            return (
              (m.state.key = f.state.key),
              (m.state.__TSR_key = f.state.__TSR_key),
              delete m.state.__tempLocation,
              { ...m, maskedLocation: f }
            );
          }
          return f;
        }),
        (this.resolvePathWithBase = (s, u) =>
          AS({
            base: s,
            to: u.includes("//") ? Nc(u) : u,
            trailingSlash: this.options.trailingSlash,
            cache: this.resolvePathCache,
          })),
        (this.matchRoutes = (s, u, c) =>
          typeof s == "string"
            ? this.matchRoutesInternal({ pathname: s, search: u }, c)
            : this.matchRoutesInternal(s, u)),
        (this.getMatchedRoutes = (s) =>
          JS({ pathname: s, routesById: this.routesById, processedTree: this.processedTree })),
        (this.cancelMatch = (s) => {
          const u = this.getMatch(s);
          u &&
            (u.abortController.abort(),
            clearTimeout(u._nonReactive.pendingTimeout),
            (u._nonReactive.pendingTimeout = void 0));
        }),
        (this.cancelMatches = () => {
          (this.stores.pendingIds.get().forEach((s) => {
            this.cancelMatch(s);
          }),
            this.stores.matchesId.get().forEach((s) => {
              if (this.stores.pendingMatchStores.has(s)) return;
              const u = this.stores.matchStores.get(s)?.get();
              u && (u.status === "pending" || u.isFetching === "loader") && this.cancelMatch(s);
            }));
        }),
        (this.buildLocation = (s) => {
          const u = (f = {}) => {
              const h = f._fromLocation || this.pendingBuiltLocation || this.latestLocation,
                y = this.matchRoutesLightweight(h);
              f.from;
              const m = f.unsafeRelative === "path" ? h.pathname : (f.from ?? y.fullPath),
                p = f.to ? `${f.to}` : void 0,
                S = y.search,
                g = Object.assign(Object.create(null), y.params),
                _ = p?.charCodeAt(0) === 47 ? "/" : this.resolvePathWithBase(m, "."),
                E = p ? this.resolvePathWithBase(_, p) : _,
                x =
                  f.params === !1 || f.params === null
                    ? Object.create(null)
                    : (f.params ?? !0) === !0
                      ? g
                      : Object.assign(g, Oa(f.params, g)),
                z = this.routesByPath[An(E)];
              let R;
              if (z) R = this.getRouteBranch(z);
              else if (E.includes("$")) R = [];
              else {
                const st = this.getMatchedRoutes(E);
                ((R = st.matchedRoutes),
                  this.options.notFoundRoute &&
                    (!st.foundRoute || (st.foundRoute.path !== "/" && st.routeParams["**"])) &&
                    (R = [...R, this.options.notFoundRoute]));
              }
              if (R.length && np(x))
                for (const st of R) {
                  const lt = st.options.params?.stringify ?? st.options.stringifyParams;
                  if (lt)
                    try {
                      Object.assign(x, lt(x));
                    } catch {}
                }
              const w = s.leaveParams
                ? E
                : zl(
                    uy({
                      path: E,
                      params: x,
                      decoder: this.pathParamsDecoder,
                      server: this.isServer,
                    }).interpolatedPath,
                  ).path;
              let N = S;
              if (s._includeValidateSearch && this.options.search?.strict) {
                const st = {};
                (R.forEach((lt) => {
                  if (lt.options.validateSearch)
                    try {
                      Object.assign(st, _r(lt.options.validateSearch, { ...st, ...N }));
                    } catch {}
                }),
                  (N = st));
              }
              ((N = kS({
                search: N,
                dest: f,
                destRoutes: R,
                _includeValidateSearch: s._includeValidateSearch,
              })),
                (N = wa(S, N)));
              const X = this.options.stringifySearch(N),
                Y = f.hash === !0 ? h.hash : f.hash ? Oa(f.hash, h.hash) : void 0,
                K = Y ? `#${Y}` : "";
              let J = f.state === !0 ? h.state : f.state ? Oa(f.state, h.state) : {};
              J = za(h.state, J);
              const q = `${w}${X}${K}`;
              let Z,
                I,
                ut = !1;
              if (this.rewrite) {
                const st = new URL(q, this.origin),
                  lt = yp(this.rewrite, st);
                ((Z = st.href.replace(st.origin, "")),
                  lt.origin !== this.origin
                    ? ((I = lt.href), (ut = !0))
                    : (I = lt.pathname + lt.search + lt.hash));
              } else ((Z = dS(q)), (I = Z));
              return {
                publicHref: I,
                href: Z,
                pathname: w,
                search: N,
                searchStr: X,
                state: J,
                hash: Y ?? "",
                external: ut,
                unmaskOnReload: f.unmaskOnReload,
              };
            },
            c = (f = {}, h) => {
              const y = u(f);
              let m = h ? u(h) : void 0;
              if (!m) {
                const p = Object.create(null);
                if (this.options.routeMasks) {
                  const S = pS(y.pathname, this.processedTree);
                  if (S) {
                    Object.assign(p, S.rawParams);
                    const { from: g, params: _, ...E } = S.route,
                      x =
                        _ === !1 || _ === null
                          ? Object.create(null)
                          : (_ ?? !0) === !0
                            ? p
                            : Object.assign(p, Oa(_, p));
                    ((h = { from: s.from, ...E, params: x }), (m = u(h)));
                  }
                }
              }
              return (m && (y.maskedLocation = m), y);
            };
          return s.mask ? c(s, { from: s.from, ...s.mask }) : c(s);
        }),
        (this.commitLocation = async ({ viewTransition: s, ignoreBlocker: u, ...c }) => {
          let f;
          const h = () => {
              const p = ["key", "__TSR_key", "__TSR_index", "__hashScrollIntoViewOptions"];
              p.forEach((g) => {
                c.state[g] = this.latestLocation.state[g];
              });
              const S = he(c.state, this.latestLocation.state);
              return (
                p.forEach((g) => {
                  delete c.state[g];
                }),
                S
              );
            },
            y = An(this.latestLocation.href) === An(c.href);
          let m = this.commitLocationPromise;
          if (
            ((this.commitLocationPromise = Na(() => {
              (m?.resolve(), (m = void 0));
            })),
            y && h())
          )
            this.load();
          else {
            let { maskedLocation: p, hashScrollIntoView: S, ...g } = c;
            (p &&
              ((g = {
                ...p,
                state: {
                  ...p.state,
                  __tempKey: void 0,
                  __tempLocation: {
                    ...g,
                    search: g.searchStr,
                    state: {
                      ...g.state,
                      __tempKey: void 0,
                      __tempLocation: void 0,
                      __TSR_key: void 0,
                      key: void 0,
                    },
                  },
                },
              }),
              (g.unmaskOnReload ?? this.options.unmaskOnReload ?? !1) &&
                (g.state.__tempKey = this.tempLocationKey)),
              (g.state.__hashScrollIntoViewOptions =
                S ?? this.options.defaultHashScrollIntoView ?? !0),
              (this.shouldViewTransition = s),
              (f = c.replace ? "REPLACE" : "PUSH"),
              this.history[f === "REPLACE" ? "replace" : "push"](g.publicHref, g.state, {
                ignoreBlocker: u,
              }));
          }
          return (
            (this.resetNextScroll = c.resetScroll ?? !0),
            this.history.subscribers.size || this.load(f ? { action: { type: f } } : void 0),
            this.commitLocationPromise
          );
        }),
        (this.buildAndCommitLocation = ({
          replace: s,
          resetScroll: u,
          hashScrollIntoView: c,
          viewTransition: f,
          ignoreBlocker: h,
          href: y,
          ...m
        } = {}) => {
          if (y) {
            const g = this.history.location.state.__TSR_index,
              _ = Mr(y, { __TSR_index: s ? g : g + 1 }),
              E = new URL(_.pathname, this.origin);
            ((m.to = Tc(this.rewrite, E).pathname),
              (m.search = this.options.parseSearch(_.search)),
              (m.hash = _.hash.slice(1)));
          }
          const p = this.buildLocation({ ...m, _includeValidateSearch: !0 });
          this.pendingBuiltLocation = p;
          const S = this.commitLocation({
            ...p,
            viewTransition: f,
            replace: s,
            resetScroll: u,
            hashScrollIntoView: c,
            ignoreBlocker: h,
          });
          return (
            Promise.resolve().then(() => {
              this.pendingBuiltLocation === p && (this.pendingBuiltLocation = void 0);
            }),
            S
          );
        }),
        (this.navigate = async ({ to: s, reloadDocument: u, href: c, publicHref: f, ...h }) => {
          let y = !1;
          if (c)
            try {
              (new URL(`${c}`), (y = !0));
            } catch {}
          if ((y && !u && (u = !0), u)) {
            if (s !== void 0 || !c) {
              const p = this.buildLocation({ to: s, ...h });
              ((c = c ?? p.publicHref), (f = f ?? p.publicHref));
            }
            const m = !y && f ? f : c;
            if (Ar(m, this.protocolAllowlist)) return Promise.resolve();
            if (!h.ignoreBlocker) {
              const p = this.history.getBlockers?.() ?? [];
              for (const S of p)
                if (
                  S?.blockerFn &&
                  (await S.blockerFn({
                    currentLocation: this.latestLocation,
                    nextLocation: this.latestLocation,
                    action: "PUSH",
                  }))
                )
                  return Promise.resolve();
            }
            return (
              h.replace ? window.location.replace(m) : (window.location.href = m),
              Promise.resolve()
            );
          }
          return this.buildAndCommitLocation({ ...h, href: c, to: s, _isNavigate: !0 });
        }),
        (this.beforeLoad = () => {
          (this.cancelMatches(), this.updateLatestLocation());
          const s = this.matchRoutes(this.latestLocation),
            u = this.stores.cachedMatches.get().filter((c) => !s.some((f) => f.id === c.id));
          this.batch(() => {
            (this.stores.status.set("pending"),
              this.stores.statusCode.set(200),
              this.stores.isLoading.set(!0),
              this.stores.location.set(this.latestLocation),
              this.stores.setPending(s),
              this.stores.setCached(u));
          });
        }),
        (this.load = async (s) => {
          const u = s?.action?.type;
          let c, f, h;
          const y = this.stores.resolvedLocation.get() ?? this.stores.location.get();
          for (
            h = new Promise((p) => {
              this.startTransition(async () => {
                try {
                  (this.beforeLoad(),
                    u ? Rc.set(this.latestLocation, u) : Rc.delete(this.latestLocation));
                  const S = this.latestLocation,
                    g = zi(S, this.stores.resolvedLocation.get());
                  (this.stores.redirect.get() || this.emit({ type: "onBeforeNavigate", ...g }),
                    this.emit({ type: "onBeforeLoad", ...g }),
                    await hy({
                      router: this,
                      sync: s?.sync,
                      forceStaleReload: y.href === S.href,
                      matches: this.stores.pendingMatches.get(),
                      location: S,
                      updateMatch: this.updateMatch,
                      onReady: async () => {
                        this.startTransition(() => {
                          this.startViewTransition(async () => {
                            let _ = null,
                              E = null,
                              x = null,
                              z = null;
                            this.batch(() => {
                              const R = this.stores.pendingMatches.get(),
                                w = R.length,
                                N = this.stores.matches.get();
                              _ = w
                                ? N.filter((K) => !this.stores.pendingMatchStores.has(K.id))
                                : null;
                              const X = new Set();
                              for (const K of this.stores.pendingMatchStores.values())
                                K.routeId && X.add(K.routeId);
                              const Y = new Set();
                              for (const K of this.stores.matchStores.values())
                                K.routeId && Y.add(K.routeId);
                              ((E = w ? N.filter((K) => !X.has(K.routeId)) : null),
                                (x = w ? R.filter((K) => !Y.has(K.routeId)) : null),
                                (z = w ? R.filter((K) => Y.has(K.routeId)) : N),
                                this.stores.isLoading.set(!1),
                                this.stores.loadedAt.set(Date.now()),
                                w &&
                                  (this.stores.setMatches(R),
                                  this.stores.setPending([]),
                                  this.stores.setCached([
                                    ...this.stores.cachedMatches.get(),
                                    ..._.filter(
                                      (K) =>
                                        K.status !== "error" &&
                                        K.status !== "notFound" &&
                                        K.status !== "redirected",
                                    ),
                                  ]),
                                  this.clearExpiredCache()));
                            });
                            for (const [R, w] of [
                              [E, "onLeave"],
                              [x, "onEnter"],
                              [z, "onStay"],
                            ])
                              if (R)
                                for (const N of R) this.looseRoutesById[N.routeId].options[w]?.(N);
                          });
                        });
                      },
                    }));
                } catch (S) {
                  Ee(S)
                    ? ((c = S), this.navigate({ ...c.options, replace: !0, ignoreBlocker: !0 }))
                    : ne(S) && (f = S);
                  const g = c
                    ? c.status
                    : f
                      ? 404
                      : this.stores.matches.get().some((_) => _.status === "error")
                        ? 500
                        : 200;
                  this.batch(() => {
                    (this.stores.statusCode.set(g), this.stores.redirect.set(c));
                  });
                }
                (this.latestLoadPromise === h &&
                  (this.commitLocationPromise?.resolve(),
                  (this.latestLoadPromise = void 0),
                  (this.commitLocationPromise = void 0)),
                  p());
              });
            }),
              this.latestLoadPromise = h,
              await h;
            this.latestLoadPromise && h !== this.latestLoadPromise;
          )
            await this.latestLoadPromise;
          let m;
          (this.hasNotFoundMatch()
            ? (m = 404)
            : this.stores.matches.get().some((p) => p.status === "error") && (m = 500),
            m !== void 0 && this.stores.statusCode.set(m));
        }),
        (this.startViewTransition = (s) => {
          const u = this.shouldViewTransition ?? this.options.defaultViewTransition;
          if (
            ((this.shouldViewTransition = void 0),
            u &&
              typeof document < "u" &&
              "startViewTransition" in document &&
              typeof document.startViewTransition == "function")
          ) {
            let c;
            if (typeof u == "object" && this.isViewTransitionTypesSupported) {
              const f = this.latestLocation,
                h = this.stores.resolvedLocation.get(),
                y = typeof u.types == "function" ? u.types(zi(f, h)) : u.types;
              if (y === !1) {
                s();
                return;
              }
              c = { update: s, types: y };
            } else c = s;
            document.startViewTransition(c);
          } else s();
        }),
        (this.updateMatch = (s, u) => {
          this.startTransition(() => {
            const c = this.stores.pendingMatchStores.get(s);
            if (c) {
              c.set(u);
              return;
            }
            const f = this.stores.matchStores.get(s);
            if (f) {
              f.set(u);
              return;
            }
            const h = this.stores.cachedMatchStores.get(s);
            if (h) {
              const y = u(h.get());
              y.status === "redirected"
                ? this.stores.cachedMatchStores.delete(s) &&
                  this.stores.cachedIds.set((m) => m.filter((p) => p !== s))
                : h.set(y);
            }
          });
        }),
        (this.getMatch = (s) =>
          this.stores.cachedMatchStores.get(s)?.get() ??
          this.stores.pendingMatchStores.get(s)?.get() ??
          this.stores.matchStores.get(s)?.get()),
        (this.invalidate = (s) => {
          const u = (c) =>
            (s?.filter?.(c) ?? !0)
              ? {
                  ...c,
                  invalid: !0,
                  ...(s?.forcePending || c.status === "error" || c.status === "notFound"
                    ? { status: "pending", error: void 0 }
                    : void 0),
                }
              : c;
          return (
            this.batch(() => {
              (this.stores.setMatches(this.stores.matches.get().map(u)),
                this.stores.setCached(this.stores.cachedMatches.get().map(u)),
                this.stores.setPending(this.stores.pendingMatches.get().map(u)));
            }),
            (this.shouldViewTransition = !1),
            this.load({ sync: s?.sync })
          );
        }),
        (this.getParsedLocationHref = (s) => s.publicHref || "/"),
        (this.resolveRedirect = (s) => {
          const u = s.headers.get("Location");
          if (!s.options.href || s.options._builtLocation) {
            const c = s.options._builtLocation ?? this.buildLocation(s.options),
              f = this.getParsedLocationHref(c);
            ((s.options.href = f), s.headers.set("Location", f));
          } else if (u)
            try {
              const c = new URL(u);
              if (this.origin && c.origin === this.origin) {
                const f = c.pathname + c.search + c.hash;
                ((s.options.href = f), s.headers.set("Location", f));
              }
            } catch {}
          if (
            s.options.href &&
            !s.options._builtLocation &&
            Ar(s.options.href, this.protocolAllowlist)
          )
            throw new Error("Redirect blocked: unsafe protocol");
          return (s.headers.get("Location") || s.headers.set("Location", s.options.href), s);
        }),
        (this.clearCache = (s) => {
          const u = s?.filter;
          u !== void 0
            ? this.stores.setCached(this.stores.cachedMatches.get().filter((c) => !u(c)))
            : this.stores.setCached([]);
        }),
        (this.clearExpiredCache = () => {
          const s = Date.now(),
            u = (c) => {
              const f = this.looseRoutesById[c.routeId];
              if (!f.options.loader) return !0;
              const h =
                (c.preload
                  ? (f.options.preloadGcTime ?? this.options.defaultPreloadGcTime)
                  : (f.options.gcTime ?? this.options.defaultGcTime)) ?? 300 * 1e3;
              return c.status === "error" ? !0 : s - c.updatedAt >= h;
            };
          this.clearCache({ filter: u });
        }),
        (this.loadRouteChunk = Gl),
        (this.preloadRoute = async (s) => {
          const u = s._builtLocation ?? this.buildLocation(s);
          let c = this.matchRoutes(u, { throwOnError: !0, preload: !0, dest: s });
          const f = new Set([...this.stores.matchesId.get(), ...this.stores.pendingIds.get()]),
            h = new Set([...f, ...this.stores.cachedIds.get()]),
            y = c.filter((m) => !h.has(m.id));
          if (y.length) {
            const m = this.stores.cachedMatches.get();
            this.stores.setCached([...m, ...y]);
          }
          try {
            return (
              (c = await hy({
                router: this,
                matches: c,
                location: u,
                preload: !0,
                updateMatch: (m, p) => {
                  f.has(m) ? (c = c.map((S) => (S.id === m ? p(S) : S))) : this.updateMatch(m, p);
                },
              })),
              c
            );
          } catch (m) {
            if (Ee(m))
              return m.options.reloadDocument
                ? void 0
                : await this.preloadRoute({ ...m.options, _fromLocation: u });
            ne(m) || console.error(m);
            return;
          }
        }),
        (this.matchRoute = (s, u) => {
          const c = {
              ...s,
              to: s.to ? this.resolvePathWithBase(s.from || "", s.to) : void 0,
              params: s.params || {},
              leaveParams: !0,
            },
            f = this.buildLocation(c);
          if (u?.pending && this.stores.status.get() !== "pending") return !1;
          const h = (u?.pending === void 0 ? !this.stores.isLoading.get() : u.pending)
              ? this.latestLocation
              : this.stores.resolvedLocation.get() || this.stores.location.get(),
            y = gS(
              f.pathname,
              u?.caseSensitive ?? !1,
              u?.fuzzy ?? !1,
              h.pathname,
              this.processedTree,
            );
          return !y || (s.params && !he(y.rawParams, s.params, { partial: !0 }))
            ? !1
            : (u?.includeSearch ?? !0)
              ? he(h.search, f.search, { partial: !0 })
                ? y.rawParams
                : !1
              : y.rawParams;
        }),
        (this.hasNotFoundMatch = () =>
          this.stores.matches.get().some((s) => s.status === "notFound" || s.globalNotFound)),
        (this.getStoreConfig = l),
        this.update({
          defaultPreloadDelay: 50,
          defaultPendingMs: 1e3,
          defaultPendingMinMs: 500,
          context: void 0,
          ...n,
          caseSensitive: n.caseSensitive ?? !1,
          notFoundMode: n.notFoundMode ?? "fuzzy",
          stringifySearch: n.stringifySearch ?? wS,
          parseSearch: n.parseSearch ?? CS,
          protocolAllowlist: n.protocolAllowlist ?? uS,
        }),
        typeof document < "u" && (self.__TSR_ROUTER__ = this));
    }
    isShell() {
      return !!this.options.isShell;
    }
    isPrerendering() {
      return !!this.options.isPrerendering;
    }
    get state() {
      return this.stores.__store.get();
    }
    setRoutes({ routesById: n, routesByPath: l, processedTree: s }) {
      ((this.routesById = n), (this.routesByPath = l), (this.processedTree = s));
      const u = this.options.notFoundRoute;
      u && (u.init({ originalIndex: 99999999999 }), (this.routesById[u.id] = u));
    }
    getRouteBranch(n) {
      let l = this.routeBranchCache.get(n);
      return (l || ((l = sp(n)), this.routeBranchCache.set(n, l)), l);
    }
    get looseRoutesById() {
      return this.routesById;
    }
    getParentContext(n) {
      return n?.id
        ? (n.context ?? this.options.context ?? void 0)
        : (this.options.context ?? void 0);
    }
    matchRoutesInternal(n, l) {
      const s = this.getMatchedRoutes(n.pathname),
        { foundRoute: u, routeParams: c } = s;
      let { matchedRoutes: f } = s,
        h = !1;
      (u ? u.path !== "/" && c["**"] : An(n.pathname)) &&
        (this.options.notFoundRoute ? (f = [...f, this.options.notFoundRoute]) : (h = !0));
      const y = h ? $S(this.options.notFoundMode, f) : void 0,
        m = new Array(f.length),
        p = new Map();
      for (const S of this.stores.matchStores.values()) S.routeId && p.set(S.routeId, S.get());
      for (let S = 0; S < f.length; S++) {
        const g = f[S],
          _ = m[S - 1];
        let E, x, z;
        {
          const lt = _?.search ?? n.search,
            Wt = _?._strictSearch ?? void 0;
          try {
            const Nt = _r(g.options.validateSearch, { ...lt }) ?? void 0;
            ((E = { ...lt, ...Nt }), (x = { ...Wt, ...Nt }), (z = void 0));
          } catch (Nt) {
            let Gt = Nt;
            if ((Nt instanceof Cr || (Gt = new Cr(Nt.message, { cause: Nt })), l?.throwOnError))
              throw Gt;
            ((E = lt), (x = {}), (z = Gt));
          }
        }
        const R = g.options.loaderDeps?.({ search: E }) ?? "",
          w = R ? JSON.stringify(R) : "",
          { interpolatedPath: N, usedParams: X } = uy({
            path: g.fullPath,
            params: c,
            decoder: this.pathParamsDecoder,
            server: this.isServer,
          }),
          Y = g.id + N + w,
          K = this.getMatch(Y),
          J = p.get(g.id),
          q = K?._strictParams ?? X;
        let Z;
        if (!K)
          try {
            vy(g, q);
          } catch (lt) {
            if (
              (ne(lt) || Ee(lt) ? (Z = lt) : (Z = new PS(lt.message, { cause: lt })),
              l?.throwOnError)
            )
              throw Z;
          }
        Object.assign(c, q);
        const I = J ? "stay" : "enter";
        let ut;
        if (K)
          ut = {
            ...K,
            cause: I,
            params: J?.params ?? c,
            _strictParams: q,
            search: wa(J ? J.search : K.search, E),
            _strictSearch: x,
          };
        else {
          const lt =
            g.options.loader || g.options.beforeLoad || g.lazyFn || mp(g) ? "pending" : "success";
          ut = {
            id: Y,
            ssr: g.options.ssr,
            index: S,
            routeId: g.id,
            params: J?.params ?? c,
            _strictParams: q,
            pathname: N,
            updatedAt: Date.now(),
            search: J ? wa(J.search, E) : E,
            _strictSearch: x,
            searchError: void 0,
            status: lt,
            isFetching: !1,
            error: void 0,
            paramsError: Z,
            __routeContext: void 0,
            _nonReactive: { loadPromise: Na() },
            __beforeLoadContext: void 0,
            context: {},
            abortController: new AbortController(),
            fetchCount: 0,
            cause: I,
            loaderDeps: J ? za(J.loaderDeps, R) : R,
            invalid: !1,
            preload: !1,
            links: void 0,
            scripts: void 0,
            headScripts: void 0,
            meta: void 0,
            staticData: g.options.staticData || {},
            fullPath: g.fullPath,
          };
        }
        (l?.preload || (ut.globalNotFound = y === g.id), (ut.searchError = z));
        const st = this.getParentContext(_);
        ((ut.context = { ...st, ...ut.__routeContext, ...ut.__beforeLoadContext }), (m[S] = ut));
      }
      for (let S = 0; S < m.length; S++) {
        const g = m[S],
          _ = this.looseRoutesById[g.routeId],
          E = this.getMatch(g.id),
          x = p.get(g.routeId);
        if (((g.params = x ? wa(x.params, c) : c), !E)) {
          const z = m[S - 1],
            R = this.getParentContext(z);
          if (_.options.context) {
            const w = {
              deps: g.loaderDeps,
              params: g.params,
              context: R ?? {},
              location: n,
              navigate: (N) => this.navigate({ ...N, _fromLocation: n }),
              buildLocation: this.buildLocation,
              cause: g.cause,
              abortController: g.abortController,
              preload: !!g.preload,
              matches: m,
              routeId: _.id,
            };
            g.__routeContext = _.options.context(w) ?? void 0;
          }
          g.context = { ...R, ...g.__routeContext, ...g.__beforeLoadContext };
        }
      }
      return m;
    }
    matchRoutesLightweight(n) {
      const { matchedRoutes: l, routeParams: s } = this.getMatchedRoutes(n.pathname),
        u = Hl(l),
        c = { ...n.search };
      for (const p of l)
        try {
          Object.assign(c, _r(p.options.validateSearch, c));
        } catch {}
      const f = Hl(this.stores.matchesId.get()),
        h = f && this.stores.matchStores.get(f)?.get(),
        y = h && h.routeId === u.id && h.pathname === n.pathname;
      let m;
      if (y) m = h.params;
      else {
        const p = Object.assign(Object.create(null), s);
        for (const S of l)
          try {
            vy(S, p);
          } catch {}
        m = p;
      }
      return { matchedRoutes: l, fullPath: u.fullPath, search: c, params: m };
    }
  },
  Cr = class extends Error {},
  PS = class extends Error {};
function FS(n) {
  return {
    loadedAt: 0,
    isLoading: !1,
    isTransitioning: !1,
    status: "idle",
    resolvedLocation: void 0,
    location: n,
    matches: [],
    statusCode: 200,
  };
}
function _r(n, l) {
  if (n == null) return {};
  if ("~standard" in n) {
    const s = n["~standard"].validate(l);
    if (s instanceof Promise) throw new Cr("Async validation not supported");
    if (s.issues) throw new Cr(JSON.stringify(s.issues, void 0, 2), { cause: s });
    return s.value;
  }
  return "parse" in n ? n.parse(l) : typeof n == "function" ? n(l) : {};
}
function JS({ pathname: n, routesById: l, processedTree: s }) {
  const u = Object.create(null),
    c = An(n);
  let f;
  const h = vS(c, s, !0);
  return (
    h && ((f = h.route), Object.assign(u, h.rawParams)),
    { matchedRoutes: h?.branch || [l.__root__], routeParams: u, foundRoute: f }
  );
}
function kS({ search: n, dest: l, destRoutes: s, _includeValidateSearch: u }) {
  return IS(s)(n, l, u ?? !1);
}
function IS(n) {
  const l = { dest: null, _includeValidateSearch: !1, middlewares: [] };
  for (const c of n) {
    if ("search" in c.options)
      c.options.search?.middlewares && l.middlewares.push(...c.options.search.middlewares);
    else if (c.options.preSearchFilters || c.options.postSearchFilters) {
      const f = ({ search: h, next: y }) => {
        let m = h;
        "preSearchFilters" in c.options &&
          c.options.preSearchFilters &&
          (m = c.options.preSearchFilters.reduce((S, g) => g(S), h));
        const p = y(m);
        return "postSearchFilters" in c.options && c.options.postSearchFilters
          ? c.options.postSearchFilters.reduce((S, g) => g(S), p)
          : p;
      };
      l.middlewares.push(f);
    }
    if (c.options.validateSearch) {
      const f = ({ search: h, next: y }) => {
        const m = y(h);
        if (!l._includeValidateSearch) return m;
        try {
          return { ...m, ...(_r(c.options.validateSearch, m) ?? void 0) };
        } catch {
          return m;
        }
      };
      l.middlewares.push(f);
    }
  }
  const s = ({ search: c }) => {
    const f = l.dest;
    return f.search ? (f.search === !0 ? c : Oa(f.search, c)) : {};
  };
  l.middlewares.push(s);
  const u = (c, f, h) => {
    if (c >= h.length) return f;
    const y = h[c];
    return y({ search: f, next: (p) => u(c + 1, p, h) });
  };
  return function (f, h, y) {
    return ((l.dest = h), (l._includeValidateSearch = y), u(0, f, l.middlewares));
  };
}
function $S(n, l) {
  if (n !== "root")
    for (let s = l.length - 1; s >= 0; s--) {
      const u = l[s];
      if (u.children) return u.id;
    }
  return Ua;
}
function vy(n, l) {
  const s = n.options.params?.parse ?? n.options.parseParams;
  if (s) {
    const u = s(l);
    if (u === !1) throw new Error("Route params.parse returned false for a matched route");
    Object.assign(l, u);
  }
}
function WS() {
  try {
    return sessionStorage;
  } catch {
    return;
  }
}
const tb = "tsr-scroll-restoration-v1_3",
  pp = WS();
function eb() {
  try {
    return JSON.parse(pp?.getItem("tsr-scroll-restoration-v1_3") || "{}");
  } catch {
    return {};
  }
}
function nb() {
  try {
    pp?.setItem(tb, JSON.stringify(xi));
  } catch {}
}
const xi = eb(),
  Sy = "data-scroll-restoration-id",
  ab = (n) => n.state.__TSR_key || n.href;
function ib(n) {
  const l = n.getAttribute(Sy);
  if (l) return `[${Sy}="${l}"]`;
  let s = "",
    u = n,
    c;
  for (; (c = u.parentNode); ) {
    let f = 1,
      h = u;
    for (; (h = h.previousElementSibling); ) f++;
    const y = `${u.localName}:nth-child(${f})`;
    ((s = s ? `${y} > ${s}` : y), (u = c));
  }
  return s;
}
let mr = !1;
const Mi = "window";
function Ac(n) {
  try {
    return typeof n == "function" ? n() : document.querySelector(n);
  } catch {}
}
function by(n) {
  const l = [];
  for (const s of n) {
    if (s === Mi) continue;
    const u = Ac(s);
    u && l.push(u);
  }
  return l;
}
function lb(n, l) {
  if ((n.options.scrollRestoration && (n.isScrollRestoring = !0), n.isScrollRestorationSetup))
    return;
  ((n.isScrollRestorationSetup = !0), (mr = !1));
  const s = n.options.getScrollRestorationKey || ab,
    u = new Map(),
    c = (y, m, p) => {
      const S = u.get(y) || {};
      ((S.scrollX = m), (S.scrollY = p), u.set(y, S));
    };
  history.scrollRestoration = "manual";
  const f = (y) => {
      if (!(mr || !n.isScrollRestoring))
        if (y.target === document) c(Mi, scrollX, scrollY);
        else {
          const m = y.target;
          c(m, m.scrollLeft, m.scrollTop);
        }
    },
    h = (y) => {
      if (!n.isScrollRestoring) return;
      const m = (xi[y] ||= {});
      for (const [p, S] of u) p === Mi ? (m[Mi] = S) : p.isConnected && (m[ib(p)] = S);
    };
  (document.addEventListener("scroll", f, !0),
    n.subscribe("onBeforeLoad", (y) => {
      (y.fromLocation && h(s(y.fromLocation)), u.clear());
    }),
    addEventListener("pagehide", () => {
      (h(s(n.stores.resolvedLocation.get() ?? n.stores.location.get())), nb());
    }),
    n.subscribe("onRendered", (y) => {
      const m = n.options.scrollRestorationBehavior,
        p = n.options.scrollToTopSelectors,
        S = n.resetNextScroll;
      let g;
      if (
        (u.clear(),
        S || (n.resetNextScroll = !0),
        typeof n.options.scrollRestoration == "function" &&
          !n.options.scrollRestoration({ location: n.latestLocation }))
      )
        return;
      const _ = s(y.toLocation),
        E = y.fromLocation && s(y.fromLocation);
      if (n.isScrollRestoring && E && E !== _) {
        const x = xi[E];
        if (x) {
          let z = xi[_];
          for (const R in x) {
            if (R === Mi) {
              if (S) continue;
            } else {
              const w = Ac(R);
              if (!w || (S && p && ((g ??= by(p)), g.includes(w)))) continue;
            }
            (z || (z = xi[_] = {}), (z[R] ??= x[R]));
          }
        }
      }
      mr = !0;
      try {
        const x = y.toLocation.hash,
          z = y.toLocation.state.__hashScrollIntoViewOptions ?? !0;
        let R = !1;
        if (S) {
          const w = Rc.get(y.toLocation),
            N = x && z && (w === "PUSH" || w === "REPLACE"),
            X = n.isScrollRestoring ? xi[_] : void 0;
          if (X)
            for (const Y in X) {
              const { scrollX: K, scrollY: J } = X[Y];
              if (Y === Mi) {
                if (N) continue;
                (scrollTo({ top: J, left: K, behavior: m }), (R = !0));
              } else {
                const q = Ac(Y);
                q && ((q.scrollLeft = K), (q.scrollTop = J));
              }
            }
          if (!R && !x) {
            const Y = { top: 0, left: 0, behavior: m };
            if ((scrollTo(Y), p)) {
              g ??= by(p);
              for (const K of g) K.scrollTo(Y);
            }
          }
        }
        !R && x && z && document.getElementById(x)?.scrollIntoView(z);
      } finally {
        mr = !1;
      }
    }));
}
const en = Symbol.for("TSR_DEFERRED_PROMISE");
function sb(n, l) {
  const s = n;
  return (
    s[en] ||
      ((s[en] = { status: "pending" }),
      s
        .then((u) => {
          ((s[en].status = "success"), (s[en].data = u));
        })
        .catch((u) => {
          ((s[en].status = "error"), (s[en].error = { data: KS(u), __isServerError: !0 }));
        })),
    s
  );
}
const rb = "Error preloading route! ☝️";
function gp(n, l) {
  if (n) return typeof n == "string" ? n : n[l];
}
function ub(n) {
  return n?.scriptFormat ?? "module";
}
function ob(n, l, s) {
  const u = cb(l),
    c = gp(s, "script") ?? u.crossOrigin;
  return {
    ...(ub(n) === "iife" ? { rel: "preload", as: "script" } : { rel: "modulepreload" }),
    href: u.href,
    ...(c ? { crossOrigin: c } : {}),
  };
}
function cb(n) {
  return typeof n == "string" ? { href: n, crossOrigin: void 0 } : n;
}
function yr(n, l) {
  if (l.length === 0) return;
  if (l.length === 1) {
    n.push(l[0]);
    return;
  }
  const s = new Set();
  for (const u of l) {
    const c = JSON.stringify(u);
    s.has(c) || (s.add(c), n.push(u));
  }
}
function fb(n) {
  return typeof n == "string" ? { href: n, crossOrigin: void 0 } : n;
}
var vp = class {
    get to() {
      return this._to;
    }
    get id() {
      return this._id;
    }
    get path() {
      return this._path;
    }
    get fullPath() {
      return this._fullPath;
    }
    constructor(n) {
      if (
        ((this.init = (l) => {
          this.originalIndex = l.originalIndex;
          const s = this.options,
            u = !s?.path && !s?.id;
          ((this.parentRoute = this.options.getParentRoute?.()),
            u ? (this._path = Ua) : this.parentRoute || Te());
          let c = u ? Ua : s?.path;
          c && c !== "/" && (c = rp(c));
          const f = s?.id || c;
          let h = u ? Ua : Sr([this.parentRoute.id === "__root__" ? "" : this.parentRoute.id, f]);
          (c === "__root__" && (c = "/"), h !== "__root__" && (h = Sr(["/", h])));
          const y = h === "__root__" ? "/" : Sr([this.parentRoute.fullPath, c]);
          ((this._path = c), (this._id = h), (this._fullPath = y), (this._to = An(y)));
        }),
        (this.addChildren = (l) => this._addFileChildren(l)),
        (this._addFileChildren = (l) => (
          Array.isArray(l) && (this.children = l),
          typeof l == "object" && l !== null && (this.children = Object.values(l)),
          this
        )),
        (this._addFileTypes = () => this),
        (this.updateLoader = (l) => (Object.assign(this.options, l), this)),
        (this.update = (l) => (Object.assign(this.options, l), this)),
        (this.lazy = (l) => ((this.lazyFn = l), this)),
        (this.redirect = (l) => cp({ from: this.fullPath, ...l })),
        (this.options = n || {}),
        (this.isRoot = !n?.getParentRoute),
        n?.id && n?.path)
      )
        throw new Error("Route cannot have both an 'id' and a 'path' option.");
    }
  },
  db = class extends vp {
    constructor(n) {
      super(n);
    }
  },
  hb = ((n) => (
    (n[(n.AggregateError = 1)] = "AggregateError"),
    (n[(n.ArrowFunction = 2)] = "ArrowFunction"),
    (n[(n.ErrorPrototypeStack = 4)] = "ErrorPrototypeStack"),
    (n[(n.ObjectAssign = 8)] = "ObjectAssign"),
    (n[(n.BigIntTypedArray = 16)] = "BigIntTypedArray"),
    (n[(n.RegExp = 32)] = "RegExp"),
    n
  ))(hb || {}),
  xn = Symbol.asyncIterator,
  Sp = Symbol.hasInstance,
  Di = Symbol.isConcatSpreadable,
  Mn = Symbol.iterator,
  bp = Symbol.match,
  _p = Symbol.matchAll,
  Ep = Symbol.replace,
  Tp = Symbol.search,
  Rp = Symbol.species,
  Ap = Symbol.split,
  xp = Symbol.toPrimitive,
  Li = Symbol.toStringTag,
  Mp = Symbol.unscopables,
  Cp = {
    [xn]: 0,
    [Sp]: 1,
    [Di]: 2,
    [Mn]: 3,
    [bp]: 4,
    [_p]: 5,
    [Ep]: 6,
    [Tp]: 7,
    [Rp]: 8,
    [Ap]: 9,
    [xp]: 10,
    [Li]: 11,
    [Mp]: 12,
  },
  mb = {
    0: xn,
    1: Sp,
    2: Di,
    3: Mn,
    4: bp,
    5: _p,
    6: Ep,
    7: Tp,
    8: Rp,
    9: Ap,
    10: xp,
    11: Li,
    12: Mp,
  },
  b = void 0,
  yb = {
    2: !0,
    3: !1,
    1: b,
    0: null,
    4: -0,
    5: Number.POSITIVE_INFINITY,
    6: Number.NEGATIVE_INFINITY,
    7: Number.NaN,
  },
  pb = {
    0: "Error",
    1: "EvalError",
    2: "RangeError",
    3: "ReferenceError",
    4: "SyntaxError",
    5: "TypeError",
    6: "URIError",
  },
  gb = {
    0: Error,
    1: EvalError,
    2: RangeError,
    3: ReferenceError,
    4: SyntaxError,
    5: TypeError,
    6: URIError,
  };
function _t(n, l, s, u, c, f, h, y, m, p, S, g) {
  return { t: n, i: l, s, c: u, m: c, p: f, e: h, a: y, f: m, b: p, o: S, l: g };
}
function ra(n) {
  return _t(2, b, n, b, b, b, b, b, b, b, b, b);
}
var wp = ra(2),
  Op = ra(3),
  vb = ra(1),
  Sb = ra(0),
  bb = ra(4),
  _b = ra(5),
  Eb = ra(6),
  Tb = ra(7);
function Rb(n) {
  switch (n) {
    case '"':
      return '\\"';
    case "\\":
      return "\\\\";
    case `
`:
      return "\\n";
    case "\r":
      return "\\r";
    case "\b":
      return "\\b";
    case "	":
      return "\\t";
    case "\f":
      return "\\f";
    case "<":
      return "\\x3C";
    case "\u2028":
      return "\\u2028";
    case "\u2029":
      return "\\u2029";
    default:
      return b;
  }
}
function ua(n) {
  let l = "",
    s = 0,
    u;
  for (let c = 0, f = n.length; c < f; c++)
    ((u = Rb(n[c])), u && ((l += n.slice(s, c) + u), (s = c + 1)));
  return (s === 0 ? (l = n) : (l += n.slice(s)), l);
}
function Ab(n) {
  switch (n) {
    case "\\\\":
      return "\\";
    case '\\"':
      return '"';
    case "\\n":
      return `
`;
    case "\\r":
      return "\r";
    case "\\b":
      return "\b";
    case "\\t":
      return "	";
    case "\\f":
      return "\f";
    case "\\x3C":
      return "<";
    case "\\u2028":
      return "\u2028";
    case "\\u2029":
      return "\u2029";
    default:
      return n;
  }
}
function oa(n) {
  return n.replace(/(\\\\|\\"|\\n|\\r|\\b|\\t|\\f|\\u2028|\\u2029|\\x3C)/g, Ab);
}
var pr = "__SEROVAL_REFS__",
  zp = new Map(),
  wi = new Map();
function Dp(n) {
  return zp.has(n);
}
function xb(n) {
  return wi.has(n);
}
function Mb(n) {
  if (Dp(n)) return zp.get(n);
  throw new i1(n);
}
function Cb(n) {
  if (xb(n)) return wi.get(n);
  throw new l1(n);
}
typeof globalThis < "u"
  ? Object.defineProperty(globalThis, pr, {
      value: wi,
      configurable: !0,
      writable: !1,
      enumerable: !1,
    })
  : typeof window < "u"
    ? Object.defineProperty(window, pr, {
        value: wi,
        configurable: !0,
        writable: !1,
        enumerable: !1,
      })
    : typeof self < "u"
      ? Object.defineProperty(self, pr, {
          value: wi,
          configurable: !0,
          writable: !1,
          enumerable: !1,
        })
      : typeof global < "u" &&
        Object.defineProperty(global, pr, {
          value: wi,
          configurable: !0,
          writable: !1,
          enumerable: !1,
        });
function Hc(n) {
  return n instanceof EvalError
    ? 1
    : n instanceof RangeError
      ? 2
      : n instanceof ReferenceError
        ? 3
        : n instanceof SyntaxError
          ? 4
          : n instanceof TypeError
            ? 5
            : n instanceof URIError
              ? 6
              : 0;
}
function wb(n) {
  let l = pb[Hc(n)];
  return n.name !== l
    ? { name: n.name }
    : n.constructor.name !== l
      ? { name: n.constructor.name }
      : {};
}
function Lp(n, l) {
  let s = wb(n),
    u = Object.getOwnPropertyNames(n);
  for (let c = 0, f = u.length, h; c < f; c++)
    ((h = u[c]),
      h !== "name" &&
        h !== "message" &&
        (h === "stack" ? l & 4 && ((s = s || {}), (s[h] = n[h])) : ((s = s || {}), (s[h] = n[h]))));
  return s;
}
function Up(n) {
  return Object.isFrozen(n) ? 3 : Object.isSealed(n) ? 2 : Object.isExtensible(n) ? 0 : 1;
}
function Ob(n) {
  switch (n) {
    case Number.POSITIVE_INFINITY:
      return _b;
    case Number.NEGATIVE_INFINITY:
      return Eb;
  }
  return n !== n ? Tb : Object.is(n, -0) ? bb : _t(0, b, n, b, b, b, b, b, b, b, b, b);
}
function Bp(n) {
  return _t(1, b, ua(n), b, b, b, b, b, b, b, b, b);
}
function zb(n) {
  return _t(3, b, "" + n, b, b, b, b, b, b, b, b, b);
}
function Db(n) {
  return _t(4, n, b, b, b, b, b, b, b, b, b, b);
}
function Lb(n, l) {
  let s = l.valueOf();
  return _t(5, n, s !== s ? "" : l.toISOString(), b, b, b, b, b, b, b, b, b);
}
function Ub(n, l) {
  return _t(6, n, b, ua(l.source), l.flags, b, b, b, b, b, b, b);
}
function Bb(n, l) {
  return _t(17, n, Cp[l], b, b, b, b, b, b, b, b, b);
}
function Nb(n, l) {
  return _t(18, n, ua(Mb(l)), b, b, b, b, b, b, b, b, b);
}
function jb(n, l, s) {
  return _t(25, n, s, ua(l), b, b, b, b, b, b, b, b);
}
function Hb(n, l, s) {
  return _t(9, n, b, b, b, b, b, s, b, b, Up(l), b);
}
function qb(n, l) {
  return _t(21, n, b, b, b, b, b, b, l, b, b, b);
}
function Yb(n, l, s) {
  return _t(15, n, b, l.constructor.name, b, b, b, b, s, l.byteOffset, b, l.length);
}
function Gb(n, l, s) {
  return _t(16, n, b, l.constructor.name, b, b, b, b, s, l.byteOffset, b, l.byteLength);
}
function Qb(n, l, s) {
  return _t(20, n, b, b, b, b, b, b, s, l.byteOffset, b, l.byteLength);
}
function Vb(n, l, s) {
  return _t(13, n, Hc(l), b, ua(l.message), s, b, b, b, b, b, b);
}
function Xb(n, l, s) {
  return _t(14, n, Hc(l), b, ua(l.message), s, b, b, b, b, b, b);
}
function Kb(n, l) {
  return _t(7, n, b, b, b, b, b, l, b, b, b, b);
}
function Zb(n, l) {
  return _t(28, b, b, b, b, b, b, [n, l], b, b, b, b);
}
function Pb(n, l) {
  return _t(30, b, b, b, b, b, b, [n, l], b, b, b, b);
}
function Fb(n, l, s) {
  return _t(31, n, b, b, b, b, b, s, l, b, b, b);
}
function Jb(n, l) {
  return _t(32, n, b, b, b, b, b, b, l, b, b, b);
}
function kb(n, l) {
  return _t(33, n, b, b, b, b, b, b, l, b, b, b);
}
function Ib(n, l) {
  return _t(34, n, b, b, b, b, b, b, l, b, b, b);
}
function $b(n, l, s, u) {
  return _t(35, n, s, b, b, b, b, l, b, b, b, u);
}
var Wb = { parsing: 1, serialization: 2, deserialization: 3 };
function t1(n) {
  return `Seroval Error (step: ${Wb[n]})`;
}
var e1 = (n, l) => t1(n),
  Np = class extends Error {
    constructor(n, l) {
      (super(e1(n)), (this.cause = l));
    }
  },
  _y = class extends Np {
    constructor(n) {
      super("parsing", n);
    }
  },
  n1 = class extends Np {
    constructor(n) {
      super("deserialization", n);
    }
  };
function Cn(n) {
  return `Seroval Error (specific: ${n})`;
}
var Br = class extends Error {
    constructor(l) {
      (super(Cn(1)), (this.value = l));
    }
  },
  jp = class extends Error {
    constructor(n) {
      super(Cn(2));
    }
  },
  a1 = class extends Error {
    constructor(n) {
      super(Cn(3));
    }
  },
  Zl = class extends Error {
    constructor(n) {
      super(Cn(4));
    }
  },
  i1 = class extends Error {
    constructor(n) {
      (super(Cn(5)), (this.value = n));
    }
  },
  l1 = class extends Error {
    constructor(n) {
      super(Cn(6));
    }
  },
  s1 = class extends Error {
    constructor(n) {
      super(Cn(7));
    }
  },
  wn = class extends Error {
    constructor(n) {
      super(Cn(8));
    }
  },
  r1 = class extends Error {
    constructor(n) {
      super(Cn(9));
    }
  },
  u1 = class {
    constructor(n, l) {
      ((this.value = n), (this.replacement = l));
    }
  },
  Nr = () => {
    let n = { p: 0, s: 0, f: 0 };
    return (
      (n.p = new Promise((l, s) => {
        ((n.s = l), (n.f = s));
      })),
      n
    );
  },
  o1 = (n, l) => {
    (n.s(l), (n.p.s = 1), (n.p.v = l));
  },
  c1 = (n, l) => {
    (n.f(l), (n.p.s = 2), (n.p.v = l));
  };
Nr.toString();
o1.toString();
c1.toString();
var f1 = () => {
    let n = [],
      l = [],
      s = !0,
      u = !1,
      c = 0,
      f = (m, p, S) => {
        for (S = 0; S < c; S++) l[S] && l[S][p](m);
      },
      h = (m, p, S, g) => {
        for (p = 0, S = n.length; p < S; p++)
          ((g = n[p]), !s && p === S - 1 ? m[u ? "return" : "throw"](g) : m.next(g));
      },
      y = (m, p) => (
        s && ((p = c++), (l[p] = m)),
        h(m),
        () => {
          s && ((l[p] = l[c]), (l[c--] = void 0));
        }
      );
    return {
      __SEROVAL_STREAM__: !0,
      on: (m) => y(m),
      next: (m) => {
        s && (n.push(m), f(m, "next"));
      },
      throw: (m) => {
        s && (n.push(m), f(m, "throw"), (s = !1), (u = !1), (l.length = 0));
      },
      return: (m) => {
        s && (n.push(m), f(m, "return"), (s = !1), (u = !0), (l.length = 0));
      },
    };
  },
  d1 = (n) => (l) => () => {
    let s = 0,
      u = {
        [n]: () => u,
        next: () => {
          if (s > l.d) return { done: !0, value: void 0 };
          let c = s++,
            f = l.v[c];
          if (c === l.t) throw f;
          return { done: c === l.d, value: f };
        },
      };
    return u;
  },
  h1 = (n, l) => (s) => () => {
    let u = 0,
      c = -1,
      f = !1,
      h = [],
      y = [],
      m = (S = 0, g = y.length) => {
        for (; S < g; S++) y[S].s({ done: !0, value: void 0 });
      };
    s.on({
      next: (S) => {
        let g = y.shift();
        (g && g.s({ done: !1, value: S }), h.push(S));
      },
      throw: (S) => {
        let g = y.shift();
        (g && g.f(S), m(), (c = h.length), (f = !0), h.push(S));
      },
      return: (S) => {
        let g = y.shift();
        (g && g.s({ done: !0, value: S }), m(), (c = h.length), h.push(S));
      },
    });
    let p = {
      [n]: () => p,
      next: () => {
        if (c === -1) {
          let _ = u++;
          if (_ >= h.length) {
            let E = l();
            return (y.push(E), E.p);
          }
          return { done: !1, value: h[_] };
        }
        if (u > c) return { done: !0, value: void 0 };
        let S = u++,
          g = h[S];
        if (S !== c) return { done: !1, value: g };
        if (f) throw g;
        return { done: !0, value: g };
      },
    };
    return p;
  },
  Hp = (n) => {
    let l = atob(n),
      s = l.length,
      u = new Uint8Array(s);
    for (let c = 0; c < s; c++) u[c] = l.charCodeAt(c);
    return u.buffer;
  };
Hp.toString();
function m1(n) {
  return "__SEROVAL_SEQUENCE__" in n;
}
function qp(n, l, s) {
  return { __SEROVAL_SEQUENCE__: !0, v: n, t: l, d: s };
}
function y1(n) {
  let l = [],
    s = -1,
    u = -1,
    c = n[Mn]();
  for (;;)
    try {
      let f = c.next();
      if ((l.push(f.value), f.done)) {
        u = l.length - 1;
        break;
      }
    } catch (f) {
      ((s = l.length), l.push(f));
    }
  return qp(l, s, u);
}
var p1 = d1(Mn);
function g1(n) {
  return p1(n);
}
var v1 = {},
  S1 = {},
  b1 = { 0: {}, 1: {}, 2: {}, 3: {}, 4: {}, 5: {} };
function _1(n) {
  return "__SEROVAL_STREAM__" in n;
}
function ja() {
  return f1();
}
function E1(n) {
  let l = ja(),
    s = n[xn]();
  async function u() {
    try {
      let c = await s.next();
      c.done ? l.return(c.value) : (l.next(c.value), await u());
    } catch (c) {
      l.throw(c);
    }
  }
  return (u().catch(() => {}), l);
}
var T1 = h1(xn, Nr);
function R1(n) {
  return T1(n);
}
async function A1(n) {
  try {
    return [1, await n];
  } catch (l) {
    return [0, l];
  }
}
function x1(n, l) {
  return {
    plugins: l.plugins,
    mode: n,
    marked: new Set(),
    features: 63 ^ (l.disabledFeatures || 0),
    refs: l.refs || new Map(),
    depthLimit: l.depthLimit || 1e3,
  };
}
function Er(n, l) {
  n.marked.add(l);
}
function M1(n, l) {
  let s = n.refs.size;
  return (n.refs.set(l, s), s);
}
function jr(n, l) {
  let s = n.refs.get(l);
  return s != null ? (Er(n, s), { type: 1, value: Db(s) }) : { type: 0, value: M1(n, l) };
}
function qc(n, l) {
  let s = jr(n, l);
  return s.type === 1 ? s : Dp(l) ? { type: 2, value: Nb(s.value, l) } : s;
}
function La(n, l) {
  let s = qc(n, l);
  if (s.type !== 0) return s.value;
  if (l in Cp) return Bb(s.value, l);
  throw new Br(l);
}
function Hr(n, l) {
  let s = jr(n, b1[l]);
  return s.type === 1 ? s.value : _t(26, s.value, l, b, b, b, b, b, b, b, b, b);
}
function C1(n) {
  let l = jr(n, v1);
  return l.type === 1 ? l.value : _t(27, l.value, b, b, b, b, b, b, La(n, Mn), b, b, b);
}
function w1(n) {
  let l = jr(n, S1);
  return l.type === 1 ? l.value : _t(29, l.value, b, b, b, b, b, [Hr(n, 1), La(n, xn)], b, b, b, b);
}
function O1(n, l, s, u) {
  return _t(s ? 11 : 10, n, b, b, b, u, b, b, b, b, Up(l), b);
}
function z1(n, l, s, u) {
  return _t(8, l, b, b, b, b, { k: s, v: u }, b, Hr(n, 0), b, b, b);
}
function D1(n, l, s) {
  let u = new Uint8Array(s),
    c = "";
  for (let f = 0, h = u.length; f < h; f++) c += String.fromCharCode(u[f]);
  return _t(19, l, ua(btoa(c)), b, b, b, b, b, Hr(n, 5), b, b, b);
}
function L1(n, l) {
  return { base: x1(n, l), child: void 0 };
}
var U1 = class {
  constructor(n, l) {
    ((this._p = n), (this.depth = l));
  }
  parse(n) {
    return It(this._p, this.depth, n);
  }
};
async function B1(n, l, s) {
  let u = [];
  for (let c = 0, f = s.length; c < f; c++) c in s ? (u[c] = await It(n, l, s[c])) : (u[c] = 0);
  return u;
}
async function N1(n, l, s, u) {
  return Hb(s, u, await B1(n, l, u));
}
async function Yc(n, l, s) {
  let u = Object.entries(s),
    c = [],
    f = [];
  for (let h = 0, y = u.length; h < y; h++) (c.push(ua(u[h][0])), f.push(await It(n, l, u[h][1])));
  return (
    Mn in s && (c.push(La(n.base, Mn)), f.push(Zb(C1(n.base), await It(n, l, y1(s))))),
    xn in s && (c.push(La(n.base, xn)), f.push(Pb(w1(n.base), await It(n, l, E1(s))))),
    Li in s && (c.push(La(n.base, Li)), f.push(Bp(s[Li]))),
    Di in s && (c.push(La(n.base, Di)), f.push(s[Di] ? wp : Op)),
    { k: c, v: f }
  );
}
async function hc(n, l, s, u, c) {
  return O1(s, u, c, await Yc(n, l, u));
}
async function j1(n, l, s, u) {
  return qb(s, await It(n, l, u.valueOf()));
}
async function H1(n, l, s, u) {
  return Yb(s, u, await It(n, l, u.buffer));
}
async function q1(n, l, s, u) {
  return Gb(s, u, await It(n, l, u.buffer));
}
async function Y1(n, l, s, u) {
  return Qb(s, u, await It(n, l, u.buffer));
}
async function Ey(n, l, s, u) {
  let c = Lp(u, n.base.features);
  return Vb(s, u, c ? await Yc(n, l, c) : b);
}
async function G1(n, l, s, u) {
  let c = Lp(u, n.base.features);
  return Xb(s, u, c ? await Yc(n, l, c) : b);
}
async function Q1(n, l, s, u) {
  let c = [],
    f = [];
  for (let [h, y] of u.entries()) (c.push(await It(n, l, h)), f.push(await It(n, l, y)));
  return z1(n.base, s, c, f);
}
async function V1(n, l, s, u) {
  let c = [];
  for (let f of u.keys()) c.push(await It(n, l, f));
  return Kb(s, c);
}
async function Yp(n, l, s, u) {
  let c = n.base.plugins;
  if (c)
    for (let f = 0, h = c.length; f < h; f++) {
      let y = c[f];
      if (y.parse.async && y.test(u))
        return jb(s, y.tag, await y.parse.async(u, new U1(n, l), { id: s }));
    }
  return b;
}
async function X1(n, l, s, u) {
  let [c, f] = await A1(u);
  return _t(12, s, c, b, b, b, b, b, await It(n, l, f), b, b, b);
}
function K1(n, l, s, u, c) {
  let f = [],
    h = s.on({
      next: (y) => {
        (Er(this.base, l),
          It(this, n, y).then(
            (m) => {
              f.push(Jb(l, m));
            },
            (m) => {
              (c(m), h());
            },
          ));
      },
      throw: (y) => {
        (Er(this.base, l),
          It(this, n, y).then(
            (m) => {
              (f.push(kb(l, m)), u(f), h());
            },
            (m) => {
              (c(m), h());
            },
          ));
      },
      return: (y) => {
        (Er(this.base, l),
          It(this, n, y).then(
            (m) => {
              (f.push(Ib(l, m)), u(f), h());
            },
            (m) => {
              (c(m), h());
            },
          ));
      },
    });
}
async function Z1(n, l, s, u) {
  return Fb(s, Hr(n.base, 4), await new Promise(K1.bind(n, l, s, u)));
}
async function P1(n, l, s, u) {
  let c = [];
  for (let f = 0, h = u.v.length; f < h; f++) c[f] = await It(n, l, u.v[f]);
  return $b(s, c, u.t, u.d);
}
async function F1(n, l, s, u) {
  if (Array.isArray(u)) return N1(n, l, s, u);
  if (_1(u)) return Z1(n, l, s, u);
  if (m1(u)) return P1(n, l, s, u);
  let c = u.constructor;
  if (c === u1) return It(n, l, u.replacement);
  let f = await Yp(n, l, s, u);
  if (f) return f;
  switch (c) {
    case Object:
      return hc(n, l, s, u, !1);
    case b:
      return hc(n, l, s, u, !0);
    case Date:
      return Lb(s, u);
    case Error:
    case EvalError:
    case RangeError:
    case ReferenceError:
    case SyntaxError:
    case TypeError:
    case URIError:
      return Ey(n, l, s, u);
    case Number:
    case Boolean:
    case String:
    case BigInt:
      return j1(n, l, s, u);
    case ArrayBuffer:
      return D1(n.base, s, u);
    case Int8Array:
    case Int16Array:
    case Int32Array:
    case Uint8Array:
    case Uint16Array:
    case Uint32Array:
    case Uint8ClampedArray:
    case Float32Array:
    case Float64Array:
      return H1(n, l, s, u);
    case DataView:
      return Y1(n, l, s, u);
    case Map:
      return Q1(n, l, s, u);
    case Set:
      return V1(n, l, s, u);
  }
  if (c === Promise || u instanceof Promise) return X1(n, l, s, u);
  let h = n.base.features;
  if (h & 32 && c === RegExp) return Ub(s, u);
  if (h & 16)
    switch (c) {
      case BigInt64Array:
      case BigUint64Array:
        return q1(n, l, s, u);
    }
  if (h & 1 && typeof AggregateError < "u" && (c === AggregateError || u instanceof AggregateError))
    return G1(n, l, s, u);
  if (u instanceof Error) return Ey(n, l, s, u);
  if (Mn in u || xn in u) return hc(n, l, s, u, !!c);
  throw new Br(u);
}
async function J1(n, l, s) {
  let u = qc(n.base, s);
  if (u.type !== 0) return u.value;
  let c = await Yp(n, l, u.value, s);
  if (c) return c;
  throw new Br(s);
}
async function It(n, l, s) {
  switch (typeof s) {
    case "boolean":
      return s ? wp : Op;
    case "undefined":
      return vb;
    case "string":
      return Bp(s);
    case "number":
      return Ob(s);
    case "bigint":
      return zb(s);
    case "object": {
      if (s) {
        let u = qc(n.base, s);
        return u.type === 0 ? await F1(n, l + 1, u.value, s) : u.value;
      }
      return Sb;
    }
    case "symbol":
      return La(n.base, s);
    case "function":
      return J1(n, l, s);
    default:
      throw new Br(s);
  }
}
async function k1(n, l) {
  try {
    return await It(n, 0, l);
  } catch (s) {
    throw s instanceof _y ? s : new _y(s);
  }
}
var I1 = ((n) => ((n[(n.Vanilla = 1)] = "Vanilla"), (n[(n.Cross = 2)] = "Cross"), n))(I1 || {});
function Gp(n, l) {
  for (let s = 0, u = l.length; s < u; s++) {
    let c = l[s];
    n.has(c) || (n.add(c), c.extends && Gp(n, c.extends));
  }
}
function Qp(n) {
  if (n) {
    let l = new Set();
    return (Gp(l, n), [...l]);
  }
}
function $1(n) {
  switch (n) {
    case "Int8Array":
      return Int8Array;
    case "Int16Array":
      return Int16Array;
    case "Int32Array":
      return Int32Array;
    case "Uint8Array":
      return Uint8Array;
    case "Uint16Array":
      return Uint16Array;
    case "Uint32Array":
      return Uint32Array;
    case "Uint8ClampedArray":
      return Uint8ClampedArray;
    case "Float32Array":
      return Float32Array;
    case "Float64Array":
      return Float64Array;
    case "BigInt64Array":
      return BigInt64Array;
    case "BigUint64Array":
      return BigUint64Array;
    default:
      throw new s1(n);
  }
}
var W1 = 1e6,
  t_ = 1e4,
  e_ = 2e4;
function Vp(n, l) {
  switch (l) {
    case 3:
      return Object.freeze(n);
    case 1:
      return Object.preventExtensions(n);
    case 2:
      return Object.seal(n);
    default:
      return n;
  }
}
var n_ = 1e3;
function a_(n, l) {
  var s;
  let u = l.refs || new Map();
  return (
    "types" in u || Object.assign(u, { types: new Map() }),
    {
      mode: n,
      plugins: l.plugins,
      refs: u,
      features: (s = l.features) != null ? s : 63 ^ (l.disabledFeatures || 0),
      depthLimit: l.depthLimit || n_,
    }
  );
}
function i_(n) {
  return { mode: 2, base: a_(2, n), child: b };
}
var l_ = class {
  constructor(n, l) {
    ((this._p = n), (this.depth = l));
  }
  deserialize(n) {
    return Dt(this._p, this.depth, n);
  }
};
function Xp(n, l) {
  if (l < 0 || !Number.isFinite(l) || !Number.isInteger(l)) throw new wn({ t: 4, i: l });
  if (n.refs.has(l)) throw new Error("Conflicted ref id: " + l);
}
function s_(n, l, s) {
  return (Xp(n.base, l), n.state.marked.has(l) && n.base.refs.set(l, s), s);
}
function r_(n, l, s) {
  return (Xp(n.base, l), n.base.refs.set(l, s), s);
}
function $t(n, l, s) {
  return n.mode === 1 ? s_(n, l, s) : r_(n, l, s);
}
function xc(n, l, s) {
  if (Object.hasOwn(l, s)) return l[s];
  throw new wn(n);
}
function u_(n, l) {
  return $t(n, l.i, Cb(oa(l.s)));
}
function o_(n, l, s) {
  let u = s.a,
    c = u.length,
    f = $t(n, s.i, new Array(c));
  for (let h = 0, y; h < c; h++) ((y = u[h]), y && (f[h] = Dt(n, l, y)));
  return (Vp(f, s.o), f);
}
function c_(n) {
  switch (n) {
    case "constructor":
    case "__proto__":
    case "prototype":
    case "__defineGetter__":
    case "__defineSetter__":
    case "__lookupGetter__":
    case "__lookupSetter__":
      return !1;
    default:
      return !0;
  }
}
function f_(n) {
  switch (n) {
    case xn:
    case Di:
    case Li:
    case Mn:
      return !0;
    default:
      return !1;
  }
}
function Ty(n, l, s) {
  c_(l)
    ? (n[l] = s)
    : Object.defineProperty(n, l, { value: s, configurable: !0, enumerable: !0, writable: !0 });
}
function d_(n, l, s, u, c) {
  if (typeof u == "string") Ty(s, oa(u), Dt(n, l, c));
  else {
    let f = Dt(n, l, u);
    switch (typeof f) {
      case "string":
        Ty(s, f, Dt(n, l, c));
        break;
      case "symbol":
        f_(f) && (s[f] = Dt(n, l, c));
        break;
      default:
        throw new wn(u);
    }
  }
}
function Kp(n, l, s) {
  n.base.refs.types.set(l, s);
}
function Pl(n, l, s, u) {
  if (n.base.refs.types.get(s) !== u) throw new wn(l);
}
function Zp(n, l, s, u) {
  let c = s.k;
  if (c.length > 0) for (let f = 0, h = s.v, y = c.length; f < y; f++) d_(n, l, u, c[f], h[f]);
  return u;
}
function h_(n, l, s) {
  let u = $t(n, s.i, s.t === 10 ? {} : Object.create(null));
  return (Zp(n, l, s.p, u), Vp(u, s.o), u);
}
function m_(n, l) {
  return $t(n, l.i, new Date(l.s));
}
function y_(n, l) {
  if (n.base.features & 32) {
    let s = oa(l.c);
    if (s.length > e_) throw new wn(l);
    return $t(n, l.i, new RegExp(s, l.m));
  }
  throw new jp(l);
}
function p_(n, l, s) {
  let u = $t(n, s.i, new Set());
  for (let c = 0, f = s.a, h = f.length; c < h; c++) u.add(Dt(n, l, f[c]));
  return u;
}
function g_(n, l, s) {
  let u = $t(n, s.i, new Map());
  for (let c = 0, f = s.e.k, h = s.e.v, y = f.length; c < y; c++)
    u.set(Dt(n, l, f[c]), Dt(n, l, h[c]));
  return u;
}
function v_(n, l) {
  if (l.s.length > W1) throw new wn(l);
  return $t(n, l.i, Hp(oa(l.s)));
}
function S_(n, l, s) {
  var u;
  let c = $1(s.c),
    f = Dt(n, l, s.f),
    h = (u = s.b) != null ? u : 0;
  if (h < 0 || h > f.byteLength) throw new wn(s);
  return $t(n, s.i, new c(f, h, s.l));
}
function b_(n, l, s) {
  var u;
  let c = Dt(n, l, s.f),
    f = (u = s.b) != null ? u : 0;
  if (f < 0 || f > c.byteLength) throw new wn(s);
  return $t(n, s.i, new DataView(c, f, s.l));
}
function Pp(n, l, s, u) {
  if (s.p) {
    let c = Zp(n, l, s.p, {});
    Object.defineProperties(u, Object.getOwnPropertyDescriptors(c));
  }
  return u;
}
function __(n, l, s) {
  let u = $t(n, s.i, new AggregateError([], oa(s.m)));
  return Pp(n, l, s, u);
}
function E_(n, l, s) {
  let u = xc(s, gb, s.s),
    c = $t(n, s.i, new u(oa(s.m)));
  return Pp(n, l, s, c);
}
function T_(n, l, s) {
  let u = Nr(),
    c = $t(n, s.i, u.p),
    f = Dt(n, l, s.f);
  return (s.s ? u.s(f) : u.f(f), c);
}
function R_(n, l, s) {
  return $t(n, s.i, Object(Dt(n, l, s.f)));
}
function A_(n, l, s) {
  let u = n.base.plugins;
  if (u) {
    let c = oa(s.c);
    for (let f = 0, h = u.length; f < h; f++) {
      let y = u[f];
      if (y.tag === c) return $t(n, s.i, y.deserialize(s.s, new l_(n, l), { id: s.i }));
    }
  }
  throw new a1(s.c);
}
function x_(n, l) {
  let s = $t(n, l.i, $t(n, l.s, Nr()).p);
  return (Kp(n, l.s, 22), s);
}
function M_(n, l, s) {
  let u = n.base.refs.get(s.i);
  if (u) return (Pl(n, s, s.i, 22), u.s(Dt(n, l, s.a[1])), b);
  throw new Zl("Promise");
}
function C_(n, l, s) {
  let u = n.base.refs.get(s.i);
  if (u) return (Pl(n, s, s.i, 22), u.f(Dt(n, l, s.a[1])), b);
  throw new Zl("Promise");
}
function w_(n, l, s) {
  Dt(n, l, s.a[0]);
  let u = Dt(n, l, s.a[1]);
  return g1(u);
}
function O_(n, l, s) {
  Dt(n, l, s.a[0]);
  let u = Dt(n, l, s.a[1]);
  return R1(u);
}
function z_(n, l, s) {
  let u = $t(n, s.i, ja());
  Kp(n, s.i, 31);
  let c = s.a,
    f = c.length;
  if (f) for (let h = 0; h < f; h++) Dt(n, l, c[h]);
  return u;
}
function D_(n, l, s) {
  let u = n.base.refs.get(s.i);
  if (u) return (Pl(n, s, s.i, 31), u.next(Dt(n, l, s.f)), b);
  throw new Zl("Stream");
}
function L_(n, l, s) {
  let u = n.base.refs.get(s.i);
  if (u) return (Pl(n, s, s.i, 31), u.throw(Dt(n, l, s.f)), b);
  throw new Zl("Stream");
}
function U_(n, l, s) {
  let u = n.base.refs.get(s.i);
  if (u) return (Pl(n, s, s.i, 31), u.return(Dt(n, l, s.f)), b);
  throw new Zl("Stream");
}
function B_(n, l, s) {
  return (Dt(n, l, s.f), b);
}
function N_(n, l, s) {
  return (Dt(n, l, s.a[1]), b);
}
function j_(n, l, s) {
  let u = $t(n, s.i, qp([], s.s, s.l));
  for (let c = 0, f = s.a.length; c < f; c++) u.v[c] = Dt(n, l, s.a[c]);
  return u;
}
function Dt(n, l, s) {
  if (l > n.base.depthLimit) throw new r1(n.base.depthLimit);
  switch (((l += 1), s.t)) {
    case 2:
      return xc(s, yb, s.s);
    case 0:
      return Number(s.s);
    case 1:
      return oa(String(s.s));
    case 3:
      if (String(s.s).length > t_) throw new wn(s);
      return BigInt(s.s);
    case 4:
      return n.base.refs.get(s.i);
    case 18:
      return u_(n, s);
    case 9:
      return o_(n, l, s);
    case 10:
    case 11:
      return h_(n, l, s);
    case 5:
      return m_(n, s);
    case 6:
      return y_(n, s);
    case 7:
      return p_(n, l, s);
    case 8:
      return g_(n, l, s);
    case 19:
      return v_(n, s);
    case 16:
    case 15:
      return S_(n, l, s);
    case 20:
      return b_(n, l, s);
    case 14:
      return __(n, l, s);
    case 13:
      return E_(n, l, s);
    case 12:
      return T_(n, l, s);
    case 17:
      return xc(s, mb, s.s);
    case 21:
      return R_(n, l, s);
    case 25:
      return A_(n, l, s);
    case 22:
      return x_(n, s);
    case 23:
      return M_(n, l, s);
    case 24:
      return C_(n, l, s);
    case 28:
      return w_(n, l, s);
    case 30:
      return O_(n, l, s);
    case 31:
      return z_(n, l, s);
    case 32:
      return D_(n, l, s);
    case 33:
      return L_(n, l, s);
    case 34:
      return U_(n, l, s);
    case 27:
      return B_(n, l, s);
    case 29:
      return N_(n, l, s);
    case 35:
      return j_(n, l, s);
    default:
      throw new jp(s);
  }
}
function H_(n, l) {
  try {
    return Dt(n, 0, l);
  } catch (s) {
    throw new n1(s);
  }
}
var q_ = () => T;
q_.toString();
function Ry(n, l) {
  let s = Qp(l.plugins),
    u = i_({
      plugins: s,
      refs: l.refs,
      features: l.features,
      disabledFeatures: l.disabledFeatures,
      depthLimit: l.depthLimit,
    });
  return H_(u, n);
}
async function Y_(n, l = {}) {
  let s = Qp(l.plugins),
    u = L1(1, { plugins: s, disabledFeatures: l.disabledFeatures });
  return { t: await k1(u, n), f: u.base.features, m: Array.from(u.base.marked) };
}
function G_(n) {
  return {
    tag: "$TSR/t/" + n.key,
    test: n.test,
    parse: {
      sync(l, s, u) {
        return { v: s.parse(n.toSerializable(l)) };
      },
      async async(l, s, u) {
        return { v: await s.parse(n.toSerializable(l)) };
      },
      stream(l, s, u) {
        return { v: s.parse(n.toSerializable(l)) };
      },
    },
    serialize: void 0,
    deserialize(l, s, u) {
      return n.fromSerializable(s.deserialize(l.v));
    },
  };
}
var Q_ = class {
  constructor(n, l) {
    ((this.stream = n), (this.hint = l?.hint ?? "binary"));
  }
};
const wr = globalThis.Buffer,
  Fp = !!wr && typeof wr.from == "function";
function Jp(n) {
  if (n.length === 0) return "";
  if (Fp) return wr.from(n).toString("base64");
  const l = 32768,
    s = [];
  for (let u = 0; u < n.length; u += l) {
    const c = n.subarray(u, u + l);
    s.push(String.fromCharCode.apply(null, c));
  }
  return btoa(s.join(""));
}
function kp(n) {
  if (n.length === 0) return new Uint8Array(0);
  if (Fp) {
    const u = wr.from(n, "base64");
    return new Uint8Array(u.buffer, u.byteOffset, u.byteLength);
  }
  const l = atob(n),
    s = new Uint8Array(l.length);
  for (let u = 0; u < l.length; u++) s[u] = l.charCodeAt(u);
  return s;
}
const Ll = Object.create(null),
  Ul = Object.create(null),
  V_ = (n) =>
    new ReadableStream({
      start(l) {
        n.on({
          next(s) {
            try {
              l.enqueue(kp(s));
            } catch {}
          },
          throw(s) {
            l.error(s);
          },
          return() {
            try {
              l.close();
            } catch {}
          },
        });
      },
    }),
  X_ = new TextEncoder(),
  K_ = (n) =>
    new ReadableStream({
      start(l) {
        n.on({
          next(s) {
            try {
              typeof s == "string" ? l.enqueue(X_.encode(s)) : l.enqueue(kp(s.$b64));
            } catch {}
          },
          throw(s) {
            l.error(s);
          },
          return() {
            try {
              l.close();
            } catch {}
          },
        });
      },
    }),
  Z_ =
    "(s=>new ReadableStream({start(c){s.on({next(b){try{const d=atob(b),a=new Uint8Array(d.length);for(let i=0;i<d.length;i++)a[i]=d.charCodeAt(i);c.enqueue(a)}catch(_){}},throw(e){c.error(e)},return(){try{c.close()}catch(_){}}})}}))",
  P_ =
    "(s=>{const e=new TextEncoder();return new ReadableStream({start(c){s.on({next(v){try{if(typeof v==='string'){c.enqueue(e.encode(v))}else{const d=atob(v.$b64),a=new Uint8Array(d.length);for(let i=0;i<d.length;i++)a[i]=d.charCodeAt(i);c.enqueue(a)}}catch(_){}},throw(x){c.error(x)},return(){try{c.close()}catch(_){}}})}})})";
function Ay(n) {
  const l = ja(),
    s = n.getReader();
  return (
    (async () => {
      try {
        for (;;) {
          const { done: u, value: c } = await s.read();
          if (u) {
            l.return(void 0);
            break;
          }
          l.next(Jp(c));
        }
      } catch (u) {
        l.throw(u);
      } finally {
        s.releaseLock();
      }
    })(),
    l
  );
}
function xy(n) {
  const l = ja(),
    s = n.getReader(),
    u = new TextDecoder("utf-8", { fatal: !0 });
  return (
    (async () => {
      try {
        for (;;) {
          const { done: c, value: f } = await s.read();
          if (c) {
            try {
              const h = u.decode();
              h.length > 0 && l.next(h);
            } catch {}
            l.return(void 0);
            break;
          }
          try {
            const h = u.decode(f, { stream: !0 });
            h.length > 0 && l.next(h);
          } catch {
            l.next({ $b64: Jp(f) });
          }
        }
      } catch (c) {
        l.throw(c);
      } finally {
        s.releaseLock();
      }
    })(),
    l
  );
}
const F_ = {
  tag: "tss/RawStream",
  extends: [
    {
      tag: "tss/RawStreamFactory",
      test(n) {
        return n === Ll;
      },
      parse: {
        sync(n, l, s) {
          return {};
        },
        async async(n, l, s) {
          return {};
        },
        stream(n, l, s) {
          return {};
        },
      },
      serialize(n, l, s) {
        return Z_;
      },
      deserialize(n, l, s) {
        return Ll;
      },
    },
    {
      tag: "tss/RawStreamFactoryText",
      test(n) {
        return n === Ul;
      },
      parse: {
        sync(n, l, s) {
          return {};
        },
        async async(n, l, s) {
          return {};
        },
        stream(n, l, s) {
          return {};
        },
      },
      serialize(n, l, s) {
        return P_;
      },
      deserialize(n, l, s) {
        return Ul;
      },
    },
  ],
  test(n) {
    return n instanceof Q_;
  },
  parse: {
    sync(n, l, s) {
      const u = n.hint === "text" ? Ul : Ll;
      return { hint: l.parse(n.hint), factory: l.parse(u), stream: l.parse(ja()) };
    },
    async async(n, l, s) {
      const u = n.hint === "text" ? Ul : Ll,
        c = n.hint === "text" ? xy(n.stream) : Ay(n.stream);
      return { hint: await l.parse(n.hint), factory: await l.parse(u), stream: await l.parse(c) };
    },
    stream(n, l, s) {
      const u = n.hint === "text" ? Ul : Ll,
        c = n.hint === "text" ? xy(n.stream) : Ay(n.stream);
      return { hint: l.parse(n.hint), factory: l.parse(u), stream: l.parse(c) };
    },
  },
  serialize(n, l, s) {
    return "(" + l.serialize(n.factory) + ")(" + l.serialize(n.stream) + ")";
  },
  deserialize(n, l, s) {
    const u = l.deserialize(n.stream);
    return l.deserialize(n.hint) === "text" ? K_(u) : V_(u);
  },
};
function J_(n) {
  return {
    tag: "tss/RawStream",
    test: () => !1,
    parse: {},
    serialize() {
      throw new Error(
        "RawStreamDeserializePlugin.serialize should not be called. Client only deserializes.",
      );
    },
    deserialize(l, s, u) {
      return n(typeof s?.deserialize == "function" ? s.deserialize(l.streamId) : l.streamId);
    },
  };
}
const k_ = {
  tag: "$TSR/Error",
  test(n) {
    return n instanceof Error;
  },
  parse: {
    sync(n, l) {
      return { message: l.parse(n.message) };
    },
    async async(n, l) {
      return { message: await l.parse(n.message) };
    },
    stream(n, l) {
      return { message: l.parse(n.message) };
    },
  },
  serialize(n, l) {
    return "new Error(" + l.serialize(n.message) + ")";
  },
  deserialize(n, l) {
    return new Error(l.deserialize(n.message));
  },
};
var aa = {},
  Ip = (n) =>
    new ReadableStream({
      start: (l) => {
        n.on({
          next: (s) => {
            try {
              l.enqueue(s);
            } catch {}
          },
          throw: (s) => {
            l.error(s);
          },
          return: () => {
            try {
              l.close();
            } catch {}
          },
        });
      },
    }),
  I_ = {
    tag: "seroval-plugins/web/ReadableStreamFactory",
    test(n) {
      return n === aa;
    },
    parse: {
      sync() {
        return aa;
      },
      async async() {
        return await Promise.resolve(aa);
      },
      stream() {
        return aa;
      },
    },
    serialize() {
      return Ip.toString();
    },
    deserialize() {
      return aa;
    },
  };
function My(n) {
  let l = ja(),
    s = n.getReader();
  async function u() {
    try {
      let c = await s.read();
      c.done ? l.return(c.value) : (l.next(c.value), await u());
    } catch (c) {
      l.throw(c);
    }
  }
  return (u().catch(() => {}), l);
}
var $_ = {
    tag: "seroval/plugins/web/ReadableStream",
    extends: [I_],
    test(n) {
      return typeof ReadableStream > "u" ? !1 : n instanceof ReadableStream;
    },
    parse: {
      sync(n, l) {
        return { factory: l.parse(aa), stream: l.parse(ja()) };
      },
      async async(n, l) {
        return { factory: await l.parse(aa), stream: await l.parse(My(n)) };
      },
      stream(n, l) {
        return { factory: l.parse(aa), stream: l.parse(My(n)) };
      },
    },
    serialize(n, l) {
      return "(" + l.serialize(n.factory) + ")(" + l.serialize(n.stream) + ")";
    },
    deserialize(n, l) {
      let s = l.deserialize(n.stream);
      return Ip(s);
    },
  },
  W_ = $_;
const tE = [k_, F_, W_];
function eE() {
  return [...(Wy()?.serializationAdapters?.map(G_) ?? []), ...tE];
}
var Cy = new TextDecoder(),
  nE = new Uint8Array(0),
  wy = 16 * 1024 * 1024,
  Oy = 32 * 1024 * 1024,
  zy = 1024,
  Dy = 1e5;
function aE(n) {
  const l = new Map(),
    s = new Map(),
    u = new Set();
  let c = !1,
    f = null,
    h = 0,
    y;
  const m = new ReadableStream({
    start(g) {
      y = g;
    },
    cancel() {
      c = !0;
      try {
        f?.cancel();
      } catch {}
      (l.forEach((g) => {
        try {
          g.error(new Error("Framed response cancelled"));
        } catch {}
      }),
        l.clear(),
        s.clear(),
        u.clear());
    },
  });
  function p(g) {
    const _ = s.get(g);
    if (_) return _;
    if (u.has(g))
      return new ReadableStream({
        start(x) {
          x.close();
        },
      });
    if (s.size >= zy) throw new Error(`Too many raw streams in framed response (max ${zy})`);
    const E = new ReadableStream({
      start(x) {
        l.set(g, x);
      },
      cancel() {
        (u.add(g), l.delete(g), s.delete(g));
      },
    });
    return (s.set(g, E), E);
  }
  function S(g) {
    return (p(g), l.get(g));
  }
  return (
    (async () => {
      const g = n.getReader();
      f = g;
      const _ = [];
      let E = 0;
      function x() {
        if (E < 9) return null;
        const R = _[0];
        if (R.length >= 9)
          return {
            type: R[0],
            streamId: ((R[1] << 24) | (R[2] << 16) | (R[3] << 8) | R[4]) >>> 0,
            length: ((R[5] << 24) | (R[6] << 16) | (R[7] << 8) | R[8]) >>> 0,
          };
        const w = new Uint8Array(9);
        let N = 0,
          X = 9;
        for (let Y = 0; Y < _.length && X > 0; Y++) {
          const K = _[Y],
            J = Math.min(K.length, X);
          (w.set(K.subarray(0, J), N), (N += J), (X -= J));
        }
        return {
          type: w[0],
          streamId: ((w[1] << 24) | (w[2] << 16) | (w[3] << 8) | w[4]) >>> 0,
          length: ((w[5] << 24) | (w[6] << 16) | (w[7] << 8) | w[8]) >>> 0,
        };
      }
      function z(R) {
        if (R === 0) return nE;
        const w = new Uint8Array(R);
        let N = 0,
          X = R;
        for (; X > 0 && _.length > 0; ) {
          const Y = _[0];
          if (!Y) break;
          const K = Math.min(Y.length, X);
          (w.set(Y.subarray(0, K), N),
            (N += K),
            (X -= K),
            K === Y.length ? _.shift() : (_[0] = Y.subarray(K)));
        }
        return ((E -= R), w);
      }
      try {
        for (;;) {
          const { done: R, value: w } = await g.read();
          if (c || R) break;
          if (w) {
            if (E + w.length > Oy) throw new Error(`Framed response buffer exceeded ${Oy} bytes`);
            for (_.push(w), E += w.length; ; ) {
              const N = x();
              if (!N) break;
              const { type: X, streamId: Y, length: K } = N;
              if (X !== Rn.JSON && X !== Rn.CHUNK && X !== Rn.END && X !== Rn.ERROR)
                throw new Error(`Unknown frame type: ${X}`);
              if (X === Rn.JSON) {
                if (Y !== 0) throw new Error("Invalid JSON frame streamId (expected 0)");
              } else if (Y === 0) throw new Error("Invalid raw frame streamId (expected non-zero)");
              if (K > wy) throw new Error(`Frame payload too large: ${K} bytes (max ${wy})`);
              const J = 9 + K;
              if (E < J) break;
              if (++h > Dy) throw new Error(`Too many frames in framed response (max ${Dy})`);
              z(9);
              const q = z(K);
              switch (X) {
                case Rn.JSON:
                  try {
                    y.enqueue(Cy.decode(q));
                  } catch {}
                  break;
                case Rn.CHUNK: {
                  const Z = S(Y);
                  Z && Z.enqueue(q);
                  break;
                }
                case Rn.END: {
                  const Z = S(Y);
                  if ((u.add(Y), Z)) {
                    try {
                      Z.close();
                    } catch {}
                    l.delete(Y);
                  }
                  break;
                }
                case Rn.ERROR: {
                  const Z = S(Y);
                  if ((u.add(Y), Z)) {
                    const I = Cy.decode(q);
                    (Z.error(new Error(I)), l.delete(Y));
                  }
                  break;
                }
              }
            }
          }
        }
        if (E !== 0) throw new Error("Incomplete frame at end of framed response");
        try {
          y.close();
        } catch {}
        (l.forEach((R) => {
          try {
            R.close();
          } catch {}
        }),
          l.clear());
      } catch (R) {
        try {
          y.error(R);
        } catch {}
        (l.forEach((w) => {
          try {
            w.error(R);
          } catch {}
        }),
          l.clear());
      } finally {
        try {
          g.releaseLock();
        } catch {}
        f = null;
      }
    })(),
    { getOrCreateStream: p, jsonChunks: m }
  );
}
var Ql = null;
async function Mc(n) {
  n.length > 0 && (await Promise.allSettled(n));
}
var iE = Object.prototype.hasOwnProperty;
function $p(n) {
  for (const l in n) if (iE.call(n, l)) return !0;
  return !1;
}
async function lE(n, l, s) {
  Ql || (Ql = eE());
  const u = l[0],
    c = u.fetch ?? s,
    f = u.data instanceof FormData ? "formData" : "payload",
    h = u.headers ? new Headers(u.headers) : new Headers();
  if (
    (h.set("x-tsr-serverFn", "true"),
    f === "payload" && h.set("accept", `${tS}, application/x-ndjson, application/json`),
    u.method === "GET")
  ) {
    if (f === "formData") throw new Error("FormData is not supported with GET requests");
    const m = await Wp(u);
    if (m !== void 0) {
      const p = op({ payload: m });
      n.includes("?") ? (n += `&${p}`) : (n += `?${p}`);
    }
  }
  let y;
  if (u.method === "POST") {
    const m = await sE(u);
    (m?.contentType && h.set("content-type", m.contentType), (y = m?.body));
  }
  return await rE(async () => c(n, { method: u.method, headers: h, signal: u.signal, body: y }));
}
async function Wp(n) {
  let l = !1;
  const s = {};
  if (
    (n.data !== void 0 && ((l = !0), (s.data = n.data)),
    n.context && $p(n.context) && ((l = !0), (s.context = n.context)),
    l)
  )
    return tg(s);
}
async function tg(n) {
  return JSON.stringify(await Promise.resolve(Y_(n, { plugins: Ql })));
}
async function sE(n) {
  if (n.data instanceof FormData) {
    let s;
    return (
      n.context && $p(n.context) && (s = await tg(n.context)),
      s !== void 0 && n.data.set(W0, s),
      { body: n.data }
    );
  }
  const l = await Wp(n);
  if (l) return { body: l, contentType: "application/json" };
}
async function rE(n) {
  let l;
  try {
    l = await n();
  } catch (u) {
    if (u instanceof Response) l = u;
    else throw (console.log(u), u);
  }
  if (l.headers.get("x-tss-raw") === "true") return l;
  const s = l.headers.get("content-type");
  if ((s || Te(), l.headers.get("x-tss-serialized"))) {
    let u;
    if (s.includes("application/x-tss-framed")) {
      if ((aS(s), !l.body)) throw new Error("No response body for framed response");
      const { getOrCreateStream: c, jsonChunks: f } = aE(l.body),
        h = [J_(c), ...(Ql || [])],
        y = new Map();
      u = await uE({
        jsonStream: f,
        onMessage: (m) => Ry(m, { refs: y, plugins: h }),
        onError(m, p) {
          console.error(m, p);
        },
      });
    } else if (s.includes("application/json")) {
      const c = await l.json(),
        f = [];
      ((u = Ry(c, { plugins: Ql })), await Mc(f));
    }
    if ((u || Te(), u instanceof Error)) throw u;
    return u;
  }
  if (s.includes("application/json")) {
    const u = await l.json(),
      c = DS(u);
    if (c) throw c;
    if (ne(u)) throw u;
    return u;
  }
  if (!l.ok) throw new Error(await l.text());
  return l;
}
async function uE({ jsonStream: n, onMessage: l, onError: s }) {
  const u = n.getReader(),
    { value: c, done: f } = await u.read();
  if (f || !c) throw new Error("Stream ended before first object");
  const h = JSON.parse(c);
  let y = !1;
  const m = (async () => {
    try {
      for (;;) {
        const { value: g, done: _ } = await u.read();
        if (_) break;
        if (g)
          try {
            const E = [];
            try {
              l(JSON.parse(g));
            } finally {
            }
            await Mc(E);
          } catch (E) {
            s?.(`Invalid JSON: ${g}`, E);
          }
      }
    } catch (g) {
      y || s?.("Stream processing error:", g);
    }
  })();
  let p;
  const S = [];
  try {
    p = l(h);
  } catch (g) {
    throw ((y = !0), u.cancel().catch(() => {}), g);
  }
  return (
    await Mc(S),
    Promise.resolve(p).catch(() => {
      ((y = !0), u.cancel().catch(() => {}));
    }),
    m.finally(() => {
      try {
        u.releaseLock();
      } catch {}
    }),
    p
  );
}
function oE(n) {
  const l = "/_serverFn/" + n;
  return Object.assign(
    (...c) => {
      const f = Wy()?.serverFns?.fetch;
      return lE(l, c, f ?? fetch);
    },
    { url: l, serverFnMeta: { id: n }, [_c]: !0 },
  );
}
var cE = {
  key: "$TSS/serverfn",
  test: (n) => (typeof n != "function" || !(_c in n) ? !1 : !!n[_c]),
  toSerializable: ({ serverFnMeta: n }) => ({ functionId: n.id }),
  fromSerializable: ({ functionId: n }) => oE(n),
};
function Ly(n) {
  return n.replaceAll("\0", "/").replaceAll("�", "/");
}
function fE(n, l) {
  ((n.id = l.i),
    (n.__beforeLoadContext = l.b),
    (n.loaderData = l.l),
    (n.status = l.s),
    (n.ssr = l.ssr),
    (n.updatedAt = l.u),
    (n.error = l.e),
    l.g !== void 0 && (n.globalNotFound = l.g));
}
async function dE(n) {
  window.$_TSR || Te();
  const l = n.options.serializationAdapters;
  if (l?.length) {
    const R = new Map();
    (l.forEach((w) => {
      R.set(w.key, w.fromSerializable);
    }),
      (window.$_TSR.t = R),
      window.$_TSR.buffer.forEach((w) => w()));
  }
  ((window.$_TSR.initialized = !0), window.$_TSR.router || Te());
  const s = window.$_TSR.router;
  (s.matches.forEach((R) => {
    R.i = Ly(R.i);
  }),
    s.lastMatchId && (s.lastMatchId = Ly(s.lastMatchId)));
  const { manifest: u, dehydratedData: c, lastMatchId: f } = s;
  n.ssr = { manifest: u };
  const h = document.querySelector('meta[property="csp-nonce"]')?.content;
  ((n.options.ssr = { nonce: h }), await n.options.hydrate?.(c));
  const y = n.matchRoutes(n.stores.location.get()),
    m = Promise.all(y.map((R) => n.loadRouteChunk(n.looseRoutesById[R.routeId])));
  function p(R) {
    const w = n.looseRoutesById[R.routeId].options.pendingMinMs ?? n.options.defaultPendingMinMs;
    if (w) {
      const N = Na();
      ((R._nonReactive.minPendingPromise = N),
        (R._forcePending = !0),
        setTimeout(() => {
          (N.resolve(),
            n.updateMatch(
              R.id,
              (X) => ((X._nonReactive.minPendingPromise = void 0), { ...X, _forcePending: void 0 }),
            ));
        }, w));
    }
  }
  function S(R) {
    const w = n.looseRoutesById[R.routeId];
    w && (w.options.ssr = R.ssr);
  }
  let g;
  (y.forEach((R) => {
    const w = s.matches.find((N) => N.i === R.id);
    if (!w) {
      ((R._nonReactive.dehydrated = !1), (R.ssr = !1), S(R));
      return;
    }
    (fE(R, w),
      S(R),
      (R._nonReactive.dehydrated = R.ssr !== !1),
      (R.ssr === "data-only" || R.ssr === !1) && g === void 0 && ((g = R.index), p(R)));
  }),
    n.stores.setMatches(y));
  const _ = n.stores.matches.get(),
    E = n.stores.location.get();
  await Promise.all(
    _.map(async (R) => {
      try {
        const w = n.looseRoutesById[R.routeId],
          N = _[R.index - 1]?.context ?? n.options.context;
        if (w.options.context) {
          const J = {
            deps: R.loaderDeps,
            params: R.params,
            context: N ?? {},
            location: E,
            navigate: (q) => n.navigate({ ...q, _fromLocation: E }),
            buildLocation: n.buildLocation,
            cause: R.cause,
            abortController: R.abortController,
            preload: !1,
            matches: y,
            routeId: w.id,
          };
          R.__routeContext = w.options.context(J) ?? void 0;
        }
        R.context = { ...N, ...R.__routeContext, ...R.__beforeLoadContext };
        const X = {
            ssr: n.options.ssr,
            matches: _,
            match: R,
            params: R.params,
            loaderData: R.loaderData,
          },
          Y = await w.options.head?.(X),
          K = await w.options.scripts?.(X);
        ((R.meta = Y?.meta),
          (R.links = Y?.links),
          (R.headScripts = Y?.scripts),
          (R.styles = Y?.styles),
          (R.scripts = K));
      } catch (w) {
        if (ne(w))
          ((R.error = { isNotFound: !0 }),
            console.error(`NotFound error during hydration for routeId: ${R.routeId}`, w));
        else
          throw (
            (R.error = w),
            console.error(`Error during hydration for route ${R.routeId}:`, w),
            w
          );
      }
    }),
  );
  const x = y[y.length - 1].id !== f;
  if (!y.some((R) => R.ssr === !1) && !x)
    return (
      y.forEach((R) => {
        R._nonReactive.dehydrated = void 0;
      }),
      n.stores.resolvedLocation.set(n.stores.location.get()),
      m
    );
  const z = Promise.resolve()
    .then(() => n.load())
    .catch((R) => {
      console.error("Error during router hydration:", R);
    });
  if (x) {
    const R = y[1];
    (R || Te(),
      p(R),
      (R._displayPending = !0),
      (R._nonReactive.displayPendingPromise = z),
      z.then(() => {
        n.batch(() => {
          (n.stores.status.get() === "pending" &&
            (n.stores.status.set("idle"), n.stores.resolvedLocation.set(n.stores.location.get())),
            n.updateMatch(R.id, (w) => ({
              ...w,
              _displayPending: void 0,
              displayPendingPromise: void 0,
            })));
        });
      }));
  }
  return m;
}
var Or = tt.use,
  Nl = typeof window < "u" ? tt.useLayoutEffect : tt.useEffect;
function mc(n) {
  const l = tt.useRef({ value: n, prev: null }),
    s = l.current.value;
  return (n !== s && (l.current = { value: n, prev: s }), l.current.prev);
}
function hE(n, l, s = {}, u = {}) {
  tt.useEffect(() => {
    if (!n.current || u.disabled || typeof IntersectionObserver != "function") return;
    const c = new IntersectionObserver(([f]) => {
      l(f);
    }, s);
    return (
      c.observe(n.current),
      () => {
        c.disconnect();
      }
    );
  }, [l, s, u.disabled, n]);
}
function mE(n) {
  const l = tt.useRef(null);
  return (tt.useImperativeHandle(n, () => l.current, []), l);
}
function yE({ promise: n }) {
  if (Or) return Or(n);
  const l = sb(n);
  if (l[en].status === "pending") throw l;
  if (l[en].status === "error") throw l[en].error;
  return l[en].data;
}
function pE(n) {
  const l = P.jsx(gE, { ...n });
  return n.fallback ? P.jsx(tt.Suspense, { fallback: n.fallback, children: l }) : l;
}
function gE(n) {
  const l = yE(n);
  return n.children(l);
}
function Gc(n) {
  const l = n.errorComponent ?? Qc;
  return P.jsx(vE, {
    getResetKey: n.getResetKey,
    onCatch: n.onCatch,
    children: ({ error: s, reset: u }) =>
      s ? tt.createElement(l, { error: s, reset: u }) : n.children,
  });
}
var vE = class extends tt.Component {
  constructor(...n) {
    (super(...n), (this.state = { error: null }));
  }
  static getDerivedStateFromProps(n, l) {
    const s = n.getResetKey();
    return l.error && l.resetKey !== s ? { resetKey: s, error: null } : { resetKey: s };
  }
  static getDerivedStateFromError(n) {
    return { error: n };
  }
  reset() {
    this.setState({ error: null });
  }
  componentDidCatch(n, l) {
    this.props.onCatch && this.props.onCatch(n, l);
  }
  render() {
    return this.props.children({
      error: this.state.error,
      reset: () => {
        this.reset();
      },
    });
  }
};
function Qc({ error: n }) {
  const [l, s] = tt.useState(!1);
  return P.jsxs("div", {
    style: { padding: ".5rem", maxWidth: "100%" },
    children: [
      P.jsxs("div", {
        style: { display: "flex", alignItems: "center", gap: ".5rem" },
        children: [
          P.jsx("strong", { style: { fontSize: "1rem" }, children: "Something went wrong!" }),
          P.jsx("button", {
            style: {
              appearance: "none",
              fontSize: ".6em",
              border: "1px solid currentColor",
              padding: ".1rem .2rem",
              fontWeight: "bold",
              borderRadius: ".25rem",
            },
            onClick: () => s((u) => !u),
            children: l ? "Hide Error" : "Show Error",
          }),
        ],
      }),
      P.jsx("div", { style: { height: ".25rem" } }),
      l
        ? P.jsx("div", {
            children: P.jsx("pre", {
              style: {
                fontSize: ".7em",
                border: "1px solid red",
                borderRadius: ".25rem",
                padding: ".3rem",
                color: "red",
                overflow: "auto",
              },
              children: n.message ? P.jsx("code", { children: n.message }) : null,
            }),
          })
        : null,
    ],
  });
}
function SE({ children: n, fallback: l = null }) {
  return Vc() ? P.jsx(jl.Fragment, { children: n }) : P.jsx(jl.Fragment, { children: l });
}
function Vc() {
  return jl.useSyncExternalStore(
    bE,
    () => !0,
    () => !1,
  );
}
function bE() {
  return () => {};
}
var eg = tt.createContext(null);
function oe(n) {
  return tt.useContext(eg);
}
var qr = tt.createContext(void 0),
  _E = tt.createContext(void 0),
  Ht = ((n) => (
    (n[(n.None = 0)] = "None"),
    (n[(n.Mutable = 1)] = "Mutable"),
    (n[(n.Watching = 2)] = "Watching"),
    (n[(n.RecursedCheck = 4)] = "RecursedCheck"),
    (n[(n.Recursed = 8)] = "Recursed"),
    (n[(n.Dirty = 16)] = "Dirty"),
    (n[(n.Pending = 32)] = "Pending"),
    n
  ))(Ht || {});
function EE({ update: n, notify: l, unwatched: s }) {
  return { link: u, unlink: c, propagate: f, checkDirty: h, shallowPropagate: y };
  function u(p, S, g) {
    const _ = S.depsTail;
    if (_ !== void 0 && _.dep === p) return;
    const E = _ !== void 0 ? _.nextDep : S.deps;
    if (E !== void 0 && E.dep === p) {
      ((E.version = g), (S.depsTail = E));
      return;
    }
    const x = p.subsTail;
    if (x !== void 0 && x.version === g && x.sub === S) return;
    const z =
      (S.depsTail =
      p.subsTail =
        { version: g, dep: p, sub: S, prevDep: _, nextDep: E, prevSub: x, nextSub: void 0 });
    (E !== void 0 && (E.prevDep = z),
      _ !== void 0 ? (_.nextDep = z) : (S.deps = z),
      x !== void 0 ? (x.nextSub = z) : (p.subs = z));
  }
  function c(p, S = p.sub) {
    const g = p.dep,
      _ = p.prevDep,
      E = p.nextDep,
      x = p.nextSub,
      z = p.prevSub;
    return (
      E !== void 0 ? (E.prevDep = _) : (S.depsTail = _),
      _ !== void 0 ? (_.nextDep = E) : (S.deps = E),
      x !== void 0 ? (x.prevSub = z) : (g.subsTail = z),
      z !== void 0 ? (z.nextSub = x) : (g.subs = x) === void 0 && s(g),
      E
    );
  }
  function f(p) {
    let S = p.nextSub,
      g;
    t: do {
      const _ = p.sub;
      let E = _.flags;
      if (
        (E & 60
          ? E & 12
            ? E & 4
              ? !(E & 48) && m(p, _)
                ? ((_.flags = E | 40), (E &= 1))
                : (E = 0)
              : (_.flags = (E & -9) | 32)
            : (E = 0)
          : (_.flags = E | 32),
        E & 2 && l(_),
        E & 1)
      ) {
        const x = _.subs;
        if (x !== void 0) {
          const z = (p = x).nextSub;
          z !== void 0 && ((g = { value: S, prev: g }), (S = z));
          continue;
        }
      }
      if ((p = S) !== void 0) {
        S = p.nextSub;
        continue;
      }
      for (; g !== void 0; )
        if (((p = g.value), (g = g.prev), p !== void 0)) {
          S = p.nextSub;
          continue t;
        }
      break;
    } while (!0);
  }
  function h(p, S) {
    let g,
      _ = 0,
      E = !1;
    t: do {
      const x = p.dep,
        z = x.flags;
      if (S.flags & 16) E = !0;
      else if ((z & 17) === 17) {
        if (n(x)) {
          const R = x.subs;
          (R.nextSub !== void 0 && y(R), (E = !0));
        }
      } else if ((z & 33) === 33) {
        ((p.nextSub !== void 0 || p.prevSub !== void 0) && (g = { value: p, prev: g }),
          (p = x.deps),
          (S = x),
          ++_);
        continue;
      }
      if (!E) {
        const R = p.nextDep;
        if (R !== void 0) {
          p = R;
          continue;
        }
      }
      for (; _--; ) {
        const R = S.subs,
          w = R.nextSub !== void 0;
        if ((w ? ((p = g.value), (g = g.prev)) : (p = R), E)) {
          if (n(S)) {
            (w && y(R), (S = p.sub));
            continue;
          }
          E = !1;
        } else S.flags &= -33;
        S = p.sub;
        const N = p.nextDep;
        if (N !== void 0) {
          p = N;
          continue t;
        }
      }
      return E;
    } while (!0);
  }
  function y(p) {
    do {
      const S = p.sub,
        g = S.flags;
      (g & 48) === 32 && ((S.flags = g | 16), (g & 6) === 2 && l(S));
    } while ((p = p.nextSub) !== void 0);
  }
  function m(p, S) {
    let g = S.depsTail;
    for (; g !== void 0; ) {
      if (g === p) return !0;
      g = g.prevDep;
    }
    return !1;
  }
}
function TE(n, l, s) {
  const u = typeof n == "object",
    c = u ? n : void 0;
  return {
    next: (u ? n.next : n)?.bind(c),
    error: (u ? n.error : l)?.bind(c),
    complete: (u ? n.complete : s)?.bind(c),
  };
}
const Cc = [];
let Tr = 0;
const {
  link: Uy,
  unlink: RE,
  propagate: AE,
  checkDirty: ng,
  shallowPropagate: By,
} = EE({
  update(n) {
    return n._update();
  },
  notify(n) {
    ((Cc[wc++] = n), (n.flags &= ~Ht.Watching));
  },
  unwatched(n) {
    n.depsTail !== void 0 && ((n.depsTail = void 0), (n.flags = Ht.Mutable | Ht.Dirty), zr(n));
  },
});
let gr = 0,
  wc = 0,
  tn,
  Oc = 0;
function ag(n) {
  try {
    (++Oc, n());
  } finally {
    --Oc || ig();
  }
}
function zr(n) {
  const l = n.depsTail;
  let s = l !== void 0 ? l.nextDep : n.deps;
  for (; s !== void 0; ) s = RE(s, n);
}
function ig() {
  if (!(Oc > 0)) {
    for (; gr < wc; ) {
      const n = Cc[gr];
      ((Cc[gr++] = void 0), n.notify());
    }
    ((gr = 0), (wc = 0));
  }
}
function Ny(n, l) {
  const s = typeof n == "function",
    u = n,
    c = {
      _snapshot: s ? void 0 : n,
      subs: void 0,
      subsTail: void 0,
      deps: void 0,
      depsTail: void 0,
      flags: s ? Ht.None : Ht.Mutable,
      get() {
        return (tn !== void 0 && Uy(c, tn, Tr), c._snapshot);
      },
      subscribe(f) {
        const h = TE(f),
          y = { current: !1 },
          m = xE(() => {
            (c.get(), y.current ? h.next?.(c._snapshot) : (y.current = !0));
          });
        return {
          unsubscribe: () => {
            m.stop();
          },
        };
      },
      _update(f) {
        const h = tn,
          y = l?.compare ?? Object.is;
        if (s) ((tn = c), ++Tr, (c.depsTail = void 0));
        else if (f === void 0) return !1;
        s && (c.flags = Ht.Mutable | Ht.RecursedCheck);
        try {
          const m = c._snapshot,
            p = typeof f == "function" ? f(m) : f === void 0 && s ? u(m) : f;
          return m === void 0 || !y(m, p) ? ((c._snapshot = p), !0) : !1;
        } finally {
          ((tn = h), s && (c.flags &= ~Ht.RecursedCheck), zr(c));
        }
      },
    };
  return (
    s
      ? ((c.flags = Ht.Mutable | Ht.Dirty),
        (c.get = function () {
          const f = c.flags;
          if (f & Ht.Dirty || (f & Ht.Pending && ng(c.deps, c))) {
            if (c._update()) {
              const h = c.subs;
              h !== void 0 && By(h);
            }
          } else f & Ht.Pending && (c.flags = f & ~Ht.Pending);
          return (tn !== void 0 && Uy(c, tn, Tr), c._snapshot);
        }))
      : (c.set = function (f) {
          if (c._update(f)) {
            const h = c.subs;
            h !== void 0 && (AE(h), By(h), ig());
          }
        }),
    c
  );
}
function xE(n) {
  const l = () => {
      const u = tn;
      ((tn = s), ++Tr, (s.depsTail = void 0), (s.flags = Ht.Watching | Ht.RecursedCheck));
      try {
        return n();
      } finally {
        ((tn = u), (s.flags &= ~Ht.RecursedCheck), zr(s));
      }
    },
    s = {
      deps: void 0,
      depsTail: void 0,
      subs: void 0,
      subsTail: void 0,
      flags: Ht.Watching | Ht.RecursedCheck,
      notify() {
        const u = this.flags;
        u & Ht.Dirty || (u & Ht.Pending && ng(this.deps, this)) ? l() : (this.flags = Ht.Watching);
      },
      stop() {
        ((this.flags = Ht.None), (this.depsTail = void 0), zr(this));
      },
    };
  return (l(), s);
}
var yc = { exports: {} },
  pc = {},
  gc = { exports: {} },
  vc = {};
var jy;
function ME() {
  if (jy) return vc;
  jy = 1;
  var n = Kl();
  function l(g, _) {
    return (g === _ && (g !== 0 || 1 / g === 1 / _)) || (g !== g && _ !== _);
  }
  var s = typeof Object.is == "function" ? Object.is : l,
    u = n.useState,
    c = n.useEffect,
    f = n.useLayoutEffect,
    h = n.useDebugValue;
  function y(g, _) {
    var E = _(),
      x = u({ inst: { value: E, getSnapshot: _ } }),
      z = x[0].inst,
      R = x[1];
    return (
      f(
        function () {
          ((z.value = E), (z.getSnapshot = _), m(z) && R({ inst: z }));
        },
        [g, E, _],
      ),
      c(
        function () {
          return (
            m(z) && R({ inst: z }),
            g(function () {
              m(z) && R({ inst: z });
            })
          );
        },
        [g],
      ),
      h(E),
      E
    );
  }
  function m(g) {
    var _ = g.getSnapshot;
    g = g.value;
    try {
      var E = _();
      return !s(g, E);
    } catch {
      return !0;
    }
  }
  function p(g, _) {
    return _();
  }
  var S =
    typeof window > "u" ||
    typeof window.document > "u" ||
    typeof window.document.createElement > "u"
      ? p
      : y;
  return (
    (vc.useSyncExternalStore = n.useSyncExternalStore !== void 0 ? n.useSyncExternalStore : S),
    vc
  );
}
var Hy;
function CE() {
  return (Hy || ((Hy = 1), (gc.exports = ME())), gc.exports);
}
var qy;
function wE() {
  if (qy) return pc;
  qy = 1;
  var n = Kl(),
    l = CE();
  function s(p, S) {
    return (p === S && (p !== 0 || 1 / p === 1 / S)) || (p !== p && S !== S);
  }
  var u = typeof Object.is == "function" ? Object.is : s,
    c = l.useSyncExternalStore,
    f = n.useRef,
    h = n.useEffect,
    y = n.useMemo,
    m = n.useDebugValue;
  return (
    (pc.useSyncExternalStoreWithSelector = function (p, S, g, _, E) {
      var x = f(null);
      if (x.current === null) {
        var z = { hasValue: !1, value: null };
        x.current = z;
      } else z = x.current;
      x = y(
        function () {
          function w(J) {
            if (!N) {
              if (((N = !0), (X = J), (J = _(J)), E !== void 0 && z.hasValue)) {
                var q = z.value;
                if (E(q, J)) return (Y = q);
              }
              return (Y = J);
            }
            if (((q = Y), u(X, J))) return q;
            var Z = _(J);
            return E !== void 0 && E(q, Z) ? ((X = J), q) : ((X = J), (Y = Z));
          }
          var N = !1,
            X,
            Y,
            K = g === void 0 ? null : g;
          return [
            function () {
              return w(S());
            },
            K === null
              ? void 0
              : function () {
                  return w(K());
                },
          ];
        },
        [S, g, _, E],
      );
      var R = c(p, x[0], x[1]);
      return (
        h(
          function () {
            ((z.hasValue = !0), (z.value = R));
          },
          [R],
        ),
        m(R),
        R
      );
    }),
    pc
  );
}
var Yy;
function OE() {
  return (Yy || ((Yy = 1), (yc.exports = wE())), yc.exports);
}
var zE = OE();
function DE(n, l) {
  return n === l;
}
function Kt(n, l, s = DE) {
  const u = tt.useCallback(
      (h) => {
        if (!n) return () => {};
        const { unsubscribe: y } = n.subscribe(h);
        return y;
      },
      [n],
    ),
    c = tt.useCallback(() => n?.get(), [n]);
  return zE.useSyncExternalStoreWithSelector(u, c, c, l, s);
}
var LE = { get: () => {}, subscribe: () => ({ unsubscribe: () => {} }) };
function Ha(n) {
  const l = oe(),
    s = tt.useContext(n.from ? _E : qr),
    u = n.from ?? s,
    c = u ? (n.from ? l.stores.getRouteMatchStore(u) : l.stores.matchStores.get(u)) : void 0,
    f = tt.useRef(void 0);
  return Kt(c ?? LE, (h) => {
    if (((n.shouldThrow ?? !0) && !h && Te(), h === void 0)) return;
    const y = n.select ? n.select(h) : h;
    if (n.structuralSharing ?? l.options.defaultStructuralSharing) {
      const m = za(f.current, y);
      return ((f.current = m), m);
    }
    return y;
  });
}
function lg(n) {
  return Ha({
    from: n.from,
    strict: n.strict,
    structuralSharing: n.structuralSharing,
    select: (l) => (n.select ? n.select(l.loaderData) : l.loaderData),
  });
}
function sg(n) {
  const { select: l, ...s } = n;
  return Ha({ ...s, select: (u) => (l ? l(u.loaderDeps) : u.loaderDeps) });
}
function rg(n) {
  return Ha({
    from: n.from,
    shouldThrow: n.shouldThrow,
    structuralSharing: n.structuralSharing,
    strict: n.strict,
    select: (l) => {
      const s = n.strict === !1 ? l.params : l._strictParams;
      return n.select ? n.select(s) : s;
    },
  });
}
function ug(n) {
  return Ha({
    from: n.from,
    strict: n.strict,
    shouldThrow: n.shouldThrow,
    structuralSharing: n.structuralSharing,
    select: (l) => (n.select ? n.select(l.search) : l.search),
  });
}
function og(n) {
  const l = oe();
  return tt.useCallback((s) => l.navigate({ ...s, from: s.from ?? n?.from }), [n?.from, l]);
}
function cg(n) {
  return Ha({ ...n, select: (l) => (n.select ? n.select(l.context) : l.context) });
}
var UE = $y();
function BE(n, l) {
  const s = oe(),
    u = mE(l),
    {
      activeProps: c,
      inactiveProps: f,
      activeOptions: h,
      to: y,
      preload: m,
      preloadDelay: p,
      preloadIntentProximity: S,
      hashScrollIntoView: g,
      replace: _,
      startTransition: E,
      resetScroll: x,
      viewTransition: z,
      children: R,
      target: w,
      disabled: N,
      style: X,
      className: Y,
      onClick: K,
      onBlur: J,
      onFocus: q,
      onMouseEnter: Z,
      onMouseLeave: I,
      onTouchStart: ut,
      ignoreBlocker: st,
      params: lt,
      search: Wt,
      hash: Nt,
      state: Gt,
      mask: j,
      reloadDocument: F,
      unsafeRelative: it,
      from: Et,
      _fromLocation: Rt,
      ...C
    } = n,
    Q = Vc(),
    k = tt.useMemo(
      () => n,
      [
        s,
        n.from,
        n._fromLocation,
        n.hash,
        n.to,
        n.search,
        n.params,
        n.state,
        n.mask,
        n.unsafeRelative,
      ],
    ),
    $ = Kt(
      s.stores.location,
      (Lt) => Lt,
      (Lt, ae) => Lt.href === ae.href,
    ),
    nt = tt.useMemo(() => {
      const Lt = { _fromLocation: $, ...k };
      return s.buildLocation(Lt);
    }, [s, $, k]),
    dt = nt.maskedLocation ? nt.maskedLocation.publicHref : nt.publicHref,
    vt = nt.maskedLocation ? nt.maskedLocation.external : nt.external,
    Qt = tt.useMemo(() => GE(dt, vt, s.history, N), [N, vt, dt, s.history]),
    Ot = tt.useMemo(() => {
      if (Qt?.external) return Ar(Qt.href, s.protocolAllowlist) ? void 0 : Qt.href;
      if (!QE(y) && !(typeof y != "string" || y.indexOf(":") === -1))
        try {
          return (new URL(y), Ar(y, s.protocolAllowlist) ? void 0 : y);
        } catch {}
    }, [y, Qt, s.protocolAllowlist]),
    nn = tt.useMemo(() => {
      if (Ot) return !1;
      if (h?.exact) {
        if (!RS($.pathname, nt.pathname, s.basepath)) return !1;
      } else {
        const Lt = xr($.pathname, s.basepath),
          ae = xr(nt.pathname, s.basepath);
        if (!(Lt.startsWith(ae) && (Lt.length === ae.length || Lt[ae.length] === "/"))) return !1;
      }
      return (h?.includeSearch ?? !0) &&
        !he($.search, nt.search, { partial: !h?.exact, ignoreUndefined: !h?.explicitUndefined })
        ? !1
        : h?.includeHash
          ? Q && $.hash === nt.hash
          : !0;
    }, [
      h?.exact,
      h?.explicitUndefined,
      h?.includeHash,
      h?.includeSearch,
      $,
      Ot,
      Q,
      nt.hash,
      nt.pathname,
      nt.search,
      s.basepath,
    ]),
    an = nn ? (Oa(c, {}) ?? NE) : Sc,
    On = nn ? Sc : (Oa(f, {}) ?? Sc),
    Ui = [Y, an.className, On.className].filter(Boolean).join(" "),
    Fe = (X || an.style || On.style) && { ...X, ...an.style, ...On.style },
    [Bi, qa] = tt.useState(!1),
    Fl = tt.useRef(!1),
    ln = n.reloadDocument || Ot ? !1 : (m ?? s.options.defaultPreload),
    ca = p ?? s.options.defaultPreloadDelay ?? 0,
    Ve = tt.useCallback(() => {
      s.preloadRoute({ ...k, _builtLocation: nt }).catch((Lt) => {
        (console.warn(Lt), console.warn(rb));
      });
    }, [s, k, nt]);
  (hE(
    u,
    tt.useCallback(
      (Lt) => {
        Lt?.isIntersecting && Ve();
      },
      [Ve],
    ),
    YE,
    { disabled: !!N || ln !== "viewport" },
  ),
    tt.useEffect(() => {
      Fl.current || (!N && ln === "render" && (Ve(), (Fl.current = !0)));
    }, [N, Ve, ln]));
  const Ni = (Lt) => {
    const ae = Lt.currentTarget.getAttribute("target"),
      Je = w !== void 0 ? w : ae;
    if (!N && !VE(Lt) && !Lt.defaultPrevented && (!Je || Je === "_self") && Lt.button === 0) {
      (Lt.preventDefault(),
        UE.flushSync(() => {
          qa(!0);
        }));
      const Ya = s.subscribe("onResolved", () => {
        (Ya(), qa(!1));
      });
      s.navigate({
        ...k,
        replace: _,
        resetScroll: x,
        hashScrollIntoView: g,
        startTransition: E,
        viewTransition: z,
        ignoreBlocker: st,
      });
    }
  };
  if (Ot)
    return {
      ...C,
      ref: u,
      href: Ot,
      ...(R && { children: R }),
      ...(w && { target: w }),
      ...(N && { disabled: N }),
      ...(X && { style: X }),
      ...(Y && { className: Y }),
      ...(K && { onClick: K }),
      ...(J && { onBlur: J }),
      ...(q && { onFocus: q }),
      ...(Z && { onMouseEnter: Z }),
      ...(I && { onMouseLeave: I }),
      ...(ut && { onTouchStart: ut }),
    };
  const Jl = (Lt) => {
      if (N || ln !== "intent") return;
      if (!ca) {
        Ve();
        return;
      }
      const ae = Lt.currentTarget;
      if (Bl.has(ae)) return;
      const Je = setTimeout(() => {
        (Bl.delete(ae), Ve());
      }, ca);
      Bl.set(ae, Je);
    },
    Gr = (Lt) => {
      N || ln !== "intent" || Ve();
    },
    ce = (Lt) => {
      if (N || !ln || !ca) return;
      const ae = Lt.currentTarget,
        Je = Bl.get(ae);
      Je && (clearTimeout(Je), Bl.delete(ae));
    };
  return {
    ...C,
    ...an,
    ...On,
    href: Qt?.href,
    ref: u,
    onClick: Ai([K, Ni]),
    onBlur: Ai([J, ce]),
    onFocus: Ai([q, Jl]),
    onMouseEnter: Ai([Z, Jl]),
    onMouseLeave: Ai([I, ce]),
    onTouchStart: Ai([ut, Gr]),
    disabled: !!N,
    target: w,
    ...(Fe && { style: Fe }),
    ...(Ui && { className: Ui }),
    ...(N && jE),
    ...(nn && HE),
    ...(Q && Bi && qE),
  };
}
var Sc = {},
  NE = { className: "active" },
  jE = { role: "link", "aria-disabled": !0 },
  HE = { "data-status": "active", "aria-current": "page" },
  qE = { "data-transitioning": "transitioning" },
  Bl = new WeakMap(),
  YE = { rootMargin: "100px" },
  Ai = (n) => (l) => {
    for (const s of n)
      if (s) {
        if (l.defaultPrevented) return;
        s(l);
      }
  };
function GE(n, l, s, u) {
  if (!u) return l ? { href: n, external: !0 } : { href: s.createHref(n) || "/", external: !1 };
}
function QE(n) {
  if (typeof n != "string") return !1;
  const l = n.charCodeAt(0);
  return l === 47 ? n.charCodeAt(1) !== 47 : l === 46;
}
var Xc = tt.forwardRef((n, l) => {
  const { _asChild: s, ...u } = n,
    { type: c, ...f } = BE(u, l),
    h =
      typeof u.children == "function"
        ? u.children({ isActive: f["data-status"] === "active" })
        : u.children;
  if (!s) {
    const { disabled: y, ...m } = f;
    return tt.createElement("a", m, h);
  }
  return tt.createElement(s, f, h);
});
function VE(n) {
  return !!(n.metaKey || n.altKey || n.ctrlKey || n.shiftKey);
}
var XE = class extends vp {
  constructor(l) {
    (super(l),
      (this.useMatch = (s) =>
        Ha({ select: s?.select, from: this.id, structuralSharing: s?.structuralSharing })),
      (this.useRouteContext = (s) => cg({ ...s, from: this.id })),
      (this.useSearch = (s) =>
        ug({ select: s?.select, structuralSharing: s?.structuralSharing, from: this.id })),
      (this.useParams = (s) =>
        rg({ select: s?.select, structuralSharing: s?.structuralSharing, from: this.id })),
      (this.useLoaderDeps = (s) => sg({ ...s, from: this.id })),
      (this.useLoaderData = (s) => lg({ ...s, from: this.id })),
      (this.useNavigate = () => og({ from: this.fullPath })),
      (this.Link = jl.forwardRef((s, u) => P.jsx(Xc, { ref: u, from: this.fullPath, ...s }))));
  }
};
function KE(n) {
  return new XE(n);
}
function ZE() {
  return (n) => FE(n);
}
var PE = class extends db {
  constructor(n) {
    (super(n),
      (this.useMatch = (l) =>
        Ha({ select: l?.select, from: this.id, structuralSharing: l?.structuralSharing })),
      (this.useRouteContext = (l) => cg({ ...l, from: this.id })),
      (this.useSearch = (l) =>
        ug({ select: l?.select, structuralSharing: l?.structuralSharing, from: this.id })),
      (this.useParams = (l) =>
        rg({ select: l?.select, structuralSharing: l?.structuralSharing, from: this.id })),
      (this.useLoaderDeps = (l) => sg({ ...l, from: this.id })),
      (this.useLoaderData = (l) => lg({ ...l, from: this.id })),
      (this.useNavigate = () => og({ from: this.fullPath })),
      (this.Link = jl.forwardRef((l, s) => P.jsx(Xc, { ref: s, from: this.fullPath, ...l }))));
  }
};
function FE(n) {
  return new PE(n);
}
function JE(n) {
  return new kE(n, { silent: !0 }).createRoute;
}
var kE = class {
  constructor(n, l) {
    ((this.path = n),
      (this.createRoute = (s) => {
        const u = KE(s);
        return ((u.isRoot = !1), u);
      }),
      (this.silent = l?.silent));
  }
};
function IE(n, l) {
  let s, u, c, f;
  const h = () => (
      s ||
        (s = n()
          .then((m) => {
            ((s = void 0), (u = m[l]));
          })
          .catch((m) => {
            if (
              ((c = m),
              sS(c) && c instanceof Error && typeof window < "u" && typeof sessionStorage < "u")
            ) {
              const p = `tanstack_router_reload:${c.message}`;
              sessionStorage.getItem(p) || (sessionStorage.setItem(p, "1"), (f = !0));
            }
          })),
      s
    ),
    y = function (p) {
      if (f) throw (window.location.reload(), new Promise(() => {}));
      if (c) throw c;
      if (!u)
        if (Or) Or(h());
        else throw h();
      return tt.createElement(u, p);
    };
  return ((y.preload = h), y);
}
function $E(n) {
  const l = oe(),
    s = `not-found-${Kt(l.stores.location, (u) => u.pathname)}-${Kt(l.stores.status, (u) => u)}`;
  return P.jsx(Gc, {
    getResetKey: () => s,
    onCatch: (u, c) => {
      if (ne(u)) n.onCatch?.(u, c);
      else throw u;
    },
    errorComponent: ({ error: u }) => {
      if (ne(u)) return n.fallback?.(u);
      throw u;
    },
    children: n.children,
  });
}
function WE() {
  return P.jsx("p", { children: "Not Found" });
}
function Ci(n) {
  return P.jsx(P.Fragment, { children: n.children });
}
function fg(n, l, s) {
  return l.options.notFoundComponent
    ? P.jsx(l.options.notFoundComponent, { ...s })
    : n.options.defaultNotFoundComponent
      ? P.jsx(n.options.defaultNotFoundComponent, { ...s })
      : P.jsx(WE, {});
}
function tT(n) {
  return null;
}
function eT() {
  return (tT(oe()), null);
}
var dg = tt.memo(function ({ matchId: l }) {
  const s = oe(),
    u = s.stores.matchStores.get(l);
  u || Te();
  const c = Kt(s.stores.loadedAt, (h) => h),
    f = Kt(u, (h) => h);
  return P.jsx(nT, {
    router: s,
    matchId: l,
    resetKey: c,
    matchState: tt.useMemo(() => {
      const h = f.routeId,
        y = s.routesById[h].parentRoute?.id;
      return { routeId: h, ssr: f.ssr, _displayPending: f._displayPending, parentRouteId: y };
    }, [f._displayPending, f.routeId, f.ssr, s.routesById]),
  });
});
function nT({ router: n, matchId: l, resetKey: s, matchState: u }) {
  const c = n.routesById[u.routeId],
    f = c.options.pendingComponent ?? n.options.defaultPendingComponent,
    h = f ? P.jsx(f, {}) : null,
    y = c.options.errorComponent ?? n.options.defaultErrorComponent,
    m = c.options.onCatch ?? n.options.defaultOnCatch,
    p = c.isRoot
      ? (c.options.notFoundComponent ?? n.options.notFoundRoute?.options.component)
      : c.options.notFoundComponent,
    S = u.ssr === !1 || u.ssr === "data-only",
    g =
      (!c.isRoot || c.options.wrapInSuspense || S) &&
      (c.options.wrapInSuspense ?? f ?? (c.options.errorComponent?.preload || S))
        ? tt.Suspense
        : Ci,
    _ = y ? Gc : Ci,
    E = p ? $E : Ci;
  return P.jsxs(c.isRoot ? (c.options.shellComponent ?? Ci) : Ci, {
    children: [
      P.jsx(qr.Provider, {
        value: l,
        children: P.jsx(g, {
          fallback: h,
          children: P.jsx(_, {
            getResetKey: () => s,
            errorComponent: y || Qc,
            onCatch: (x, z) => {
              if (ne(x)) throw ((x.routeId ??= u.routeId), x);
              m?.(x, z);
            },
            children: P.jsx(E, {
              fallback: (x) => {
                if (
                  ((x.routeId ??= u.routeId),
                  !p || (x.routeId && x.routeId !== u.routeId) || (!x.routeId && !c.isRoot))
                )
                  throw x;
                return tt.createElement(p, x);
              },
              children:
                S || u._displayPending
                  ? P.jsx(SE, { fallback: h, children: P.jsx(Gy, { matchId: l }) })
                  : P.jsx(Gy, { matchId: l }),
            }),
          }),
        }),
      }),
      u.parentRouteId === Ua
        ? P.jsxs(P.Fragment, {
            children: [
              P.jsx(aT, { resetKey: s }),
              n.options.scrollRestoration && tp ? P.jsx(eT, {}) : null,
            ],
          })
        : null,
    ],
  });
}
function aT({ resetKey: n }) {
  const l = oe(),
    s = tt.useRef(void 0);
  return (
    Nl(() => {
      const u = l.latestLocation.href;
      (s.current === void 0 || s.current !== u) &&
        (l.emit({
          type: "onRendered",
          ...zi(l.stores.location.get(), l.stores.resolvedLocation.get()),
        }),
        (s.current = u));
    }, [l.latestLocation.state.__TSR_key, n, l]),
    null
  );
}
var Gy = tt.memo(function ({ matchId: l }) {
    const s = oe(),
      u = (S, g) => s.getMatch(S.id)?._nonReactive[g] ?? S._nonReactive[g],
      c = s.stores.matchStores.get(l);
    c || Te();
    const f = Kt(c, (S) => S),
      h = f.routeId,
      y = s.routesById[h],
      m = tt.useMemo(() => {
        const S = (s.routesById[h].options.remountDeps ?? s.options.defaultRemountDeps)?.({
          routeId: h,
          loaderDeps: f.loaderDeps,
          params: f._strictParams,
          search: f._strictSearch,
        });
        return S ? JSON.stringify(S) : void 0;
      }, [
        h,
        f.loaderDeps,
        f._strictParams,
        f._strictSearch,
        s.options.defaultRemountDeps,
        s.routesById,
      ]),
      p = tt.useMemo(() => {
        const S = y.options.component ?? s.options.defaultComponent;
        return S ? P.jsx(S, {}, m) : P.jsx(hg, {});
      }, [m, y.options.component, s.options.defaultComponent]);
    if (f._displayPending) throw u(f, "displayPendingPromise");
    if (f._forcePending) throw u(f, "minPendingPromise");
    if (f.status === "pending") {
      const S = y.options.pendingMinMs ?? s.options.defaultPendingMinMs;
      if (S) {
        const g = s.getMatch(f.id);
        if (g && !g._nonReactive.minPendingPromise) {
          const _ = Na();
          ((g._nonReactive.minPendingPromise = _),
            setTimeout(() => {
              (_.resolve(), (g._nonReactive.minPendingPromise = void 0));
            }, S));
        }
      }
      throw u(f, "loadPromise");
    }
    if (f.status === "notFound") return (ne(f.error) || Te(), fg(s, y, f.error));
    if (f.status === "redirected") throw (Ee(f.error) || Te(), u(f, "loadPromise"));
    if (f.status === "error") throw f.error;
    return p;
  }),
  hg = tt.memo(function () {
    const l = oe(),
      s = tt.useContext(qr);
    let u,
      c = !1,
      f;
    {
      const p = s ? l.stores.matchStores.get(s) : void 0;
      (([u, c] = Kt(p, (S) => [S?.routeId, S?.globalNotFound ?? !1])),
        (f = Kt(l.stores.matchesId, (S) => S[S.findIndex((g) => g === s) + 1])));
    }
    const h = u ? l.routesById[u] : void 0,
      y = l.options.defaultPendingComponent ? P.jsx(l.options.defaultPendingComponent, {}) : null;
    if (c) return (h || Te(), fg(l, h, void 0));
    if (!f) return null;
    const m = P.jsx(dg, { matchId: f });
    return u === Ua ? P.jsx(tt.Suspense, { fallback: y, children: m }) : m;
  });
function iT() {
  const n = oe(),
    l = tt.useRef({ router: n, mounted: !1 }),
    [s, u] = tt.useState(!1),
    c = Kt(n.stores.isLoading, (g) => g),
    f = Kt(n.stores.hasPending, (g) => g),
    h = mc(c),
    y = c || s || f,
    m = mc(y),
    p = c || f,
    S = mc(p);
  return (
    (n.startTransition = (g) => {
      (u(!0),
        tt.startTransition(() => {
          (g(), u(!1));
        }));
    }),
    tt.useEffect(() => {
      const g = n.history.subscribe(n.load),
        _ = n.buildLocation({
          to: n.latestLocation.pathname,
          search: !0,
          params: !0,
          hash: !0,
          state: !0,
          _includeValidateSearch: !0,
        });
      return (
        An(n.latestLocation.publicHref) !== An(_.publicHref) &&
          n.commitLocation({ ..._, replace: !0 }),
        () => {
          g();
        }
      );
    }, [n, n.history]),
    Nl(() => {
      if ((typeof window < "u" && n.ssr) || (l.current.router === n && l.current.mounted)) return;
      ((l.current = { router: n, mounted: !0 }),
        (async () => {
          try {
            await n.load();
          } catch (_) {
            console.error(_);
          }
        })());
    }, [n]),
    Nl(() => {
      h &&
        !c &&
        n.emit({ type: "onLoad", ...zi(n.stores.location.get(), n.stores.resolvedLocation.get()) });
    }, [h, n, c]),
    Nl(() => {
      S &&
        !p &&
        n.emit({
          type: "onBeforeRouteMount",
          ...zi(n.stores.location.get(), n.stores.resolvedLocation.get()),
        });
    }, [p, S, n]),
    Nl(() => {
      if (m && !y) {
        const g = zi(n.stores.location.get(), n.stores.resolvedLocation.get());
        (n.emit({ type: "onResolved", ...g }),
          ag(() => {
            (n.stores.status.set("idle"), n.stores.resolvedLocation.set(n.stores.location.get()));
          }));
      }
    }, [y, m, n]),
    null
  );
}
function lT() {
  const n = oe(),
    l = n.routesById[Ua].options.pendingComponent ?? n.options.defaultPendingComponent,
    s = l ? P.jsx(l, {}) : null,
    u = P.jsxs(typeof document < "u" && n.ssr ? Ci : tt.Suspense, {
      fallback: s,
      children: [P.jsx(iT, {}), P.jsx(sT, {})],
    });
  return n.options.InnerWrap ? P.jsx(n.options.InnerWrap, { children: u }) : u;
}
function sT() {
  const n = oe(),
    l = Kt(n.stores.firstId, (c) => c),
    s = Kt(n.stores.loadedAt, (c) => c),
    u = l ? P.jsx(dg, { matchId: l }) : null;
  return P.jsx(qr.Provider, {
    value: l,
    children: n.options.disableGlobalCatchBoundary
      ? u
      : P.jsx(Gc, { getResetKey: () => s, errorComponent: Qc, onCatch: void 0, children: u }),
  });
}
var rT = (n) => ({ createMutableStore: Ny, createReadonlyStore: Ny, batch: ag }),
  uT = (n) => new oT(n),
  oT = class extends ZS {
    constructor(n) {
      super(n, rT);
    }
  };
function cT({ router: n, children: l, ...s }) {
  np(s) && n.update({ ...n.options, ...s, context: { ...n.options.context, ...s.context } });
  const u = P.jsx(eg.Provider, { value: n, children: l });
  return n.options.Wrap ? P.jsx(n.options.Wrap, { children: u }) : u;
}
function fT({ router: n, ...l }) {
  return P.jsx(cT, { router: n, ...l, children: P.jsx(lT, {}) });
}
function Qy(n, l) {
  if (l)
    for (const [s, u] of Object.entries(l))
      s !== "suppressHydrationWarning" &&
        u !== void 0 &&
        u !== !1 &&
        n.setAttribute(s, typeof u == "boolean" ? "" : String(u));
}
function mg(n) {
  const { attrs: l, children: s, nonce: u, preventScriptHoist: c } = n;
  switch (n.tag) {
    case "title":
      return P.jsx("title", { ...l, suppressHydrationWarning: !0, children: s });
    case "meta":
      return P.jsx("meta", { ...l, suppressHydrationWarning: !0 });
    case "link":
      return P.jsx("link", {
        ...l,
        precedence: l?.precedence ?? (l?.rel === "stylesheet" ? "default" : void 0),
        nonce: u,
        suppressHydrationWarning: !0,
      });
    case "style":
      return (
        n.inlineCss,
        P.jsx("style", { ...l, dangerouslySetInnerHTML: { __html: s }, nonce: u })
      );
    case "script":
      return P.jsx(dT, { attrs: l, preventScriptHoist: c, children: s });
    default:
      return null;
  }
}
function dT({ attrs: n, children: l, preventScriptHoist: s }) {
  oe();
  const u = Vc(),
    c =
      typeof n?.type == "string" &&
      n.type !== "" &&
      n.type !== "text/javascript" &&
      n.type !== "module";
  if (
    (tt.useEffect(() => {
      if (!c) {
        if (n?.src) {
          const f = (() => {
            try {
              const y = document.baseURI || window.location.href;
              return new URL(n.src, y).href;
            } catch {
              return n.src;
            }
          })();
          for (const y of document.querySelectorAll("script[src]")) if (y.src === f) return;
          const h = document.createElement("script");
          return (Qy(h, n), document.head.appendChild(h), () => h.remove());
        }
        if (typeof l == "string") {
          const f = typeof n?.type == "string" ? n.type : "text/javascript",
            h = typeof n?.nonce == "string" ? n.nonce : void 0;
          for (const m of document.querySelectorAll("script:not([src])")) {
            if (!(m instanceof HTMLScriptElement)) continue;
            const p = m.getAttribute("type") ?? "text/javascript",
              S = m.getAttribute("nonce") ?? void 0;
            if (m.textContent === l && p === f && S === h) return;
          }
          const y = document.createElement("script");
          return ((y.textContent = l), Qy(y, n), document.head.appendChild(y), () => y.remove());
        }
      }
    }, [n, l, c]),
    c && typeof l == "string")
  )
    return P.jsx("script", {
      ...n,
      suppressHydrationWarning: !0,
      dangerouslySetInnerHTML: { __html: l },
    });
  if (!u) {
    if (n?.src) return P.jsx("script", { ...n, suppressHydrationWarning: !0 });
    if (typeof l == "string")
      return P.jsx("script", {
        ...n,
        dangerouslySetInnerHTML: { __html: l },
        suppressHydrationWarning: !0,
      });
  }
  return null;
}
var hT = (n) => {
  const l = oe(),
    s = l.options.ssr?.nonce,
    u = Kt(l.stores.matches, (g) => g.map((_) => _.meta).filter((_) => _ !== void 0), he),
    c = tt.useMemo(() => {
      const g = [],
        _ = {};
      let E;
      for (let x = u.length - 1; x >= 0; x--) {
        const z = u[x];
        for (let R = z.length - 1; R >= 0; R--) {
          const w = z[R];
          if (w)
            if (w.title) E || (E = { tag: "title", children: w.title });
            else if ("script:ld+json" in w)
              try {
                const N = JSON.stringify(w["script:ld+json"]);
                g.push({ tag: "script", attrs: { type: "application/ld+json" }, children: fS(N) });
              } catch {}
            else {
              const N = w.name ?? w.property;
              if (N) {
                if (_[N]) continue;
                _[N] = !0;
              }
              g.push({ tag: "meta", attrs: { ...w, nonce: s } });
            }
        }
      }
      return (
        E && g.push(E),
        s && g.push({ tag: "meta", attrs: { property: "csp-nonce", content: s } }),
        g.reverse(),
        g
      );
    }, [u, s]),
    f = Kt(
      l.stores.matches,
      (g) =>
        g
          .flatMap((_) => _.links ?? [])
          .filter((_) => _ !== void 0)
          .map((_) => ({ tag: "link", attrs: { ..._, nonce: s } })),
      he,
    ),
    h = Kt(
      l.stores.matches,
      (g) => {
        const _ = l.ssr?.manifest,
          E = [];
        return (
          _ &&
            (g.forEach((x) => {
              _.routes[x.routeId]?.css?.forEach((z) => {
                const R = fb(z);
                E.push({
                  tag: "link",
                  attrs: {
                    rel: "stylesheet",
                    ...R,
                    crossOrigin: gp(n, "stylesheet") ?? R.crossOrigin,
                    suppressHydrationWarning: !0,
                    nonce: s,
                  },
                });
              });
            }),
            _.inlineStyle &&
              E.push({
                tag: "style",
                attrs: { ..._.inlineStyle.attrs, nonce: s },
                children: _.inlineStyle.children,
                inlineCss: !0,
              })),
          E
        );
      },
      he,
    ),
    y = Kt(
      l.stores.matches,
      (g) => {
        const _ = [],
          E = l.ssr?.manifest;
        return (
          E &&
            g.forEach((x) => {
              E.routes[x.routeId]?.preloads?.forEach((z) => {
                _.push({ tag: "link", attrs: { ...ob(E, z, n), nonce: s } });
              });
            }),
          _
        );
      },
      he,
    ),
    m = Kt(
      l.stores.matches,
      (g) =>
        g
          .flatMap((_) => _.styles ?? [])
          .filter((_) => _ !== void 0)
          .map(({ children: _, ...E }) => ({
            tag: "style",
            attrs: { ...E, nonce: s },
            children: _,
          })),
      he,
    ),
    p = Kt(
      l.stores.matches,
      (g) =>
        g
          .flatMap((_) => _.headScripts ?? [])
          .filter((_) => _ !== void 0)
          .map(({ children: _, ...E }) => ({
            tag: "script",
            attrs: { ...E, nonce: s },
            children: _,
          })),
      he,
    ),
    S = [];
  return (yr(S, c), S.push(...y), yr(S, f), S.push(...h), yr(S, m), yr(S, p), S);
};
function mT(n) {
  const l = hT(n.assetCrossOrigin),
    s = oe().options.ssr?.nonce;
  return P.jsx(P.Fragment, {
    children: l.map((u) =>
      tt.createElement(mg, { ...u, key: `tsr-meta-${JSON.stringify(u)}`, nonce: s }),
    ),
  });
}
var yT = () => {
  const n = oe(),
    l = n.options.ssr?.nonce,
    s = (f) => {
      const h = [],
        y = n.ssr?.manifest;
      if (!y) return [];
      for (const m of f) {
        const p = y.routes[m.routeId]?.scripts;
        if (p)
          for (const S of p)
            h.push({
              tag: "script",
              attrs: { ...S.attrs, nonce: l },
              children: S.children,
              ...(typeof S.attrs?.src == "string" ? { preventScriptHoist: !0 } : {}),
            });
      }
      return h;
    },
    u = (f) =>
      f
        .map((h) => h.scripts)
        .flat(1)
        .filter(Boolean)
        .map(({ children: h, ...y }) => ({
          tag: "script",
          attrs: { ...y, suppressHydrationWarning: !0, nonce: l },
          children: h,
        })),
    c = Kt(n.stores.matches, s, he);
  return pT(n, Kt(n.stores.matches, u, he), c);
};
function pT(n, l, s) {
  const u = [...l, ...s];
  return P.jsx(P.Fragment, {
    children: u.map((c, f) => tt.createElement(mg, { ...c, key: `tsr-scripts-${c.tag}-${f}` })),
  });
}
var Oi = (n, l) => {
  const s = { type: "request", ...(l || n) };
  return {
    options: s,
    middleware: (u) => Oi({}, Object.assign(s, { middleware: u })),
    inputValidator: (u) => Oi({}, Object.assign(s, { inputValidator: u })),
    client: (u) => Oi({}, Object.assign(s, { client: u })),
    server: (u) => Oi({}, Object.assign(s, { server: u })),
  };
};
function yg(n, l) {
  for (let s = 0, u = l.length; s < u; s++) {
    const c = l[s];
    n.has(c) || (n.add(c), c.extends && yg(n, c.extends));
  }
}
var gT = (n) => ({
  getOptions: async () => {
    const l = await n();
    if (l.serializationAdapters) {
      const s = new Set();
      (yg(s, l.serializationAdapters), (l.serializationAdapters = Array.from(s)));
    }
    return l;
  },
  createMiddleware: Oi,
});
const vT = Oi(),
  Vy = gT(() => ({ requestMiddleware: [vT] }));
var Yr = class {
    constructor() {
      ((this.listeners = new Set()), (this.subscribe = this.subscribe.bind(this)));
    }
    subscribe(n) {
      return (
        this.listeners.add(n),
        this.onSubscribe(),
        () => {
          (this.listeners.delete(n), this.onUnsubscribe());
        }
      );
    }
    hasListeners() {
      return this.listeners.size > 0;
    }
    onSubscribe() {}
    onUnsubscribe() {}
  },
  ST = class extends Yr {
    #t;
    #e;
    #n;
    constructor() {
      (super(),
        (this.#n = (n) => {
          if (typeof window < "u" && window.addEventListener) {
            const l = () => n();
            return (
              window.addEventListener("visibilitychange", l, !1),
              () => {
                window.removeEventListener("visibilitychange", l);
              }
            );
          }
        }));
    }
    onSubscribe() {
      this.#e || this.setEventListener(this.#n);
    }
    onUnsubscribe() {
      this.hasListeners() || (this.#e?.(), (this.#e = void 0));
    }
    setEventListener(n) {
      ((this.#n = n),
        this.#e?.(),
        (this.#e = n((l) => {
          typeof l == "boolean" ? this.setFocused(l) : this.onFocus();
        })));
    }
    setFocused(n) {
      this.#t !== n && ((this.#t = n), this.onFocus());
    }
    onFocus() {
      const n = this.isFocused();
      this.listeners.forEach((l) => {
        l(n);
      });
    }
    isFocused() {
      return typeof this.#t == "boolean"
        ? this.#t
        : globalThis.document?.visibilityState !== "hidden";
    }
  },
  pg = new ST(),
  bT = {
    setTimeout: (n, l) => setTimeout(n, l),
    clearTimeout: (n) => clearTimeout(n),
    setInterval: (n, l) => setInterval(n, l),
    clearInterval: (n) => clearInterval(n),
  },
  _T = class {
    #t = bT;
    #e = !1;
    setTimeoutProvider(n) {
      this.#t = n;
    }
    setTimeout(n, l) {
      return this.#t.setTimeout(n, l);
    }
    clearTimeout(n) {
      this.#t.clearTimeout(n);
    }
    setInterval(n, l) {
      return this.#t.setInterval(n, l);
    }
    clearInterval(n) {
      this.#t.clearInterval(n);
    }
  },
  zc = new _T();
function ET(n) {
  setTimeout(n, 0);
}
var TT = typeof window > "u" || "Deno" in globalThis;
function Pe() {}
function RT(n, l) {
  return typeof n == "function" ? n(l) : n;
}
function AT(n) {
  return typeof n == "number" && n >= 0 && n !== 1 / 0;
}
function xT(n, l) {
  return Math.max(n + (l || 0) - Date.now(), 0);
}
function Dc(n, l) {
  return typeof n == "function" ? n(l) : n;
}
function MT(n, l) {
  return typeof n == "function" ? n(l) : n;
}
function Xy(n, l) {
  const { type: s = "all", exact: u, fetchStatus: c, predicate: f, queryKey: h, stale: y } = n;
  if (h) {
    if (u) {
      if (l.queryHash !== Kc(h, l.options)) return !1;
    } else if (!Xl(l.queryKey, h)) return !1;
  }
  if (s !== "all") {
    const m = l.isActive();
    if ((s === "active" && !m) || (s === "inactive" && m)) return !1;
  }
  return !(
    (typeof y == "boolean" && l.isStale() !== y) ||
    (c && c !== l.state.fetchStatus) ||
    (f && !f(l))
  );
}
function Ky(n, l) {
  const { exact: s, status: u, predicate: c, mutationKey: f } = n;
  if (f) {
    if (!l.options.mutationKey) return !1;
    if (s) {
      if (Vl(l.options.mutationKey) !== Vl(f)) return !1;
    } else if (!Xl(l.options.mutationKey, f)) return !1;
  }
  return !((u && l.state.status !== u) || (c && !c(l)));
}
function Kc(n, l) {
  return (l?.queryKeyHashFn || Vl)(n);
}
function Vl(n) {
  return JSON.stringify(n, (l, s) =>
    Lc(s)
      ? Object.keys(s)
          .sort()
          .reduce((u, c) => ((u[c] = s[c]), u), {})
      : s,
  );
}
function Xl(n, l) {
  return n === l
    ? !0
    : typeof n != typeof l
      ? !1
      : n && l && typeof n == "object" && typeof l == "object"
        ? Object.keys(l).every((s) => Xl(n[s], l[s]))
        : !1;
}
var CT = Object.prototype.hasOwnProperty;
function gg(n, l, s = 0) {
  if (n === l) return n;
  if (s > 500) return l;
  const u = Zy(n) && Zy(l);
  if (!u && !(Lc(n) && Lc(l))) return l;
  const f = (u ? n : Object.keys(n)).length,
    h = u ? l : Object.keys(l),
    y = h.length,
    m = u ? new Array(y) : {};
  let p = 0;
  for (let S = 0; S < y; S++) {
    const g = u ? S : h[S],
      _ = n[g],
      E = l[g];
    if (_ === E) {
      ((m[g] = _), (u ? S < f : CT.call(n, g)) && p++);
      continue;
    }
    if (_ === null || E === null || typeof _ != "object" || typeof E != "object") {
      m[g] = E;
      continue;
    }
    const x = gg(_, E, s + 1);
    ((m[g] = x), x === _ && p++);
  }
  return f === y && p === f ? n : m;
}
function Zy(n) {
  return Array.isArray(n) && n.length === Object.keys(n).length;
}
function Lc(n) {
  if (!Py(n)) return !1;
  const l = n.constructor;
  if (l === void 0) return !0;
  const s = l.prototype;
  return !(
    !Py(s) ||
    !s.hasOwnProperty("isPrototypeOf") ||
    Object.getPrototypeOf(n) !== Object.prototype
  );
}
function Py(n) {
  return Object.prototype.toString.call(n) === "[object Object]";
}
function wT(n) {
  return new Promise((l) => {
    zc.setTimeout(l, n);
  });
}
function OT(n, l, s) {
  return typeof s.structuralSharing == "function"
    ? s.structuralSharing(n, l)
    : s.structuralSharing !== !1
      ? gg(n, l)
      : l;
}
function zT(n, l, s = 0) {
  const u = [...n, l];
  return s && u.length > s ? u.slice(1) : u;
}
function DT(n, l, s = 0) {
  const u = [l, ...n];
  return s && u.length > s ? u.slice(0, -1) : u;
}
var Zc = Symbol();
function vg(n, l) {
  return !n.queryFn && l?.initialPromise
    ? () => l.initialPromise
    : !n.queryFn || n.queryFn === Zc
      ? () => Promise.reject(new Error(`Missing queryFn: '${n.queryHash}'`))
      : n.queryFn;
}
function LT(n, l, s) {
  let u = !1,
    c;
  return (
    Object.defineProperty(n, "signal", {
      enumerable: !0,
      get: () => (
        (c ??= l()),
        u || ((u = !0), c.aborted ? s() : c.addEventListener("abort", s, { once: !0 })),
        c
      ),
    }),
    n
  );
}
var Sg = (() => {
  let n = () => TT;
  return {
    isServer() {
      return n();
    },
    setIsServer(l) {
      n = l;
    },
  };
})();
function UT() {
  let n, l;
  const s = new Promise((c, f) => {
    ((n = c), (l = f));
  });
  ((s.status = "pending"), s.catch(() => {}));
  function u(c) {
    (Object.assign(s, c), delete s.resolve, delete s.reject);
  }
  return (
    (s.resolve = (c) => {
      (u({ status: "fulfilled", value: c }), n(c));
    }),
    (s.reject = (c) => {
      (u({ status: "rejected", reason: c }), l(c));
    }),
    s
  );
}
var BT = ET;
function NT() {
  let n = [],
    l = 0,
    s = (y) => {
      y();
    },
    u = (y) => {
      y();
    },
    c = BT;
  const f = (y) => {
      l
        ? n.push(y)
        : c(() => {
            s(y);
          });
    },
    h = () => {
      const y = n;
      ((n = []),
        y.length &&
          c(() => {
            u(() => {
              y.forEach((m) => {
                s(m);
              });
            });
          }));
    };
  return {
    batch: (y) => {
      let m;
      l++;
      try {
        m = y();
      } finally {
        (l--, l || h());
      }
      return m;
    },
    batchCalls:
      (y) =>
      (...m) => {
        f(() => {
          y(...m);
        });
      },
    schedule: f,
    setNotifyFunction: (y) => {
      s = y;
    },
    setBatchNotifyFunction: (y) => {
      u = y;
    },
    setScheduler: (y) => {
      c = y;
    },
  };
}
var me = NT(),
  jT = class extends Yr {
    #t = !0;
    #e;
    #n;
    constructor() {
      (super(),
        (this.#n = (n) => {
          if (typeof window < "u" && window.addEventListener) {
            const l = () => n(!0),
              s = () => n(!1);
            return (
              window.addEventListener("online", l, !1),
              window.addEventListener("offline", s, !1),
              () => {
                (window.removeEventListener("online", l), window.removeEventListener("offline", s));
              }
            );
          }
        }));
    }
    onSubscribe() {
      this.#e || this.setEventListener(this.#n);
    }
    onUnsubscribe() {
      this.hasListeners() || (this.#e?.(), (this.#e = void 0));
    }
    setEventListener(n) {
      ((this.#n = n), this.#e?.(), (this.#e = n(this.setOnline.bind(this))));
    }
    setOnline(n) {
      this.#t !== n &&
        ((this.#t = n),
        this.listeners.forEach((s) => {
          s(n);
        }));
    }
    isOnline() {
      return this.#t;
    }
  },
  Dr = new jT();
function HT(n) {
  return Math.min(1e3 * 2 ** n, 3e4);
}
function bg(n) {
  return (n ?? "online") === "online" ? Dr.isOnline() : !0;
}
var Uc = class extends Error {
  constructor(n) {
    (super("CancelledError"), (this.revert = n?.revert), (this.silent = n?.silent));
  }
};
function _g(n) {
  let l = !1,
    s = 0,
    u;
  const c = UT(),
    f = () => c.status !== "pending",
    h = (z) => {
      if (!f()) {
        const R = new Uc(z);
        (_(R), n.onCancel?.(R));
      }
    },
    y = () => {
      l = !0;
    },
    m = () => {
      l = !1;
    },
    p = () => pg.isFocused() && (n.networkMode === "always" || Dr.isOnline()) && n.canRun(),
    S = () => bg(n.networkMode) && n.canRun(),
    g = (z) => {
      f() || (u?.(), c.resolve(z));
    },
    _ = (z) => {
      f() || (u?.(), c.reject(z));
    },
    E = () =>
      new Promise((z) => {
        ((u = (R) => {
          (f() || p()) && z(R);
        }),
          n.onPause?.());
      }).then(() => {
        ((u = void 0), f() || n.onContinue?.());
      }),
    x = () => {
      if (f()) return;
      let z;
      const R = s === 0 ? n.initialPromise : void 0;
      try {
        z = R ?? n.fn();
      } catch (w) {
        z = Promise.reject(w);
      }
      Promise.resolve(z)
        .then(g)
        .catch((w) => {
          if (f()) return;
          const N = n.retry ?? (Sg.isServer() ? 0 : 3),
            X = n.retryDelay ?? HT,
            Y = typeof X == "function" ? X(s, w) : X,
            K = N === !0 || (typeof N == "number" && s < N) || (typeof N == "function" && N(s, w));
          if (l || !K) {
            _(w);
            return;
          }
          (s++,
            n.onFail?.(s, w),
            wT(Y)
              .then(() => (p() ? void 0 : E()))
              .then(() => {
                l ? _(w) : x();
              }));
        });
    };
  return {
    promise: c,
    status: () => c.status,
    cancel: h,
    continue: () => (u?.(), c),
    cancelRetry: y,
    continueRetry: m,
    canStart: S,
    start: () => (S() ? x() : E().then(x), c),
  };
}
var Eg = class {
  #t;
  destroy() {
    this.clearGcTimeout();
  }
  scheduleGc() {
    (this.clearGcTimeout(),
      AT(this.gcTime) &&
        (this.#t = zc.setTimeout(() => {
          this.optionalRemove();
        }, this.gcTime)));
  }
  updateGcTime(n) {
    this.gcTime = Math.max(this.gcTime || 0, n ?? (Sg.isServer() ? 1 / 0 : 300 * 1e3));
  }
  clearGcTimeout() {
    this.#t !== void 0 && (zc.clearTimeout(this.#t), (this.#t = void 0));
  }
};
function qT(n) {
  return {
    onFetch: (l, s) => {
      const u = l.options,
        c = l.fetchOptions?.meta?.fetchMore?.direction,
        f = l.state.data?.pages || [],
        h = l.state.data?.pageParams || [];
      let y = { pages: [], pageParams: [] },
        m = 0;
      const p = async () => {
        let S = !1;
        const g = (x) => {
            LT(
              x,
              () => l.signal,
              () => (S = !0),
            );
          },
          _ = vg(l.options, l.fetchOptions),
          E = async (x, z, R) => {
            if (S) return Promise.reject(l.signal.reason);
            if (z == null && x.pages.length) return Promise.resolve(x);
            const N = (() => {
                const J = {
                  client: l.client,
                  queryKey: l.queryKey,
                  pageParam: z,
                  direction: R ? "backward" : "forward",
                  meta: l.options.meta,
                };
                return (g(J), J);
              })(),
              X = await _(N),
              { maxPages: Y } = l.options,
              K = R ? DT : zT;
            return { pages: K(x.pages, X, Y), pageParams: K(x.pageParams, z, Y) };
          };
        if (c && f.length) {
          const x = c === "backward",
            z = x ? YT : Fy,
            R = { pages: f, pageParams: h },
            w = z(u, R);
          y = await E(R, w, x);
        } else {
          const x = n ?? f.length;
          do {
            const z = m === 0 ? (h[0] ?? u.initialPageParam) : Fy(u, y);
            if (m > 0 && z == null) break;
            ((y = await E(y, z)), m++);
          } while (m < x);
        }
        return y;
      };
      l.options.persister
        ? (l.fetchFn = () =>
            l.options.persister?.(
              p,
              { client: l.client, queryKey: l.queryKey, meta: l.options.meta, signal: l.signal },
              s,
            ))
        : (l.fetchFn = p);
    },
  };
}
function Fy(n, { pages: l, pageParams: s }) {
  const u = l.length - 1;
  return l.length > 0 ? n.getNextPageParam(l[u], l, s[u], s) : void 0;
}
function YT(n, { pages: l, pageParams: s }) {
  return l.length > 0 ? n.getPreviousPageParam?.(l[0], l, s[0], s) : void 0;
}
var GT = class extends Eg {
  #t;
  #e;
  #n;
  #i;
  #l;
  #a;
  #u;
  #s;
  constructor(n) {
    (super(),
      (this.#s = !1),
      (this.#u = n.defaultOptions),
      this.setOptions(n.options),
      (this.observers = []),
      (this.#l = n.client),
      (this.#i = this.#l.getQueryCache()),
      (this.queryKey = n.queryKey),
      (this.queryHash = n.queryHash),
      (this.#e = ky(this.options)),
      (this.state = n.state ?? this.#e),
      this.scheduleGc());
  }
  get meta() {
    return this.options.meta;
  }
  get queryType() {
    return this.#t;
  }
  get promise() {
    return this.#a?.promise;
  }
  setOptions(n) {
    if (
      ((this.options = { ...this.#u, ...n }),
      n?._type && (this.#t = n._type),
      this.updateGcTime(this.options.gcTime),
      this.state && this.state.data === void 0)
    ) {
      const l = ky(this.options);
      l.data !== void 0 && (this.setState(Jy(l.data, l.dataUpdatedAt)), (this.#e = l));
    }
  }
  optionalRemove() {
    !this.observers.length && this.state.fetchStatus === "idle" && this.#i.remove(this);
  }
  setData(n, l) {
    const s = OT(this.state.data, n, this.options);
    return (
      this.#r({ data: s, type: "success", dataUpdatedAt: l?.updatedAt, manual: l?.manual }),
      s
    );
  }
  setState(n) {
    this.#r({ type: "setState", state: n });
  }
  cancel(n) {
    const l = this.#a?.promise;
    return (this.#a?.cancel(n), l ? l.then(Pe).catch(Pe) : Promise.resolve());
  }
  destroy() {
    (super.destroy(), this.cancel({ silent: !0 }));
  }
  get resetState() {
    return this.#e;
  }
  reset() {
    (this.destroy(), this.setState(this.resetState));
  }
  isActive() {
    return this.observers.some((n) => MT(n.options.enabled, this) !== !1);
  }
  isDisabled() {
    return this.getObserversCount() > 0
      ? !this.isActive()
      : this.options.queryFn === Zc || !this.isFetched();
  }
  isFetched() {
    return this.state.dataUpdateCount + this.state.errorUpdateCount > 0;
  }
  isStatic() {
    return this.getObserversCount() > 0
      ? this.observers.some((n) => Dc(n.options.staleTime, this) === "static")
      : !1;
  }
  isStale() {
    return this.getObserversCount() > 0
      ? this.observers.some((n) => n.getCurrentResult().isStale)
      : this.state.data === void 0 || this.state.isInvalidated;
  }
  isStaleByTime(n = 0) {
    return this.state.data === void 0
      ? !0
      : n === "static"
        ? !1
        : this.state.isInvalidated
          ? !0
          : !xT(this.state.dataUpdatedAt, n);
  }
  onFocus() {
    (this.observers.find((l) => l.shouldFetchOnWindowFocus())?.refetch({ cancelRefetch: !1 }),
      this.#a?.continue());
  }
  onOnline() {
    (this.observers.find((l) => l.shouldFetchOnReconnect())?.refetch({ cancelRefetch: !1 }),
      this.#a?.continue());
  }
  addObserver(n) {
    this.observers.includes(n) ||
      (this.observers.push(n),
      this.clearGcTimeout(),
      this.#i.notify({ type: "observerAdded", query: this, observer: n }));
  }
  removeObserver(n) {
    this.observers.includes(n) &&
      ((this.observers = this.observers.filter((l) => l !== n)),
      this.observers.length ||
        (this.#a && (this.#s || this.#o() ? this.#a.cancel({ revert: !0 }) : this.#a.cancelRetry()),
        this.scheduleGc()),
      this.#i.notify({ type: "observerRemoved", query: this, observer: n }));
  }
  getObserversCount() {
    return this.observers.length;
  }
  #o() {
    return this.state.fetchStatus === "paused" && this.state.status === "pending";
  }
  invalidate() {
    this.state.isInvalidated || this.#r({ type: "invalidate" });
  }
  async fetch(n, l) {
    if (this.state.fetchStatus !== "idle" && this.#a?.status() !== "rejected") {
      if (this.state.data !== void 0 && l?.cancelRefetch) this.cancel({ silent: !0 });
      else if (this.#a) return (this.#a.continueRetry(), this.#a.promise);
    }
    if ((n && this.setOptions(n), !this.options.queryFn)) {
      const m = this.observers.find((p) => p.options.queryFn);
      m && this.setOptions(m.options);
    }
    const s = new AbortController(),
      u = (m) => {
        Object.defineProperty(m, "signal", {
          enumerable: !0,
          get: () => ((this.#s = !0), s.signal),
        });
      },
      c = () => {
        const m = vg(this.options, l),
          S = (() => {
            const g = { client: this.#l, queryKey: this.queryKey, meta: this.meta };
            return (u(g), g);
          })();
        return ((this.#s = !1), this.options.persister ? this.options.persister(m, S, this) : m(S));
      },
      h = (() => {
        const m = {
          fetchOptions: l,
          options: this.options,
          queryKey: this.queryKey,
          client: this.#l,
          state: this.state,
          fetchFn: c,
        };
        return (u(m), m);
      })();
    ((this.#t === "infinite" ? qT(this.options.pages) : this.options.behavior)?.onFetch(h, this),
      (this.#n = this.state),
      (this.state.fetchStatus === "idle" || this.state.fetchMeta !== h.fetchOptions?.meta) &&
        this.#r({ type: "fetch", meta: h.fetchOptions?.meta }),
      (this.#a = _g({
        initialPromise: l?.initialPromise,
        fn: h.fetchFn,
        onCancel: (m) => {
          (m instanceof Uc && m.revert && this.setState({ ...this.#n, fetchStatus: "idle" }),
            s.abort());
        },
        onFail: (m, p) => {
          this.#r({ type: "failed", failureCount: m, error: p });
        },
        onPause: () => {
          this.#r({ type: "pause" });
        },
        onContinue: () => {
          this.#r({ type: "continue" });
        },
        retry: h.options.retry,
        retryDelay: h.options.retryDelay,
        networkMode: h.options.networkMode,
        canRun: () => !0,
      })));
    try {
      const m = await this.#a.start();
      if (m === void 0) throw new Error(`${this.queryHash} data is undefined`);
      return (
        this.setData(m),
        this.#i.config.onSuccess?.(m, this),
        this.#i.config.onSettled?.(m, this.state.error, this),
        m
      );
    } catch (m) {
      if (m instanceof Uc) {
        if (m.silent) return this.#a.promise;
        if (m.revert) {
          if (this.state.data === void 0) throw m;
          return this.state.data;
        }
      }
      throw (
        this.#r({ type: "error", error: m }),
        this.#i.config.onError?.(m, this),
        this.#i.config.onSettled?.(this.state.data, m, this),
        m
      );
    } finally {
      this.scheduleGc();
    }
  }
  #r(n) {
    const l = (s) => {
      switch (n.type) {
        case "failed":
          return { ...s, fetchFailureCount: n.failureCount, fetchFailureReason: n.error };
        case "pause":
          return { ...s, fetchStatus: "paused" };
        case "continue":
          return { ...s, fetchStatus: "fetching" };
        case "fetch":
          return { ...s, ...QT(s.data, this.options), fetchMeta: n.meta ?? null };
        case "success":
          const u = {
            ...s,
            ...Jy(n.data, n.dataUpdatedAt),
            dataUpdateCount: s.dataUpdateCount + 1,
            ...(!n.manual && {
              fetchStatus: "idle",
              fetchFailureCount: 0,
              fetchFailureReason: null,
            }),
          };
          return ((this.#n = n.manual ? u : void 0), u);
        case "error":
          const c = n.error;
          return {
            ...s,
            error: c,
            errorUpdateCount: s.errorUpdateCount + 1,
            errorUpdatedAt: Date.now(),
            fetchFailureCount: s.fetchFailureCount + 1,
            fetchFailureReason: c,
            fetchStatus: "idle",
            status: "error",
            isInvalidated: !0,
          };
        case "invalidate":
          return { ...s, isInvalidated: !0 };
        case "setState":
          return { ...s, ...n.state };
      }
    };
    ((this.state = l(this.state)),
      me.batch(() => {
        (this.observers.forEach((s) => {
          s.onQueryUpdate();
        }),
          this.#i.notify({ query: this, type: "updated", action: n }));
      }));
  }
};
function QT(n, l) {
  return {
    fetchFailureCount: 0,
    fetchFailureReason: null,
    fetchStatus: bg(l.networkMode) ? "fetching" : "paused",
    ...(n === void 0 && { error: null, status: "pending" }),
  };
}
function Jy(n, l) {
  return {
    data: n,
    dataUpdatedAt: l ?? Date.now(),
    error: null,
    isInvalidated: !1,
    status: "success",
  };
}
function ky(n) {
  const l = typeof n.initialData == "function" ? n.initialData() : n.initialData,
    s = l !== void 0,
    u = s
      ? typeof n.initialDataUpdatedAt == "function"
        ? n.initialDataUpdatedAt()
        : n.initialDataUpdatedAt
      : 0;
  return {
    data: l,
    dataUpdateCount: 0,
    dataUpdatedAt: s ? (u ?? Date.now()) : 0,
    error: null,
    errorUpdateCount: 0,
    errorUpdatedAt: 0,
    fetchFailureCount: 0,
    fetchFailureReason: null,
    fetchMeta: null,
    isInvalidated: !1,
    status: s ? "success" : "pending",
    fetchStatus: "idle",
  };
}
var VT = class extends Eg {
  #t;
  #e;
  #n;
  #i;
  constructor(n) {
    (super(),
      (this.#t = n.client),
      (this.mutationId = n.mutationId),
      (this.#n = n.mutationCache),
      (this.#e = []),
      (this.state = n.state || XT()),
      this.setOptions(n.options),
      this.scheduleGc());
  }
  setOptions(n) {
    ((this.options = n), this.updateGcTime(this.options.gcTime));
  }
  get meta() {
    return this.options.meta;
  }
  addObserver(n) {
    this.#e.includes(n) ||
      (this.#e.push(n),
      this.clearGcTimeout(),
      this.#n.notify({ type: "observerAdded", mutation: this, observer: n }));
  }
  removeObserver(n) {
    ((this.#e = this.#e.filter((l) => l !== n)),
      this.scheduleGc(),
      this.#n.notify({ type: "observerRemoved", mutation: this, observer: n }));
  }
  optionalRemove() {
    this.#e.length || (this.state.status === "pending" ? this.scheduleGc() : this.#n.remove(this));
  }
  continue() {
    return this.#i?.continue() ?? this.execute(this.state.variables);
  }
  async execute(n) {
    const l = () => {
        this.#l({ type: "continue" });
      },
      s = { client: this.#t, meta: this.options.meta, mutationKey: this.options.mutationKey };
    this.#i = _g({
      fn: () =>
        this.options.mutationFn
          ? this.options.mutationFn(n, s)
          : Promise.reject(new Error("No mutationFn found")),
      onFail: (f, h) => {
        this.#l({ type: "failed", failureCount: f, error: h });
      },
      onPause: () => {
        this.#l({ type: "pause" });
      },
      onContinue: l,
      retry: this.options.retry ?? 0,
      retryDelay: this.options.retryDelay,
      networkMode: this.options.networkMode,
      canRun: () => this.#n.canRun(this),
    });
    const u = this.state.status === "pending",
      c = !this.#i.canStart();
    try {
      if (u) l();
      else {
        (this.#l({ type: "pending", variables: n, isPaused: c }),
          this.#n.config.onMutate && (await this.#n.config.onMutate(n, this, s)));
        const h = await this.options.onMutate?.(n, s);
        h !== this.state.context &&
          this.#l({ type: "pending", context: h, variables: n, isPaused: c });
      }
      const f = await this.#i.start();
      return (
        await this.#n.config.onSuccess?.(f, n, this.state.context, this, s),
        await this.options.onSuccess?.(f, n, this.state.context, s),
        await this.#n.config.onSettled?.(
          f,
          null,
          this.state.variables,
          this.state.context,
          this,
          s,
        ),
        await this.options.onSettled?.(f, null, n, this.state.context, s),
        this.#l({ type: "success", data: f }),
        f
      );
    } catch (f) {
      try {
        await this.#n.config.onError?.(f, n, this.state.context, this, s);
      } catch (h) {
        Promise.reject(h);
      }
      try {
        await this.options.onError?.(f, n, this.state.context, s);
      } catch (h) {
        Promise.reject(h);
      }
      try {
        await this.#n.config.onSettled?.(
          void 0,
          f,
          this.state.variables,
          this.state.context,
          this,
          s,
        );
      } catch (h) {
        Promise.reject(h);
      }
      try {
        await this.options.onSettled?.(void 0, f, n, this.state.context, s);
      } catch (h) {
        Promise.reject(h);
      }
      throw (this.#l({ type: "error", error: f }), f);
    } finally {
      this.#n.runNext(this);
    }
  }
  #l(n) {
    const l = (s) => {
      switch (n.type) {
        case "failed":
          return { ...s, failureCount: n.failureCount, failureReason: n.error };
        case "pause":
          return { ...s, isPaused: !0 };
        case "continue":
          return { ...s, isPaused: !1 };
        case "pending":
          return {
            ...s,
            context: n.context,
            data: void 0,
            failureCount: 0,
            failureReason: null,
            error: null,
            isPaused: n.isPaused,
            status: "pending",
            variables: n.variables,
            submittedAt: Date.now(),
          };
        case "success":
          return {
            ...s,
            data: n.data,
            failureCount: 0,
            failureReason: null,
            error: null,
            status: "success",
            isPaused: !1,
          };
        case "error":
          return {
            ...s,
            data: void 0,
            error: n.error,
            failureCount: s.failureCount + 1,
            failureReason: n.error,
            isPaused: !1,
            status: "error",
          };
      }
    };
    ((this.state = l(this.state)),
      me.batch(() => {
        (this.#e.forEach((s) => {
          s.onMutationUpdate(n);
        }),
          this.#n.notify({ mutation: this, type: "updated", action: n }));
      }));
  }
};
function XT() {
  return {
    context: void 0,
    data: void 0,
    error: null,
    failureCount: 0,
    failureReason: null,
    isPaused: !1,
    status: "idle",
    variables: void 0,
    submittedAt: 0,
  };
}
var KT = class extends Yr {
  constructor(n = {}) {
    (super(), (this.config = n), (this.#t = new Set()), (this.#e = new Map()), (this.#n = 0));
  }
  #t;
  #e;
  #n;
  build(n, l, s) {
    const u = new VT({
      client: n,
      mutationCache: this,
      mutationId: ++this.#n,
      options: n.defaultMutationOptions(l),
      state: s,
    });
    return (this.add(u), u);
  }
  add(n) {
    this.#t.add(n);
    const l = vr(n);
    if (typeof l == "string") {
      const s = this.#e.get(l);
      s ? s.push(n) : this.#e.set(l, [n]);
    }
    this.notify({ type: "added", mutation: n });
  }
  remove(n) {
    if (this.#t.delete(n)) {
      const l = vr(n);
      if (typeof l == "string") {
        const s = this.#e.get(l);
        if (s)
          if (s.length > 1) {
            const u = s.indexOf(n);
            u !== -1 && s.splice(u, 1);
          } else s[0] === n && this.#e.delete(l);
      }
    }
    this.notify({ type: "removed", mutation: n });
  }
  canRun(n) {
    const l = vr(n);
    if (typeof l == "string") {
      const u = this.#e.get(l)?.find((c) => c.state.status === "pending");
      return !u || u === n;
    } else return !0;
  }
  runNext(n) {
    const l = vr(n);
    return typeof l == "string"
      ? (this.#e
          .get(l)
          ?.find((u) => u !== n && u.state.isPaused)
          ?.continue() ?? Promise.resolve())
      : Promise.resolve();
  }
  clear() {
    me.batch(() => {
      (this.#t.forEach((n) => {
        this.notify({ type: "removed", mutation: n });
      }),
        this.#t.clear(),
        this.#e.clear());
    });
  }
  getAll() {
    return Array.from(this.#t);
  }
  find(n) {
    const l = { exact: !0, ...n };
    return this.getAll().find((s) => Ky(l, s));
  }
  findAll(n = {}) {
    return this.getAll().filter((l) => Ky(n, l));
  }
  notify(n) {
    me.batch(() => {
      this.listeners.forEach((l) => {
        l(n);
      });
    });
  }
  resumePausedMutations() {
    const n = this.getAll().filter((l) => l.state.isPaused);
    return me.batch(() => Promise.all(n.map((l) => l.continue().catch(Pe))));
  }
};
function vr(n) {
  return n.options.scope?.id;
}
var ZT = class extends Yr {
    constructor(n = {}) {
      (super(), (this.config = n), (this.#t = new Map()));
    }
    #t;
    build(n, l, s) {
      const u = l.queryKey,
        c = l.queryHash ?? Kc(u, l);
      let f = this.get(c);
      return (
        f ||
          ((f = new GT({
            client: n,
            queryKey: u,
            queryHash: c,
            options: n.defaultQueryOptions(l),
            state: s,
            defaultOptions: n.getQueryDefaults(u),
          })),
          this.add(f)),
        f
      );
    }
    add(n) {
      this.#t.has(n.queryHash) ||
        (this.#t.set(n.queryHash, n), this.notify({ type: "added", query: n }));
    }
    remove(n) {
      const l = this.#t.get(n.queryHash);
      l &&
        (n.destroy(),
        l === n && this.#t.delete(n.queryHash),
        this.notify({ type: "removed", query: n }));
    }
    clear() {
      me.batch(() => {
        this.getAll().forEach((n) => {
          this.remove(n);
        });
      });
    }
    get(n) {
      return this.#t.get(n);
    }
    getAll() {
      return [...this.#t.values()];
    }
    find(n) {
      const l = { exact: !0, ...n };
      return this.getAll().find((s) => Xy(l, s));
    }
    findAll(n = {}) {
      const l = this.getAll();
      return Object.keys(n).length > 0 ? l.filter((s) => Xy(n, s)) : l;
    }
    notify(n) {
      me.batch(() => {
        this.listeners.forEach((l) => {
          l(n);
        });
      });
    }
    onFocus() {
      me.batch(() => {
        this.getAll().forEach((n) => {
          n.onFocus();
        });
      });
    }
    onOnline() {
      me.batch(() => {
        this.getAll().forEach((n) => {
          n.onOnline();
        });
      });
    }
  },
  PT = class {
    #t;
    #e;
    #n;
    #i;
    #l;
    #a;
    #u;
    #s;
    constructor(n = {}) {
      ((this.#t = n.queryCache || new ZT()),
        (this.#e = n.mutationCache || new KT()),
        (this.#n = n.defaultOptions || {}),
        (this.#i = new Map()),
        (this.#l = new Map()),
        (this.#a = 0));
    }
    mount() {
      (this.#a++,
        this.#a === 1 &&
          ((this.#u = pg.subscribe(async (n) => {
            n && (await this.resumePausedMutations(), this.#t.onFocus());
          })),
          (this.#s = Dr.subscribe(async (n) => {
            n && (await this.resumePausedMutations(), this.#t.onOnline());
          }))));
    }
    unmount() {
      (this.#a--,
        this.#a === 0 && (this.#u?.(), (this.#u = void 0), this.#s?.(), (this.#s = void 0)));
    }
    isFetching(n) {
      return this.#t.findAll({ ...n, fetchStatus: "fetching" }).length;
    }
    isMutating(n) {
      return this.#e.findAll({ ...n, status: "pending" }).length;
    }
    getQueryData(n) {
      const l = this.defaultQueryOptions({ queryKey: n });
      return this.#t.get(l.queryHash)?.state.data;
    }
    ensureQueryData(n) {
      const l = this.defaultQueryOptions(n),
        s = this.#t.build(this, l),
        u = s.state.data;
      return u === void 0
        ? this.fetchQuery(n)
        : (n.revalidateIfStale && s.isStaleByTime(Dc(l.staleTime, s)) && this.prefetchQuery(l),
          Promise.resolve(u));
    }
    getQueriesData(n) {
      return this.#t.findAll(n).map(({ queryKey: l, state: s }) => {
        const u = s.data;
        return [l, u];
      });
    }
    setQueryData(n, l, s) {
      const u = this.defaultQueryOptions({ queryKey: n }),
        f = this.#t.get(u.queryHash)?.state.data,
        h = RT(l, f);
      if (h !== void 0) return this.#t.build(this, u).setData(h, { ...s, manual: !0 });
    }
    setQueriesData(n, l, s) {
      return me.batch(() =>
        this.#t.findAll(n).map(({ queryKey: u }) => [u, this.setQueryData(u, l, s)]),
      );
    }
    getQueryState(n) {
      const l = this.defaultQueryOptions({ queryKey: n });
      return this.#t.get(l.queryHash)?.state;
    }
    removeQueries(n) {
      const l = this.#t;
      me.batch(() => {
        l.findAll(n).forEach((s) => {
          l.remove(s);
        });
      });
    }
    resetQueries(n, l) {
      const s = this.#t;
      return me.batch(
        () => (
          s.findAll(n).forEach((u) => {
            u.reset();
          }),
          this.refetchQueries({ type: "active", ...n }, l)
        ),
      );
    }
    cancelQueries(n, l = {}) {
      const s = { revert: !0, ...l },
        u = me.batch(() => this.#t.findAll(n).map((c) => c.cancel(s)));
      return Promise.all(u).then(Pe).catch(Pe);
    }
    invalidateQueries(n, l = {}) {
      return me.batch(
        () => (
          this.#t.findAll(n).forEach((s) => {
            s.invalidate();
          }),
          n?.refetchType === "none"
            ? Promise.resolve()
            : this.refetchQueries({ ...n, type: n?.refetchType ?? n?.type ?? "active" }, l)
        ),
      );
    }
    refetchQueries(n, l = {}) {
      const s = { ...l, cancelRefetch: l.cancelRefetch ?? !0 },
        u = me.batch(() =>
          this.#t
            .findAll(n)
            .filter((c) => !c.isDisabled() && !c.isStatic())
            .map((c) => {
              let f = c.fetch(void 0, s);
              return (
                s.throwOnError || (f = f.catch(Pe)),
                c.state.fetchStatus === "paused" ? Promise.resolve() : f
              );
            }),
        );
      return Promise.all(u).then(Pe);
    }
    fetchQuery(n) {
      const l = this.defaultQueryOptions(n);
      l.retry === void 0 && (l.retry = !1);
      const s = this.#t.build(this, l);
      return s.isStaleByTime(Dc(l.staleTime, s)) ? s.fetch(l) : Promise.resolve(s.state.data);
    }
    prefetchQuery(n) {
      return this.fetchQuery(n).then(Pe).catch(Pe);
    }
    fetchInfiniteQuery(n) {
      return ((n._type = "infinite"), this.fetchQuery(n));
    }
    prefetchInfiniteQuery(n) {
      return this.fetchInfiniteQuery(n).then(Pe).catch(Pe);
    }
    ensureInfiniteQueryData(n) {
      return ((n._type = "infinite"), this.ensureQueryData(n));
    }
    resumePausedMutations() {
      return Dr.isOnline() ? this.#e.resumePausedMutations() : Promise.resolve();
    }
    getQueryCache() {
      return this.#t;
    }
    getMutationCache() {
      return this.#e;
    }
    getDefaultOptions() {
      return this.#n;
    }
    setDefaultOptions(n) {
      this.#n = n;
    }
    setQueryDefaults(n, l) {
      this.#i.set(Vl(n), { queryKey: n, defaultOptions: l });
    }
    getQueryDefaults(n) {
      const l = [...this.#i.values()],
        s = {};
      return (
        l.forEach((u) => {
          Xl(n, u.queryKey) && Object.assign(s, u.defaultOptions);
        }),
        s
      );
    }
    setMutationDefaults(n, l) {
      this.#l.set(Vl(n), { mutationKey: n, defaultOptions: l });
    }
    getMutationDefaults(n) {
      const l = [...this.#l.values()],
        s = {};
      return (
        l.forEach((u) => {
          Xl(n, u.mutationKey) && Object.assign(s, u.defaultOptions);
        }),
        s
      );
    }
    defaultQueryOptions(n) {
      if (n._defaulted) return n;
      const l = { ...this.#n.queries, ...this.getQueryDefaults(n.queryKey), ...n, _defaulted: !0 };
      return (
        l.queryHash || (l.queryHash = Kc(l.queryKey, l)),
        l.refetchOnReconnect === void 0 && (l.refetchOnReconnect = l.networkMode !== "always"),
        l.throwOnError === void 0 && (l.throwOnError = !!l.suspense),
        !l.networkMode && l.persister && (l.networkMode = "offlineFirst"),
        l.queryFn === Zc && (l.enabled = !1),
        l
      );
    }
    defaultMutationOptions(n) {
      return n?._defaulted
        ? n
        : {
            ...this.#n.mutations,
            ...(n?.mutationKey && this.getMutationDefaults(n.mutationKey)),
            ...n,
            _defaulted: !0,
          };
    }
    clear() {
      (this.#t.clear(), this.#e.clear());
    }
  },
  FT = tt.createContext(void 0),
  JT = ({ client: n, children: l }) => (
    tt.useEffect(
      () => (
        n.mount(),
        () => {
          n.unmount();
        }
      ),
      [n],
    ),
    P.jsx(FT.Provider, { value: n, children: l })
  );
const kT = {
    en: {
      nav: {
        experience: "Experience",
        services: "Services",
        artisans: "Artisans",
        gallery: "Gallery",
        visit: "Visit",
        book: "Book Now",
      },
      hero: {
        location: "· Leander, Texas · Est. 2014 ·",
        title1: "Modern ",
        title2: "Grooming",
        title3: "& Beauty Lifestyle",
        desc: "More than a barbershop and salon — we are a modern space to relax, renew your image, and feel confident, backed by over 12 years of experience.",
        reviews: "170+ Reviews",
        btnBook: "Book Appointment",
        btnExplore: "Explore Services",
        scroll: "Scroll ↓",
      },
      marquee: [
        "2405 S Hwy 183 #102, Leander TX",
        "•",
        "Est. 2014",
        "•",
        "Premium Cuts & Couture Beauty",
        "•",
        "Open 7 Days",
        "•",
      ],
      space: {
        subtitle: "— The Space",
        title1: "Designed for ",
        title2: "your comfort.",
        desc: "A modern sanctuary designed for your comfort. Enjoy personalized attention, updated techniques, and premium products to guarantee an excellent experience on every visit.",
      },
      philosophy: {
        subtitle: "— The Philosophy",
        title1: "The hush of a ",
        title2: "Lounge.",
        title3: "The polish of a ",
        title4: "Couture ",
        title5: "studio.",
        desc: "Our goal has always been to deliver results that make you feel secure, confident, and stylish. We blend the precision of professional barbering with the artistry of a modern beauty salon.",
        stat1: "12+",
        stat1Desc: "Years Crafting",
        stat2: "8",
        stat2Desc: "Master Artisans",
        stat3: "170+",
        stat3Desc: "5★ Reviews",
        label1: "Barber Craft",
        label2: "Beauty Couture",
      },
      services: {
        barberSubtitle: "— For Him",
        barberTitle: "Master Barbering",
        barberMenu: "The Barber Menu →",
        barberList: [
          { name: "Modern Cuts & Fades", price: "$45+" },
          { name: "Professional Barbering", price: "$40+" },
          { name: "Razor Line Up", price: "$35+" },
          { name: "Beard Sculpting", price: "$30+" },
          { name: "Kids Cuts", price: "$25+" },
          { name: "Hot Towel Shave", price: "$50+" },
        ],
        beautySubtitle: "— For Her",
        beautyTitle: "Artisan Beauty",
        beautyMenu: "The Beauty Menu →",
        beautyList: [
          { name: "Color & Balayage", price: "$150+" },
          { name: "Highlights", price: "$120+" },
          { name: "Hair Treatments", price: "$85+" },
          { name: "Custom Facials", price: "$95+" },
          { name: "Men's & Women's Beauty", price: "$65+" },
          { name: "Lash & Brow Artistry", price: "$45+" },
        ],
      },
      team: {
        subtitle: "— The Founder",
        title1: "Our ",
        title2: "Story.",
        p1: "At Lifestyle Barber & Beauty Salon, we bring over 12 years of experience offering barbering and beauty services with uncompromising quality, professionalism, and dedication.",
        p2: "Thank you for being part of Lifestyle and allowing us to grow alongside you. Our goal will always be to provide personalized attention and results that make you feel secure, confident, and stylish.",
        badge: "Master Barber",
        name: "Alberto",
        role: "CEO & Manager",
        founder: "Founder",
      },
      gallery: {
        subtitle: "— Visual Archive",
        title1: "Moments, ",
        title2: "captured.",
        alt1: "Razor line up",
        alt2: "Salon interior",
        alt3: "Balayage detail",
        alt4: "Beauty atelier",
        alt5: "Editorial portrait",
        alt6: "Beauty editorial",
      },
      reels: {
        subtitle: "— @lifestyle_barberstx",
        title1: "The Ritual ",
        title2: "In Motion",
        follow: "Follow on Instagram →",
        label: "Reel",
        items: [
          { title: "Hot Towel Ritual" },
          { title: "Balayage Magic" },
          { title: "The Skin Fade" },
          { title: "Beauty Glow Up" },
          { title: "Beard Sculpt" },
          { title: "Color Story" },
        ],
      },
      testimonials: {
        subtitle: "— Voices",
        title1: "A standard ",
        title2: "our guests",
        title3: " recognize.",
        verified: "Verified Guest",
        reviews: [
          {
            quote:
              "Excellent service and amazing attention to detail. The best taper fade I've had in years.",
            name: "Marcus R.",
            rating: 5,
          },
          {
            quote:
              "Professional, modern, and highly recommended. Walked out feeling like a new person.",
            name: "Sofia K.",
            rating: 5,
          },
          {
            quote:
              "Best barber shop in Leander. The atmosphere is incredible and the cuts are precision-level.",
            name: "Daniel V.",
            rating: 5,
          },
        ],
      },
      contact: {
        subtitle: "— Reserve Your Seat",
        title1: "Book your ",
        title2: "appointment",
        title3: " today.",
        nameLabel: "Name",
        namePlaceholder: "Your full name",
        phoneLabel: "Phone",
        phonePlaceholder: "(830) 555-0000",
        serviceLabel: "Service",
        servicePlaceholder: "What can we do for you?",
        btnBook: "Request Booking",
        btnWhatsApp: "WhatsApp Us",
      },
      footer: {
        visit: "Visit",
        hours: "Hours",
        connect: "Connect",
        monSat: "Mon–Sat: 9AM – 8PM",
        sun: "Sun: 10AM – 6PM",
        copy: "© 2026 Lifestyle Barber & Beauty Salon",
        crafted: "Crafted with precision in Leander, TX",
      },
      cta: { book: "Book" },
    },
    es: {
      nav: {
        experience: "Experiencia",
        services: "Servicios",
        artisans: "Artistas",
        gallery: "Galería",
        visit: "Visítanos",
        book: "Reservar",
      },
      hero: {
        location: "· Leander, Texas · Est. 2014 ·",
        title1: "Estilo ",
        title2: "Moderno",
        title3: "& Belleza Exclusiva",
        desc: "Más que una barbería y salón — somos un espacio moderno para relajarte, renovar tu imagen y sentirte en confianza, con más de 12 años de experiencia.",
        reviews: "170+ Reseñas",
        btnBook: "Agendar Cita",
        btnExplore: "Explorar Servicios",
        scroll: "Deslizar ↓",
      },
      marquee: [
        "2405 S Hwy 183 #102, Leander TX",
        "•",
        "Est. 2014",
        "•",
        "Cortes Premium y Alta Belleza",
        "•",
        "Abierto los 7 días",
        "•",
      ],
      space: {
        subtitle: "— El Lugar",
        title1: "Diseñado para ",
        title2: "tu comodidad.",
        desc: "Un santuario moderno diseñado para tu comodidad. Disfruta de atención personalizada, técnicas actualizadas y productos de alta calidad para garantizar una excelente experiencia en cada visita.",
      },
      philosophy: {
        subtitle: "— La Filosofía",
        title1: "La paz de un ",
        title2: "Lounge.",
        title3: "La elegancia de un ",
        title4: "estudio de ",
        title5: "Belleza.",
        desc: "Nuestro objetivo siempre ha sido brindar resultados que te hagan sentir seguridad, confianza y estilo. Combinamos la precisión de la barbería profesional con el arte de un estudio de belleza.",
        stat1: "12+",
        stat1Desc: "Años de Arte",
        stat2: "8",
        stat2Desc: "Máster Artistas",
        stat3: "170+",
        stat3Desc: "Reseñas 5★",
        label1: "Arte Barbería",
        label2: "Alta Belleza",
      },
      services: {
        barberSubtitle: "— Para Él",
        barberTitle: "Barbería Máster",
        barberMenu: "Menú de Barbería →",
        barberList: [
          { name: "Cortes Modernos & Fades", price: "$45+" },
          { name: "Barbería Profesional", price: "$40+" },
          { name: "Alineación a Navaja", price: "$35+" },
          { name: "Esculpido de Barba", price: "$30+" },
          { name: "Cortes para Niños", price: "$25+" },
          { name: "Afeitado con Toalla Caliente", price: "$50+" },
        ],
        beautySubtitle: "— Para Ella",
        beautyTitle: "Belleza Artesanal",
        beautyMenu: "Menú de Belleza →",
        beautyList: [
          { name: "Coloración & Balayage", price: "$150+" },
          { name: "Highlights", price: "$120+" },
          { name: "Tratamientos Capilares", price: "$85+" },
          { name: "Faciales a la Medida", price: "$95+" },
          { name: "Belleza para Él y Ella", price: "$65+" },
          { name: "Arte en Pestañas y Cejas", price: "$45+" },
        ],
      },
      team: {
        subtitle: "— El Fundador",
        title1: "Nuestra ",
        title2: "Historia.",
        p1: "En Lifestyle Barber & Beauty Salon contamos con más de 12 años de experiencia ofreciendo servicios de barbería y belleza con calidad, profesionalismo y dedicación.",
        p2: "Gracias por formar parte de Lifestyle y permitirnos seguir creciendo junto a ustedes. Nuestro objetivo siempre será brindar atención personalizada y resultados que te hagan sentir seguridad, confianza y estilo.",
        badge: "Máster Barbero",
        name: "Alberto",
        role: "CEO y Mánager",
        founder: "Fundador",
      },
      gallery: {
        subtitle: "— Archivo Visual",
        title1: "Momentos, ",
        title2: "capturados.",
        alt1: "Alineación a navaja",
        alt2: "Interior del salón",
        alt3: "Detalle de Balayage",
        alt4: "Atelier de belleza",
        alt5: "Retrato editorial",
        alt6: "Belleza editorial",
      },
      reels: {
        subtitle: "— @lifestyle_barberstx",
        title1: "El Ritual ",
        title2: "En Movimiento",
        follow: "Síguenos en Instagram →",
        label: "Reel",
        items: [
          { title: "Ritual de Toalla Caliente" },
          { title: "Magia del Balayage" },
          { title: "El Skin Fade" },
          { title: "Resplandor de Belleza" },
          { title: "Esculpido de Barba" },
          { title: "Historia de Color" },
        ],
      },
      testimonials: {
        subtitle: "— Voces",
        title1: "Un estándar que ",
        title2: "nuestros clientes",
        title3: " reconocen.",
        verified: "Cliente Verificado",
        reviews: [
          {
            quote:
              "Excelente servicio y una increíble atención al detalle. El mejor Taper Fade que me han hecho en años.",
            name: "Marcus R.",
            rating: 5,
          },
          {
            quote:
              "Profesional, moderno y altamente recomendado. Salí sintiéndome como una persona nueva.",
            name: "Sofia K.",
            rating: 5,
          },
          {
            quote:
              "La mejor barbería en Leander. El ambiente es increíble y los cortes son de nivel de precisión.",
            name: "Daniel V.",
            rating: 5,
          },
        ],
      },
      contact: {
        subtitle: "— Reserva Tu Asiento",
        title1: "Agenda tu ",
        title2: "cita",
        title3: " hoy.",
        nameLabel: "Nombre",
        namePlaceholder: "Tu nombre completo",
        phoneLabel: "Teléfono",
        phonePlaceholder: "(830) 555-0000",
        serviceLabel: "Servicio",
        servicePlaceholder: "¿Qué podemos hacer por ti?",
        btnBook: "Solicitar Reserva",
        btnWhatsApp: "Escríbenos por WhatsApp",
      },
      footer: {
        visit: "Visítanos",
        hours: "Horarios",
        connect: "Conectar",
        monSat: "Lun–Sáb: 9AM – 8PM",
        sun: "Dom: 10AM – 6PM",
        copy: "© 2026 Lifestyle Barber & Beauty Salon",
        crafted: "Creado con precisión en Leander, TX",
      },
      cta: { book: "Reservar" },
    },
  },
  Tg = tt.createContext(void 0),
  IT = ({ children: n }) => {
    const [l, s] = tt.useState("en");
    tt.useEffect(() => {
      const f = localStorage.getItem("preferredLanguage");
      if (f === "es" || f === "en") s(f);
      else {
        const h = navigator.language.startsWith("es") ? "es" : "en";
        s(h);
      }
    }, []);
    const u = (f) => {
        (s(f), localStorage.setItem("preferredLanguage", f));
      },
      c = kT[l];
    return P.jsx(Tg.Provider, { value: { language: l, setLanguage: u, t: c }, children: n });
  },
  v2 = () => {
    const n = tt.useContext(Tg);
    if (n === void 0) throw new Error("useLanguage must be used within a LanguageProvider");
    return n;
  },
  $T = "/assets/styles-B4vZB6lv.css";
function WT() {
  return P.jsx("div", {
    className: "flex min-h-screen items-center justify-center bg-background px-4",
    children: P.jsxs("div", {
      className: "max-w-md text-center",
      children: [
        P.jsx("h1", { className: "text-7xl font-bold text-foreground", children: "404" }),
        P.jsx("h2", {
          className: "mt-4 text-xl font-semibold text-foreground",
          children: "Page not found",
        }),
        P.jsx("p", {
          className: "mt-2 text-sm text-muted-foreground",
          children: "The page you're looking for doesn't exist or has been moved.",
        }),
        P.jsx("div", {
          className: "mt-6",
          children: P.jsx(Xc, {
            to: "/",
            className:
              "inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90",
            children: "Go home",
          }),
        }),
      ],
    }),
  });
}
function t2({ error: n, reset: l }) {
  console.error(n);
  const s = oe();
  return P.jsx("div", {
    className: "flex min-h-screen items-center justify-center bg-background px-4",
    children: P.jsxs("div", {
      className: "max-w-md text-center",
      children: [
        P.jsx("h1", {
          className: "text-xl font-semibold tracking-tight text-foreground",
          children: "This page didn't load",
        }),
        P.jsx("p", {
          className: "mt-2 text-sm text-muted-foreground",
          children: "Something went wrong on our end. You can try refreshing or head back home.",
        }),
        P.jsxs("div", {
          className: "mt-6 flex flex-wrap justify-center gap-2",
          children: [
            P.jsx("button", {
              onClick: () => {
                (s.invalidate(), l());
              },
              className:
                "inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90",
              children: "Try again",
            }),
            P.jsx("a", {
              href: "/",
              className:
                "inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent",
              children: "Go home",
            }),
          ],
        }),
      ],
    }),
  });
}
const Pc = ZE()({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=device-width, initial-scale=1" },
      { name: "theme-color", content: "#1A1A1A" },
      { title: "Lifestyle Barber & Beauty Salon — Leander, TX" },
      {
        name: "description",
        content:
          "Premium barber and beauty experience in Leander, Texas. Tailored cuts, beard sculpting, hair coloring, facials & more. Book today.",
      },
      { name: "author", content: "Lifestyle Barber & Beauty Salon" },
      { property: "og:title", content: "Lifestyle Barber & Beauty Salon — Leander, TX" },
      {
        property: "og:description",
        content:
          "A cinematic fusion of luxury barbering and modern beauty atelier in Leander, Texas.",
      },
      { property: "og:type", content: "website" },
      { property: "og:image", content: "/og-image.jpg" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "twitter:title", content: "Lifestyle Barber & Beauty Salon — Leander, TX" },
      {
        name: "twitter:description",
        content:
          "Premium barber and beauty experience in Leander, Texas. Tailored cuts, beard sculpting, hair coloring, facials & more. Book today.",
      },
      { name: "twitter:image", content: "/og-image.jpg" },
    ],
    links: [
      { rel: "icon", type: "image/png", href: "/favicon.png" },
      { rel: "apple-touch-icon", href: "/favicon.png" },
      { rel: "stylesheet", href: $T },
    ],
  }),
  shellComponent: e2,
  component: n2,
  notFoundComponent: WT,
  errorComponent: t2,
});
function e2({ children: n }) {
  return P.jsxs("html", {
    lang: "en",
    children: [
      P.jsx("head", { children: P.jsx(mT, {}) }),
      P.jsxs("body", { children: [n, P.jsx(yT, {})] }),
    ],
  });
}
function n2() {
  const { queryClient: n } = Pc.useRouteContext();
  return P.jsx(IT, { children: P.jsx(JT, { client: n, children: P.jsx(hg, {}) }) });
}
const a2 = "modulepreload",
  i2 = function (n) {
    return "/" + n;
  },
  Iy = {},
  l2 = function (l, s, u) {
    let c = Promise.resolve();
    if (s && s.length > 0) {
      let m = function (p) {
        return Promise.all(
          p.map((S) =>
            Promise.resolve(S).then(
              (g) => ({ status: "fulfilled", value: g }),
              (g) => ({ status: "rejected", reason: g }),
            ),
          ),
        );
      };
      document.getElementsByTagName("link");
      const h = document.querySelector("meta[property=csp-nonce]"),
        y = h?.nonce || h?.getAttribute("nonce");
      c = m(
        s.map((p) => {
          if (((p = i2(p)), p in Iy)) return;
          Iy[p] = !0;
          const S = p.endsWith(".css"),
            g = S ? '[rel="stylesheet"]' : "";
          if (document.querySelector(`link[href="${p}"]${g}`)) return;
          const _ = document.createElement("link");
          if (
            ((_.rel = S ? "stylesheet" : a2),
            S || (_.as = "script"),
            (_.crossOrigin = ""),
            (_.href = p),
            y && _.setAttribute("nonce", y),
            document.head.appendChild(_),
            S)
          )
            return new Promise((E, x) => {
              (_.addEventListener("load", E),
                _.addEventListener("error", () => x(new Error(`Unable to preload CSS for ${p}`))));
            });
        }),
      );
    }
    function f(h) {
      const y = new Event("vite:preloadError", { cancelable: !0 });
      if (((y.payload = h), window.dispatchEvent(y), !y.defaultPrevented)) throw h;
    }
    return c.then((h) => {
      for (const y of h || []) y.status === "rejected" && f(y.reason);
      return l().catch(f);
    });
  },
  s2 = () => l2(() => import("./index-DFPWmEA5.js"), []),
  r2 = JE("/")({
    head: () => ({
      meta: [
        { title: "Lifestyle Barber & Beauty Salon — Premium Cuts & Beauty in Leander, TX" },
        {
          name: "description",
          content:
            "A cinematic fusion of luxury barbering and modern beauty atelier in Leander, Texas. Tailored cuts, beard sculpting, hair coloring, facials & more. Book today.",
        },
        { property: "og:title", content: "Lifestyle Barber & Beauty Salon — Leander, TX" },
        {
          property: "og:description",
          content: "Premium cuts, beauty services, grooming & style in Leander, Texas.",
        },
      ],
    }),
    component: IE(s2, "component"),
  }),
  u2 = r2.update({ id: "/", path: "/", getParentRoute: () => Pc }),
  o2 = { IndexRoute: u2 },
  c2 = Pc._addFileChildren(o2),
  f2 = () => {
    const n = new PT();
    return uT({
      routeTree: c2,
      context: { queryClient: n },
      scrollRestoration: !0,
      defaultPreloadStaleTime: 0,
    });
  };
async function d2() {
  const n = await f2();
  let l;
  if (Vy) {
    const s = await Vy.getOptions();
    ((s.serializationAdapters = s.serializationAdapters ?? []),
      (window.__TSS_START_OPTIONS__ = s),
      (l = s.serializationAdapters),
      (n.options.defaultSsr = s.defaultSsr));
  } else ((l = []), (window.__TSS_START_OPTIONS__ = { serializationAdapters: l }));
  return (
    l.push(cE),
    n.options.serializationAdapters && l.push(...n.options.serializationAdapters),
    n.update({ basepath: "", serializationAdapters: l }),
    n.stores.matchesId.get().length || (await dE(n)),
    n
  );
}
var h2 = d2;
async function m2() {
  const n = await h2();
  return (window.$_TSR?.h(), n);
}
var bc;
function y2() {
  return (bc || (bc = m2()), P.jsx(pE, { promise: bc, children: (n) => P.jsx(fT, { router: n }) }));
}
tt.startTransition(() => {
  $0.hydrateRoot(document, P.jsx(tt.StrictMode, { children: P.jsx(y2, {}) }));
});
export { P as j, tt as r, v2 as u };
